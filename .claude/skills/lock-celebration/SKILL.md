...
2. Telegram (SILENT — never loud), as a POST (the route is POST-only since 2026-09-17; a GET is refused with 405): `curl -sS -X POST -H "x-premium-key: $PREMIUM_ACCESS_KEY" "https://clucknorris.app/api/tg-test?photo={rawUrl}&text={pending.tgText}%0A%0A🐦 On X — like %26 repost: https://x.com/FireChicken007/status/{id}"`
