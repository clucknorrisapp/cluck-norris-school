...
const library = read("src/sections/Library.jsx");
...
const counts = {
  ...
  LIBRARY_TOPICS: countLessons(library, "LIBRARY_TOPICS"),
  LIBRARY_LIQUIDITY: countLessons(library, "LIBRARY_LIQUIDITY"),
};
...
// 6. data/curriculum.json — the Live Classroom's server-side copy of the whole curriculum
//    (server.js ~76). It is regenerated by `node scripts/extract-curriculum.js`, which reads
//    App.jsx, LPLab.jsx AND Library.jsx (fixed 2026-09-18 — the old version only read App.jsx,
//    so a re-run silently dropped the LP Lab and Deep Dive courses). Pin the per-course counts
//    here, derived from the same sources, so a lesson added to one array without regenerating
//    the JSON fails CI instead of leaving the classroom teaching a stale roster. This is a fast
//    count-only check; `node scripts/extract-curriculum.js --check` (also run in CI) catches
//    content drift the counts alone would miss.
const CURRICULUM_PATH = path.join(ROOT, "data", "curriculum.json");
const expectedCourseCounts = {
  fundamentals: counts.LESSONS,
  basics: counts.INCUBATOR_LESSONS,
  lp: counts.LP_LESSONS + counts.LIBRARY_LIQUIDITY,   // LP Lab lessons + folded-in library reference lessons
  deepdive: counts.LIBRARY_TOPICS,
};
if (!fs.existsSync(CURRICULUM_PATH)) {
  problems.push("data/curriculum.json is missing. Run: node scripts/extract-curriculum.js");
} else {
  let curriculum;
  try { curriculum = JSON.parse(fs.readFileSync(CURRICULUM_PATH, "utf8")); }
  catch (e) { problems.push(`data/curriculum.json does not parse as JSON: ${e.message}`); curriculum = null; }
  if (curriculum) {
    const byId = {};
    for (const c of curriculum.courses || []) byId[c.id] = (c.lessons || []).length;
    for (const [id, expected] of Object.entries(expectedCourseCounts)) {
      const actual = byId[id];
      if (actual === undefined) {
        problems.push(`data/curriculum.json — course "${id}" is missing. Run: node scripts/extract-curriculum.js`);
      } else if (actual !== expected) {
        problems.push(
          `data/curriculum.json — course "${id}" has ${actual} lessons but the sources have ${expected}. ` +
