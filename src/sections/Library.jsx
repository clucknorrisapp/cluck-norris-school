      ...
      {tab==="deepdives" && (
        <div>
          <div style={{background:"rgba(255,182,39,0.08)",border:"1px solid rgba(255,182,39,0.2)",borderRadius:10,padding:"12px 14px",marginBottom:16}}>
            <p style={{fontFamily:"'Anton',sans-serif",fontSize:13,color:"#FFB627",letterSpacing:1,margin:0,lineHeight:1.7}}>
                ...
                <div key={topic.id} style={{marginBottom:8}}>
                  <button onClick={()=>setOpenTopic(openTopic===topic.id?null:topic.id)} style={{width:"100%",background:openTopic===topic.id?"rgba(255,182,39,0.08)":"rgba(255,122,24,0.05)",border:`1px solid ${openTopic===topic.id?"rgba(255,182,39,0.4)":"rgba(255,122,24,0.18)"}`,borderRadius:openTopic===topic.id?"12px 12px 0 0":"12px",padding:"12px 16px",display:"flex",justifyContent:"space-between",alignItems:"center",cursor:"pointer"}}>
                    <div style={{display:"flex",alignItems:"center",gap:10,textAlign:"left"}}>
                      ...
                      <div>
                        <div style={{fontFamily:"'Anton',sans-serif",fontSize:15,fontWeight:700,color:openTopic===topic.id?"#FFB627":"#D1D5DB",letterSpacing:1}}>{topic.title}</div>
                        <div style={{fontFamily:"'Anton',sans-serif",fontSize:9,color:"#6B7280",letterSpacing:0.5,marginTop:2}}>{topic.summary}</div>
                      </div>
                    </div>
                    <span style={{color:openTopic===topic.id?"#FFB627":"#6B7280",fontSize:15.5,flexShrink:0}}>{openTopic===topic.id?"▲":"▼"}</span>
                  </button>
                  {openTopic===topic.id && (
      ...
      {tab==="liquidity" && (
        <div>
          <div style={{background:"rgba(6,182,212,0.08)",border:"1px solid rgba(6,182,212,0.2)",borderRadius:10,padding:"12px 14px",marginBottom:16}}>
            <p style={{fontFamily:"'Anton',sans-serif",fontSize:13,color:"#06B6D4",letterSpacing:1,margin:0,lineHeight:1.7}}>
              ...
              <div key={item.id} style={{background:"rgba(255,122,24,0.05)",border:`1px solid ${expanded===item.id?"rgba(6,182,212,0.4)":"rgba(255,122,24,0.16)"}`,borderRadius:10,overflow:"hidden"}}>
                <button onClick={()=>setExpanded(expanded===item.id?null:item.id)} style={{width:"100%",background:"none",border:"none",padding:"14px 16px",cursor:"pointer",textAlign:"left",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <div style={{display:"flex",alignItems:"center",gap:10}}>
                    ...
                    <div>
                      <div style={{fontFamily:"'Anton',sans-serif",fontSize:15.5,fontWeight:700,color:"#F9FAFB"}}>{item.title}</div>
                      <div style={{fontFamily:"'Anton',sans-serif",fontSize:12.5,color:"#6B7280",marginTop:2}}>{item.summary}</div>
                    </div>
                  </div>
      ...
      {tab==="glossary" && (
        <div>
          <input
            ...
            style={{width:"100%",background:"rgba(255,122,24,0.07)",border:"1px solid rgba(255,122,24,0.2)",borderRadius:8,padding:"10px 14px",color:"#F9FAFB",fontFamily:"'Anton',sans-serif",fontSize:13.5,letterSpacing:1,marginBottom:14,boxSizing:"border-box",outline:"none"}}
          />
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            {filteredGlossary.map(g=>(
              <div key={g.term} style={{background:"rgba(255,122,24,0.05)",border:"1px solid rgba(167,139,250,0.15)",borderRadius:10,padding:"12px 14px"}}>
                <div style={{fontFamily:"'Anton',sans-serif",fontSize:15,fontWeight:700,color:"#A78BFA",marginBottom:4}}>{g.term}</div>
                <div style={{fontSize:13.5,color:"#9CA3AF",lineHeight:1.6}}>{g.def}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:20}}>
          {LIBRARY_RESOURCES.map(cat=>(
            <div key={cat.category}>
              <div style={{fontFamily:"'Anton',sans-serif",fontSize:13,letterSpacing:2,color:"#FF7A18",marginBottom:10}}>{cat.category}</div>
              <div style={{display:"flex",flexDirection:"column",gap:8}}>
                {cat.links.map(link=>(
                  <a key={link.name} href={link.url} target="_blank" rel="noreferrer" style={{display:"flex",justifyContent:"space-between",alignItems:"center",background:"rgba(255,122,24,0.05)",border:"1px solid rgba(255,122,24,0.16)",borderRadius:10,padding:"12px 14px",textDecoration:"none"}}>
                    <div>
                      <div style={{fontFamily:"'Anton',sans-serif",fontSize:15,fontWeight:700,color:"#F9FAFB",marginBottom:2}}>{link.name}</div>
                      <div style={{fontSize:13,color:"#6B7280"}}>{link.desc}</div>
                    </div>
                    <span style={{color:"#FF7A18",fontSize:13.5,flexShrink:0,marginLeft:8}}>→</span>
                  </a>
                ))}
              </div>
            </div>
