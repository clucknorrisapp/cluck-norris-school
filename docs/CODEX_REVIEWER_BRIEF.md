...
`no-mutating-get` hook — Round 26's scanner concurrency question and Round 25's hook-parsing
review, both answered with real bugs this time). Note on state: at the time you found these,
#416's changes were already on `main` and #418's were on `develop` (see Round 26); this PR carries
the fixes on top of `develop` — promotion to `main` is still the owner's call.

| # | Finding | Fix | Pinned by |
|---|---|---|---|
| 1 | **P1** — `lib/cuna-giveaway.js` `scanOnce()`, the unconditional `s.cursorMs = to;` after the awaited tape request: pause a scan mid-await at minute 60, rewind the cursor to minute 0 while it's suspended, let it resume — the resumed slice blindly overwrote the rewound cursor with its own stale `to`, silently undoing the rewind. The 0–60 "recovery range" (the entire reason to rewind) was never re-walked even though the cursor read as caught up | A generation counter (`s.gen`, bumped by `rewindCursor()` and `resetLedger()` only when they actually move the cursor) is captured before each slice's tape await and re-read after it. A mismatch means an external rewind/reset landed mid-flight: the slice returns immediately with `{ok:true, superseded:true, cursorMs, ...}` WITHOUT writing `s.cursorMs = to` and without crediting that slice's trades — the next tick re-walks from wherever the rewind/reset left the cursor. Every credit remains deduped by signature regardless | `scripts/cuna-giveaway-scan-test.cjs`, new section "a rewind mid-scan does not get clobbered": a real concurrency race using a controllable pause on the stub tape fetch — pauses `scanOnce()` mid-await at minute 60, rewinds to 0 (adding a trade only visible after the pause, modelling the late-Helius-enrichment case rewind exists for), releases it, asserts `superseded:true`, the cursor left at 0, and a follow-up `catchUp()` reaching the TRUE 0–90 ceiling with the recovery-range trade credited |
| 2 | **P2** — `catchUp()`'s `budgetMs` was only checked BETWEEN complete `scanOnce()` calls; each call's own up-to-8-slice loop had no internal time check, so one call could itself run far past the budget if the tape was slow (measured: mocked 20s-per-request tape, 75s budget → 160s actual) | `scanOnce(deps, opts)` now takes an optional `opts.deadlineMs`. The slice loop (a) stops STARTING a new slice once the deadline has passed — same shape as the existing `MAX_SLICES_PER_RUN` cap — and (b) bounds each individual tape request with `withTimeout()` (`Promise.race` against a timer sized to the remaining time), treating a timeout exactly like an incomplete slice: keep whatever was already credited, do not advance the cursor past it, retry next tick. `catchUp()` computes one absolute `deadlineMs = startedAt + budgetMs` and passes it to every `scanOnce()` call it makes. The plain 5-minute tick (`scanOnce(deps)`, no `opts`) is completely unaffected — no timer, no deadline check, identical to before | Same file, new section "a single scanOnce() call self-enforces a deadline": a 24h backlog with a mocked 20s-per-request tape and a 75s deadline returns after ~4 slices (well under the 8-slice cap) instead of running the full cap; every earlier test in the file still calls `scanOnce(deps)` with no `opts` and passes unchanged, pinning the "unaffected" half |
| 3 | **P2** — `.claude/hooks/no-mutating-get.js` short-flag cluster parsing: (a) `-o/dev/null` was misparsed as carrying data — the scanner kept walking cluster characters past `o` (not in the old `SHORT_VALUE_FLAGS` set at all) and hit the `d` inside `/dev/null`, wrongly setting `hasData`; (b) `curl -X POST <safe-url> --next GET /api/…/admin` — `--next` starts a brand-new curl request whose method curl documents as RESET to the default, but the whole word list was judged as one method, so the POST before `--next` was read as covering the admin GET after it | (a) Every short flag curl actually documents as value-taking is now recognised (`o,H,A,u,b,c,e,m,w,U,x,y,z,K,E` plus the existing `X,d,G,I,T,F`); the FIRST value-taking flag hit in a cluster consumes the rest of the word (or the next word) as its value and stops the scan of that cluster right there — nothing inside a flag's own value is ever re-scanned as more flags. Only `X/d/T/F` (value) and `G/I` (no value) affect the computed method; the rest are consumed opaquely. (b) `splitOnNext()` splits a segment's tokenized words on `--next` and each resulting part is judged independently — its own `computeEffectiveMethod()`, its own admin-path/mutating-flag check — exactly like separate invocations | `scripts/no-mutating-get-hook-test.cjs`: `curl -o/dev/null ".../admin?draw=1"` and the separate-word form both block; `curl -X POST https://example.com/hook --next ".../admin?draw=1"` blocks (method reset by `--next`) while the same command with an explicit `-X POST` on the `--next` side is allowed |
| 4 | **P2** — same hook, `MUTATING_FLAG_RE` tested only the raw command TEXT, never the query curl actually sends: `curl -G --data-urlencode draw=1 …/api/cuna-giveaway/admin` moves `draw=1` onto the URL as a query parameter (that's what `-G` does), but in the raw text `draw=1` has no `?`/`&` immediately before it — it's a separate `--data-urlencode` argument — so the regex never matched and the whole thing was allowed as a "flag-less read" | `computeEffectiveMethod()` now optionally collects the raw value of every data/form flag (`-d`, `-F`, `--data*`, `--json`, `--form*`) into a `dataValuesOut` array. When the effective method resolves to GET, `main()` reconstructs the query a real request would send — the admin URL's own query string (found by `extractAdminUrlQuery()`, a direct regex over the segment/part text so it also catches `--url` or a URL in a later position) plus every collected data value, joined with `&` — and runs `MUTATING_FLAG_RE` against THAT combined string too | Same test file: `curl -G --data-urlencode draw=1 …/admin?key=k` (Codex's exact string), `curl -G -d 'draw=1' …/admin?key=k`, and `curl -G --data-raw 'x=1&draw=1' …/admin?key=k` all block; `curl -G -d 'foo=bar' …/admin?key=k` (no mutating flag anywhere in the reconstructed query) stays allowed |

Also run and unchanged: `scripts/mutating-get-guard-test.cjs` (the server-side POST-only inventory
— this PR touches no route, only the hook and the scanner) and `node --check` on every file
touched. Where to look hardest: whether the generation-counter fix (#1) has any window where a
rewind's OWN write can itself be superseded by a second concurrent rewind (two operators, or a
scheduler self-heal racing a manual `&rewind=`) — the fix only protects a scan's write against a
rewind, not two rewinds against each other, which was out of scope for this round's reproduction.

### Round 32b (second lens) — same PR, an adversarial re-review of the round-32 fixes themselves — fixed

Same branch, one more commit. Seven follow-ups on the four fixes above, each with its own attack
string and its own test.

| # | Finding | Fix | Pinned by |
|---|---|---|---|
| 1 | **P2** — the short-flag cluster loop `break`s right after `G`/`I` even though neither takes a value, so `-Gd draw=1` / `-sGd draw=1` never reached the `d` two characters later — the query-reconstruction data value was silently dropped | `G`/`I` now `continue` the cluster scan instead of `break`ing it — only a flag that actually CONSUMED a value (`X`/`d`/`T`/`F`, or an opaque one) stops the scan of that word | `scripts/no-mutating-get-hook-test.cjs`: `-Gd 'draw=1'`, `-sGd 'draw=1'` block; `-IsXPOST` still resolves to POST (the later explicit `-X` wins), not misread as HEAD, now that the loop doesn't stop early |
| 2 | **P2** — `configure()` writes `s.cursorMs = c.startMs` but never bumped `gen`, so a scan from the OLD promo that captured `gen` before a reconfigure could still credit trades scored against a mint/window that no longer applies, and overwrite the cursor the reconfigure just set — the identical hazard #1 above fixes for `rewindCursor()`/`resetLedger()`, just not closed here too | `configure()` now bumps `s.gen` whenever `mint`, `pool`, `startMs`, `endMs`, `minUsd` or `entryMode` actually change (cosmetic fields — symbol, displayUsd, exclude, bonusPct, chatId, mode — don't, since they don't change what an already-scanned slice means) | `scripts/cuna-giveaway-scan-test.cjs`, new section "reconfiguring mid-scan supersedes it too": pauses a scan mid-await under the old config, reconfigures to a new mint/window while it's suspended, releases it, asserts `superseded:true`, zero credited from the stale slice, and the cursor reflecting the NEW config's `startMs` |
| 3 | **P3** — `--url-query`/`--url-query=` (curl always appends these to the URL's query, independent of method) were unrecognised long flags and silently ignored | `computeEffectiveMethod()` now recognises both forms, pushes the value into `dataValuesOut`, and returns a new `hasUrlQueryData` flag alongside `forceGet` | Same test file: `curl --url-query draw=1 …/admin?key=k` and the `--url-query=draw=1` inline form both block |
| 4 | **P3** — value-taking short flags `-D`/`-r`/`-Y`/`-Q`/`-C`/`-t`/`-P` (dump-header file, range, speed-limit, quote, resume-from, telnet option, ftp-port) were missing from the opaque-value set, so e.g. `-D/dev/stderr` fell through as an unrecognised flag | Added to `OPAQUE_VALUE_FLAGS` | Same test file: `curl -D/dev/stderr ".../admin?key=k&draw=1"` blocks |
| 5 | **P3** — two fail-open gaps in the `-G` query reconstruction: (a) a data value curl reads from a file (`@file`, `name@file`) has unknown contents — assuming it's safe because its literal text has no mutating flag is exactly backwards; (b) the reconstructed-query check only ran when the FINAL resolved method was GET, so `-I -G -d draw=1` / `--head --get --data draw=1` / `-X HEAD -G -d draw=1` (method resolves to HEAD, but `-G` still moves the data onto the URL) slipped through | (a) any collected data value containing `@` fails CLOSED — treated as mutating unconditionally, never pattern-matched. (b) `computeEffectiveMethod()` now returns `{method, forceGet, hasUrlQueryData}`; `main()` rebuilds and tests the query whenever `forceGet \|\| hasUrlQueryData` is true, regardless of what `method` ends up resolving to | Same test file: `-G -d @payload.txt` and `-G -d name@payload.txt` both block (fail-closed); `-I -G -d draw=1`, `--head --get --data draw=1`, `-X HEAD -G -d draw=1` all block |
| 6 | **P3** — `lib/cuna-giveaway.js` `scanOnce()`'s deadline-timeout path folded itself into `s.incompleteSlices` and `stalled:true`, the same signal as a genuine tape fault — an operator watching a scan hit its OWN budget would be told "the tape is stalled, go look at Helius" for a cutoff that had nothing to do with the tape | A deadline timeout now has its own counter (`s.deadlineHits`) and its own result flag (`deadline:true`); it no longer touches `incompleteSlices` or sets `stalled`. Cursor treatment is unchanged (never advances past the cut-off slice either way) | Same scan test file, new section "a deadline cutoff is reported separately from a tape stall": a GENUINE real-wall-clock race (a new `realDelayMs` test knob, distinct from the existing fake-clock `clockStepMs`) actually loses the `Promise.race` and asserts `deadline:true`, `deadlineHits` moved by exactly one, `stalled:false`, and `incompleteSlices` unchanged — by delta, since `resetLedger()` deliberately leaves this historical counter alone, same as `incompleteSlices` |
| 7 | **P3** — the generation-counter fix (#1 above) reads as protecting every rewind/reset/reconfigure against a racing scan; it does not say what it does NOT cover | One comment at the generation check in `scanOnce()`: `gen` lives in this PROCESS's in-memory `_mem` and only protects a same-process race — Railway runs more than one process, and a scan's `save()` on process A can still overwrite a rewind process B wrote moments earlier (an overlapping deploy is the likeliest window), since A's `gen` was read before B's write landed. That cross-process race is not closed by this fix | Comment only, no behaviour change — nothing to pin beyond the existing single-process tests already passing |

Also re-run: `node --check` on every touched file, and `scripts/mutating-get-guard-test.cjs`
(unchanged — still no route touched). Where to look hardest: whether `dataValuesOut` should also
capture `-F`/`--form*` values for the `@`-fail-closed check (currently any `@` anywhere in a
collected value blocks, which already covers `-F name=@file` since that value is pushed the same
way) — say if you find a data-flag form whose value reaches curl without ever passing through
`dataValuesOut` at all.
