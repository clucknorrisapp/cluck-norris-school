...
30's instruction-semantics verifier and the `processed`-never-expired fix, still on the same money
path.

| # | Finding | Fix | Pinned by |
|---|---|---|---|
| 1 | **P1** — round 30's verifier checked every instruction's OWN fields but never a transaction's TOTAL effect: `routeTail` was simply OVERWRITTEN by whichever Jupiter instruction came last, so a second route instruction rode along unnoticed as long as the last one matched the quote; and each System `Transfer`'s amount was capped individually against the quote's `inAmount`, so two transfers each under the cap could SUM past it | `src/seeker/swap-verify.js` now enforces four limits TRANSACTION-WIDE, after/alongside the per-instruction loop rather than per instruction: exactly ONE Jupiter-program instruction of any kind (a second refuses immediately, before its shape is even decoded, as "more than one swap instruction"); the SUM of every System `Transfer`'s lamports checked ONCE against the quote's `inAmount` (and refused outright — any amount — the instant one appears while the quote's input isn't SOL); at most one wSOL `SyncNative` and at most one `CloseAccount`; at most one Associated-Token-Account create per target ATA address (i.e. per mint this swap actually touches, tracked in a `Set`) | `scripts/seeker-swap-verify-test.cjs` §4: Codex's exact case — a duplicated Jupiter route instruction → refused, never signed; two wrap transfers each individually ≤ `inAmount` but SUMMING above it → refused; a System transfer present when the quote's input isn't SOL → refused; a second `SyncNative`/`CloseAccount`/ATA-create-for-the-same-mint → each refused |
| 2 | **P1** — the round-30 priority-fee ceiling check only ran when the transaction carried BOTH a `SetComputeUnitLimit` and a `SetComputeUnitPrice`; Codex's exploit dropped the limit instruction and set the price to 1,000,000,000 micro-lamports/CU, and the ceiling check silently never ran at all. The math was also truncating (`/1000000n`) rather than the ceiling Solana actually charges | A transaction with no `SetComputeUnitLimit` gets Solana's own runtime default (solana.com/docs/core/fees/fee-structure, "Compute Budget"): 200,000 CU per non-ComputeBudget instruction, capped at 1,400,000 CU total — computed from the instruction count and used as `cuLimit` for the ceiling math whenever the caller passed a ceiling to check, whether or not either ComputeBudget instruction was present; no `SetComputeUnitPrice` defaults the price to 0. The fee is now `ceil(cuLimit × cuPriceMicroLamports / 1e6)` — BigInt CEILING division (`(a*b + 999_999n) / 1_000_000n`), never truncating, so a fee that rounds up past the ceiling by even one lamport still refuses. A second `SetComputeUnitLimit` or `SetComputeUnitPrice` in the same transaction also refuses outright | `scripts/seeker-swap-verify-test.cjs` §5: Codex's exact case — the limit instruction removed and the price set to 1e9 micro-lamports/CU → refused (the runtime default still catches it); the recorded fixture's real fee recomputed with ceiling division (441,362, not the truncated 441,361) — a ceiling of 441,361 (one lamport below the true ceiling-divided fee) refuses, 441,362 passes; a duplicated `SetComputeUnitLimit` → refused |
| 3 | **P2** — a pane whose `onSigned` write throws (a full/unavailable `localStorage`) had the failure silently swallowed and the swap still submitted anyway — for the Swap pane specifically, `onSigned`'s write is the ONLY record that lets a person resume checking an in-flight signature, so a write that failed left them with a broadcast transaction and no way to look it up again | `signSendConfirm` (`sign.js`) gains `requireOnSigned: true`: on the `signTransaction` path (the one path where nothing has reached a node yet when `onSigned` runs), an `onSigned` that throws OR returns `false` STOPS the submission before `submitSigned` is ever called and returns `{status:"failed", error:"Could not save the recovery record — nothing was sent. Free some storage and try again.", sig}` — the signature is still reported (it's real, just never broadcast) so it could be looked up. The `signAndSendTransaction` path is unaffected by `requireOnSigned` — that provider has already broadcast by the time `onSigned` runs, so there is nothing left to stop. `Swap.jsx` passes `requireOnSigned: true`, and its `savePending()` now THROWS on a failed write: it writes, reads the value back, and throws if the read-back doesn't match byte-for-byte — never assuming a `setItem` call that didn't throw actually wrote anything | `scripts/seeker-sign-versioned-test.cjs` §9: Codex's exact case — an `onSigned` that throws "storage unavailable" with `requireOnSigned: true` → the fake `rpc`'s `sendTransaction` is NEVER called, the result is `failed` with the recovery-record message, carrying the local signature; the same case with `onSigned` returning `false` instead of throwing; a legacy caller passing no `onSigned` at all (Firepit/ProjectBurn/LockerRoom's shape) → unaffected even with `requireOnSigned: true` set |
| 4 | **P2** — `checkPendingSwap`'s final check treated ANY falsy status entry the same as an explicit, well-formed `null` — `{}`, `{value:[]}`, a non-array `value`, or a thrown-then-caught RPC read all collapsed to the same "truly nothing, ever" branch and were reported `expired` once the block height had passed, even though none of them are an actual answer from the node | `checkPendingSwap` (`sign.js`) now only calls a status "well-formed null" — the ONLY shape allowed to ever become `expired` — when `result.value` is an array with an entry at index 0 that is EXACTLY `null` (`isWellFormedNullStatus`). Every other shape (missing/empty/non-array `value`, `null`/`undefined` result, a thrown RPC error) falls through `statusOutcome`'s own `null` return and stays `pending` on both the first and the final check — never guessed as `expired`, `sent`, or `failed` | new case in `scripts/seeker-pending-swap-test.cjs` §7: Codex's three malformed shapes (`{}`, `{value:[]}`, `null`/`undefined`) plus a non-array `value`, each with the height already past `lastValidBlockHeight` → every one stays `pending`, never `expired`; an RPC error on the very first check → `pending`; controls confirming a genuinely well-formed `null` → `expired` and a well-formed `confirmed` → `sent` still work exactly as round 30 left them |

Where to look hardest:

- **The runtime compute-unit default.** `min(200000 × nonComputeBudgetInstructions, 1400000)` is
  read off solana.com/docs/core/fees/fee-structure's stated per-instruction default and overall
  cap — has that default actually changed since, and does the runtime count ComputeBudget
  instructions themselves toward the "per instruction" multiplier the way this file assumes it
  does not?
- **Whether Jupiter ever legitimately emits more than one route instruction.** This round's fix 6
  hard-refuses a second Jupiter-program instruction of ANY kind in the same transaction. Every
  fixture and every documented route shape we've seen has exactly one — is there a real route
  (a split-route across two legs each needing its own `route` call, some multi-hop shape the
  fixture never exercised) that legitimately needs two, which this now blocks outright rather than
  verifying?
- **Whether `requireOnSigned` can strand a person whose storage genuinely is full.** The message
  ("Free some storage and try again") tells them what to do, but there is no in-app storage
  clearer — does a person on a truly full device have any real path forward besides manually
  clearing site data, and does the pane make that obvious enough?
- **Any other status-response shape `checkPendingSwap` hasn't been tested against** — a `value`
  array with MORE than one entry (a batched multi-signature response this call never sends but a
  proxy could still return), a status object with `confirmationStatus` set to something outside
  the four known strings, or `err` present alongside a non-object shape.
