...
Why I want to work on this: the retail wave arriving through Solana launchpads is full of first-time users who don't know what a token approval is or what a rug looks like — and Solana's fees are low enough that safety can be taught by doing, not by slideshow. The ecosystem above is fully built, live in production, free, and on the Solana Seeker dApp Store. What it needs now is not construction but reach — this application is a plan to introduce more people to it. Our growth strategy is the product itself: every free tool we give another project — locks, holder snapshots, buy competitions — puts the school in front of that project's whole community, and we're extending the same idea to new kinds of first touch, including a retro browser game (now in private playtest with a partner token community) built to meet people where they play and walk them into the free education layer.
...
Requesting $6,000 total, milestone-based. The product is fully built, so the grant pays the real operating bill of giving it away free at greater scale — RPC infrastructure (Helius), AI inference for the free tutor and the machine-translation layer (Anthropic's Claude API), text-to-speech and translation services for the multilingual school, and AI media tooling for social content in every language we teach in. No salary, no runway: every dollar maps to a cost of adoption. Every completion criterion is verifiable by the Foundation without trusting us: product work ships in the MIT-licensed public repository, and adoption numbers come from on-chain data, public pages the Foundation can enumerate itself, or a third-party analytics dashboard shared read-only.

$1,500 due upon completion of Milestone 1 deliverable (Product for growth — target: month 2): (a) every Locker Room lock transaction is tagged with an on-chain attribution memo, making third-party use of the free locker permanently countable by anyone with a block explorer — this shipped ahead of this application, so the counting mechanism for Milestone 3 is already live; (b) the school's full translation layer extended to two additional languages (seven → nine), live with text-to-speech; (c) the crawlable curriculum layer and shareable outputs (verifiable transcripts, Lock of Fame reports) expanded so every learner artifact is a public, linkable page. Verification: all live in production, visible in the public repo's commit history. Funds cover the translation and text-to-speech service costs of the two new language packs (translation software, native-speaker review, speech synthesis) and the RPC/AI infrastructure behind the new content.

$2,250 due upon completion of Milestone 2 deliverable (Adoption — people — target: month 4): 2,500 unique monthly visitors and 1,500 monthly free-tool runs across the school and tools, measured by Google Analytics (already installed) through a read-only dashboard shared with the Foundation (not self-reported summaries). Funds cover the outreach that causes those numbers — AI-generated multilingual social content (video and imagery produced with media tooling like Higgsfield, in every language the school teaches), translated launch pushes, and content collaborations with Solana communities — plus the marginal infrastructure of serving free users at that scale (Helius RPC, Claude-powered tutor and translation inference, text-to-speech).
...
- We practice what we teach: over 40% of our own token's supply — more than 400 million tokens — is locked across 52 on-chain escrows, publicly visible in the Lock of Fame we built — the same standard of public verifiability the milestones above use.
- The app ships in seven languages with text-to-speech and has shipped continuously since April 2026 — the public commit history shows the cadence. An active Telegram community keeps the free AI tutor answering learner questions in-chat daily.
- Web traffic is the early metric, and we're straightforward about that: the product's breadth is ahead of its audience, which is precisely what this application funds. We've just instrumented the site with Google Analytics, and Milestone 2's visitor and tool-run targets are measured on that instrumentation through a read-only dashboard shared with the Foundation — we'd rather grow a number the Foundation can watch than quote one it can't verify.
