...
// A row is only ever { t, action, reason, price } in production (recordDecision's allow-list,
// above) — never enough to rebuild rollGate's or buybackDecision's full input set. This is
...
async function timeline({ vault, kv, clknMint, id, hours, helius }) {
  ...
  for (const r of replayDecisions(decisionRows)) {
    rows.push({ t: r.t, cls: "decision", action: r.action, reason: r.reason, price: r.price,
