...
// Reads the kv ring `engineLog:<project>` written by the per-tick choke points in
// lib/whirlpool-vault.js (the `tick`/`tickAskWall`/`tickSol`/`tickBtc`/`tickJup`/`tickTreasury`/
// `concentrate` wrappers — see recordDecision there). A local kv read, no network, no memo
// needed. While an engine is paused, a tick returns { action: "none", reason: "paused" } BEFORE
// doing anything and recordDecision() deliberately never logs that — so an empty log on a paused
// project means exactly what it looks like, and says so instead of reading as missing data.
// `source` names which sleeve made the call (`"base"`, `"askWall"`, `"sol"`, `"btc"`, `"jup"`,
// `"treasury"`, `"concentrate"`) — null on rows retained before batch 8 added it, never fabricated.
...
// An illustrative scenario is dated to its incident WINDOW ("2026-08-27/28"), not a moment —
// that is as precise as the record gets (see that file's header). For the merged timeline it
// still needs a single sortable `t`; take the first day of the window at 00:00 UTC, which is
// as precise a claim as the label already makes and never a false "just now".
function incidentWindowTs(win) {
  const m = /^(\d{4})-(\d{2})-(\d{2})/.exec(String(win || ""));
  return m ? Date.UTC(Number(m[1]), Number(m[2]) - 1, Number(m[3])) : null;
}

// ── X5: replay a retained decision row against the real pure gates ──────────────
// A row is only ever { t, action, reason, price, source } in production (recordDecision's
// allow-list, above) — never enough to rebuild rollGate's or buybackDecision's full input set. This is
// deliberate, not a gap to patch: the pure gates are documented in lib/engine-decisions.js as
// needing wallet floats, staged/idle inventory, range position and day counters that never
// flow into a tick() result in the first place, so replay is honest about the ceiling — most
// real rows report "inputs not retained" rather than a fabricated match. What CAN be replayed:
// a row that itself carries a full input set (an explicit `gate` plus that gate's own
// parameters) — exercised by scripts/jvp-dashboard-test.cjs against lib/engine-sim-scenarios.js,
// where the inputs are retained by construction and the replay is expected to match.
const ROLL_ONLY_ACTIONS = new Set(["hold", "dwell", "roll"]);
const BUYBACK_ONLY_ACTIONS = new Set(["buy", "none"]);
const ROLL_GATE_KEYS = ["cfg", "nowMs", "frac", "sinceLastRollSec", "dayActions", "deployStagedUsd", "idlePairUsd", "widthOffPct"];
const BUYBACK_GATE_KEYS = ["cfg", "st", "float", "nowMs", "todayStamp"];
function inferGate(row) {
  if (row.gate === "rollGate" || row.gate === "buybackDecision") return row.gate;
  if (ROLL_ONLY_ACTIONS.has(row.action)) return "rollGate";
  if (BUYBACK_ONLY_ACTIONS.has(row.action)) return "buybackDecision";
  // "capped"/"deferred" are ambiguous by action alone — both gates return them — so fall back
  // to whichever gate's own inputs the row actually carries.
  if (row.frac !== undefined || row.deployStagedUsd !== undefined || row.idlePairUsd !== undefined || row.widthOffPct !== undefined) return "rollGate";
  if (row.float !== undefined || row.st !== undefined || row.fromSym !== undefined) return "buybackDecision";
  return null;
}
function hasAll(row, keys) { return keys.every((k) => row[k] !== undefined); }
function replayOne(row) {
  const gate = inferGate(row);
  try {
    if (gate === "rollGate" && hasAll(row, ROLL_GATE_KEYS)) {
      const replayed = rollGate({ cfg: row.cfg, nowMs: row.nowMs, frac: row.frac, oorSince: row.oorSince === undefined ? null : row.oorSince,
        sinceLastRollSec: row.sinceLastRollSec, dayActions: row.dayActions, deployStagedUsd: row.deployStagedUsd, idlePairUsd: row.idlePairUsd, widthOffPct: row.widthOffPct });
      return { gate, replayed, matches: replayed.action === row.action };
    }
    if (gate === "buybackDecision" && hasAll(row, BUYBACK_GATE_KEYS)) {
      const replayed = buybackDecision({ cfg: row.cfg, st: row.st, float: row.float, prices: row.prices === undefined ? null : row.prices, nowMs: row.nowMs, todayStamp: row.todayStamp });
      return { gate, replayed, matches: replayed.action === row.action };
    }
  } catch (e) { return { gate, replayed: null, note: "replay failed" }; }
  return { gate: gate || null, replayed: null, note: "inputs not retained" };
}
// Pure — never calls the vault, never reads live state, never starts anything.
function replayDecisions(rows) {
  return (rows || []).map((row) => Object.assign({}, row, replayOne(row)));
}

// ── X5: the merged timeline — three evidence classes, one time-ordered array ─────
// Never mixed silently: every row keeps its class in `cls` and exactly the fields that class
// already exposes elsewhere on this page (plus `t`/`cls` themselves, and the decision replay,
// which this function attaches). Illustrative rows are timestamped by their incident date so
// they take their real place in the order — never rendered as if they just happened.
const TIMELINE_CAP = 300;
async function timeline({ vault, kv, clknMint, id, hours, helius }) {
  const all = vault.listProjects();
  const project = all[id];
  if (!project) return null;
  const hrs = Math.max(1, Math.min(720, Number(hours) || 168));
  const cutoffMs = Date.now() - hrs * 3600e3;
  let status = null;
  try { status = await memo("status:" + id, TTL.status, () => vault.status(id)); } catch (_) { status = null; }
  const paused = !!(status && status.paused);
  const xfer = await operatorHistory({ helius, vault, id, project }).catch(() => null);
  const dLog = decisionLog(kv, id, paused);

  const rows = [];
  for (const r of (xfer && xfer.rows) || []) {
    if (r.t && r.t < cutoffMs) continue;
    rows.push({ t: r.t, cls: "transfer", direction: r.direction, symbol: r.symbol, amount: r.amount, signature: r.signature });
  }
  const decisionRows = (dLog.rows || []).filter((r) => !r.t || r.t >= cutoffMs);
  for (const r of replayDecisions(decisionRows)) {
    rows.push({ t: r.t, cls: "decision", action: r.action, reason: r.reason, price: r.price, source: r.source || null,
      gate: r.gate || null, replayed: r.replayed || null, matches: typeof r.matches === "boolean" ? r.matches : null, note: r.note || null });
  }
  // Illustrative rows are the fixed reference table — always shown regardless of the `hours`
  // window, since they are not an observation period being windowed, they're a constant.
  for (const s of simulatorScenarios()) {
    rows.push({ t: incidentWindowTs(s.incidentWindow), cls: "illustrative", id: s.id, name: s.name, incidentWindow: s.incidentWindow,
      description: s.description, inputs: s.inputs, decision: s.decision,
      label: "illustrative — replayed from a recorded incident, not live" });
  }
  rows.sort((a, b) => (b.t || 0) - (a.t || 0));
  const capped = rows.slice(0, TIMELINE_CAP);
  const classes = { transfer: 0, decision: 0, illustrative: 0 };
  for (const r of capped) classes[r.cls] = (classes[r.cls] || 0) + 1;
  return {
    rows: capped, classes, hours: hrs,
    transfersAvailable: !!(xfer && xfer.rows),
    transfersFreshness: xfer ? freshnessOf(xfer) : { observedAt: null, stale: true, state: "unavailable" },
    decisionRetained: dLog.retained, decisionNote: dLog.note || null,
  };
}
...
module.exports = {
  ...
  // X5 — the merged timeline and replay
  timeline, replayDecisions, incidentWindowTs,
};
