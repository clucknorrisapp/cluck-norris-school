...
async function runCheckup(canonical, { sources, checks = [], deps, tier = "preview", concurrency = 4, hashLogo, howToList = null } = {}) {
  ...
  async function worker() {
    while (i < wanted.length) {
      ...
      try {
        ...
        if (r && r.extra && typeof r.extra === "object") row.extra = r.extra;   // adapter side-facts (authorities, LP lock %) the checks read — never compared
        if (row.found) {
          let logoHashes = null;
        ...
        if (row.status === "not-found" && howToList && howToList[src.id]) row.howToList = howToList[src.id];   // what the site says it needs + its form
      } catch (e) {
        row.status = "unread";
  ...
  // Batch A checks (lib/listing-checkup-checks.js): each answers one question from the canonical
  // record and the rows above. A throwing check is `unread` with its error — never a 500.
  const checksOut = {};
  const wantedChecks = (checks || []).filter((c) => tier === "full" || c.tier === "preview");
  await Promise.all(wantedChecks.map(async (c) => {
    try { checksOut[c.id] = { id: c.id, label: c.label, ...(await c.run(canonical, deps, { rows, tier })) }; }
    catch (e) { checksOut[c.id] = { id: c.id, label: c.label, status: "unread", error: String(e && e.message || e).slice(0, 200) }; }
  }));
  ...
  return { chain: "solana", mint: canonical.mint, canonical, tier, sources: rows, summary, fixFirst, checked: wanted.length, checks: checksOut };
}
