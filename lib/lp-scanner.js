...
const BASE58 = require("./solana-addr").SOL_ADDR_RE;
...
async function jupList() {
  ...
  try {
    ...
    const r = await fetch((_jk ? "https://api.jup.ag" : "https://lite-api.jup.ag") + "/tokens/v2/tag?query=verified", { headers: _jk ? { "x-api-key": _jk } : {}, signal: AbortSignal.timeout(15000) });
...
async function cgFetch(sub) {
  ...
  // Free GeckoTerminal is ~30 req/min — since the CG-API divorce (2026-07-03) ALL scanner
  // traffic lands here, and the burst fan-outs can trip 429s. One polite retry honoring
  // Retry-After keeps a rate blip from surfacing as a user-visible route error.
  for (let attempt = 0; ; attempt++) {
    ...
    if (r.status === 429 && attempt < 2) { // two patient retries — operator tool, slow beats failing
      const waitSec = Math.min(30, Number(r.headers.get("retry-after")) || 15);
...
// Concurrency-capped map — the free-GT-friendly replacement for wide Promise.all fan-outs
// (25 parallel OHLCV reads = instant 429 on the free tier; ~4 at a time stays under it).
...
async function cgPro(path) {
  // Pro host only when a key exists AND works; otherwise the FREE public host — same endpoints
  // for everything we use (simple/price, coins/{id}, search/trending, coins/markets), just
  // lower rate limits (fine: all callers are cached/daily). The $129/mo Analyst sub was
  // cancelled 2026-07-03 (owner's call after the CoinGecko listing rejection) — the key may
  // linger in env until expiry, so a dead key must fall through, not error.
  const key = process.env.COINGECKO_API_KEY;
  if (key) {
    try {
      const r = await fetch(`https://pro-api.coingecko.com/api/v3${path}`,
        { headers: { Accept: "application/json", "x-cg-pro-api-key": key }, signal: AbortSignal.timeout(12000) });
      if (r.ok) return r.json();
    } catch (_) { /* fall through to the free host */ }
  }
...
// FluxBeam, Saros (and other SPL-Token-Swap forks): canonical token-swap state layout (len 324) — the
// LP trade fee is tradeFeeNumerator(u64 @227)/tradeFeeDenominator(u64 @235). owner/host fees are
// protocol-side and don't affect LP yield, so we read only the trade fee. Confirmed against live
// pools (curveType @291 == 0 = ConstantProduct validates the layout). Sanity-bounded 0–10%.
async function splTokenSwapFeePct(poolAddress, conn) {
  try {
    const { PublicKey } = require("@solana/web3.js");
    const info = await conn.getAccountInfo(new PublicKey(poolAddress));
    if (!info || !info.data || info.data.length < 324) return null;
    const num = Number(info.data.readBigUInt64LE(227)), den = Number(info.data.readBigUInt64LE(235));
    if (!den || num <= 0) return null;
    const pct = num / den * 100;
    if (!isFinite(pct) || pct <= 0 || pct > 10) return null;
    return Number(pct.toFixed(4));
  } catch (_) { return null; }
}

// PancakeSwap v3 + Byreal (Solana) are Raydium-CLMM forks (identical PoolState layout, disc f7ede3f5…):
// the pool stores its AmmConfig pubkey at offset 9, and the AmmConfig holds trade_fee_rate
// (u32, 1e-6 units) at offset 47 → % = rate / 10000. Two on-chain reads. (validated on live pools)
async function raydiumClmmForkFeePct(poolAddress, conn) {
  try {
    const { PublicKey } = require("@solana/web3.js");
    const pool = await conn.getAccountInfo(new PublicKey(poolAddress));
    if (!pool || !pool.data || pool.data.length < 41) return null;
    const cfg = await conn.getAccountInfo(new PublicKey(pool.data.slice(9, 41)));
    if (!cfg || !cfg.data || cfg.data.length < 51) return null;
    const tradeFeeRate = cfg.data.readUInt32LE(47);
    if (!tradeFeeRate) return null;
    return Number((tradeFeeRate / 10000).toFixed(4));
  } catch (_) { return null; }
}

// Meteora DAMM v2 (cp-amm) fee via the official SDK. The base fee can be static OR a launch
// scheduler that DECAYS from a high anti-sniper fee down to a settled floor. We read the
// MIN (settled/floor) base-fee numerator — correct for static pools (min==max) and the
// honest, never-overstate number for a decaying pool that's past its launch window.
// numerator / FEE_DENOMINATOR (1e9) → %. Lazy-loaded; null on any failure.
async function meteoraDammV2FeePct(poolAddress, conn) {
  try {
    const mod = require("@meteora-ag/cp-amm-sdk");
    const { PublicKey } = require("@solana/web3.js");
    const cp = new mod.CpAmm(conn);
    const st = await cp.fetchPoolState(new PublicKey(poolAddress));
    const handler = mod.getBaseFeeHandlerFromPodAlignedData(st.poolFees.baseFee.baseFeeInfo.data);
    const minN = Number(handler.getMinFeeNumerator().toString());
    const DEN = Number(mod.FEE_DENOMINATOR) || 1e9;
    if (!isFinite(minN) || minN <= 0) return null;
    return Number((minN / DEN * 100).toFixed(4));
  } catch (_) { return null; }
}
...
const READABLE_DEXES = new Set(["meteora", "meteora-damm-v2", "orca", "raydium", "raydium-clmm", "raydium-cpmm", "pancakeswap-v3-solana", "byreal", "fluxbeam", "saros-amm"]);

// Fee-tier dispatcher by DEX. Precise on-chain/API readers (most accurate): Meteora DLMM (SDK),
// Orca (on-chain), Raydium (API), PumpSwap (documented constant). To cover EVERY OTHER venue a
// normal user can LP (Meteora DAMM v2, Lifinity, Invariant, Crema, Saber, FluxBeam, Saros, …) we
// need a per-DEX reader each — there is NO universal shortcut: verified 2026-06-15 that neither
// GeckoTerminal nor CoinGecko Pro populates pool_fee_percentage for Solana pools (field exists,
// always empty). The pool.gtFeePct fallback below is kept harmless/future-proof in case CG ever
// fills it, but today it's a no-op, so unsupported DEXs honestly show "—" until a reader is added.
async function feePctForPool(pool, conn) {
  const dex = pool.dex || "";
  let fee = null;
  if (dex === "meteora") fee = await meteoraFeePct(pool.address, conn);
  else if (dex === "orca") fee = await orcaFeePct(pool.address, conn);
  else if (dex.startsWith("raydium")) fee = await raydiumFeePct(pool.address);
  else if (dex === "pumpswap") fee = PUMPSWAP_LP_FEE_PCT;
  else if (dex === "meteora-damm-v2") fee = await meteoraDammV2FeePct(pool.address, conn);
  else if (dex === "pancakeswap-v3-solana" || dex === "byreal") fee = await raydiumClmmForkFeePct(pool.address, conn);
  else if (dex === "fluxbeam" || dex === "saros-amm") fee = await splTokenSwapFeePct(pool.address, conn);
  if (fee != null) return fee;
  // Universal fallback: GeckoTerminal's per-pool fee % (captured in shapePool), when sane.
  const gt = Number(pool.gtFeePct);
  if (isFinite(gt) && gt > 0 && gt < 50) return Number(gt.toFixed(4));
  return null;
}
...
function shapePool(p, symById) {
  ...
  return {
    ...
    // GeckoTerminal's reported swap-fee % for this pool (Pro key fills it for most DEXs).
    // Used as the universal fee fallback for any venue we don't read precisely on-chain.
    gtFeePct: (a.pool_fee_percentage != null && isFinite(Number(a.pool_fee_percentage))) ? Number(a.pool_fee_percentage) : null,
...
async function topPools(opts = {}) {
  ...
  if (kind === "trending") {
    ...
    const lists = await mapLimit([...READABLE_DEXES], 4, (dx) =>
...
async function poolDeepDive(poolAddress, opts = {}) {
  ...
  const conn = connection();
  const gtFeePct = (a.pool_fee_percentage != null && isFinite(Number(a.pool_fee_percentage))) ? Number(a.pool_fee_percentage) : null;
  const fee = await feePctForPool({ dex, address: poolAddress, gtFeePct }, conn);
...
async function debugCg() {
  ...
  try {
    const url = "https://pro-api.coingecko.com/api/v3/onchain/networks/solana/tokens/JUPyiwrYJFskUPiHa7hkeR8VUtAeFoSYbKedZNsDvCN/pools?include_fee=true";
    const r = await fetch(url, { headers: { Accept: "application/json", "x-cg-pro-api-key": key }, signal: AbortSignal.timeout(12000) });
    let feeSamples = null;
    try { const j = await r.json(); feeSamples = (j.data || []).slice(0, 6).map((p) => ({ dex: p.relationships?.dex?.data?.id, fee: p.attributes?.pool_fee_percentage })); } catch (_) {}
    return { keySet: true, proStatus: r.status, proOk: r.ok, using: r.ok ? "CoinGecko Pro onchain" : "free fallback (Pro rejected)", feeSamples };
...
// cgFetch/gtSearchTokens are exported so the rest of the app (token research, token-market, …)
