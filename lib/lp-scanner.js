...
async function topPools(opts = {}) {
  ...
  // Query the TOP pools of each real concentrated-LP DEX directly (GeckoTerminal per-DEX
  // listing), so we get the busiest pools on the venues we can price — instead of paging
  // through chain-wide volume that's dominated by pumpswap/manifest memecoin AMMs we can't
  // read. Merge, DEDUPE to distinct PAIRS (highest-volume pool per pair), rank, enrich.
  ...
  const symById = Object.assign({}, ...lists.map(symMapFromIncluded));
  const byPair = new Map(); // mint-pair key -> highest-volume shaped pool
  for (const j of lists) for (const p of (j.data || [])) {
    const shaped = shapePool(p, symById);
