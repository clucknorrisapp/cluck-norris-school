...
// feeTier/fees/yield/concentration are filled by later phases (left null here).
async function scanPair(tokenA, tokenB, opts = {}) {
  ...
  pools.sort((x, y) => y.turnover24h - x.turnover24h);
  return {
    pair: `${tokenA}/${tokenB}`.toUpperCase(),
    mintA, mintB, count: pools.length, pools,
    note: "Ranked by 24h turnover. Fee tier & yield (the money metric) land in phase 2 — turnover alone overstates low-fee/high-volume pools.",
