...
// FluxBeam (and other SPL-Token-Swap forks): canonical token-swap state layout (len 324) — the
...
const READABLE_DEXES = new Set(["meteora", "meteora-damm-v2", "orca", "raydium", "raydium-clmm", "raydium-cpmm", "pancakeswap-v3-solana", "fluxbeam"]);
...
async function feePctForPool(pool, conn) {
  ...
  else if (dex === "fluxbeam") fee = await splTokenSwapFeePct(pool.address, conn);
