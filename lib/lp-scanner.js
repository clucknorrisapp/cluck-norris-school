...
async function topPools(opts = {}) {
  ...
  const result = {
    ...
    note: kind === "trending"
      ? "The hottest Solana pairs right now (GeckoTerminal trending, 24h) across every DEX — memecoins included. Real fee yield where we can price the DEX on-chain (Meteora/Orca/Raydium), honest '—' otherwise. Ranked by yield then volume. Informational only — NOT financial advice."
