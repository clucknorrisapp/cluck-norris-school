...
// TOP POOLS — the busiest Solana pools across every DEX, refreshed hourly. Discovery surface:
// "where's the action right now, and which of the busiest pools actually pay?" Ranked by real
// fee yield (among the top-volume set) where the fee tier is read on-chain, else turnover.
let _topCache = { data: null, ts: 0 };
const TOP_TTL = 55 * 60 * 1000; // ~hourly
async function topPools(opts = {}) {
  const limit = Math.min(25, Math.max(5, Number(opts.limit) || 14));
  if (!opts.force && _topCache.data && Date.now() - _topCache.ts < TOP_TTL) return _topCache.data;
  // Pull several pages of the highest-volume pools, keep only the DEXs whose fee we can read
  // on-chain (the real concentrated-LP venues — so every row shows a REAL yield, not "—" noise
  // from pumpswap/manifest memecoin AMMs), then DEDUPE to distinct PAIRS (highest-volume pool
  // per pair) so it reads as a clean top-pairs board instead of 4× the same ticker.
  const pages = await Promise.all([1, 2, 3].map((pg) =>
    gt(`/pools?sort=h24_volume_usd_desc&include=base_token,quote_token&page=${pg}`).catch(() => ({ data: [] }))));
  const symById = Object.assign({}, ...pages.map(symMapFromIncluded));
  const byPair = new Map(); // mint-pair key -> highest-volume shaped pool
  for (const p of pages.flatMap((j) => j.data || [])) {
    const shaped = shapePool(p, symById);
    if (!shaped || !shaped.baseMint || !shaped.quoteMint) continue;
    if (!READABLE_DEXES.has(shaped.dex)) continue; // only DEXs we can price the fee for
  ...
  const result = {
    count: pools.length, pools, updatedAt: Date.now(),
    note: "The busiest Solana pools by 24h volume on the DEXs we can price on-chain (Meteora, Orca, Raydium — the real concentrated-LP venues), refreshed hourly and ranked by 7-day-avg fee yield, so you see which of the hottest pools actually PAY. One row per pair. Informational only — NOT financial advice.",
  };
  _topCache = { data: result, ts: Date.now() };
  return result;
}
...
module.exports = { scanPair, scanToken, topPools, poolDeepDive, resolveMint, searchTokens, debugFee, debugCg, debugReaders };
