...
// Fee-tier dispatcher by DEX. Precise on-chain/API readers first (most accurate): Meteora DLMM
// (SDK), Orca (on-chain), Raydium (API), PumpSwap (documented constant). For EVERY OTHER venue
// where a normal user can add liquidity (Meteora DAMM v2, Lifinity, Invariant, Crema, Saber,
// FluxBeam, Saros, …) we fall back to GeckoTerminal's reported swap-fee % — so the pool gets a
// REAL yield instead of "—". Still returns null (honest "—") when no source has a fee, and we
// never invent a number. Prop/dark AMMs (HumidiFi, SolFi, ZeroFi) and order books (Phoenix) you
// can't retail-LP simply won't carry a usable fee here, which is correct — they're not LP venues.
