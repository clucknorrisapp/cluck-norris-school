...
async function topPools(opts = {}) {
  ...
  // Pull two pages of the highest-volume pools, then DEDUPE to distinct PAIRS (keep the single
  // highest-volume pool per pair) so it reads as a clean "top pairs" board, not 4× the same pair.
  const [j1, j2] = await Promise.all([
    gt(`/pools?sort=h24_volume_usd_desc&include=base_token,quote_token&page=1`),
    gt(`/pools?sort=h24_volume_usd_desc&include=base_token,quote_token&page=2`).catch(() => ({ data: [] })),
  ]);
  const symById = { ...symMapFromIncluded(j1), ...symMapFromIncluded(j2) };
  const byPair = new Map(); // mint-pair key -> highest-volume shaped pool
  for (const p of [...(j1.data || []), ...(j2.data || [])]) {
    const shaped = shapePool(p, symById);
  ...
  const result = {
    ...
    note: "Top Solana pools by 24h volume across all DEXs, refreshed hourly. Ranked by 7-day-avg fee yield where the fee tier is read on-chain (so you see which of the busiest pools actually pay), else turnover. Informational only — NOT financial advice.",
  };
