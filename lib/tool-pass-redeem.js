 ...
 */
function redeemPaidPass({ paySig, wallet, verified, sigStore, kv, termsAt, now, usedElsewhere }) {
  ...
  if (typeof sigStore.has === "function" && sigStore.has("hub-access:" + paySig)) return { ok: false, status: 409, error: "this payment was already used for a platform-access month" };
  ...
  try { if (typeof usedElsewhere === "function" && usedElsewhere(paySig)) return { ok: false, status: 409, error: "this payment was already used for a platform-access month" }; } catch (_) { /* a broken check must not grant */ return { ok: false, status: 503, retry: true, error: "could not check the payment against the platform ledger — try again" }; }
