...
// Originally this protected the Ultimate Challenge answer key; that exam was removed
// on 2026-07-29, so nothing is gated on these answers anymore. The exclusion stays
// anyway — a crawlable answer key would let a learner skip the thinking the quiz is
// there to provoke. Question TEXTS alone are included (they're already in the public
// JS bundle, and they're good search surface).
//
...
<p>Every lesson below is free at <a href="/school">the interactive school</a> — with quizzes, read-aloud audio, and translations in English, Español, Italiano, Português, Tiếng Việt and 中文. Finish the course and earn a <a href="/school">permanent on-chain transcript</a>. This page is the plain-text course outline.</p>
...
// Lesson counts, read from the SAME arrays the app ships. The /education hub page
// advertises "7 lessons", "12 classes", "14 lessons" — hand-typing those is exactly
// how the landing page ended up claiming 72 exams against a 70-question curriculum,
// so they are injected at render time instead. Cached like render(); returns nulls
// if extraction fails, and the page then omits the numbers rather than lying.
let _counts;
function counts() {
