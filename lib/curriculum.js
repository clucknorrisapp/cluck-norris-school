...
function loadArray(file, name) {
  ...
  // The arrays carry edition ternaries since store-edition v1.1.0 (`STORE ? … : …`, and the LP
  // Lab / Library worked-example ticker `TOK`). This helper serves the WEBSITE edition, so the
  // literal is evaluated with STORE = false and the file's own `const TOK = …` line in scope.
  const tok = /const TOK = [^\n]+;/.exec(src);
  const arr = new Function("const STORE = false; " + (tok ? tok[0] + " " : "") + "return (" + lit + ")")();
...
// The ids of the core LESSONS array, for guards that point at a lesson from outside the school
// (the Project Hub's lesson map, Addendum C4): a renamed or deleted lesson must break the build,
// not ship a dead link. Null when extraction fails, like counts().
let _ids;
function lessonIds() {
  if (_ids !== undefined) return _ids;
  try { _ids = loadArray("src/App.jsx", "LESSONS").map((l) => l && l.id).filter(Boolean); }
  catch (_) { _ids = null; }
  return _ids;
}

module.exports = { counts, lessonIds };
