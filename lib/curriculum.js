...
// The real lesson ids the graduation-gate ledger (lib/school-progress.js) should accept —
// pulled from the same LESSONS array the client walks (App.jsx's finish() only ever fires
// lesson_complete for a LESSONS entry). Cached; null on any extraction failure so a caller
// can fall back to "don't filter" rather than breaking every learner on a parse hiccup.
let _lessonIds;
function lessonIds() {
  if (_lessonIds !== undefined) return _lessonIds;
  try {
    const arr = loadArray("src/App.jsx", "LESSONS");
    const ids = new Set(arr.map((l) => String((l && l.id) || "").toLowerCase()).filter(Boolean));
    _lessonIds = ids.size ? ids : null;
  } catch (_) {
    _lessonIds = null;
  }
  return _lessonIds;
