...
// Closed two days early on 2026-09-17 (deep dive P0-004): the bf flag has no legitimacy check, so
// a script could mark 20 lessons as "backfilled" in one second and skip the only anti-farm
// control (live marks across distinct 5-minute windows) in front of a treasury-signed diploma
// mint. Backfilled marks still COUNT toward the lesson total; they no longer waive the spread.
const BACKFILL_SUNSET = Date.UTC(2026, 8, 17); // 2026-09-17 (month is 0-based) — was 09-19
...
(function load() {
  try {
    ...
    if (fs.existsSync(FILE)) {
      ...
      if (o && typeof o === "object") sessions = o;
    }
    prune();
...
function prune() {
  ...
  if (left.length > MAX_SIDS) {
    left.sort((a, b) => (sessions[a].lastAt || 0) - (sessions[b].lastAt || 0));
...
//
// MERGE BEFORE WRITING. This file was read ONCE at boot and then rewritten wholesale every 30s
// from the in-memory map. Railway runs more than one process (kvstore.js grew an mtime refresh
// for exactly this reason), and a wholesale rewrite from a stale snapshot does not just miss the
// other process's work — it DELETES it. Learner marks a lesson on instance A; instance B's timer
// fires and writes its own copy, which never saw that mark; the mark is gone. Since this store is
// the graduation gate's evidence, the visible symptom is a learner who finished the course being
// told they have not.
//
// So: re-read the file when its mtime has moved, union it with what we hold, then write. The
// merge follows this store's own rule — FIRST SIGHTING WINS for a lesson mark, so a replay can
// never relabel genuine progression, and nothing already recorded is ever downgraded.
let fileMtime = 0;
function mergeFromDisk() {
  if (!persistent) return;
  let st;
  try { st = fs.statSync(FILE); } catch (_) { return; }        // no file yet — nothing to merge
  if (st.mtimeMs === fileMtime) return;                        // we wrote it last; already current
  let onDisk;
  try { onDisk = JSON.parse(fs.readFileSync(FILE, "utf8")); }
  catch (e) { console.warn(`[school-progress] merge read failed: ${e.message}`); return; }
  if (!onDisk || typeof onDisk !== "object") return;
  for (const sid of Object.keys(onDisk)) {
    const theirs = onDisk[sid];
    if (!theirs || typeof theirs !== "object") continue;
    const ours = sessions[sid];
    if (!ours) { sessions[sid] = theirs; continue; }
    ours.createdAt = Math.min(Number(ours.createdAt) || Infinity, Number(theirs.createdAt) || Infinity);
    ours.lastAt = Math.max(Number(ours.lastAt) || 0, Number(theirs.lastAt) || 0);
    if (!ours.wallet && theirs.wallet) ours.wallet = theirs.wallet;
    const theirMarks = (theirs.marks && typeof theirs.marks === "object") ? theirs.marks : {};
    ours.marks = ours.marks || {};
    for (const lid of Object.keys(theirMarks)) {
      const mine = ours.marks[lid], yours = theirMarks[lid];
      if (!yours) continue;
      // First sighting wins — and a live mark is never replaced by a backfill of the same lesson.
      if (!mine) { ours.marks[lid] = yours; continue; }
      if (Number(yours.t) < Number(mine.t)) ours.marks[lid] = yours;
      if (mine.bf && !yours.bf) ours.marks[lid].bf = 0;
    }
  }
}
function persist() {
  ...
  try {
    mergeFromDisk();
    require("./atomic-write").atomicWriteFileSync(FILE, JSON.stringify(sessions));
...
function mark(sid, lessonId, { backfill = false } = {}) {
  ...
  } else if (s.marks[lessonId] && !backfill) {
    // A live RE-PASS of a lesson already on record: keep the first sighting (`t`) untouched and
    // remember the latest live re-pass (`r`). Codex on #333: when a learner's marks are replayed
    // together (a flushed queue, the claim-time re-send, a reset browser id) they all land in ONE
    // five-minute window, and with first-sighting-only the spread check could never be repaired —
    // "revisit a lesson and pass its quiz again" was a promise the ledger could not keep. A re-pass
    // timestamp counts toward the spread exactly like a first live sighting would have, which is
    // no easier for a script (it could always have sent the first sighting later) and lets a real
    // learner recover by doing real lessons a few minutes apart.
    s.marks[lessonId].r = now;
  }
  dirty = true;
  return true;
}
...
function evaluate(sid, wallet, { requiredLessons = 12, minAgeMs = 15 * 60000, minSpreadBuckets = 3 } = {}) {
  ...
  // Live windows: every live first sighting PLUS every live re-pass (`r`, set on any mark — a
  // backfilled lesson re-passed live counts as live from that moment).
  const liveTimes = live.map((m) => m.t).concat(marks.filter((m) => m.r).map((m) => m.r));
  const liveBuckets = new Set(liveTimes.map((t) => Math.floor(t / 300000))).size;
...
function statusFor(sid) {
  ...
  if (!s) return null;
  const marks = Object.values(s.marks || {});
  return { createdAt: s.createdAt, lastAt: s.lastAt, wallet: s.wallet ? s.wallet.slice(0, 6) + "…" : null, lessons: marks.length, backfilled: marks.filter((m) => m.bf).length, repassed: marks.filter((m) => m.r).length };
}
