...
// so this never reaches into another pool's idle float or the SOL gas reserve, and calls
// manualSwap with the direction/amount below.
//   clknUi     — the project token freed by the close (ui units, ≥0)
...
//   maxSwapUsdPerCycle — hard cap on the swap, in USD (cfg.maxSwapUsdPerCycle)
...
function rollRebalanceDecision({ clknUi, quoteUi, price, quoteUsd, minUsd = 5, maxSwapUsdPerCycle }) {
  ...
  const capQuote = (Number.isFinite(maxSwapUsdPerCycle) && maxSwapUsdPerCycle > 0) ? maxSwapUsdPerCycle / qUsd : Infinity;
  const rawHalfQuote = Math.abs(diff) / 2;
  const halfQuote = Math.min(rawHalfQuote, capQuote);
...
// "insufficient funds" deterministically (audit F3, addLiquidity ~line 3405). Extracted
// (BULLEN production incident, 2026-09-26 — evenPools' DIRECT SHIFT add step hit exactly this
// moving into the lean pool and stranded 621 JUP) so evenPools' post-failure retry draws from
// the SAME tested arithmetic as addLiquidity's own inline clamp, instead of a second copy that
// can drift out of sync with it the way tickSol's SOL-guard copy once did.
...
module.exports = { buybackDecision, rollGate, spendableSol, rollRebalanceDecision, spendableForAdd };
