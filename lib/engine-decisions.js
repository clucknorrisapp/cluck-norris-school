...
// so scripts/engine-sim-test.cjs can replay thousands of ticks before a behavior change ships.
// Born from the 2026-08-28 retro: the decay loop, the buyback stall, and the sleeve starvation
// were all pure decision bugs — each would have shown up in a 200-tick replay that takes
// milliseconds. If you change a gate here, the simulator's scenario suite is the contract
// you're editing.
//
// ⚠️ READ THIS BEFORE TRUSTING A GREEN RUN (audit 2026-09-03). Exactly ONE of the two
// functions below is production-wired:
//   • buybackDecision — CALLED by lib/whirlpool-vault.js buyback(). Simulating it simulates
//     production.
//   • rollGate       — a SPEC. NOTHING calls it: the four sleeve ticks (tick/tickSol/tickJup/
//     tickBtc) each re-implement the roll gate inline, and they have already drifted from it
//     (near-edge trigger, ROOM gate, evenness gate; tickBtc has no OOR dwell and no 2×
//     emergency ceiling). Editing rollGate changes NOTHING that runs. The simulator pins that
//     divergence explicitly (Scenario 6) so a green run can't be mistaken for coverage of the
//     live roll logic; wiring the sleeves to call rollGate is a deliberate behaviour change,
//     not a refactor, and Scenario 6 fails the moment anyone does it.
...
// ── Sleeve roll gate: dwell / urgency / pacing / day-cap SPEC (NOT production-wired) ──
// The sleeve ticks in whirlpool-vault.js implement this SHAPE inline (base uses forceRoll
// where the sleeves use deployDriven — same "urgent" semantics) but none of them call this
// function, so a scenario passing here proves the SPEC is self-consistent, not that the live
// engine behaves this way. Scenario 6 of the simulator pins the known divergences. Keep the
// inline gates and this spec in lockstep when either changes.
