...
async function recordDrop({ kv, dropId, mint, decimals, createdAt, rows, operator, getTx, now }) {
  ...
  const firstIndexOfSig = new Map(); // the same signature twice in one call is answered once
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (firstIndexOfSig.has(sig)) { results[i] = { dupOf: firstIndexOfSig.get(sig) }; continue; }
    firstIndexOfSig.set(sig, i);
  ...
  for (const c of candidates) {
    // Another call landed this signature on THIS receipt while we were reading the chain.
    if (fresh.rows[c.sig]) { results[c.i] = { wallet: fresh.rows[c.sig].wallet, sig: c.sig, amount: fresh.rows[c.sig].amount, verified: true, reason: null, alreadyRecorded: true }; continue; }
    ...
    fresh.rows[c.sig] = c.row;
    entries[sigKey(c.sig)] = dropId;
    stored++;
...
module.exports = { feePayerOf,
  ...
  toRaw, rawToUi, dayKeyUtc, wellFormedSig, kvKey, sigKey, SIG_KEY_PREFIX, receiptOfSig,
