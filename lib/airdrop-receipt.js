...
//   operator  — the pass-holder's wallet, for the daily-drop cap only; never stored publicly and
//               fine to omit (the cap is skipped, not enforced against nobody).
...
async function recordDrop({ kv, dropId, mint, decimals, createdAt, rows, operator, getTx, now }) {
  ...
  if (dropId) {
    ...
    if (operator) {
      ...
      const key = opDayKey(operator, day);
      const seen = kv.get(key, []);
      const list = Array.isArray(seen) ? seen : [];
      if (list.length >= MAX_DROPS_PER_OPERATOR_PER_DAY) {
  ...
  for (const r of rows) {
    ...
    const v = payoutVerify.rowPaidBy(tx, { mint, wallet, minRaw: raw.toString(), notBefore: notBeforeSec });
    drop.rows[sig] = { wallet, amount: String(amount), sig, verified: !!v.ok, reason: v.ok ? null : v.why, deltaRaw: v.deltaRaw || null, recordedAt: now };
    results.push({ wallet, sig, verified: !!v.ok, reason: v.ok ? null : v.why });
  }
...
module.exports = {
  MAX_ROWS_PER_DROP, MAX_DROPS_PER_OPERATOR_PER_DAY,
  toRaw, rawToUi, dayKeyUtc, wellFormedSig, kvKey,
  recordDrop, loadDrop, publicDrop, publicRow,
};
