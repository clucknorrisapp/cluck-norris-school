...
// sig -> dropId for every VERIFIED row ever stored. One signature, one receipt: a stranger who
// knows a public signature cannot start a second drop with it (and consume its payer's quota).
const SIG_INDEX_KV = "airdropReceiptSigIndex";
...
//   (`airdropReceiptSigIndex`), so a public signature cannot start a second drop.
...
//               operator is never stored publicly. A new drop whose first transactions cannot be
//               read is not created (409) — retry once they are readable.
...
async function recordDrop({ kv, dropId, mint, decimals, createdAt, rows, operator, getTx, now }) {
  ...
  const sigIndexRaw = kv.get(SIG_INDEX_KV, {});
  const sigIndex = sigIndexRaw && typeof sigIndexRaw === "object" ? sigIndexRaw : {};
  const results = [];
  let stored = 0;
  ...
  for (const r of rows) {
    ...
    // Idempotent on sig: a sig already on this receipt is a no-op — return what's on file rather
    // than re-hit the chain. (Only verified rows are ever on file.)
    ...
    // One signature, one receipt.
    if (sigIndex[sig] && sigIndex[sig] !== dropId) { results.push({ wallet, sig, verified: false, reason: "already recorded on another receipt" }); continue; }

    const raw = toRaw(amount, dec);
    if (raw === null) { results.push({ wallet, sig, verified: false, reason: "invalid amount" }); continue; }

    let tx = null, fetchErr = null;
    try { tx = await getTx(sig); } catch (e) { fetchErr = String((e && e.message) || e); }
    if (fetchErr) { results.push({ wallet, sig, verified: false, reason: `could not read the chain: ${fetchErr}` }); continue; }
    ...
    let op = drop.operator || null;
    if (!op) {
      op = feePayerOf(tx);
      if (!op) { results.push({ wallet, sig, verified: false, reason: "transaction names no fee payer" }); continue; }
    }
    const v = native
    ...
    if (ok) {
      const fundedByOperator = native
        ? nativeSourceIsOperator(tx, { operator: op, minRaw: raw })
        : sourceIsOperator(tx, { mint, operator: op, minRaw: raw });
      if (!fundedByOperator) { ok = false; reason = SOURCE_MISMATCH_REASON; }
    }
    if (!ok) { results.push({ wallet, sig, verified: false, reason }); continue; }
    ...
    if (!drop.operator) {
      if (isNew && capReached(kv, op, now)) return capRefusal();
      drop.operator = op;
    }
    drop.rows[sig] = { wallet, amount: String(amount), sig, verified: true, reason: null, deltaRaw: v.deltaRaw || null, recordedAt: now };
    sigIndex[sig] = dropId;
    stored++;
    results.push({ wallet, sig, verified: true, reason: null });
  }
  ...
  if (stored === 0) {
    ...
    if (isNew) return { ok: false, status: 409, error: "no row could be verified on-chain yet — nothing was recorded; retry once the transactions are readable", results };
    return { ok: true, dropId, drop, results, nothingNew: true, totals: { rows: Object.keys(drop.rows).length, verified: Object.keys(drop.rows).length } };
  }

  drop.updatedAt = now;
  // The drop and the signature index in ONE persist where the store supports it (kvstore's
  // setManyVerified), so a crash between the two can never leave a stored row unindexed.
  const persisted = typeof kv.setManyVerified === "function"
    ? kv.setManyVerified({ [kvKey(dropId)]: drop, [SIG_INDEX_KV]: sigIndex }) === true
    : (saveDrop(kv, drop) && (kv.set(SIG_INDEX_KV, sigIndex), true));
