...
async function recordDrop({ kv, dropId, mint, decimals, createdAt, rows, operator, getTx, now }) {
  ...
  for (const r of rows) {
    ...
    const v = payoutVerify.rowPaidBy(tx, { mint, wallet, minRaw: raw.toString(), notBefore: notBeforeSec, nowUnix: Math.floor(now / 1000) });
    ...
    if (ok && drop.operator && !sourceIsOperator(tx, { mint, operator: drop.operator, minRaw: raw })) {
