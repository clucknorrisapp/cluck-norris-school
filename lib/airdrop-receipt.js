...
// Did `operator` fund THIS row? See sourceIsOperator / nativeSourceIsOperator below: the
// operator's OWN balance of the claimed asset must have gone down by at least the row's amount
// (and, for SOL, a parsed SystemProgram transfer from the operator to this recipient must exist).
// Returns false (never throws) when `operator` is falsy.
// ── native SOL ───────────────────────────────────────────────────────────────────────────────
...
// Did the operator fund THIS row? Two things, both required (Codex round 19, 2026-09-22: the
// lamport delta alone let a fee payer "fund" a row smaller than the fee — X co-signs a
// transaction in which Y pays A, X's lamports drop by the fee, and X could claim A's row):
//  1) a parsed SystemProgram transfer, top-level or inner, FROM the operator TO this recipient of
//     at least the row's amount — the exact instruction the airdropper emits
//     (public/airdrop-engine.js createSolTransferInstruction); and
//  2) the operator's lamports went DOWN by at least that amount (the net effect agrees).
// Deliberately ignores the fee in (2): a drop's fee is far below any real row and demanding
// `amount + fee` would fail honest rows.
const SYSTEM_PROGRAM = "11111111111111111111111111111111";
function nativeSourceIsOperator(tx, { operator, wallet, minRaw }) {
  if (!operator || !wallet) return false;
  const need = typeof minRaw === "bigint" ? minRaw : BigInt(String(minRaw || "0"));
  const delta = lamportDelta(tx, operator);
  if (delta === null || delta > -need) return false;
  for (const ix of allParsedInstructions(tx)) {
    const parsed = ix && ix.parsed;
    if (!parsed || typeof parsed !== "object" || parsed.type !== "transfer") continue;
    if (ix.programId && ix.programId !== SYSTEM_PROGRAM) continue;
    const info = parsed.info || {};
    if (info.source !== operator || info.destination !== wallet) continue;
    let lamports; try { lamports = BigInt(String(info.lamports)); } catch (_) { continue; }
    if (lamports >= need) return true;
  }
  return false;
}

// The token-balance row (owner, mint) behind a token ACCOUNT address a parsed instruction names —
// pre/postTokenBalances carry `accountIndex` + `owner` + `mint` but not the account's own
// address, so the account-keys list is the bridge. Post first: the recipient's ATA may have been
// created in this same transaction and has no pre row.
function tokenAccountRow(tx, addr) {
  if (!addr) return null;
  const idx = accountKeysOf(tx).indexOf(addr);
  if (idx < 0) return null;
  const meta = (tx && tx.meta) || {};
  return (meta.postTokenBalances || []).find((b) => b && b.accountIndex === idx)
    || (meta.preTokenBalances || []).find((b) => b && b.accountIndex === idx) || null;
}
const TOKEN_PROGRAMS = new Set(["TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA", "TokenzQdBNbLqP5VEhdkAS6EPFLC1PHnBqCXEpPxuEb"]);
// Is there a parsed spl-token `transfer` / `transferChecked` (top-level or inner) whose SOURCE
// account is owned by the operator, whose DESTINATION account is owned by this recipient, of this
// mint, of at least the row's amount? This is what binds "the operator lost the mint" to "…and
// paid THIS recipient with it".
function boundTokenTransferExists(tx, { mint, operator, wallet, need }) {
  for (const ix of allParsedInstructions(tx)) {
    const parsed = ix && ix.parsed;
    if (!parsed || typeof parsed !== "object") continue;
    if (parsed.type !== "transfer" && parsed.type !== "transferChecked") continue;
    if (ix.programId && !TOKEN_PROGRAMS.has(ix.programId)) continue;
    const info = parsed.info || {};
    const src = tokenAccountRow(tx, info.source), dst = tokenAccountRow(tx, info.destination);
    if (!src || !dst) continue;
    if (src.mint !== mint || dst.mint !== mint || (info.mint && info.mint !== mint)) continue;
    if (src.owner !== operator || dst.owner !== wallet) continue;
    const raw = info.tokenAmount ? info.tokenAmount.amount : info.amount;
    let amount; try { amount = BigInt(String(raw)); } catch (_) { continue; }
    if (amount >= need) return true;
  }
  return false;
}

// Did the operator fund THIS row? Two things, both required:
//  1) the operator's OWN balance of THIS mint went down by at least the row's amount — the
//     transaction's net effect from pre/postTokenBalances, so a DEX-routed inner CPI (the pool
//     loses the balance, never the operator) is refused, and a delegate moving the operator's
//     tokens still counts (the operator's account is what drains); and
//  2) a parsed spl-token transfer FROM an account the operator owns TO an account this recipient
//     owns, of this mint, of at least the amount (boundTokenTransferExists) — the instruction the
//     airdropper emits (public/airdrop-engine.js createTransferCheckedInstruction).
// ⚠️ History, so nobody loosens it: rounds 8–18 accepted any parsed transfer whose authority was
// the operator, unbound to mint/recipient/amount (Codex round 19: X moved an unrelated token in
// the transaction where Y paid A, and X claimed A's row). Round 19 kept only (1), and (1) alone
// still let X, who paid B the same mint in the transaction where Y paid A, claim A's row (Codex
// round 20). Both together bind operator → recipient, mint and amount.
function sourceIsOperator(tx, { mint, operator, wallet, minRaw }) {
  if (!operator || !wallet) return false;
  const need = typeof minRaw === "bigint" ? minRaw : BigInt(minRaw || 0);
  let lost = false;
  for (const [owner, delta] of payoutVerify.tokenDeltas(tx, mint)) {
    if (owner === operator && delta <= -need) { lost = true; break; }
  }
  if (!lost) return false;
  return boundTokenTransferExists(tx, { mint, operator, wallet, need });
}
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (ok) {
      const fundedByOperator = native
        ? nativeSourceIsOperator(tx, { operator: rowOp, wallet, minRaw: raw })
        : sourceIsOperator(tx, { mint, operator: rowOp, wallet, minRaw: raw });
