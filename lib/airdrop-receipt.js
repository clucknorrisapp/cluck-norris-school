...
// Did the operator fund THIS row? The operator's OWN balance of THIS mint must have gone down by
// at least the row's amount — the transaction's net effect, read from pre/postTokenBalances, so
// a DEX-routed inner CPI (the pool loses the balance, never the operator) is refused without
// understanding the DEX's instruction shape, and a delegate moving the operator's tokens still
// counts (the operator's account is what drains).
// ⚠️ Nothing else counts (Codex round 19, 2026-09-22). Rounds 8–18 had a second, unbound branch:
// any parsed spl-token transfer whose authority (or source owner) was the operator passed —
// without binding its mint, its recipient or its amount to the row. A signed wallet X could
// therefore claim a transaction in which X moved an unrelated token while Y paid the claimed
// mint to A, and Y's own recording then got "already recorded on another receipt". The net delta
// of the claimed mint is the only funding evidence now.
