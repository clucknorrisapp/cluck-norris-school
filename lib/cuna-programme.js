...
function validateConfig(patch, base) {
  ...
  for (const w of c.fundedBy) if (!B58.test(w)) throw new Error(`fundedBy has a bad address: ${w}`);
...
// How many distinct UTC DAYS the ledger touches. The ledger is keyed per hourly slice
// ("YYYY-MM-DDTHH"), so Object.keys(days).length is the SLICE count — the admin and payout
// routes reported that as daysAccrued after the hourly move, and the payout verifier's only
// magnitude ceiling (dailyPool × days) inherited a 24× overstatement (audit 2026-09-05 #4).
// Legacy whole-day keys ("YYYY-MM-DD") count as one day each.
function daysAccruedFrom(days) {
  const set = new Set();
  for (const k of Object.keys(days || {})) { const d = String(k).slice(0, 10); if (/^\d{4}-\d{2}-\d{2}$/.test(d)) set.add(d); }
  return set.size;
}
// The most the programme can have emitted across N accrued slices: N/24 of the daily pool,
// rounded up so the remainder-carrying last slice of a day never trips it. This is the payout
// verifier's ceiling.
function maxEmittedRaw(dailyRaw, sliceCount) {
  const daily = BigInt(dailyRaw || 0), n = BigInt(Math.max(0, Number(sliceCount) || 0));
  return (daily * n + BigInt(SLICES_PER_DAY - 1)) / BigInt(SLICES_PER_DAY);
}
...
module.exports = {
  SLICES_PER_DAY, sliceKey, sliceIndexOf, slicePoolRaw, missedSlices, daysAccruedFrom, maxEmittedRaw,
