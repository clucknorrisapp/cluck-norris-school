...
function missedDays({ programme, paidDays, nowUnix }) {
  ...
  const MAX = 400;
  // Walk from YESTERDAY back to the calendar day that contains startedAt — inclusive. The old
  // loop stopped at the first instant before startedAt, which is inside the arm day itself, so a
  // programme armed at 23:30 UTC whose first tick fired after midnight lost its launch day and
  // could never report it.
  const firstKey = dayKey(p.startedAt);
  for (let i = 1; i <= MAX; i++) {
    const t = now - i * DAY;
    const k = dayKey(t);
    if (k < firstKey) break;
