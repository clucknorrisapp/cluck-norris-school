...
function locateTransferInstruction(tx, { mint, wallet, fundingWallet, fundedBy, nowUnix } = {}) {
  ...
  // N3 (docs/HUB_JOURNAL_VERIFY_2026-09-18.md, Round 3): the settlement journal wants to record WHO
  // actually paid, not just that the payer was on the allowlist — so the receipt can say "paid from"
  // when an allowlisted-but-not-funding wallet covered it. `found.sourceOwner` is already computed
  // above (never trusted from the instruction's own `authority`, only from the source token
  // account's owner in preTokenBalances) — just carried through on the happy path too.
  return { instructionIndex: m.instructionIndex, innerIndex: m.innerIndex, amountRaw: instAmt.toString(), slot: Number(tx.slot) || 0, sourceWallet: (found && found.sourceOwner) || null };
}
