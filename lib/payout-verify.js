  ...
  for (const r of results || []) {
    ...
    if (!txBySig || typeof txBySig.has !== "function" || !txBySig.has(sg)) v = { ok: false, why: "no transaction signature" };
      ...
      v = prior ? { ok: false, why: "this signature already paid " + w.slice(0, 6) + "… in batch " + prior }
                : rowPaidBy(txBySig.get(sg), { mint, wallet: w, minRaw: (amounts || {})[w], notBefore, fundingWallet, fundedBy, nowUnix });
    }
...
module.exports = { tokenDeltas, rowPaidBy, sigAlreadyUsed, verifyBatchRows, locateTransferInstruction, transfersIntoWallet, allowedSourceSet, NOT_BEFORE_SKEW_S, FUTURE_SKEW_S };
