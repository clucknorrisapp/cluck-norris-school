...
// shared by both /cuna-payout and /api/hub/:project/payout (lib/hub/routes.js) — a lock-to-earn
// batch itself carries no program-version hash of its own. W3 wired the Addendum-B settlement
// journal (lib/hub/ledger.js) into that same route as a dual-write on top of the legacy batch, so
// `buildInputsForWallet`/`buildBatchInputs` now fill in `programVersion` — {version, hash}, the
// program version IN FORCE when the batch was built — for any (batch, wallet) row that already has
// a journal event; a row with no journal event (predates the dual-write, or its transfer could not
// be attributed to one instruction) still reports `hash: null` honestly, never invented. Buy
// competitions, Buy Special draws and the CUNA giveaway are
...
function buildInputsForWallet({ batch, wallet, days, batches, journal = null, programState = null }) {
  ...
  if (journal && programState) {
    ...
    if (hasJournalEntry) {
      ...
      const { versionFor } = require("./project");
