...
// /api/hub/:project/payout (lib/hub/routes.js) — NOT the hashed-program-version settlement journal
// in lib/hub/ledger.js (Addendum B), which is pure and tested (scripts/hub-core-test.cjs) but not
// yet wired to any live route. So a lock-to-earn batch carries no program-version hash of its own
// today; `reproduce()` reports that honestly (hash: null) rather than inventing one. A buy-comp row
// reproduces through `reproduceBuyCompRow` below, from exactly the fields the public standings
// route already publishes (terms, this wallet's rank, its review row) — `projectReproducibility`
// runs it over every winner of every SEALED comp, and reports an unsealed comp's rows as
// MISSING_INPUTS ("not sealed") rather than guessing at a claim that could still change. Buy
// Special draws and the CUNA giveaway are still their own, not-yet-covered kinds — `reproduce()`
// and `projectReproducibility()` say so as MISSING_INPUTS, naming the kind, rather than silently
