...
const { canonicalJson, sha256 } = require("./project");
...
function compView(c) {
  ...
  return {
    ...
    totals: {
      winners: results.length, owedUi: sumUi(owed, (r) => r.amount),
      paidUi: sumUi(settled, (p) => p.amountUi), submittedUi: sumUi(submitted, (p) => p.amountUi),
      settled: settled.length, submitted: submitted.length, unpaid: owed.filter((r) => !paidSet.has(r.wallet)).length,
...
function giveawayView({ draw, payouts, cfg } = {}) {
  ...
  return {
    ...
    totals: { winners: real.length, owedUi: sumUi(real, (w) => w.prizeUi), paidUi: sumUi(settled, (p) => p.amountUi), submittedUi: sumUi(submitted, (p) => p.amountUi),
      settled: settled.length, submitted: submitted.length, unpaid: real.filter((w) => !rows.some((r) => r.wallet === w.wallet)).length },
  };
}

// ── lock-to-earn (the accrual ledger + the airdropper batches with their signatures) ─────────
function stakeView({ days, paid, batches, decimals = 9 } = {}) {
  ...
  const batchViews = Object.values(batches || {}).filter(Boolean).map((b) => ({
    ...
    rows: Object.entries(b.amounts || {}).map(([wallet, raw]) => {
      const s = b.sent && b.sent[wallet];
      return { wallet, amountUi: rawToUi(raw, decimals), sig: s && s.sig ? String(s.sig) : null, at: s ? num(s.at) : null,
        state: s ? (s.sig ? (s.pending ? "submitted" : "settled") : "confirmed-manually") : (b.state === "cancelled" ? "cancelled" : "owed") };
    }),
  ...
  return {
    ...
    status: dayKeys.length ? "accruing" : "not-started",
    accrual: { days: dayKeys.length, firstDay: dayKeys[0] || null, lastDay: dayKeys[dayKeys.length - 1] || null, wallets: Object.keys(accrued).length },
    totals: { accruedUi: rawToUi(accruedTotal, decimals), paidUi: rawToUi(paidTotal, decimals), owedUi: rawToUi(owedRaw < 0n ? 0n : owedRaw, decimals),
...
function walletLookup(view, wallet) {
  ...
  for (const p of view.programs || []) {
    ...
    for (const b of p.batches || []) for (const r of b.rows || []) if (r.wallet === w && !r.sig) entries.push({ ...head, role: r.state, amountUi: r.amountUi, batchId: b.id });
  }
...
function findReceipt(view, sig) {
  ...
  for (const p of view.programs || []) {
    ...
    if (row) return { projectId: view.id, symbol: view.symbol, program: { kind: p.kind, id: p.id, label: p.label, ticker: p.ticker || view.symbol, mint: p.mint, prizeMint: p.prizeMint || p.mint, termsHash: p.termsHash || null }, receipt: row };
  }
  return null;
}
