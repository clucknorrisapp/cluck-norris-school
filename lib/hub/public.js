...
const { attributePeriods, explainReceipt, explainBuyCompRow } = require("./explain");
...
// One review row's hold-through status, from what was actually observed (Colosseum roadmap §7 —
// "say what's on-chain, never why"): "held" (still qualified), "sold" / "moved-out" (the two ways
// a DQ actually happens — never lumped together as one generic disqualification), "locked-not-a-
// sell" (PR #298 — the wallet's whole buy went into a lock escrow, which is the opposite of an
// exit, so this is NOT a disqualification, just a fact worth surfacing plainly), or "unverified"
// (a genuine lookup gap — coverage or RPC miss — with nothing observed either way).
function holdThroughOf(r) {
  const fact = String(r.note || "");
  if (r.status === "qualified") return "held";
  if (r.status === "dq") return /sold on-chain/.test(fact) ? "sold" : "moved-out";
  if (r.status === "manual" && Number(r.locksSeen) > 0) return "locked-not-a-sell";
  return "unverified";
}
...
function compView(c) {
  ...
  // Every candidate the hold check looked at, with the observed fact — built FIRST so a winner's
  // own row (below) can carry its hold-through label and evidence signature too, never just
  // "held" asserted with nothing behind it.
  const review = (Array.isArray(c.verifyResults) ? c.verifyResults : []).map((r) => ({
    ...
    holdThrough: holdThroughOf(r), evidenceSig: SIG_RE.test(String(r.evidenceSig || "")) ? r.evidenceSig : null,
  }));
  const reviewByWallet = {}; for (const r of review) reviewByWallet[r.wallet] = r;
  const results = (Array.isArray(c.verified) ? c.verified : []).map((v) => {
    const wallet = String(v.wallet || ""), rank = num(v.rank), amount = num(v.amount);
    const rv = reviewByWallet[wallet] || {};
    // "How this number was computed" (roadmap E5): derived by the exact split function
    // buyCompVerify used (lib/buycomp-payout.js splitAmount, via lib/hub/explain.js), never a
    // fabricated walkthrough — degrades to {steps:[], missing:[...]} when a rank or the terms'
    // places are unavailable, same discipline as the lock-to-earn mirror above.
    const explanation = explainBuyCompRow({ terms, rank, tokensBought: rv.tokensBought, valueSol: rv.value, ticker: terms.ticker, expectedAmount: amount });
    // A winner is, by construction, the "held" case among the review rows — buyCompVerify only
    // ever seals a wallet whose status was "qualified" — so its hold-through evidence is simply
    // "nothing bad was observed", not a signature.
    return {
      rank, wallet, amount, amountUnit: v.amountUnit === "sol" ? "sol" : "token", amountNote: v.amountNote || null,
      status: String(v.status || ""), note: String(v.note || ""), holdThrough: "held", evidenceSig: null, explanation,
    };
  });
  ...
  // sealed: the winner list is final (buyCompVerify ran, or the operator set it by hand) — never
  // inferred from results.length, which is legitimately 0 for a sealed comp with no qualifiers.
  // resultsHash covers only the decision-relevant fields (rank/wallet/amount/amountUnit/status),
  // never bookkeeping or explanations, so a reader can tell if the SEALED CLAIM itself ever
  // changes without it being upset by a payout landing or a walkthrough improving later.
  const sealed = String(c.status || "") === "verified";
  const resultsHash = sha256(canonicalJson(results.map((r) => ({ rank: r.rank, wallet: r.wallet, amount: r.amount, amountUnit: r.amountUnit, status: r.status }))));
  return {
    ...
    status: String(c.status || ""), sealed, sealedAt: sealed ? num(c.verifiedAt) : null, resultsHash,
    createdAt: num(c.createdAt), verifiedAt: num(c.verifiedAt),
...
// ── the standalone public standings + hold-through proof view (Colosseum roadmap §7, the W4
// deferred "public Buy Special standings" item) ───────────────────────────────────────────────
// A leaner, explicitly GATED contract for ONE competition — distinct from compView's role inside
// the whole-project page (which a live comp already renders empty-but-present arrays for). Before
// the comp is sealed, this returns the published RULES ONLY: no `results`, no `review`, no
// `payouts` key at all — not even an empty array a reader could mistake for "nobody has qualified
// yet". A live, mid-window leaderboard through this route would let a whale time the last minute;
// the whole point of sealing is that nothing here is published before the window (and its hold)
// actually ends.
function compStandingsView(c) {
  const sealed = String(c.status || "") === "verified";
  if (!sealed) {
    const terms = compTerms(c);
    return {
      kind: "buy-comp", id: terms.id, label: String(c.label || c.ticker || ""), ticker: terms.ticker, mint: terms.mint,
      status: String(c.status || ""), sealed: false,
      terms, termsHash: termsHash(terms),
      message: "Standings are sealed at the window's end, after the hold-through check runs — nothing is published before then.",
    };
  }
  return compView(c);   // sealed → the full public view, already carrying sealed/sealedAt/resultsHash
}
...
function stakeView({ days, paid, batches, decimals = 9, project = null, programState = null } = {}) {
  ...
  // Roadmap CC1: the program version in force for the LAST period this row drew on — whitelisted
  // down to {version, hash} (never the full terms; a reader gets those from
  // GET /api/hub/:project/program/:version, the one canonical place they're published) so a
  // receipt can name which rules it was paid under and a page can tell a holder "the rules
  // changed since this receipt" when a later version now exists. Null exactly when explanationOf
  // is also degraded (no project/programState, or periods could not be attributed) — never
  // invented from a receipt that cannot actually be traced to one version.
  const versionOf = {};            // batchId -> wallet -> {version, hash} | null
  for (const b of orderedBatches) {
    explanationOf[b.id] = {}; versionOf[b.id] = {};
    for (const [wallet, raw] of Object.entries(b.amounts || {})) {
      ...
      explanationOf[b.id][wallet] = (() => {
        ...
        pv = programState ? versionFor(programState, lastPeriod) : null;
      ...
      versionOf[b.id][wallet] = (pv && pv.hash) ? { version: pv.version, hash: pv.hash } : null;
  ...
  const batchViews = orderedBatches.map((b) => ({
    ...
    rows: Object.entries(b.amounts || {}).filter(([wallet]) => b.sent && b.sent[wallet]).map(([wallet, raw]) => {
      ...
      return { wallet, amountUi: rawToUi(raw, decimals), sig: s.sig ? String(s.sig) : null, at: num(s.at),
        ...
        explanation: explanationOf[b.id][wallet], programVersion: versionOf[b.id][wallet] };
    }),
...
// `opts.full` (AA2 bug fix, 2026-09-18): the plain view below is what a page that only DISPLAYS
// terms has ever needed, and stays exactly as it was. But `hash` is a sha256 of the canonical JSON
// of the FULL version record (lib/hub/project.js versionRecord/verifyVersionHash) — which also
// carries projectId, mint, rewardMint, rewardDecimals, rewardTokenProgram, fundingResponsibility,
// signer and exclusions. Every one of those is public information already (the project id, the
// mint/reward-mint/reward-decimals/reward-token-program — program terms and a token account's own
// public fields — the funding wallet named twice as fundingResponsibility/signer, and the Rule-B
// exclusion list of public wallet addresses) — nothing here is a secret. Omitting them from what a
// reader can recompute the hash FROM made "recompute the hash" fail on every untampered version,
// for any project (found by the BB2 docs builder): the served document was missing fields the hash
// was actually taken over. `full:true` returns every one of those fields, alongside the plain view,
// so a caller that needs a reader to be able to verify the hash (the evidence bundle, the
// program-version route `/hub/verify` reads) can serve a document that actually recomputes.
function programVersionView(v, opts) {
  if (!v) return null;
  const base = { version: v.version, effectiveFrom: v.effectiveFrom, effectiveTo: v.effectiveTo != null ? v.effectiveTo : null, hash: v.hash, terms: v.terms, commitment: commitmentView(v) };
  if (!(opts && opts.full === true)) return base;
  return {
    ...base,
    projectId: v.projectId != null ? String(v.projectId) : null,
    mint: v.mint != null ? String(v.mint) : null,
    rewardMint: v.rewardMint != null ? String(v.rewardMint) : null,
    rewardDecimals: Number.isInteger(v.rewardDecimals) ? v.rewardDecimals : null,
    rewardTokenProgram: v.rewardTokenProgram != null ? String(v.rewardTokenProgram) : null,
    fundingResponsibility: v.fundingResponsibility != null ? String(v.fundingResponsibility) : null,
    signer: v.signer != null ? String(v.signer) : null,
    exclusions: v.exclusions && typeof v.exclusions === "object"
      ? { rule: v.exclusions.rule != null ? String(v.exclusions.rule) : null, wallets: Array.isArray(v.exclusions.wallets) ? v.exclusions.wallets.map(String) : [] }
      : null,
  };
}

// ── one project, every program ───────────────────────────────────────────────────────────────
// Roadmap CC1 ("What changed in the rules"): the public index of program versions a project has
// published, whitelisted to exactly {version, hash, publishedAt} — never `terms` (a reader fetches
// the full document, including its terms, from GET /api/hub/:project/program/:version, the one
// canonical place they're published) and never `effectiveTo`/`commitment` bookkeeping. `publishedAt`
// is the version's own `effectiveFrom` (a program version has no separate wall-clock publish time —
// it takes effect, and is published, on that UTC day). Callers pass the project's raw
// state.versions (lib/hub/project.js records); an absent/empty list is honestly `[]`, never
// invented.
function versionsPublic(versions) {
  return (Array.isArray(versions) ? versions : [])
    .filter((v) => v && v.version != null && v.hash)
    .map((v) => ({ version: v.version, hash: v.hash, publishedAt: v.effectiveFrom || null }));
}
...
// `holderSnapshot` (optional): { id, at, holderCount, listHash } from lib/holders-snapshot.js
// `latest()` — Colosseum roadmap §8 X7. Whitelisted to those four fields only: no wallets, no
// top list. A fact line, never a judgement ("Holders: N (snapshot <date>, hash …)"), never
// "growing"/"healthy" — CLAUDE.md: say what's on-chain, never why.
function projectView({ project, comps = [], draws = [], stake = null, giveaway = null, lessonReads = null, holderSnapshot = null, versions = [] }) {
  ...
  return {
    ...
    // CC1: every published program version, oldest first (the order lib/hub/project.js already
    // keeps them in) — [] for a project that has never published one, never omitted.
    versions: versionsPublic(versions),
    ...
    holders: (holderSnapshot && holderSnapshot.id) ? {
      id: String(holderSnapshot.id), at: num(holderSnapshot.at),
      holderCount: Number(holderSnapshot.holderCount) || 0,
      listHash: String(holderSnapshot.listHash || ""),
    } : null,
  };
}
...
module.exports = { rawToUi, compTerms, termsHash, compView, compStandingsView, drawView, giveawayView, stakeView, projectView, walletLookup, findReceipt, commitmentView, programVersionView, B58 };
