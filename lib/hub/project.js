...
const { canonicalJson, versionHashInput } = require("./canonical");
...
const RESERVED_PROJECT_IDS = new Set([
  "apply", "verify", "status", "wallet", "trust", "judge", "schema", "registry", "glossary", "hub",
...
// canonicalJson (keys sorted at every level, BigInts as strings — the only serialisation a hash
// may be taken over) lives in ./canonical, shared with the browser bundle (Y1) so there is one
// function, not two.
...
function validateProject(input, mintInfo) {
  ...
  // P3-09 (docs/HUB_PUBLIC_SURFACES_VERIFY_2026-09-18.md): a Unicode bidi/format override
  // character (RLO and friends) is invisible in almost every approval UI, so the label an owner
  // reads and approves is not necessarily the label a share card renders — stripped here, the
  // same way brandLib.stripBidiControls already strips a tagline, before the length check runs
  // (so a label that is ONLY a bidi character, or empty once they're removed, is refused rather
  // than silently approved as a label with no visible content).
  const label = brandLib.stripBidiControls(String(p.label || "")).trim();
...
//
// N-2 (Round 4, docs/HUB_JOURNAL_VERIFY_2026-09-18.md): the Round 3 fix above still exempted a
// SUSPENDED project from both the id and the mint check, on the theory that suspension was purely
// the owner's business. But `requireNew` is the SELF-SERVE path — an application should never be
// able to take over ANY existing registry row, suspended or not: a suspended project already has a
// fundingWallet, operatorWallets and (per the routes.js guard) possibly a full payout history, and
// none of that is the applicant's to inherit. So under `requireNew` the suspended exemption is
// dropped for BOTH checks; the owner's own edit path (`requireNew` false) keeps the exemption
// exactly as before — re-approving or un-suspending the same project, or relaunching a suspended
// mint under a brand-new id, are still the owner's call to make.
function approveProject(registry, project, { nowUnix, reserved, appliedAt, requireNew = false } = {}) {
  ...
  for (const [otherId, other] of Object.entries(reg)) {
    if (otherId !== project.id && other && other.mint === project.mint && (requireNew || other.status !== "suspended")) {
      throw new Error(`mint ${project.mint} is already registered as project "${otherId}"`);
    }
  }
  if (requireNew && reg[project.id]) {
  ...
  // NEW-3 (Round 5, docs/HUB_JOURNAL_VERIFY_2026-09-18.md): re-issuing an id to a DIFFERENT mint
  // (only reachable here on a SUSPENDED row — the check just above blocks it for an approved one)
  // is a fresh project by every measure that matters, even though the id is old. `storeIsEmptyFor`
  // (lib/hub/store.js, checked by the caller before this runs) only guards the money/receipt
  // parts; the registry row itself still carried the PREVIOUS project's paid-through date and
  // onboarding clock forward across the mint change, as if the new mint had been paying rent on
  // the old one's account. Only a mint change resets these — the same-mint re-approve / un-suspend
  // path (existingId reused, mint unchanged) is unaffected and keeps everything exactly as before.
  const existing = reg[project.id];
  const mintChanged = !!(existing && existing.mint !== project.mint);
  // Re-approving on the SAME mint keeps the months already paid; only the tier and note are
  // re-decided. A mint change starts access fresh — `project.access` is already the clean default
  // (access.normalizeAccess) validateProject computed from this call's own tier/note.
  const prev = !mintChanged && existing ? existing.access : null;
  const acc = prev ? { ...prev, tier: project.access.tier, note: project.access.note } : project.access;
  const prevMilestones = !mintChanged ? ((existing && existing.milestones) || (project.milestones || null)) : null;
...
function createVersion(state, project, termsInput, { effectiveFrom, todayKey }) {
  ...
  // A dry-run project (Colosseum E10) may be registered with no funding wallet at all — real terms
  // are not yet agreed, so there is nothing to fund yet either (validateProject allows this). But
  // validateTerms below unconditionally excludes project.fundingWallet from the pool (Rule B), and
  // a null value fails plain address validation with "fundedBy entry is not an address: null" —
  // true, but it names neither what is missing nor what to do about it. This is the exact
  // undocumented prerequisite the POKEAHOE rehearsal named (docs/POKEAHOE_REHEARSAL_2026-09.md
  // §4): a funding wallet is set only through the owner-gated /api/hub-registry route, never
  // through the desk. Refuse here first, naming the actual gap and the one command that closes it.
  if (!project || !project.fundingWallet) {
    const id = (project && project.id) || "<id>";
    throw new Error(`${id} has no funding wallet on its registry record yet — terms cannot be published until the owner sets one: POST /api/hub-registry?id=${id}&fundingWallet=<the project's own wallet>`);
  }
...
// versionFor lives in lib/hub/version-for.js (pure, browser-safe) and is re-exported here.
...
function verifyVersionHash(v) {
  const { hash, canon } = versionHashInput(v);
  return hash != null && sha256(canon) === hash;
}
