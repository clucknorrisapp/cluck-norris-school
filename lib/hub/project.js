...
function approveProject(registry, project, { nowUnix }) {
  const reg = registry || {};
  ...
  const approved = { ...project, status: "approved", approvedAt: Number(nowUnix), approvedBy: "owner" };
...
function validateTerms(input, project) {
  ...
  if (!["weekly", "daily", "manual"].includes(out.payoutSchedule)) throw new Error(`payoutSchedule must be weekly, daily or manual: got ${t.payoutSchedule}`);
  return out;
}
...
function verifyVersionHash(v) {
  const { hash, ...body } = v || {};
  return typeof hash === "string" && sha256(canonicalJson(body)) === hash;
}
