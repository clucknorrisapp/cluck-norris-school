...
function validateApplication(input, mintInfo, { nowUnix, id = newId }) {
  ...
  // dryRun is never applicant-set — it is an owner-only concept (a labelled placeholder for a
  // project whose terms are not yet agreed, e.g. the POKEAHOE dry run). brand IS applicant-set:
  // a project may submit its own logo/accent/tagline/socials with the application, validated the
  // identical way an owner edit is (proj.validateProject -> lib/hub/brand.js).
  const projectInput = { id: a.id, label: a.label, symbol: a.symbol, mint: a.mint, fundingWallet: a.fundingWallet, operatorWallets: a.operatorWallets, rewardMint: a.rewardMint, rewardMintInfo: a.rewardMintInfo, accessTier: tierRequested, brand: a.brand };
...
function approve(book, registry, appId, { tier, note, nowUnix, freshMintInfo, freshRewardMintInfo, reserved }) {
  ...
  // N2 (docs/HUB_JOURNAL_VERIFY_2026-09-18.md, Round 3): requireNew — this self-serve path can
  // never take over an id or mint that already belongs to a registered project, regardless of
  // whether it has program versions yet (see proj.approveProject's own comment).
  const reg = proj.approveProject(registry, project, { nowUnix, reserved, appliedAt: app.submittedAt, requireNew: true });
  const state = proj.createVersion({}, reg[project.id], app.terms, { effectiveFrom: tomorrowKey(nowUnix), todayKey: dayKey(nowUnix) });
  // The draft becomes v1 immediately on approval here (unlike a project the owner seeds directly
  // through /api/hub-registry?id=, whose first version comes later from the admin desk) — the
  // onboarding clock's firstVersionPublishedAt is this same moment.
  const withVersion = { ...reg, [project.id]: { ...reg[project.id], milestones: proj.setMilestoneOnce(reg[project.id].milestones, "firstVersionPublishedAt", nowUnix) } };
  const nextBook = { ...book, [appId]: { ...app, status: "approved", decidedAt: Number(nowUnix) } };
  return { registry: withVersion, state, book: nextBook, project: withVersion[project.id] };
}
