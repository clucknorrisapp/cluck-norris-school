...
const lessonHref = (id) => `/school#lesson=${id}`;   // the React school is served at /school, not /
...
function teachBlock({ project, version, funding, lock = null, estimate = null }) {
  ...
  const answers = [];
  const q = (key, question, text, params, derivedFrom, opts = {}) => answers.push({ key, question, text, params, derivedFrom, lesson: { id: LESSON_MAP[key], href: lessonHref(LESSON_MAP[key]) }, alwaysVisible: !!opts.alwaysVisible });
...
module.exports = { LESSON_MAP, lessonHref, RATE_LANGUAGE, containsRateLanguage, lockEndUnix, teachBlock, allText };
