...
const LESSON_MAP = Object.freeze({
  ...
  how_much: "receipt",          // a share of a shared pool, and how to reproduce it — Y2: was
                                 // "lp"; the receipt lesson (pro-rata share, settlement, reproduce
                                 // it yourself) is the better target both for this pre-lock "how
                                 // much will I get?" answer AND, via the fallback below, for
                                 // every post-payment explain.js step (lib/hub/explain.js's push()
                                 // falls back to LESSON_MAP.how_much for its own keys — term_rule,
                                 // accrual_window, pro_rata_share, subtotal, prior_payments — none
                                 // of which is listed here, by design; they all ride this one).
...
// B5 wording (Colosseum roadmap E3): the public claim upgrades from "reproducible" to
// "independently committed" ONLY once the funding wallet's on-chain memo has actually been
// OBSERVED for a version (lib/hub/commit.js) — never on the strength of a hash this same server
// computed. Exported so every surface that shows a program version's status (this block, the
// program-page route, the demo fixture) uses these exact two sentences and never invents a third.
const REPRODUCIBLE_NOTICE = "The calculation is reproducible from the published inputs.";
const COMMITTED_NOTICE = "Independently committed on-chain by the funding wallet — check the memo on any explorer.";
...
function teachBlock({ project, version, funding, lock = null, estimate = null }) {
  ...
  return {
    ...
    answers,
    notices: {
      reproducible: REPRODUCIBLE_NOTICE,   // never "independently verified" until B5 exists
    },
  };
}
...
module.exports = { LESSON_MAP, lessonHref, RATE_LANGUAGE, containsRateLanguage, lockEndUnix, teachBlock, allText, whole, short, REPRODUCIBLE_NOTICE, COMMITTED_NOTICE };
