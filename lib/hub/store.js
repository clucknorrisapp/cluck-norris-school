...
function readJournal(kv) {
  ...
  try {
    ...
    if (legacy && typeof legacy === "object") for (const [k, v] of Object.entries(legacy)) if (v) out[k] = v;
  } catch (_) { /* a store without get(k,d) — nothing legacy to merge */ }
  if (typeof kv.entriesWithPrefix === "function") {
    for (const [k, v] of kv.entriesWithPrefix(JOURNAL_PREFIX)) if (v) out[xferKeyFromEntryKey(k)] = v;
  }
  return out;
}
...
module.exports = { PARTS, LEGACY_CUNA, REGISTRY_KEY, JOURNAL_KEY, JOURNAL_PREFIX, journalEntryKey, xferKeyFromEntryKey, assertProjectId, keyFor, read, write, writeVerified, writeManyVerified, writeManyVerifiedMixed, readRegistry, writeRegistry, readJournal, writeJournal, memoryKv, lockAcquire, lockRelease, LOCK_TTL_MS };
