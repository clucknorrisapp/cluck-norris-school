...
function browserSignAlreadyNote(bs) {
  // R3-6 (docs/HUB_BROWSER_SIGN_VERIFY_2026-09-18.md, round 3): "re-broadcast from what it
  // returns" named the DOUBLE-PAY action as the recovery. The correct recovery — proven by probe
  // P3 in that round — is the opposite: a fresh sign-request rotates the nonce, then the SAME
  // already-broadcast signature settles under that new nonce; nothing is sent to the chain twice.
  if (bs && bs.state === "cleared") return "this batch's browser-signed sign-request was cleared by the owner — request a fresh sign-request, then observe the signatures you already broadcast under its new nonce — do not broadcast again";
...
function deskBatch(bt, dec, { p, who } = {}) {
  ...
  // P2-C (docs/HUB_BROWSER_SIGN_VERIFY_2026-09-18.md, round 3): the raw batch's `browserSign` used
  // to be spread as-is into every desk view — handing its `nonce`/`idempotencyKey` to ANY operator
  // token, including one that could never call sign-request (P2-5) or, before R3-4 closed it,
  // observe. That readable nonce is what let a griefer's own observe() call be accepted as the
  // current generation's. The desk only ever needs it for a wallet that can actually sign (the
  // OBSERVE WHAT I BROADCAST button) — redact it for everyone else; the batch view is otherwise
  // unchanged, including `browserSign.state`/`failed`/`observedSigs`/`wallets`, none of which are
  // secret.
  let browserSign = bt.browserSign;
  if (browserSign) {
    const accepted = !!(p && (who === "owner" || who === p.fundingWallet || (Array.isArray(p.payoutSources) && p.payoutSources.includes(who))));
    if (!accepted) { const { nonce, idempotencyKey, ...rest } = browserSign; browserSign = rest; }
  }
  return { ...bt, browserSign, rows, remainingCount: Object.keys(remaining).length, remainingRaw: remainingRaw.toString(), remainingLines: pay.toAirdropLines(remaining, dec) };
}
...
function mount(app, deps) {
    ...
    try {
      ...
      if (b.waive) {
        ...
        try {
          ...
          // R3-1 (docs/HUB_BROWSER_SIGN_VERIFY_2026-09-18.md, round 3): &waive= frees a row's
          // unsettled remainder straight back to `available` (ledger.waiveRemainder), which is
          // exactly what made a bare &cancel= a double payment (F2) — a wallet this sign-request
          // handed out that the operator already broadcast for, but never observed, has no
          // settlement on file, so waiving it and re-exporting draws the SAME amount again with
          // no warning and no trace. Same guard &cancel=/&confirm= already carry.
          if (browserSignIsLive(bt0)) return res.status(409).json({ ok: false, error: `a browser-signed payout is ${bt0.browserSign.state} for this batch — resolve it with sign-request/observe (or ask the owner to clear it) before waiving` });
      ...
      return res.status(200).json({
        ...
        pendingBatches: pending.map((x) => ({ id: x.id, at: x.at, count: x.count, totalRaw: x.totalRaw, remaining: Object.keys(pay.remainingOf(x)).length })),
        batch: b.batch && batches[String(b.batch)] ? deskBatch(batches[String(b.batch)], dec, { p, who }) : null,
        as: who,
        ...report,
      });
    } catch (e) {
  ...
  app.all("/api/hub/:project/batch/:batchId/sign-request", limited("hubheavy", { windowMs: 60000, max: 30 }), async (req, res) => {
    ...
    try {
      ...
      for (const wallet of Object.keys(bt.amounts || {}).sort()) {
        ...
        // P3-H (docs/HUB_BROWSER_SIGN_VERIFY_2026-09-18.md, round 3): after P2-4/P2-5 the signer may
        // legitimately be an allowlisted `payoutSources` wallet, not just the funding wallet — every
        // row used to declare `p.fundingWallet` regardless, which is a lie for that caller (a second
        // client that honoured `source` would build a transfer from a wallet it cannot sign for).
        rows.push({ wallet, ok: true, source: who === "owner" ? p.fundingWallet : who, destination, amountRaw: rs.remainingRaw });
      }
      ...
      const browserSign = {
        ...
        payoutSources: Array.isArray(p.payoutSources) ? [...p.payoutSources] : [],
        wallets: signable.map((r) => r.wallet),
        // R3-8 (docs/HUB_BROWSER_SIGN_VERIFY_2026-09-18.md, round 3): carrying `observedSigs`
        // forward is load-bearing (P2-6 — an already-broadcast signature must stay observable under
        // a new nonce), but carrying `failed` forward is not: a force=1 or post-clear generation
        // has touched none of these rows yet, so it should not open already reporting a stale
        // failure (R3-3's carried-forward case made that worse — a row this generation never
        // touched would read failed forever). Start clean; keep the old map, unrendered, for
        // diagnosis only.
        failed: {}, previousFailed: (bt.browserSign && bt.browserSign.failed) || {},
  ...
  app.all("/api/hub/:project/batch/:batchId/observe", limited("hubheavy", { windowMs: 60000, max: 30 }), async (req, res) => {
    ...
    if (!who) return notFound(res);
    if (p.dryRun) return res.status(409).json(dryRunSignRefusal(p));
    // P1-B/R3-4 (docs/HUB_BROWSER_SIGN_VERIFY_2026-09-18.md, round 3): P2-5 restricted sign-request
    // to owner/funding-wallet/an allowlisted payoutSources wallet; observe had no matching check —
    // any operator token could POST a handful of unrelated-but-resolvable signatures and flip
    // `signing -> submitted` (nothing here required they attribute to a named wallet), and the
    // owner's only exit from `submitted` is `clear=1`, which now demands `&confirm=abandon-broadcast`
    // — an assertion that nothing landed — for a broadcast that in this case never happened. Same
    // gate sign-request already carries.
    if (who !== "owner" && who !== p.fundingWallet && !(Array.isArray(p.payoutSources) && p.payoutSources.includes(who))) {
      return res.status(403).json({ ok: false, error: `only the project's funding wallet (${p.fundingWallet}) — or an allowlisted payout source — may report broadcast signatures for this batch` });
    }
    if (typeof getTx !== "function") return res.status(503).json({ ok: false, error: "chain reads are not configured on this server — nothing recorded" });
    const b = body(req);
    ...
    if (!nonce || nonce !== preBt.browserSign.nonce) {
      return res.status(409).json({ ok: false, error: "this observe call's nonce does not match the batch's current sign-request — request a fresh sign-request, then observe the signatures you already broadcast under its new nonce — do not broadcast again", batchId });
    }
    ...
    if (nowUnix() - (Number(preBt.browserSign.requestedAt) || 0) > BROWSER_SIGN_MAX_AGE_S) {
      return res.status(409).json({ ok: false, error: "this sign-request is more than 24 hours old — request a fresh sign-request, then observe the signatures you already broadcast under its new nonce — do not broadcast again", batchId });
    }
    ...
    try {
      ...
      // R3-3 (docs/HUB_BROWSER_SIGN_VERIFY_2026-09-18.md, round 3): N2(a)'s zero-remaining sweep
      // used to run over `failedNow` BEFORE it was merged with the carried-forward
      // `bt.browserSign.failed` — so a wallet that failed in an EARLIER pass and was later paid
      // some other way (the desk's own `&sent=` recovery button) was never re-checked: it is not in
      // `failedNow` this pass (nothing about it happened this call) and not in `r.recorded` either
      // (this pass recorded nothing for it), so the carried entry survived forever, including into
      // the terminal `settled` record. Sweep the FULL merged map instead, after both merges above.
      for (const w of Object.keys(nextFailed)) {
        const rs = ledger.rowState(finalBatch, w, finalEntries);
        if (rs && rs.remainingRaw === "0") delete nextFailed[w];
      }
      ...
      // R3-2 (docs/HUB_BROWSER_SIGN_VERIFY_2026-09-18.md, round 3): that rule bricked a batch that
      // fully settled EXCEPT for one signature the RPC never indexed (a dropped/blockhash-lapsed
      // broadcast that will never land) — `bt.state` had already flipped to `sent` (nothing legacy-
      // owed), so no still-unindexed signature could possibly settle anything more, and yet the
      // batch stayed `submitted` forever with no route able to touch it but the owner's `clear=1`.
      // Mirror the one-liner `&sent=` already gets: once nothing is legacy-owed, `settled` wins
      // regardless of what is still unindexed.
      //
      // R3-4/P1-B (docs/HUB_BROWSER_SIGN_VERIFY_2026-09-18.md, round 3): the OTHER half of the
      // same finding — this pass may have resolved signatures that touched NO named wallet at all
      // (junk-but-resolvable signatures from any operator, before the caller gate above closed
      // that route; or, even for an accepted caller, an honest mix-up). Advancing `signing ->
      // submitted` on that alone lets one POST lock a batch behind `submitted`'s narrower recovery
      // (no `force=1` escape, only the owner's `clear=1`) for a flow that never actually broadcast
      // anything new. Only advance out of `signing` when this pass attributed something — a real
      // settlement or a real, wallet-specific refusal — to a wallet this sign-request named.
      const attributedNow = r.recorded.length > 0 || Object.keys(failedNow).length > 0;
      const nextState = finalBatch.state === "sent" ? "settled"
        : (!stillOwed && !notYetIndexed.length) ? "settled"
        : !attributedNow ? bt.browserSign.state
        : "submitted";
      // R3-5/P3-G (docs/HUB_BROWSER_SIGN_VERIFY_2026-09-18.md, round 3): `observedSigs` used to
      // union in EVERY signature the caller named that the RPC could resolve, whether or not it had
      // anything to do with this batch — up to ~40 per call, 30 calls/min/IP, unbounded and never
      // cleared, on a money record every payout route reads and rewrites whole. Keep only
      // signatures that actually attributed to a named wallet this pass (a real settlement/
      // attribution-failure attempt, or a real wallet-specific refusal — never the cross-product
      // noise NOISE_WHY_RE already filters out), capped at the most recent 100.
      const attributedSigs = new Set();
      for (const x of passed) if (x && x.sig) attributedSigs.add(x.sig);
      for (const rj of alertRefusals) if (rj && rj.sig) attributedSigs.add(rj.sig);
      const newlyAttributedSigs = resolvedSigs.filter((sg) => attributedSigs.has(sg));
      const observedSigs = [...new Set([...(bt.browserSign.observedSigs || []), ...newlyAttributedSigs])].slice(-100);
      const nextBrowserSign = { ...bt.browserSign, state: nextState, lastObservedAt: now, observedSigs, failed: nextFailed };
