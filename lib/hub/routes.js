...
function mount(app, deps) {
  const { kv, adminAuthOK, publicErrMsg, vault, connection, scanDeps, alert = (m) => console.warn("[hub] " + m), isDirect = (req) => !!req.cluckDirect,
    ...
    secret = () => process.env.PREMIUM_ACCESS_KEY, verifySignature = null } = deps;
  ...
  app.get("/api/hub/:project/desk", async (req, res) => {
    ...
    try {
      ...
      return res.status(200).json({
        ...
        armed: st.armed, startedAt: st.startedAt, versions: st.versions.map((v) => ({ version: v.version, effectiveFrom: v.effectiveFrom, effectiveTo: v.effectiveTo, hash: v.hash, terms: v.terms })),
  ...
  app.all("/api/hub-apply", limited("hubapply", { windowMs: 60000, max: 12 }), async (req, res) => {
    ...
    try {
      ...
      const pv = apply.preview(app0, { nowUnix: now });
  ...
  const MAX_QUOTES_PER_PROJECT = 100;   // rate-limited to 30/min already; this bounds the kv row
  ...
  app.all("/api/hub/:project/access", limited("pay", { windowMs: 60000, max: 30 }), async (req, res) => {
    ...
    try {
      if (!paying) {
        ...
        // When the cap bites, expired quotes go first; only then the oldest live one — a quote
        // still inside its pay window must not be evicted by a burst of GETs (deep dive P1-054).
        const nowMs = Date.now();
        Object.keys(book).filter((k) => (book[k].expiresMs || 0) < nowMs).forEach((k) => { delete book[k]; });
      ...
      if (typeof sigStore.add === "function" && !sigStore.add(SIG_KEY(sig))) console.warn(`[hub] ${p.id}: access payment ${sig.slice(0, 12)}… recorded but the signature store did not persist it`);
    ...
    try {
      ...
      if (b.sent) {
        ...
        const verdictOf = (r) => {
          ...
          return payoutVerify.rowPaidBy(txBySig.get(sg), { mint, wallet: String((r && r.wallet) || ""), minRaw: (bt.amounts || {})[String((r && r.wallet) || "")] });
        };
        const ok = results.filter((r) => verdictOf(r).ok);
        ...
        report.sent = { recorded: r.recorded, ignored: [...r.ignored, ...results.filter((x) => !verdictOf(x).ok).map((x) => ({ wallet: x && x.wallet, sig: x && x.sig, why: verdictOf(x).why }))], remaining: Object.keys(r.remaining).length, state: r.batch.state };
      }
      if (b.confirm) {
