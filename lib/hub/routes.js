...
function mount(app, deps) {
  ...
  app.all("/api/hub/:project/payout", async (req, res) => {
    ...
    const payoutLockKey = `hub:${p.id}:payout`;
    ...
    try {
      ...
      // adv P0-1: which wallets a transfer is allowed to have come FROM, for every settlement in
      // this request — the project's own funding wallet, plus any extra `fundedBy` wallet the
      // program version in force names (defaults to just the funding wallet — see
      // lib/hub/project.js validateTerms). Read once; every settle call below reuses it.
      const stateForFunding = hubStore.read(kv, p.id, "state", {}) || {};
      const cfgForFunding = eng.configFor({ project: p, state: stateForFunding, nowUnix: now });
      const fundedBy = (cfgForFunding && Array.isArray(cfgForFunding.fundedBy) && cfgForFunding.fundedBy.length) ? cfgForFunding.fundedBy : [p.fundingWallet];
      ...
      if (b.sent) {
        ...
        for (const x of passed) {
          if (!r.recorded.includes(x.wallet)) { journalReport.push({ wallet: x.wallet, sig: x.sig, journaled: false, why: "row was not newly recorded (already sent, or zero amount) — an unrelated transfer for it is not journaled" }); continue; }
          const { settled } = x;
        ...
        if (!rows.length) report.sweep = { ok: true, checked: 0 };
          ...
          if (typeof getTx === "function" && rs.confirmed && rs.confirmed.length) {
            ...
            for (const row of rs.confirmed) {
              ...
              const settled = settle.settleAndPersist({ projectId: p.id, journal: curJournal, batches: { [id]: bt2 }, batchId: id, wallet: row.wallet, sig: sigTrim, tx, mint, nowUnix: now, fundingWallet: p.fundingWallet, fundedBy });
              if (settled.journaled) {
        ...
        if (!recipients.length) report.send = { action: "none", reason: "every row in this batch is already recorded as sent" };
          ...
          if (typeof getTx === "function" && Array.isArray(r.paid) && r.paid.length) {
            ...
            for (const row of r.paid) {
              const sigTrim = String(row.sig || "").trim();
              let tx = null; try { tx = await getTx(sigTrim); } catch (_) { tx = null; }
              const settled = settle.settleAndPersist({ projectId: p.id, journal: curJournal, batches: curBatches, batchId: id, wallet: row.wallet, sig: sigTrim, tx, mint, nowUnix: now, fundingWallet: p.fundingWallet, fundedBy });
              if (settled.journaled) {
