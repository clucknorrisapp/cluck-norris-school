...
function mount(app, deps) {
  ...
  app.all("/api/hub/:project/batch/:batchId/sign-request", limited("hubheavy", { windowMs: 60000, max: 30 }), async (req, res) => {
    ...
    // never legacy-`sent` and so returns to `available` on a cancel. This clears `browserSign` back
    // to null WITHOUT touching amounts, the legacy sent/paid rows or the journal — nothing here can
    // ever "un-journal" a real settlement, it only forgets the in-flight sign-request bookkeeping so
    // a fresh sign-request (or the managed payer) can be tried again. Owner-only because it is a
    ...
    if (on(b.clear)) {
      ...
      try {
        ...
        const nextBatches = { ...batches, [batchId]: { ...bt, browserSign: null } };
        ...
        return res.status(200).json({ ok: true, cleared: true, batchId });
      } catch (e) { return res.status(400).json({ ok: false, error: publicErrMsg(e) }); }
  ...
  app.all("/api/hub/:project/batch/:batchId/observe", limited("hubheavy", { windowMs: 60000, max: 30 }), async (req, res) => {
    ...
    if (preBt.browserSign.state !== "signing" && preBt.browserSign.state !== "submitted") {
      return res.status(200).json({ ok: true, already: true, batchId, browserSign: preBt.browserSign, note: "this batch's browser-signed flow is already settled" });
    }
