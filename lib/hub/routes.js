  ...
  app.all("/api/hub-registry", async (req, res) => {
    ...
    try {
      ...
      if (b.approve != null || b.reject != null) {
        ...
        const r = apply.approve(book, reg, String(b.approve), { tier: b.tier, note: b.accessNote, nowUnix: nowUnix(), freshMintInfo, freshRewardMintInfo });
  ...
  app.all("/api/hub-apply", limited("hubapply", { windowMs: 60000, max: 12 }), async (req, res) => {
    ...
    try {
      ...
      if (req.method !== "POST") {
        ...
        return res.status(200).json({ ok: true, mint: String(b.mint), ...info, supported: !bad.length && proj.TOKEN_PROGRAMS.has(info.tokenProgram), unsupportedExtensions: bad, alreadyRegistered: taken ? taken.id : null });
      }
      ...
      if (Object.values(reg).some((p) => p && p.mint === app0.project.mint && p.status !== "suspended")) return res.status(409).json({ ok: false, error: "this mint already runs a program here" });
