  ...
  app.all("/api/hub-registry", async (req, res) => {
    ...
    try {
      ...
      if (b.id != null) {
        ...
        if (existingRow && existingRow.status !== "approved" && existingRow.mint !== project.mint && !hubStore.storeIsEmptyFor(kv, project.id)) {
    ...
    try {
      ...
      if (b.waive) {
        ...
        try {
          ...
          if (!hubStore.writeManyVerifiedMixed(kv, p.id, { batches }, { [waiveEntryKey]: waiveEntry })) {
