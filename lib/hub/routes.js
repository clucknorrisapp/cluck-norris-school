  ...
  app.all("/api/hub/:project/admin", async (req, res) => {
    ...
    try {
      ...
      return res.status(200).json({
        ...
        versions: st.versions.map((v) => withSchema("program-version", { version: v.version, effectiveFrom: v.effectiveFrom, effectiveTo: v.effectiveTo, hash: v.hash, terms: v.terms })),
  ...
  app.get("/api/hub/:project/desk", async (req, res) => {
    ...
    try {
      ...
      return res.status(200).json({
        ...
        armed: st.armed, startedAt: st.startedAt, versions: st.versions.map((v) => withSchema("program-version", { version: v.version, effectiveFrom: v.effectiveFrom, effectiveTo: v.effectiveTo, hash: v.hash, terms: v.terms })),
  ...
  app.all("/api/hub/:project/payout", async (req, res) => {
    ...
    try {
      ...
      if (b.sent) {
        ...
        // W3 dual-write (Addendum B1): for each row just verified-and-recorded, ALSO locate its
        // specific transfer instruction and settle the journal + stamp the attempt — folded into
        // batches[id] before the ONE persist below, alongside the legacy sent/paid row it came
        // from. A row whose transfer cannot be attributed to one instruction (see
        // lib/payout-verify.js locateTransferInstruction) keeps its legacy row — it DID land — but
        // is reported with journaled:false rather than approximated.
        let nextBatch = r.batch, nextJournal = hubJournal;
        const journalReport = [];
        for (const row of ok) {
          const settled = settle.settleAndPersist({ projectId: p.id, journal: nextJournal, batches: { [id]: nextBatch }, batchId: id, wallet: row.wallet, sig: row.sig, tx: txBySig.get(String(row.sig)), mint, nowUnix: now });
          nextJournal = settled.journal; nextBatch = settled.batches[id];
          journalReport.push({ wallet: row.wallet, sig: row.sig, journaled: settled.journaled, why: settled.why || null });
        }
        paid = r.paid; batches = { ...batches, [id]: nextBatch }; hubJournal = nextJournal;
        if (!hubStore.writeManyVerifiedMixed(kv, p.id, { batches, paid }, { [hubStore.JOURNAL_KEY]: hubJournal })) return res.status(500).json({ ok: false, error: "recorded in memory but the volume did not take the write — STOP and check DATA_DIR before sending more" });
        ...
        report.sent = { recorded: r.recorded, ignored: [...r.ignored, ...vr.rejected], remaining: Object.keys(r.remaining).length, state: nextBatch.state, journal: journalReport };
      }
      if (b.confirm) {
        ...
        if (!rows.length) report.sweep = { ok: true, checked: 0 };
          ...
          if (typeof getTx === "function" && rs.confirmed && rs.confirmed.length) {
            ...
            for (const row of rs.confirmed) {
              let tx = null; try { tx = await getTx(row.sig); } catch (_) { tx = null; }
              const settled = settle.settleAndPersist({ projectId: p.id, journal: curJournal, batches: { [id]: bt2 }, batchId: id, wallet: row.wallet, sig: row.sig, tx, mint, nowUnix: now });
          ...
          if (!hubStore.writeManyVerifiedMixed(kv, p.id, { batches, paid }, { [hubStore.JOURNAL_KEY]: hubJournal })) return res.status(500).json({ ok: false, error: "sweep did not persist — check DATA_DIR" });
        ...
        if (!recipients.length) report.send = { action: "none", reason: "every row in this batch is already recorded as sent" };
          ...
          if (typeof getTx === "function" && Array.isArray(r.paid) && r.paid.length) {
            ...
            for (const row of r.paid) {
              let tx = null; try { tx = await getTx(row.sig); } catch (_) { tx = null; }
              const settled = settle.settleAndPersist({ projectId: p.id, journal: curJournal, batches: curBatches, batchId: id, wallet: row.wallet, sig: row.sig, tx, mint, nowUnix: now });
              curJournal = settled.journal; curBatches = settled.batches;
              journalReport.push({ wallet: row.wallet, sig: row.sig, journaled: settled.journaled, why: settled.why || null });
            }
            if (hubStore.writeManyVerifiedMixed(kv, p.id, { batches: curBatches }, { [hubStore.JOURNAL_KEY]: curJournal })) { batches = curBatches; hubJournal = curJournal; }
