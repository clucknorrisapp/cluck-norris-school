...
function mount(app, deps) {
  ...
  app.all("/api/hub/:project/batch/:batchId/sign-request", limited("hubheavy", { windowMs: 60000, max: 30 }), async (req, res) => {
    ...
    const who = authOf(req, p);
    if (!who) return notFound(res);
    // W3 (Colosseum E10): refused before any store read or lock, exactly like the /payout route's
    // dryRun check above — a demo fixture has nothing real behind it to sign for.
    ...
    try {
      ...
      if (bt.browserSign && bt.browserSign.state === "submitted") {
        return res.status(409).json({ ok: false, error: "a browser-signed payout for this batch was already broadcast — observe it (or let its blockhash lapse) before requesting a new one", browserSign: { state: bt.browserSign.state, nonce: bt.browserSign.nonce } });
      }
      ...
      for (const wallet of Object.keys(bt.amounts || {}).sort()) {
        ...
        if (!rs || rs.remainingRaw === "0") continue;
      ...
      if (!signable.length) return res.status(400).json({ ok: false, error: "nothing remaining on this batch — every row is already paid or waived", rows });
      ...
      // until a transfer is actually signed and broadcast. This one ties an observe() call back to
      // the exact set of rows/amounts a sign-request handed out, so a stale or superseded
      // sign-request can never be conflated with a fresh one on the same batch.
  ...
  app.all("/api/hub/:project/batch/:batchId/observe", limited("hubheavy", { windowMs: 60000, max: 30 }), async (req, res) => {
    ...
    try {
      ...
      if (bt.browserSign.state !== "signing" && bt.browserSign.state !== "submitted") {
        return res.status(200).json({ ok: true, already: true, batchId, browserSign: bt.browserSign, note: "this batch's browser-signed flow is already settled" });
      }
      let paid = hubStore.read(kv, p.id, "paid", {}) || {};
      let hubJournal = hubStore.readJournal(kv) || {};
      ...
      // Fetch every submitted signature ONCE — the same all-or-nothing convention the /payout
      // route's &sent= branch uses: an RPC failure on ANY of them aborts with `unavailable` before
      // any state changes (never a partial "some observed, some not" write), so a flaky RPC never
      // looks like a bad signature and the batch never regresses out of `signing`/`submitted`.
      const txBySig = new Map();
      try { for (const sg of sigs) txBySig.set(sg, (await getTx(sg)) || null); }
      catch (e) { return res.status(503).json({ ok: false, error: "could not read the transactions on chain right now — nothing recorded, try again: " + publicErrMsg(e), retry: true }); }
      ...
      const results = [];
      for (const wallet of wallets) for (const sig of sigs) results.push({ wallet, sig });
      const remainingByWallet = {};
      for (const w of wallets) {
        const rs = ledger.rowState(bt, w, ledger.journalFor(hubJournal, p.id));
        if (rs) remainingByWallet[w] = rs.remainingRaw;
      }
      const vr = payoutVerify.verifyBatchRows({ results, txBySig, batches, batchId, mint, amounts: remainingByWallet, notBefore: bt.at, fundingWallet: p.fundingWallet, fundedBy, nowUnix: now, journal: hubJournal, projectId: p.id });
      ...
      for (const [wallet, walletSigs] of byWallet) {
        ...
        const settled = settle.settleAndPersist({ projectId: p.id, journal: trialJournal, batches: { [batchId]: bt }, batchId, wallet, sig: winner.sig, tx: txBySig.get(winner.sig), mint, nowUnix: now, fundingWallet: p.fundingWallet, fundedBy });
      ...
      const stillOwed = wallets.some((w) => { const rs = ledger.rowState(finalBatch, w, ledger.journalFor(hubJournal, p.id)); return rs && rs.remainingRaw !== "0"; });
      // A wallet's failure is worth surfacing whichever pass caught it — rowPaidBy's own token-
      // balance check (vr.rejected, e.g. an undersized transfer or one not sourced from the
      // funding wallet) just as much as PASS 1's journal-side refusal (amount_mismatch against the
      // row's remaining, a duplicate candidate, row_not_in_batch). PASS 1's reason wins when both
      // fired for the same wallet — it is the more specific, journal-aware verdict.
      const failedNow = {};
      for (const rj of vr.rejected) if (rj && wallets.includes(rj.wallet)) failedNow[rj.wallet] = { why: rj.why, at: now };
      ...
      // A wallet that settled just now is cleared from `failed` — a prior bad attempt for it
      // should not still read as failed once the real payment has landed.
      for (const x of passed) { if (x.settled && x.settled.journaled) delete nextFailed[x.wallet]; }
      const nextBrowserSign = { ...bt.browserSign, state: stillOwed ? "submitted" : "settled", lastObservedAt: now, observedSigs: [...new Set([...(bt.browserSign.observedSigs || []), ...sigs])], failed: nextFailed };
      ...
      return res.status(200).json({
        ...
        remainingWallets: wallets.filter((w) => { const rs = ledger.rowState(finalBatch, w, ledger.journalFor(hubJournal, p.id)); return rs && rs.remainingRaw !== "0"; }),
      });
    } catch (e) {
