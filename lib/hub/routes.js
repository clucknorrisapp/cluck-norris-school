...
function mount(app, deps) {
  ...
  app.all("/api/hub/:project/payout", async (req, res) => {
    ...
    try {
      ...
      const vaultOperatorPubkeys = () => { try { return typeof vault.operatorPubkeys === "function" ? vault.operatorPubkeys() : []; } catch (_) { return []; } };
      ...
      if (b.sent) {
        ...
        // Round 2 #5 (docs/HUB_JOURNAL_VERIFY_2026-09-18.md): the PASS-1 alert further below only
        // ever sees rows that made it PAST this legacy check (`vr.accepted`) — a row this check
        // itself refuses as `transfer_not_from_funding_wallet` (adv P0-1, a stranger's transfer) or
        // as a cross-project signature reuse (adv P0-2a, `payoutVerify.TRANSFER_ALREADY_CONSUMED_WHY`
        // — Codex's original two-project attack) never reached the PASS-1 loop at all, so neither
        // fraud signal ever fired. Both are hard refusals here too (`vr.rejected`, never recorded),
        // so this alerts on exactly those two `why` values — the exact strings verifyBatchRows
        // produces — the moment the check runs, before anything else in this branch.
        for (const rj of vr.rejected) {
          if (rj.why === "transfer_not_from_funding_wallet" || rj.why === payoutVerify.TRANSFER_ALREADY_CONSUMED_WHY) {
            try { alert(`${p.id}: batch ${id} wallet ${String(rj.wallet || "").slice(0, 6)}… settlement REFUSED (not recorded) — ${rj.why}`); } catch (_) {}
          }
        }
        // PASS 1 — the journal's own verdict, BEFORE any legacy write (adv P0-2): a row the legacy
        // check above accepted can still be a settlement the journal must REFUSE — most
        // importantly `transfer_already_consumed` (this exact on-chain identity already settled a
        // DIFFERENT row/project, Codex's original two-project attack) and, since P0-1,
        // `transfer_not_from_funding_wallet`. Either is a hard refusal: the row is dropped here and
        // NEVER reaches recordSent, so a refused settlement can never zero what a holder is owed.
        // The one non-fraud outcome — settle.ATTRIBUTION_FAILURE_WHY ("could not attribute…") — is
        // NOT a refusal (the transfer DID land, it just cannot be keyed into the journal, e.g. a
        // batched transaction), so that row proceeds to the legacy write exactly as it always did.
        // Sequential (trialJournal carries forward) so a second row in THIS SAME request reusing an
        // identity already claimed by an earlier row in this loop is caught too.
        let trialJournal = hubJournal;
        const passed = [];        // { wallet, sig, settled } — survived the fraud filter
        const hardRejected = [];  // { wallet, sig, why } — refused, never reaches recordSent
        for (const row of vr.accepted) {
          const sigTrim = String(row.sig || "").trim();
          const settled = settle.settleAndPersist({ projectId: p.id, journal: trialJournal, batches: { [id]: bt }, batchId: id, wallet: row.wallet, sig: sigTrim, tx: txBySig.get(sigTrim), mint, nowUnix: now, fundingWallet: p.fundingWallet, fundedBy });
          if (!settled.journaled && settled.why !== settle.ATTRIBUTION_FAILURE_WHY) {
            hardRejected.push({ wallet: row.wallet, sig: row.sig, why: settled.why });
            // adv P0-1/P0-2: a hard refusal here is a fraud SIGNAL, not routine noise — a wrong
            // signature is not this. Surfaced so a pattern of these is visible, not just returned
            // to whoever submitted the request.
            try { alert(`${p.id}: batch ${id} wallet ${row.wallet.slice(0, 6)}… settlement REFUSED (not recorded) — ${settled.why}`); } catch (_) {}
            continue;
          }
          if (settled.journaled) trialJournal = settled.journal;
          passed.push({ wallet: row.wallet, sig: sigTrim, settled });
        }
        ...
        report.sent = { recorded: r.recorded, ignored: [...r.ignored, ...vr.rejected, ...hardRejected], remaining: Object.keys(r.remaining).length, state: finalBatch.state, journal: journalReport };
      }
      if (b.confirm) {
      ...
      if (on(b.sweep)) {
        ...
        if (!rows.length) report.sweep = { ok: true, checked: 0 };
          ...
          // Round 2 #4: &sweep= reconciles whatever this project's batches already record as
          // pending — it has no `from=` (unlike &send=) to say which ONE vault project signed a
          // given row, so every vault project's own operator wallet is an allowed source here.
          // Still only OUR OWN signing keys (whirlpoolMM.vault.operatorPubkeys()), never a
          // holder- or operator-supplied wallet.
          const fundedByForSweep = [...fundedBy, ...vaultOperatorPubkeys()];
          if (typeof getTx === "function" && rs.confirmed && rs.confirmed.length) {
            ...
            for (const row of rs.confirmed) {
              ...
              if (settled.journaled) {
                ...
                try { alert(`${p.id}: batch ${id} wallet ${row.wallet.slice(0, 6)}… confirmed by sweep but journal refused it — ${settled.why}`); } catch (_) {}
              } else {
                try { alert(`${p.id}: batch ${id} wallet ${row.wallet.slice(0, 6)}… confirmed by sweep but not journaled — ${settled.why}`); } catch (_) {}
              }
          ...
          if (typeof getTx === "function" && Array.isArray(r.paid) && r.paid.length) {
            ...
            for (const row of r.paid) {
              ...
              if (settled.journaled) {
                ...
                try { alert(`${p.id}: batch ${id} wallet ${row.wallet.slice(0, 6)}… paid but not journaled — ${settled.why}`); } catch (_) {}
              } else {
                // Our own broadcast, journal refused it anyway (should not happen — surfaced, not swallowed).
                try { alert(`${p.id}: batch ${id} wallet ${row.wallet.slice(0, 6)}… paid but journal refused it — ${settled.why}`); } catch (_) {}
              }
              journalReport.push({ wallet: row.wallet, sig: row.sig, journaled: settled.journaled, why: settled.why || null });
            }
