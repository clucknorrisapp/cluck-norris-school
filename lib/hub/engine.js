...
// `dryRun` (Colosseum E10): a labelled placeholder project whose terms and funding wallet are not
// yet agreed (e.g. the POKEAHOE dry run). It must never accrue or pay, so arming is refused here
// too — not only at the route (lib/hub/routes.js checks the same flag first, for a clean HTTP
// error), so a future caller of this pure function directly can never bypass it.
function arm(raw, nowUnix, { dryRun = false } = {}) {
  ...
  if (dryRun) throw new Error("this project is a DRY RUN — terms are not yet agreed, so arming is refused (nothing may accrue or pay)");
...
async function runAll({ kv, nowUnix, deps, skip = ["cuna"], onAlert = null, reason = "tick" }) {
  ...
  for (const [projectId, project] of Object.entries(registry)) {
    // dryRun projects are never armed (see arm() above), so accrualTick's gate() would already
    // refuse them — this is a belt-and-suspenders skip so a dry run never even reaches a scan.
    if (skip.includes(projectId) || !project || project.approved === false || project.dryRun === true) continue;
...
function walletView({ project, state, addr, locks, days, paid, batches, nowUnix, journal = null }) {
  const w = String(addr || "");
  ...
  // crash P1-4 (docs/HUB_JOURNAL_VERIFY_2026-09-18.md): this is the public holder-facing "owed now"
  // (GET /api/hub/:project/holder) — it must consult the same settlement journal the payout desk
  // does, or the two disagree with no alert the moment the legacy and journal witnesses diverge on
  // even one row. `journal` is optional (a caller that predates this — e.g. a unit test building a
  // walletView directly — sees exactly the behavior it always had); the live route always passes it.
  const owedRaw = pay.owedNow({ days: days || {}, paid: paid || {}, pending: batches || {}, journal, projectId: journal ? project.id : null })[w] || 0n;
