...
function rowState(batch, wallet, entries) {
  ...
  // NEW-2 (Round 5, docs/HUB_JOURNAL_VERIFY_2026-09-18.md): a row whose remainder reached 0 partly
  // or wholly through a waive was reported "paid" — indistinguishable from a row every raw unit of
  // which actually moved on chain. "partial-waived" (some applied, the rest waived) and "waived"
  // (nothing applied, all of it waived) say plainly that not everything named here was collected.
  const state = remaining === 0n
    ? (waived > 0n ? (applied > 0n ? "partial-waived" : "waived") : "paid")
    : applied > 0n ? "partial" : "pending";
...
// Round 3 #1 (docs/HUB_JOURNAL_VERIFY_2026-09-18.md): `exactOnly` is set by every LIVE caller
// (lib/hub/settle.js settleAndPersist, which is the only path lib/hub/routes.js ever calls) and
// left off by the pure ledger's own direct callers (scripts/hub-core-test.cjs) so their overpayment
// arithmetic keeps working unit-tested in isolation. With it set, a transfer whose amount is not
// EXACTLY the row's remaining is refused as `amount_mismatch` rather than capped into
// applied/excess — this is what makes PASS 1 order-independent: whichever candidate transfer
// happens to equal the remaining amount settles the row, and nothing else can, regardless of the
// order the caller submitted candidates in. Every entry `exactOnly` ever produces therefore has
// `excessRaw === "0"` — there is no excess arithmetic left to reach on the live path.
function settle({ journal, projectId, batch, wallet, transfer, nowUnix, exactOnly = false }) {
  const { sig, instructionIndex, innerIndex, amountRaw, slot, verifiedBy = "getTransaction", sourceWallet = null } = transfer || {};
  ...
  // Round 3 #1: fail closed on anything that is not an exact match, rather than cap it into
  // applied/excess — an oversized OR undersized transfer is refused outright, never journaled,
  // and (per the route's own PASS 2/3 wiring) never recorded as this row's legacy `sent` state
  // either, so the genuine matching transfer can still settle the row later, in any order.
  if (exactOnly && amount !== remaining) {
    return { ok: false, error: "amount_mismatch", expectedRaw: str(remaining), actualRaw: str(amount) };
  }
...
function batchView(batch, journal) {
  ...
  for (const w of Object.keys(batch.amounts || {})) {
    ...
    rows[w] = r;
    // NEW-2 (Round 5, docs/HUB_JOURNAL_VERIFY_2026-09-18.md): a batch is "sent" only when every row
    // was actually PAID — a row waived to zero (state "waived" or "partial-waived") has a
    // remainder that never moved on chain, so calling the whole batch "sent" would claim money was
    // delivered when part of it was written off instead.
    if (!r || r.state !== "paid") allDone = false;
  }
...
function receipt({ projectId, batch, wallet, journal, project }) {
  ...
  if (!r) return null;
  // N3 (docs/HUB_JOURNAL_VERIFY_2026-09-18.md, Round 3): `payoutSources` lets a settlement land
  // from an allowlisted wallet other than `fundingWallet` — the receipt must be able to say so,
  // never let a third-party-funded settlement look indistinguishable from the funding wallet's
  // own payment. `paidFrom` is the one wallet every settlement on this row was sourced from; a row
  // whose settlements disagree on source (mixed funding, or an older entry with no recorded
  // source) reports null rather than guess which one to show.
  const sources = new Set(r.settlements.map((e) => e.sourceWallet).filter(Boolean));
  const paidFrom = sources.size === 1 ? [...sources][0] : null;
  return {
    ...
    settlements: r.settlements.map((e) => ({ xferKey: e.xferKey, sig: e.sig, instructionIndex: e.instructionIndex, innerIndex: e.innerIndex, slot: e.slot, at: e.at, amountRaw: e.amountRaw, appliedRaw: e.appliedRaw, excessRaw: e.excessRaw, sourceWallet: e.sourceWallet || null, verifiedBy: e.verifiedBy })),
    claims: {
