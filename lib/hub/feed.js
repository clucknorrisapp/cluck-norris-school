...
// ── AA1 follow-up: "follow a wallet without a project" — the same item kinds as buildFeedItems
// above, but spanning every project a wallet actually appears in (the exact set GET
// /api/hub/wallet/:wallet already resolves via hubPublic.walletLookup). TWO kinds only, per the
// roadmap note this shipped under: `version`/`commitment` (a project this wallet is in changed its
// terms — the wallet doesn't have to be paid under a version to want to know the rules moved) and
// `batch` (a settled batch that actually PAID this wallet — never a batch it wasn't part of, and
// never an unsent/"owed" row, same B3 rule buildFeedItems already follows). No `snapshot` kind
// here — a holder-count snapshot is a fact about the MINT, not about one wallet.
//
// Item ids are prefixed with the project id (`<projectId>:version:v1`, `<projectId>:batch:<id>`)
// so two different projects' version 1 (or a coincidentally-matching batch id) can never collide
// once folded into one wallet-wide list — buildFeedItems itself never needs this, since it only
// ever looks at one project at a time.
//
// Titles/summaries are prefixed with the project's own label (`"<label> — Program version 1
// published"`) — the ONE place this module ever puts an operator-controlled string (a project
// label, never sanitised beyond a length cap — see server.js hubProjects()) into feed text, so
// toRss()'s escHtml() on title/summary is what actually keeps a label like `<b>Evil</b> & Co` from
// producing invalid XML or executing anything; JSON Feed needs no extra escaping (JSON.stringify
// already handles it). Keep this the only kind of item that ever carries a label, and never trust
// it un-escaped anywhere else this module writes to the wire.
function walletScoped(it, projectId, label) {
  if (!it) return null;
  return { ...it, id: `${projectId}:${it.id}`, title: `${label} — ${it.title}`, summary: `${label}: ${it.summary}` };
}

// `projects`: [{ projectId, label, versions, batches, receiptsByBatch }] — exactly the projects
// the caller already found this wallet in (never the full registry; a project the wallet has no
// row in contributes nothing, same honesty rule the empty per-wallet roll-up already keeps).
function buildWalletFeedItems({ wallet, projects = [], base } = {}) {
  const w = String(wallet || "");
  const originBase = trimSlash(base);
  const items = [];
  for (const proj of Array.isArray(projects) ? projects : []) {
    if (!proj || !proj.projectId) continue;
    const { projectId, label, versions = [], batches = {}, receiptsByBatch = {} } = proj;
    const projectLabel = String(label || projectId);

    for (const v of Array.isArray(versions) ? versions : []) {
      const it = walletScoped(versionItem({ v, projectId, base: originBase }), projectId, projectLabel);
      if (it) items.push(it);
    }

    const commitList = (Array.isArray(versions) ? versions : [])
      .filter((v) => v && v.commitment && v.commitment.sig)
      .map((v) => ({ version: v.version, hash: v.hash, ...v.commitment }));
    for (const c of commitList) {
      const it = walletScoped(commitmentItem({ c }), projectId, projectLabel);
      if (it) items.push(it);
    }

    // Only batches that actually sent THIS wallet money — never a batch it merely shares a
    // project with, and never a batch that hasn't reached "sent" for it yet.
    for (const [id, b] of Object.entries(batches || {})) {
      if (!b || !b.sent || !b.sent[w]) continue;
      const it = walletScoped(batchItem({ id, b, rep: receiptsByBatch ? receiptsByBatch[id] : null, projectId, base: originBase }), projectId, projectLabel);
      if (it) items.push(it);
    }
  }
...
module.exports = { buildFeedItems, buildWalletFeedItems, toJsonFeed, toRss };
