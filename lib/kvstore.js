...
// Several keys in ONE persist, verified together. The payout journal is two keys (the batch's
// sent rows and the paid totals); written one after the other, a crash between them left a row
// marked sent with nothing in paid, and the ledger math then offered that money again (Codex,
// 2026-09-17, finding 1). Both land in the same write or neither is reported as landed.
function setManyVerified(entries) {
  refresh();
  const list = Object.entries(entries || {});
  for (const [k, v] of list) state[k] = v;
  persist();
  if (!persistent || lastPersistError) return false;
  try {
    const o = JSON.parse(fs.readFileSync(FILE, "utf8"));
    return !!o && list.every(([k, v]) => Object.prototype.hasOwnProperty.call(o, k) && JSON.stringify(o[k]) === JSON.stringify(v));
  } catch (_) { return false; }
}

// Read-only: every [key, value] pair whose key starts with `prefix`. Exists so a counter can be
// DERIVED from durable per-signature audit records written one-key-per-record (e.g.
// lib/tool-pass-redeem.js's "toolPassPaid:<sig>" line) instead of a running total nothing checks.
// Never mutates; never used to walk the whole store (callers always pass a real, narrow prefix).
function entriesWithPrefix(prefix) {
  refresh();
  const p = String(prefix || "");
  if (!p) return [];
  const out = [];
  for (const k of Object.keys(state)) if (k.startsWith(p)) out.push([k, state[k]]);
  return out;
}
