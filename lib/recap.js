...
function persistSoon() {
  ...
  timer = setTimeout(() => {
    ...
    try { require("./atomic-write").atomicWriteFileSync(FILE, JSON.stringify(win)); } catch (e) { console.warn(`[recap] persist failed: ${e.message}`); }
  }, 3000);
...
function reset() {
  win = emptyWindow();
  if (persistent) { try { require("./atomic-write").atomicWriteFileSync(FILE, JSON.stringify(win)); } catch (_) {} }
}
