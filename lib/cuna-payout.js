...
function recordSent({ batch, paid, results, nowUnix }) {
  if (!batch || batch.state !== "pending") throw new Error("only a pending batch can record sends");
  const nextPaid = { ...(paid || {}) };
  ...
  const next = { ...batch, sent, state: allSent ? "sent" : "pending" };
  if (allSent) next.completedAt = Number(nowUnix) || 0;
