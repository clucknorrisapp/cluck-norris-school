...
function owedNow({ days, paid, pending, journal = null, projectId = null }) {
  ...
  for (const b of Object.values(pending || {})) {
    ...
    for (const [w, raw] of Object.entries(b.amounts)) {
      ...
      // NEW-2 (Round 5, docs/HUB_JOURNAL_VERIFY_2026-09-18.md): an owner waive (lib/hub/ledger.js
      // waiveRemainder — "still owed, the next batch draws it") frees a row's unsettled remainder
      // back to `ledger.partition`'s `available`, but this function used to hold the row's FULL
      // original amount regardless, so the desk (`owedNow`) kept reporting 0 for a wallet the
      // holder view already showed as owed again. `waivedRaw` is this row's own waived amount
      // (never another row's — `batch.waived` is keyed by wallet, one row per wallet per batch);
      // subtracting it from what stays held is what makes the two witnesses agree without needing
      // `&cancel=` first.
      const waivedRaw = big(b.waived && b.waived[w] && b.waived[w].amountRaw) || 0n;
      // An unsent row of a PENDING batch is held (it is about to be paid) minus whatever of it is
      // already accounted for — as settled (rowSettled, counted above) or as waived (freed back).
      // A sent row is in `paid`; holding it too would subtract it twice.
      if (!(b.sent && b.sent[w]) && b.state === "pending") {
        let heldAmt = v - rowSettled - waivedRaw;
        if (heldAmt < 0n) heldAmt = 0n;
        held[w] = (held[w] || 0n) + heldAmt;
      }
    }
  }
