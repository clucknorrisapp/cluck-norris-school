...
const {
  ...
  KNOWN_CEX_WALLETS, KNOWN_SERVICE_WALLETS,
} = require("./solana-addr");
const { ddEnabled, ddTokenReport } = require("./webacy-dd"); // DD.xyz (Webacy) third-party risk cross-check (no-op without WEBACY_API_KEY)
const { rpcFetch: failoverRpcFetch, primaryRpcUrl } = require("./rpc"); // failover + per-attempt timeout for JSON-RPC reads

const BAGS_BASE = "https://public-api-v2.bags.fm/api/v1/";

// "held by the launchpad" must come from the AUTHORITY ITSELF, never a vanity mint
// suffix or an authority-string prefix — both are trivially grindable (a 4-char vanity
// suffix/prefix takes seconds), so either let a hand-rolled, dev-held-authority token
// wear the "the team can't unilaterally rebrand" verdict. Exact-match against the known
// platform service wallets (the same table the acquisition tracer trusts) is the only
// thing the chain actually proves; anything else falls through to the stricter
// mutable-risk label (CLAUDE.md: "say what's on-chain, never why").
function isPlatformHeldAuthority(updateAuthority) {
  return !!(updateAuthority && KNOWN_SERVICE_WALLETS[updateAuthority]);
}
...
async function runAutopsy(mint, { nocache = false } = {}) { // eslint-disable-line no-unused-vars
  ...
  // Routed through lib/rpc's failover-aware fetch (was a bare `fetch(rpcUrl)` bound to the
  // primary Helius key with no timeout and no failover — a 429/credit-cap/stalled socket
  // hung or failed the whole report instead of rolling to FALLBACK_RPC_URL/HELIUS_API_KEY_2/
  // public). DAS-only methods (getAsset, getTokenAccounts) stay Helius-restricted inside
  // rpcFetch's own HELIUS_ONLY_METHODS guard, so this is a safe drop-in for every method here.
  function rpcCall(id, method, params) {
    return failoverRpcFetch(primaryRpcUrl(), {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  try {
    const [dexData, supplyData, mintInfoData] = await Promise.allSettled([
      fetch(`https://api.dexscreener.com/token-pairs/v1/solana/${mint}`, { signal: AbortSignal.timeout(10000) }).then(r => r.json()),
    ...
    try {
      ...
      if (asset && typeof asset.mutable === "boolean") {
        ...
        const platformHeld = isPlatformHeldAuthority(updateAuthority);
        let status, label;
        if (!asset.mutable) {
          status = "immutable";
    ...
    try {
      ...
      if (sampleArr.length > 0) {
        // Routed through the canonical batched helper (timeout + 429 retry + scanQuality
        // accounting) — was a raw fetch that silently returned nothing on a throttle/error,
        // invisible to the scan-quality banner.
        const { txs } = await heliusEnhancedBatched(sampleArr.slice(0, 100), HELIUS_KEY, "lifetime-sample", heliusTxCache, scanQuality);
        parsedTxs = txs;
      }
        ...
        if (bagsInfo.dbcPoolKey) {
          try {
            ...
            if (dbcSigs.length > 0) {
              ...
              try {
                // Canonical batched helper (timeout + 429 retry + scanQuality accounting) —
                // was a raw fetch that silently returned zero buyers on a throttle/error.
                const { txs: dData } = await heliusEnhancedBatched(sampled, HELIUS_KEY, "dbc-buyers", heliusTxCache, scanQuality);
    ...
    if (pumpInfo && pumpInfo.bondingCurve) {
      try {
        ...
        if (bcSigs.length > 0) {
          ...
          // Canonical batched helper (timeout + 429 retry + scanQuality accounting) — was
          // a raw fetch that silently returned zero buyers on a throttle/error.
          const { txs: dData } = await heliusEnhancedBatched(sampled, HELIUS_KEY, "pump-bc-buyers", heliusTxCache, scanQuality);
          for (const tx of dData) {
            if (!tx || !Array.isArray(tx.tokenTransfers)) continue;
            for (const tt of tx.tokenTransfers) {
    ...
    try {
      ...
      if (firstSigsToFetch.length > 0) {
        // Canonical batched helper (timeout + 429 retry + scanQuality accounting) — was a
        // raw fetch that, on a throttle/error, silently left every holder "UNKNOWN".
        const { txs: acqData } = await heliusEnhancedBatched(firstSigsToFetch.slice(0, 100), HELIUS_KEY, "acquisition", heliusTxCache, scanQuality);
        const acqMap = new Map();
        for (const tx of acqData) { if (tx?.signature) acqMap.set(tx.signature, tx); }
        ...
        if (distSigsToFetch.length > 0) {
          try {
            // Canonical batched helper (timeout + 429 retry + scanQuality accounting) — was
            // a raw fetch that silently left every distributor "UNKNOWN" on a throttle/error.
            const { txs: dData } = await heliusEnhancedBatched(distSigsToFetch, HELIUS_KEY, "distributor", heliusTxCache, scanQuality);
            const dMap = new Map();
            for (const t of dData) { if (t?.signature) dMap.set(t.signature, t); }
...
module.exports = { runAutopsy, bagsFetch, heliusEnhancedBatched, BAGS_BASE, isPlatformHeldAuthority };
