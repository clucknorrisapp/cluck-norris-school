...
const { cgFetch } = require("./lp-scanner"); // shared CoinGecko Pro onchain (→ free GT) fetch
const {
  isOnCurve, DEX_PROGRAMS, LOCKER_PROGRAMS, TOKEN_PROGRAMS, PROGRAM_LABELS,
  KNOWN_CEX_WALLETS,
} = require("./solana-addr");
const { ddEnabled, ddTokenReport } = require("./webacy-dd"); // DD.xyz (Webacy) third-party risk cross-check (no-op without WEBACY_API_KEY)
...
// BOUNDED, because autopsy is a public endpoint. Entries were added and never removed: an
// expired entry was overwritten only if that exact (wallet, mint) pair was asked for again, so
// running autopsies on a stream of different mints grew this Map without limit — and each entry
// holds a full trace including its teamNetwork. On a memory-capped container that is a slow OOM
// anyone can drive from outside.
//
// Insertion-ordered Map, so the oldest key is simply the first one. Map.set on an EXISTING key
// does not move it, so writes go through touch() — delete then set — which makes re-use refresh
// the entry's position and turns the front of the Map into a genuine least-recently-used end.
// Without that it would be FIFO, and a mint being autopsied repeatedly would be evicted on
// schedule regardless of how hot it was. Losing an entry costs one re-fetch, not correctness.
const CREATOR_TRACE_MAX = 500;
function touchCreatorTrace(key, value) {
  CREATOR_TRACE_CACHE.delete(key);
  CREATOR_TRACE_CACHE.set(key, value);
}
function trimCreatorTraceCache() {
  const now = Date.now();
  for (const [k, v] of CREATOR_TRACE_CACHE) {
    if (!v || now - v.savedAt > CREATOR_TRACE_TTL_MS) CREATOR_TRACE_CACHE.delete(k);
  }
  // Strictly BELOW the cap: this runs before the caller inserts, so leaving exactly MAX here
  // would settle at MAX+1. Off by one, but "bounded at the number I wrote down" is the whole
  // claim being made.
  while (CREATOR_TRACE_CACHE.size >= CREATOR_TRACE_MAX) {
    const oldest = CREATOR_TRACE_CACHE.keys().next();
    if (oldest.done) break;
    CREATOR_TRACE_CACHE.delete(oldest.value);
  }
}
function bestObservedTrace(wallet, mint, fresh) {
  trimCreatorTraceCache();
  ...
  if (!cached || Date.now() - cached.savedAt > CREATOR_TRACE_TTL_MS) {
    touchCreatorTrace(key, { trace: fresh, savedAt: Date.now() });
  ...
  touchCreatorTrace(key, { trace: merged, savedAt: Date.now() });
  if (regressedAny) {
...
// GeckoTerminal pool list mapped into the DexScreener pair shape this engine consumes.
// WHY: DexScreener silently drops some LIVE tokens (its token-pairs endpoint returns []),
// which would make the autopsy read solPairs.length === 0 and declare a tradeable token
// "GHOST / NEVER LAUNCHED" (the ROSE case: 0 DexScreener pairs, 5 real GeckoTerminal pools).
// GeckoTerminal indexes many of those tokens and returns equivalent liquidity/volume/age/
// tx data, so mapping it into the same shape keeps every downstream number AND the verdict
// correct — no single-indexer dependency. Used only as a fallback when DexScreener is empty.
async function fetchGeckoTerminalPairs(mint) {
  try {
    const r = await fetch(`https://api.geckoterminal.com/api/v2/networks/solana/tokens/${mint}/pools`, { signal: AbortSignal.timeout(10000) });
    if (!r.ok) throw new Error(`geckoterminal ${r.status}`);   // an outage must be distinguishable from a genuine empty (no pools)
    ...
    for (const d of (j.data || [])) {
      ...
      const baseAddr = String(rel.base_token?.data?.id || "").replace(/^solana_/, "");
      const quoteAddr = String(rel.quote_token?.data?.id || "").replace(/^solana_/, "");
      const nameParts = String(a.name || "").split("/").map((s) => s.trim());
      const createdMs = a.pool_created_at ? Date.parse(a.pool_created_at) : null;
      const pc = a.price_change_percentage || {};
      const tx = (a.transactions && a.transactions.h24) || {};
      out.push({
        chainId: "solana",
        dexId: String(rel.dex?.data?.id || "?"),
        pairAddress: a.address,
        baseToken: { address: baseAddr, symbol: nameParts[0] || "?" },
        quoteToken: { address: quoteAddr, symbol: nameParts[1] || "?" },
        priceUsd: a.base_token_price_usd != null ? String(a.base_token_price_usd) : null,
        priceNative: a.base_token_price_native_currency != null ? String(a.base_token_price_native_currency) : null,
        liquidity: { usd: num(a.reserve_in_usd) || 0 },
        volume: { h24: num(a.volume_usd?.h24) || 0 },
        txns: { h24: { buys: Number(tx.buys) || 0, sells: Number(tx.sells) || 0 } },
        pairCreatedAt: (createdMs && Number.isFinite(createdMs)) ? createdMs : null,
        priceChange: { h1: num(pc.h1), h6: num(pc.h6), h24: num(pc.h24) },
        _source: "geckoterminal",
      });
    }
    return out;   // a genuine 200 with no pools returns [] — distinct from the throw above
  } catch (e) { throw e; }   // let the caller distinguish an indexer OUTAGE from "no pools"
}
...
async function runAutopsy(mint, { nocache = false } = {}) { // eslint-disable-line no-unused-vars
  ...
  // DD.xyz (Webacy) third-party risk cross-check — fired in parallel so it adds
  // ~no latency, returns { available:false } when WEBACY_API_KEY is unset, and
  // can NEVER throw or block the autopsy (every DD call is timeout-bounded + caught).
  const ddPromise = ddEnabled()
    ? ddTokenReport(mint).catch(() => ({ available: false }))
  ...
  try {
    ...
    let allPairs = dexData.status === "fulfilled" && Array.isArray(dexData.value) ? dexData.value : [];
    // Indexer-independence: if DexScreener has no pairs (it drops some live tokens), fall
    // back to GeckoTerminal so a tradeable token isn't mislabeled "GHOST / NEVER LAUNCHED".
    // If BOTH indexers are unreachable we must NOT conclude "never launched" — that's a data
    // OUTAGE, not a dead token. Track it so the verdict below stays UNCLEAR instead of GHOST.
    let indexersUnavailable = false;
    if (!allPairs.length) {
      try { allPairs = await fetchGeckoTerminalPairs(mint); }
      catch (_) { allPairs = []; indexersUnavailable = true; }   // GT (our GHOST-rescue) errored → no authority to call it dead
    }
    const rawSolPairs = allPairs.filter(p => p.chainId === "solana" || !p.chainId);
    // PRICE-OUTLIER FILTER (2026-07-04, owner-diagnosed: a broken Orca pool priced CLKN ~5000x
    // high). A mispriced pool poisons EVERY USD number it reports — its own liquidity.usd and
    // volume.usd are valued at the fake price, so it wins liquidity- AND volume-based sorts and
    // inflates the aggregates. Defense: take the MEDIAN price across base-side pairs (priceUsd
    // is the base token's price, so only base-side pairs vote) and drop base-side pairs whose
    // price deviates >3x from it — a majority of independent pools can't all be wrong the same
    // way (arbitrage ties the honest ones together). One- or two-pool tokens: median of what
    // exists; a 2-pool >3x disagreement keeps both (can't adjudicate) — same as before this fix.
    const basePrices = rawSolPairs
      .filter((p) => p?.baseToken?.address === mint)
      .map((p) => parseFloat(p.priceUsd)).filter((x) => x > 0).sort((a, b) => a - b);
    const medianPrice = basePrices.length
      ? (basePrices.length % 2 ? basePrices[(basePrices.length - 1) / 2]
        : (basePrices[basePrices.length / 2 - 1] + basePrices[basePrices.length / 2]) / 2)
      : null;
    const priceOutlier = (p) => {
      if (basePrices.length < 3 || !(medianPrice > 0)) return false; // too few pools to adjudicate
      if (p?.baseToken?.address !== mint) return false;              // quote-side price is the other token's
      const x = parseFloat(p.priceUsd);
      return x > 0 && (x / medianPrice > 3 || medianPrice / x > 3);
    };
    const solPairs = rawSolPairs.filter((p) => !priceOutlier(p));
    ...
    const basePairs = solPairs.filter((p) => p?.baseToken?.address === mint);
    const topPair = (basePairs.length ? basePairs : solPairs).slice()
    ...
    const decimals = supplyData.status === "fulfilled" ? (supplyData.value?.result?.value?.decimals ?? 9) : 9;   // ?? not || — a real 0-decimal mint must stay 0, not coerce to 9
    ...
    const largestRaw = allTokenAccounts
      ...
      .slice(0, 100);
    // Honesty flag: the DAS getTokenAccounts slice caps at 5,000 accounts AND is not
    // balance-sorted (see the CEX-detection note below), so on a large-holder token a real
    // whale can sit beyond the window — making top-10 concentration a FLOOR, not the truth.
    const holdersTruncated = allTokenAccounts.length >= MAX_HOLDER_PAGES * 1000;
    ...
    let verdict;
    if (solPairs.length === 0 && indexersUnavailable) {
      // Both DexScreener and GeckoTerminal were unreachable — we literally cannot see the token's
      // pools. Declaring "NEVER LAUNCHED" here would publicly (and cache-persistently) call a live
      // token a dead rug off a transient outage. Stay honest: UNCLEAR + degraded (never cached).
      verdict = { type: "UNCLEAR", label: "Verdict unavailable — token data providers unreachable, try again", severity: "UNKNOWN", color: "#6B7280", icon: "⏳", degraded: true };
    } else if (solPairs.length === 0) {
      ...
      // Behavioral (not mechanism) honeypot: inferred from a sell count, and that count is
      // single-source (DexScreener's 24h feed). Disclose the source and the confirmation step
      // so the DEAD label isn't read as a proven on-chain mechanism the way the Token-2022 branch is.
      verdict = { type: "HONEYPOT", label: "Cause of Death: HONEYPOT", severity: "DEAD", color: "#EF4444", icon: "🪤",
        evidenceNote: `No sell in ${buys24h} recent trades per DexScreener's 24h feed (single source) — confirm with a test-sell or a second indexer before treating as certain.` };
    ...
    if (topPair && topPair.pairAddress) {
      try {
        // CoinGecko Pro onchain when COINGECKO_API_KEY is set (full lifetime candles + no
        // throttling), else free GeckoTerminal. Daily candles across the token's life.
        const gtData = await cgFetch(`/networks/solana/pools/${topPair.pairAddress}/ohlcv/day?aggregate=1&limit=365`);
    ...
    try {
      ...
      if (targets.length > 0) {
        ...
        if (behaviorBreakdown.EXITED_MOSTLY >= 3) redFlags.push(`${behaviorBreakdown.EXITED_MOSTLY} of the top ${topHolders.length} holders have sold most of their original position — distribution pattern.`);
        if (holderBreakdown.sniper >= 4) {
          ...
          redFlags.push(launchpad
            ? `${holderBreakdown.sniper} of the top ${topHolders.length} bought within the first minute — concentrated early entry. On a ${lp} token this is typically early bonding-curve buyers or snipers/bots, NOT proof of team or insider coordination.`
            : `${holderBreakdown.sniper} of the top ${topHolders.length} bought within the first minute — heavy early concentration (snipers/bots or coordinated entry; on-chain timing alone can't tell which).`);
        }
        if (holderBreakdown.locker + holderBreakdown.selflock === 0 && verdict.type !== "ALIVE" && verdict.type !== "AT_RISK") redFlags.push(`None of the top ${topHolders.length} holdings are in a recognized lock — no enforced supply restrictions.`);
      }
    } catch (err) {
    ...
    try {
      const _jk = process.env.JUPITER_API_KEY || "";
      const r = await fetch(`${_jk ? "https://api.jup.ag" : "https://lite-api.jup.ag"}/tokens/v2/search?query=${mint}`, {
        headers: _jk ? { "x-api-key": _jk } : {},
    ...
    if (creatorStatus && creatorStatus.wallet) {
      try {
        ...
        // Same honesty flag the lifetime walk uses (see `truncated` at the 5000-sig cap):
        // if we filled all 5 pages, older buys/sells may lie beyond the window — so a
        // sell count derived from this scan is a FLOOR, not a proven lifetime total.
        const creatorSigsTruncated = allCreatorSigs.length >= 5000;
        ...
        creatorTrace = {
          ...
          sigsTruncated: creatorSigsTruncated,   // scan hit the 5,000-sig cap → counts are a floor
        ...
        try {
          ...
          // Exclusion set for sub-distributor candidates: CEX deposit wallets, DEX/LP
          // programs, token programs, and any labeled program — none of these are a
          // team sub-distributor. (Lockers are handled by traceLockerSet.) This was
          // referenced below but never defined, which threw and silently disabled the
          // entire team-network trace; defining it turns the phase on.
          const excludeSet = new Set([
            ...Object.keys(KNOWN_CEX_WALLETS),
            ...DEX_PROGRAMS,
            ...TOKEN_PROGRAMS,
            ...Object.keys(PROGRAM_LABELS),
          ]);
    ...
    if (creatorStatus && (creatorTrace || (walletPnl && walletPnl.projectWallets && walletPnl.projectWallets.length > 0))) {
      ...
      if (creatorActivity) {
        ...
        // Honesty guard: if the creator scan hit its 5,000-sig cap AND no independent indexer
        // (Solana Tracker) corroborates, a low/zero sell count is a FLOOR, not a proven lifetime
        // total — say so, and render the count as "≥N", so a page-capped scan can never quietly
        // clean-wash a dumper. Suppressed when ST gives a second-source view of the same history.
        const sellsIncomplete = !!creatorActivity.sigsTruncated && !creatorActivity.solanaTrackerCrossCheck;
        const sellsFloor = sellsIncomplete ? "≥" : "";
        const sellsHedge = sellsIncomplete
          ? " ⚠️ Creator history capped at 5,000 signatures — the sell count is a floor; older sells may lie beyond the scan."
          : "";
        ...
        if ((isLocking || (tn && tn.totalLockCount > 0)) && effectiveBuys >= 3 && sellsAreNegligible) {
          ...
          creatorStatus.summary = `🔄🔒 ${handle}${rankSuffix} is running a buy-and-lock — ${effectiveBuys} market buys + ${effectiveLocks} lock deposits, ${sellsFloor}${effectiveSells} actual market sells lifetime${buyFreqStr}.${solStr}${reinvestedStr}.${dualFundingStr} ${Math.round(effectiveLockedTokens).toLocaleString()} CLKN permanently locked.${sellsHedge}`;
          ...
          creatorStatus.summary = `🔄 ${handle} is actively buying back${rankSuffix} — ${creatorActivity.buyCount} buys vs ${sellsFloor}${creatorActivity.sellCount} sells lifetime${buyFreqStr}, net +$${netUsd.toFixed(2)} into the token.${sellsHedge}`;
    ...
    try {
      ...
      if (lockHolders.length > 0) {
        // Verified team wallets — ONLY a launchpad-API-confirmed creator (bags/pump official).
        // The old gate was `verified !== false && source !== genesis`, but creatorStatus.verified
        // is never set, so it silently admitted a `solana-tracker` creator — an INDEXER GUESS — as
        // "team-verified" and surfaced it as teamVerifiedLockedPct. Per the brand rule ("only call
        // a wallet 'team' when a launchpad API confirms it"), require an official launchpad source.
        const teamWallets = new Set();
        if (creatorStatus && creatorStatus.wallet
          && (creatorStatus.source === "bags-official" || creatorStatus.source === "pump-official")) {
        ...
        for (const lh of lockHolders.slice(0, 15)) {
          try {
            ...
            const txRes = await rpcCall(`autopsy-lock-tx-${lh.rank}`, "getTransaction", [fundingSig, { encoding: "jsonParsed", maxSupportedTransactionVersion: 1 }]);
    ...
    try {
      if (distributors && distributors.length > 0 && topHolders && topHolders.length > 0) {
        ...
        for (const d of distributors) {
          ...
          const isTransferThenDump = recipientStats.length >= 2 && dumpedMostPct >= 0.5;
          if (isTransferThenDump) {
        ...
        // SAY WHAT'S ON-CHAIN, NEVER WHY. The chain shows that wallet A sent tokens to N wallets
        // and that M of them then sold most of what they received. It does NOT show that anyone
        // agreed to do it. "Coordinated" is an accusation of intent between parties, and we
        // cannot see intent — the same trace is produced by an OTC desk filling buyers who then
        // took profit, a team paying contributors, or an airdrop to people who sold. Naming the
        // pattern precisely is the forensic value; asserting the motive is what would make us
        // the thing this project exists to be an alternative to.
        //
        // So: state the transfers, state the sells, state what a simple sell scan would miss,
        // and let the reader draw the conclusion. Nothing measured is removed here.
        for (const ex of hiddenExitDistributors) {
          const srcShort = ex.sourceWallet.slice(0, 8) + "…";
          const alsoSold = ex.sourceAlsoSold
            ? ` The source wallet also sold $${ex.sourceAlsoSold.soldUsd.toFixed(0)} across ${ex.sourceAlsoSold.sellCount} swaps of its own.`
            : "";
          redFlags.push(`🚨 TRANSFER-THEN-DUMP PATTERN: wallet ${srcShort} transferred to ${ex.totalRecipients} top holders, and ${ex.dumpedMostCount} of them have since sold more than 70% of what they received.${alsoSold} A sell scan of the source wallet alone would not show this — the selling happened from the recipients. What the transfers were for is not on-chain; the transfers and the sells are.`);
        }
      }
    } catch (e) {
    ...
    if (ANTHROPIC_KEY) {
      const facts = {
        ...
        top10Concentration: top10Share !== null
          ? `${(top10Share * 100).toFixed(0)}%${holdersTruncated ? " (floor — holder scan capped at 5,000 unsorted accounts; true concentration may be higher)" : ""}`
          : null,
        top10ConcentrationPartial: holdersTruncated,
...
- READ THE TRANSFER-THEN-DUMP PATTERNS IF PRESENT (hiddenExitPatterns). These flag wallets that transferred tokens to multiple OTHER wallets which then sold most of what they received. When count > 0 this is the SHARPEST signal on the page: lead with it, and use the wording "transfer-then-dump pattern". Say what the chain shows — how many recipients, how many sold, how much — and say plainly that a sell scan of the source wallet alone would miss it. Do NOT assert WHY it happened: "coordinated", "obfuscation", "deliberately hid", "worked together" are all claims about intent and agreement that no on-chain record can support, and the identical trace is produced by an OTC desk filling buyers who took profit, a team paying contributors, or an airdrop to people who sold. When sourceAlsoSold is true, add that the source wallet also sold directly — as a second fact, not as proof of a motive. These detections exclude verified Bags/Pump creator wallets and known LP/locker addresses. The reader draws the conclusion; we hand them the evidence.
...
- No markdown asterisks, no headers. Plain prose paragraphs.`;
    ...
    return { status: 200, body: {
      ...
      symbol, name,
      verdict,
      // DD.xyz (Webacy) independent risk cross-check — null unless WEBACY_API_KEY
      // is set. A second opinion alongside our own on-chain checks; rendered as a
      // distinct panel, never overrides our verdict.
      dd: ddRisk && ddRisk.available
        ? { riskScore: ddRisk.riskScore, riskLevel: ddRisk.riskLevel, flags: (ddRisk.flags || []).slice(0, 8), source: "dd.xyz" }
        : null,
      ...
      facts: {
        ...
        top10ConcentrationPartial: holdersTruncated,   // true = scan hit the 5,000-account cap; share is a floor
        topPair: topPair ? {
          dexId: topPair.dexId,
...
module.exports = { runAutopsy, bagsFetch, heliusEnhancedBatched, BAGS_BASE,
  // Exported for scripts/cache-bound-test.cjs. The cache is fed by a PUBLIC endpoint, so its
  // bound is a memory-safety property, not an implementation detail.
  _creatorTraceCache: { map: CREATOR_TRACE_CACHE, bestObservedTrace, MAX: CREATOR_TRACE_MAX } };
