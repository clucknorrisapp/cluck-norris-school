...
// ⚠️ P1-B: classify on the EXACT integer `amount` string the RPC returns, never on `uiAmount`.
// `uiAmount` is `f64 | null` in the RPC schema (jsonParsed's own tokenAmount shape) — a
// Token-2022 account with withheld transfer fees, or any account the RPC can't ui-scale for
// whatever reason, comes back with `uiAmount: null`, and `Number(null) === 0` — so a naive
// `Number(uiAmount) || 0` reads a wallet that STILL HOLDS TOKENS as empty and offers to close it,
// losing the balance. `amount` is the raw base-unit integer as a string and is never null for an
// account that exists — it is either a real digit string or (extremely defensively) absent, and
// this function treats "absent/non-numeric" as holds_balance, NEVER reclaimable, so an unreadable
// balance can only ever err toward refusing to close, not toward closing. uiAmount is kept
// alongside for DISPLAY only (scanReclaimable below) — it never drives this decision.
function classify(raw) {
  if (raw.mint === WSOL_MINT) {
    return { status: "refused", reason: "Wrapped SOL — closing it is not offered here." };
  }
  if (typeof raw.amount === "string" && raw.amount === "0") {
  ...
  for (const programId of TOKEN_PROGRAMS) {
    ...
    for (const acc of list) {
      ...
      raw.push({
        ...
        mint: info.mint,
        program: programId,
        // The exact base-unit integer, as a string — the field classify() actually decides on.
        // Not coerced through Number(): a huge balance can exceed float precision, and the whole
        // point is to compare the string, never a lossy numeric cast of it.
        amount: typeof ta.amount === "string" ? ta.amount : null,
        // Display-only from here on (see classify()'s header comment) — never treated as zero
        // when it's actually null.
        uiAmount: typeof ta.uiAmount === "number" ? ta.uiAmount : null,
