...
// The Seeker app adds a second door (owner, 2026-09-19, docs/SEEKER_APP_PLAN.md §7): hold SKR
// worth its own dollar figure and the heavy tools are free there too — same live pricing, same
// re-check at use. Each door carries ITS OWN figure (owner, 2026-09-22: "$20 of SKR or $10 of
// CLKN"; `input.usd` for CLKN, `input.skrUsd` for SKR — both live in server.js's TOOLGATE and
// come from the environment, never from here). Two doors with three outage policies each is the
...
//   input.usd         the dollar figure of the CLKN door (TOOLGATE.usd — never a token amount)
//   input.skrUsd      the dollar figure of the SKR door (TOOLGATE.skrUsd); defaults to input.usd
...
async function qualify(input) {
  ...
  const skrUsd = Number.isFinite(Number(input.skrUsd)) && Number(input.skrUsd) > 0 ? Number(input.skrUsd) : usd;
  ...
  if (doors.includes("skr")) {
    ...
    if (!skrPrice) { log("no usable SKR price with the skr door requested — SKR not checked, CLKN answer stands"); skr = { unavailable: "price", balance: null, needed: null, priceUsd: null, holdUsd: skrUsd }; }
    else {
      let s;
      try { s = await input.readSkr(); } catch (e) { s = { unavailable: true, error: e && e.message }; }
      if (!s || s.unavailable) { log("SKR balance read unavailable — SKR not checked, CLKN answer stands: " + ((s && s.error) || "no result")); skr = { unavailable: "rpc", balance: null, needed: needed(skrUsd, skrPrice), priceUsd: skrPrice, holdUsd: skrUsd }; }
      else {
        const skrNeeded = needed(skrUsd, skrPrice);
        const skrBal = Number(s.balance) || 0;
        if (skrBal >= skrNeeded) return remember({ ok: true, via: "holder-skr", balance: skrBal, needed: skrNeeded, holdUsd: skrUsd, doors });
        skr = { balance: skrBal, needed: skrNeeded, priceUsd: skrPrice, holdUsd: skrUsd };
      }
    }
  }
  ...
  else detail = `The free tier needs about $${usd} of CLKN (~${clknNeeded.toLocaleString()} at the current price) or about $${skrUsd} of SKR (~${skr.needed.toLocaleString()}); that wallet holds ${Math.round(clknBal).toLocaleString()} CLKN and ${Math.round(skr.balance).toLocaleString()} SKR.${tail}`;
