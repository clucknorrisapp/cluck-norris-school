...
function classifyBuyersFromRawTxs(rawTxs, mint, { solUsd = 0, tokenPriceUsd = 0 } = {}) {
  ...
  for (const tx of (rawTxs || [])) {
    ...
    // PAID BY SOMEONE ELSE (owner report 2026-09-07, ROSE comp: a $49 Phantom "buy with cash" never
    // reached the board). Phantom Cash, fiat on-ramps and some relayers execute the swap from THEIR
    // signer: that wallet spends the quote (CASH/USDC/SOL), the POOL gives up the token, and the
    // token lands in the customer's wallet — which therefore "received but paid nothing" and was
    // skipped as a transfer. It is a buy, and the customer is the buyer. The tell that separates it
    // from an airdrop/transfer: an OFF-CURVE owner (a pool vault) net-lost the token in this tx AND
    // some other ON-CURVE wallet that did not receive the token spent quote. That spend is credited
    // to the receiver(s), pro-rata by tokens received when there are several.
    let poolLostMint = false, othersSpendSol = 0, unpaidMintIn = 0;
    for (const [owner, d] of byOwner) {
      const mi = d[mint] || 0;
      if (mi < -1e-9 && !isOnCurve(owner)) poolLostMint = true;
      if (mi <= 1e-9 && isOnCurve(owner)) { const sp = spendOf(owner, d); if (sp.spent) othersSpendSol += sp.sol; }
    }
    if (poolLostMint && othersSpendSol > 0) {
      for (const [owner, d] of byOwner) {
        const mi = d[mint] || 0;
    ...
    for (const [owner, d] of byOwner) {
      ...
      if (!own.spent) {
        // received the token but paid no quote → a transfer/airdrop, UNLESS someone else paid the
        // pool for it in this very tx (see above) — then it is that payer's spend, credited here.
        if (!(poolLostMint && othersSpendSol > 0 && unpaidMintIn > 0)) continue;
        quoteOutSol = othersSpendSol * (mintIn / unpaidMintIn);
      }
