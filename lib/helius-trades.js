...
const { isOnCurve, LOCKER_PROGRAMS } = require("./solana-addr");   // real wallets are on-curve; pool vaults / PDAs are off-curve
...
async function getTokenBuyersInWindowHelius(mint, fromMs, toMs, {
  ...
  for (const a of (extraTargets || [])) if (a && /^[1-9A-HJ-NP-Za-km-z]{32,44}$/.test(String(a))) targets.add(String(a));
  try {
...
async function getWalletTokenPositionHelius(wallet, mint, {
  ...
  if (sigs.size) {
    ...
    for (const tx of txs) {
      ...
      if (mintOut > 1e-9) recs.push({ quoteIn, outTo });   // classified in pass 2
    }
    ...
    for (const r of recs) {
      ...
      if (toLocker.length && !outDests.length) { locks++; continue; }
      ...
      if (r.quoteIn > 1e-6 || toPool) sells++;
      else { transfersOut++; for (const to of outDests) transferDests[to] = (transferDests[to] || 0) + r.outTo[to]; }
    }
  }
  ...
  return { balance, sells, transfersOut, locks, transferDests: dests, source: "helius" };
}
