...
async function getTokenBuyersInWindowHelius(mint, fromMs, toMs, {
  ...
  return { buyers, txsScanned: rawTxs.length, source: "helius", reachedWindowStart };
}
...
function classifyBuyersFromRawTxs(rawTxs, mint, { solUsd = 0, tokenPriceUsd = 0 } = {}) {
  ...
  for (const tx of (rawTxs || [])) {
    ...
    const w = keys[0];   // fee payer / signer

    // token deltas for the fee payer, per mint (uiAmount is decimals-adjusted)
    const d = {};
    for (const b of (tx.meta.preTokenBalances || [])) {
      if (b.owner !== w) continue;
      d[b.mint] = (d[b.mint] || 0) - (Number(b.uiTokenAmount && b.uiTokenAmount.uiAmount) || 0);
    }
    ...
    const solDelta = idx >= 0 ? ((Number(tx.meta.postBalances[idx]) || 0) - (Number(tx.meta.preBalances[idx]) || 0)) / 1e9 : 0;

    const mintIn = d[mint] || 0;
    const stableOut = Math.max(0, -(d[USDC] || 0)) + Math.max(0, -(d[USDT] || 0));
    const wsolOut = Math.max(0, -(d[WSOL] || 0));
    const jupOut = Math.max(0, -(d["JUPyiwrYJFskUPiHa7hkeR8VUtAeFoSYbKedZNsDvCN"] || 0));
