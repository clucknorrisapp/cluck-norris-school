...
const { isOnCurve, LOCKER_PROGRAMS, SOL_ADDR_RE } = require("./solana-addr");   // real wallets are on-curve; pool vaults / PDAs are off-curve
...
async function getTokenBuyersInWindowHelius(mint, fromMs, toMs, {
  ...
  // Known POOLS FIRST, largest holders last, so an indexed pool is never displaced out of the
  // target slice by large NON-pool holders (locked-LP, CEX hot wallets, treasury, airdrop
  // escrows) — that silently undercounted buys on any token whose pool vault ranked below the
  // slice by balance. Order: (1) pools pinned on the comp (`extraTargets` — the owner's own
  // Orca CUNA/USDC pool, 2026-09-09), (2) DexScreener pairs, (3) GeckoTerminal pools — that day
  // DexScreener indexed 2 CUNA pairs while GeckoTerminal listed 10, so a pool only the latter
  // knew about was invisible to the scan — then (4) the largest token accounts.
  for (const a of (extraTargets || [])) if (a && SOL_ADDR_RE.test(String(a))) targets.add(String(a));
  try {
  ...
  try {
    const gt = await (await fetch(`https://api.geckoterminal.com/api/v2/networks/solana/tokens/${mint}/pools?page=1`, {
      headers: { accept: "application/json", "user-agent": "Mozilla/5.0 (compatible; clucknorris.app buy-comp)" }, signal: AbortSignal.timeout(8000) })).json();
    for (const p of ((gt && Array.isArray(gt.data)) ? gt.data : [])) {
      const addr = p && p.attributes && p.attributes.address;
  ...
  // Coverage caps: the first `maxTargets` targets (pools first), maxSigPagesPerPool pages (12k sigs) each — plenty for
  ...
  for (const pool of [...targets].slice(0, Math.max(1, Number(maxTargets) || 14))) {   // pools first (see above)
    let before = null;
    let covered = false;
...
const CASH = "CASHx9KJUStyftLFWGvEVf59SGeG9sh5FfcnZMVPCASH";   // Phantom Cash (USD stable) — what a Phantom "buy with cash" purchase spends
...
function classifyBuyersFromRawTxs(rawTxs, mint, { solUsd = 0, tokenPriceUsd = 0 } = {}) {
  ...
  for (const tx of (rawTxs || [])) {
    ...
    // Quote spent by an owner in this tx, in SOL terms (stables via solUsd).
    const spendOf = (owner, d) => {
      const stableOut = Math.max(0, -(d[USDC] || 0)) + Math.max(0, -(d[USDT] || 0)) + Math.max(0, -(d[CASH] || 0));
      ...
      const spent = stableOut > 1e-9 || wsolOut > 1e-9 || jupOut > 1e-9 || solOut > 0;
      return { spent, sol: solOut + wsolOut + (solUsd > 0 ? stableOut / solUsd : 0) };
    };
    // THE POOL LEG DECIDES (owner reports 2026-09-07, ROSE comp — two misses in one evening).
    // (1) A $49 Phantom "buy with cash": Phantom's cash account spent CASH, the pool gave up ROSE,
    //     the ROSE landed in the customer's wallet, which itself spent nothing.
    // (2) A $3.57 buy paid with a different token: DFlow sold it through a pump.fun pool for SOL and
    //     bought ROSE — no real wallet spent SOL/USDC/anything on our quote list at all.
    // Requiring the RECEIVER (or any real wallet) to have spent quote skipped both as "transfers".
    // What the buyer paid with is irrelevant. The one tell that a token was BOUGHT is the pool:
    // an OFF-CURVE owner (a pool vault) net-LOST the token AND an off-curve owner net-GAINED quote
    // (wSOL / native SOL / USDC / USDT / CASH / JUP) in the same tx. Every on-curve wallet that
    // netted +token in such a tx is a buyer; if it spent quote itself that is its size, otherwise
    // it is credited with the pool's quote intake pro-rata by tokens received. A wallet-to-wallet
    // transfer or airdrop has no pool loss; an LP withdrawal has pool loss but no pool quote gain —
    // neither is a buy.
    // Only the pool(s) that GAVE UP the token count — a routed buy (CASH → USDC → SOL → ROSE) has a
    // pool taking in quote at every hop, and summing all of them quadrupled the Phantom Cash buy.
    let poolLostMint = false, poolQuoteGainSol = 0, unpaidMintIn = 0;
    const mintLosers = new Set();
    for (const [owner, d] of byOwner) {
      if (isOnCurve(owner) || !((d[mint] || 0) < -1e-9)) continue;
      poolLostMint = true; mintLosers.add(owner);
      const stableIn = Math.max(0, d[USDC] || 0) + Math.max(0, d[USDT] || 0) + Math.max(0, d[CASH] || 0);
      poolQuoteGainSol += Math.max(0, d[WSOL] || 0) + Math.max(0, d[JUP] || 0) + (solUsd > 0 ? stableIn / solUsd : 0);
    }
    for (const [key, dv] of solByKey) { if (dv > BC_SOL_FEE_FLOOR && mintLosers.has(key)) poolQuoteGainSol += dv; }   // native-SOL pools (bonding curves hold SOL on the curve PDA itself)
    if (poolLostMint && poolQuoteGainSol > 0) {
      for (const [owner, d] of byOwner) {
        const mi = d[mint] || 0;
        if (mi > 1e-9 && isOnCurve(owner) && !spendOf(owner, d).spent) unpaidMintIn += mi;
      }
    }
    ...
    for (const [owner, d] of byOwner) {
      ...
      const own = spendOf(owner, d);
      let quoteOutSol = own.sol;
      if (!own.spent) {
        // received the token but paid no quote itself → a transfer/airdrop, UNLESS a pool sold it
        // for quote in this very tx (see above) — then the pool's intake is this buyer's size.
        if (!(poolLostMint && poolQuoteGainSol > 0 && unpaidMintIn > 0)) continue;
        quoteOutSol = poolQuoteGainSol * (mintIn / unpaidMintIn);
      }
      ...
      const cur = buyers.get(owner) || { wallet: owner, buyCount: 0, volumeSol: 0, maxBuySol: 0, tokensBought: 0, buysSol: [] };
      cur.buyCount++; cur.volumeSol += buySol; cur.tokensBought += mintIn;
      if (cur.buysSol.length < 500) cur.buysSol.push(buySol);   // per-buy sizes (bounded) — buy-comp entries ("every $3 buy = N horses")
...
async function getWalletTokenPositionHelius(wallet, mint, {
  ...
  let sells = 0, transfersOut = 0, locks = 0;
  ...
  if (sigs.size) {
    ...
    // TWO PASSES. Pass 1 collects each tx's mint/quote legs; classification waits for pass 2,
    // because "off-curve" on its own cannot tell a POOL VAULT from a LOCK ESCROW and we have
    // to resolve the destinations' owning programs first.
    const recs = [];
    for (const tx of txs) {
      ...
      if (mintOut > 1e-9) recs.push({ quoteIn, outTo, sig: tx.signature || null });   // classified in pass 2
    }
    // Resolve the owning program of every OFF-CURVE destination, batched 100 per call. Only
    // off-curve addresses need this (a real wallet is on-curve and is never an escrow), so on a
    // normal wallet this adds no RPC at all.
    const offCurve = [...new Set(recs.flatMap((r) => Object.keys(r.outTo)).filter((o) => !isOnCurve(o)))];
    const ownerProg = new Map();
    for (let i = 0; i < offCurve.length; i += 100) {
      const chunk = offCurve.slice(i, i + 100);
      let vals = [];
      try { const r = await rpc("getMultipleAccounts", [chunk, { encoding: "base64" }]); vals = (r && r.value) || []; } catch (_) {}
      chunk.forEach((a, j) => { const v = vals[j]; if (v && v.owner) ownerProg.set(a, v.owner); });
    }
    // Pass 2 — classify each outflow. `sig` is captured on the FIRST tx of each class — the
    // public standings page's on-chain evidence link (Colosseum roadmap §7): the reader clicks
    // through to the transaction that actually decided the hold-through call, instead of taking
    // our word for "sold" / "moved out" / "locked".
    let sellSig = null, transferOutSig = null, lockSig = null;
    for (const r of recs) {
      const all = Object.keys(r.outTo);
      const toLocker = all.filter((o) => LOCKER_PROGRAMS.has(ownerProg.get(o)));
      const outDests = all.filter((o) => !LOCKER_PROGRAMS.has(ownerProg.get(o)));
      // A LOCK is the mint moving into a locker program's escrow (Jupiter Lock and friends).
      // It is the OPPOSITE of an exit — the tokens are provably immobilised — so it counts as
      // neither a sell nor a transfer out. Escrows are PDAs and therefore off-curve, so the
      // old on-curve-only test read every lock as a pool sale and disqualified the locker from
      // the very hold check the Locker Room exists to help them pass.
      if (toLocker.length && !outDests.length) { locks++; if (!lockSig) lockSig = r.sig; continue; }
      // A sell is the mint leaving the wallet for QUOTE — or for a POOL. The quote test alone
      // only saw SOL/wSOL/USDC coming back, so a sell into a CUNA/$VETT or CUNA/JUP pool (any
      // other-token pair) read as a plain transfer and the seller kept their place on the board
      // (owner, 2026-09-09: arbers round-tripping across pools must not show as buyers). Pool
      // vaults are owned by off-curve PDAs and real wallets are on-curve, so the destination
      // tells the two apart without a pool list. An off-curve destination we could NOT resolve
      // keeps the old, conservative reading: treat it as a pool.
      const toPool = outDests.some((o) => !isOnCurve(o));
      if (r.quoteIn > 1e-6 || toPool) { sells++; if (!sellSig) sellSig = r.sig; }
      else { transfersOut++; if (!transferOutSig) transferOutSig = r.sig; for (const to of outDests) transferDests[to] = (transferDests[to] || 0) + r.outTo[to]; }
    }
    ...
    return { balance, sells, transfersOut, locks, transferDests: dests, source: "helius", sellSig, transferOutSig, lockSig };
  }
  return { balance, sells, transfersOut, locks, transferDests: [], source: "helius", sellSig: null, transferOutSig: null, lockSig: null };
}
...
async function getTradeTapeHelius(mint, fromMs, toMs, {
  ...
  const sigs = new Set();
  let capped = false;
  // THREE WAYS THIS CAN MISS TRADES, AND ONLY ONE OF THEM USED TO SAY SO.
  //
  // `capped` covered the maxSigs ceiling. The other two truncated in silence:
  //   - an RPC error (thrown, or a 200 carrying {error} — throttle/quota, which is NOT an empty
  //     page) hit `catch { break }` and abandoned the rest of that pool's history;
  //   - running out of maxSigPagesPerPool before paging back past fromMs stopped mid-window.
  // Either one returns a SHORT tape that looks complete. Callers scoring a promo off it (see
  // lib/cuna-giveaway.js) would then miss buys and sells — and a missed SELL is a wallet that
  // keeps entries it should have lost.
  //
  // reachedWindowStart mirrors getTokenBuyersInWindowHelius above: true only if EVERY scanned
  // target got back past the window start or genuinely ran out of history. false = the oldest
  // in-window trades may be missing, and the caller must not treat this tape as authoritative.
  let reachedWindowStart = true;
  const poolErrors = [];
  outer:
  for (const pool of [...targets].slice(0, 10)) {
    ...
    for (let page = 0; page < maxSigPagesPerPool; page++) {
      ...
      try {
        ...
        // A 200 carrying {error} is a throttle or quota refusal, not "no more signatures".
        // Reading it as an empty page is what makes a rate-limited scan look complete.
        if (r && r.error) rpcErr = String((r.error && r.error.message) || "rpc error").slice(0, 120);
        arr = (r && r.result) || [];
      } catch (e) { rpcErr = String((e && e.message) || e).slice(0, 120); }
      if (rpcErr) { poolErrors.push({ pool, page, error: rpcErr }); break; }   // coverage unknown
      if (!arr.length) { covered = true; break; }                              // ran out of history
      let oldest = Infinity;
      for (const s of arr) { const tMs = (s.blockTime || 0) * 1000; if (tMs < oldest) oldest = tMs; if (s.err) continue; if (tMs >= fromMs && tMs <= toMs) { sigs.add(s.signature); if (sigs.size >= maxSigs) { capped = true; reachedWindowStart = false; break outer; } } }
      if (oldest < fromMs) { covered = true; break; }        // paged back past the window start
      if (arr.length < 1000) { covered = true; break; }      // short page = end of history
      before = arr[arr.length - 1].signature;
    }
    if (!covered) reachedWindowStart = false;
  }
  const coverage = { capped, reachedWindowStart, ...(poolErrors.length ? { poolErrors } : {}) };
  if (!sigs.size) return { trades: [], txsScanned: 0, ...coverage };
  ...
  // The enhanced-tx batch can also come back short of what we asked for (a dropped page, a
  // partial batch). Say so rather than letting a short tape read as a complete one.
  const txsMissing = Math.max(0, sigs.size - txs.length);
  if (txsMissing) coverage.reachedWindowStart = false;
  return { trades, txsScanned: txs.length, sigsFound: sigs.size, txsMissing, ...coverage };
}


// Buy-comp ENTRIES: how many of a wallet's buys were at least entryUsd each (each such buy
// earns entryHorses "horses"/tickets). Per-buy sizes arrive in SOL; solUsd converts them.
// Returns null when the source carried no per-buy sizes (GeckoTerminal / Solana Tracker
// fallbacks) so the board shows "—" rather than a false zero. Pure; unit-tested.
function entriesFromBuys(buysSol, solUsd, entryUsd, entryHorses = 1) {
  if (!(entryUsd > 0) || !Array.isArray(buysSol) || !(solUsd > 0)) return null;
  const entries = buysSol.filter((sol) => Number(sol) * solUsd >= entryUsd - 1e-9).length;
  return { entries, horses: entries * Math.max(1, Math.floor(entryHorses) || 1) };
}

module.exports = { entriesFromBuys, getTokenBuyersInWindowHelius, getWalletTokenPositionHelius, getTradeTapeHelius, classifyBuyersFromRawTxs };
