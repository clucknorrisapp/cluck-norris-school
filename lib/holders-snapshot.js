...
function appendSnapshot(kv, { mint, at, holderCount, top, totalSupplyRaw, fullList, symbol, decimals }) {
  ...
  const record = {
    ...
    symbol: symbol || null,
    // P2-06 (docs/HUB_PUBLIC_SURFACES_VERIFY_2026-09-18.md): `top[].amount`/`fullList[].amount` are
    // UI-scale whole tokens (the caller, lib/owners-snapshot.js, writes p.balance — see the header
    // comment above diffSnapshots), while `totalSupplyRaw` is true base units — so a diff needs the
    // token's own decimals to render both in the same unit, and shouldn't have to depend on a
    // separately gated live crawl to know them. Optional and additive: a snapshot recorded before
    // this field existed just carries `decimals: null`, and callers render "units" honestly rather
    // than guess a scale.
...
function snapshotHead(r) {
  return {
    ...
    // P2-06: carried through so a diff reader can convert `supplyDelta` (true raw units) and the
    // `top` amounts (already UI units) into the SAME unit without a second, gated fetch — null on
    // a snapshot recorded before this field existed, never guessed.
...
function diffSnapshots(a, b) {
  ...
  // P2-06: these were named `amountRaw`/`fromRaw`/`toRaw`/`deltaRaw` — a name that means base
  // units everywhere else in this repo — while they actually hold the recorded top list's values,
  // which are UI-scale whole tokens (see the header note above and appendSnapshot's own comment).
  // Renamed honestly to `*Ui`; the BigInt arithmetic is unchanged (still exact, still never a
  // float), only the field names changed, so this is a wire-shape change for any consumer reading
  // the old names.
  const entered = [];
  for (const [wallet, info] of topB) {
    if (!topA.has(wallet)) entered.push({ wallet, amountUi: toBig(info.amount).toString(), rank: info.rank });
  }
  entered.sort((x, y) => x.rank - y.rank);

  const exited = [];
  for (const [wallet, info] of topA) {
    if (!topB.has(wallet)) exited.push({ wallet, amountUi: toBig(info.amount).toString(), prevRank: info.rank });
  }
  ...
  for (const [wallet, infoA] of topA) {
    ...
    if (!infoB) continue;
    const fromUi = toBig(infoA.amount), toUi = toBig(infoB.amount);
    if (fromUi === toUi) continue; // unchanged — see the header note above
    held.push({
      wallet,
      fromUi: fromUi.toString(),
      toUi: toUi.toString(),
      deltaUi: (toUi - fromUi).toString(),
      fromRank: infoA.rank,
      toRank: infoB.rank,
    });
  }
