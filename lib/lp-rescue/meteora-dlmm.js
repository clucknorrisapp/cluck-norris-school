...
function shapePosition(poolInfo, p, { requestedOwner, indexerVisibility, lockReleasePoint } = {}) {
  ...
  if (requestedOwner && ownerStr !== requestedOwner) {
    ...
    diagnostics.push("Meteora's own indexer does NOT list this pool — the normal Meteora frontend cannot show it");
    diagnostics.push("Your liquidity still exists on-chain and can be withdrawn directly");
  } else {
    status = "RECOVERABLE";
    if (indexerVisibility === "indexed") diagnostics.push("Pool is visible to Meteora's indexer");
    else diagnostics.push("Meteora indexer visibility could not be confirmed (supplemental check failed)");
  }
