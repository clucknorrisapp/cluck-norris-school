...
async function scanWalletRaydiumClmm(ownerAddress) {
  ...
  for (const f of found) {
    ...
    diagnostics.push("Manage or withdraw at raydium.io — the position shows normally in Raydium's app (LP Rescue's in-app withdrawal currently covers Meteora DLMM)");
...
// Pool-mode: a pasted address that turned out to be a Raydium CLMM pool.
// Same self-validating raw decode as the wallet scan.
async function scanPoolRaydiumClmm(poolAddress, ownerAddress) {
  const conn = rpc.connection("confirmed");
  const acc = await conn.getAccountInfo(new PublicKey(poolAddress));
  if (!acc || !acc.owner.equals(CLMM_PROGRAM) || acc.data.length < 273) {
    return { success: true, poolFound: !!acc, positions: [], note: "Account is not a readable Raydium CLMM pool" };
  }
  const mint0 = pubkey(acc.data, S.MINT0), mint1 = pubkey(acc.data, S.MINT1);
  const m0 = await conn.getAccountInfo(mint0);
  if (!m0 || !(m0.owner.equals(TOKEN_PROGRAM) || m0.owner.equals(TOKEN_2022_PROGRAM))) {
    return { success: true, poolFound: true, positions: [], note: "Raydium CLMM pool found, but its layout could not be safely decoded" };
  }
  const pool = {
    poolAddress: String(poolAddress),
    tokenX: mint0.toBase58(), tokenY: mint1.toBase58(),
    tokenXDecimals: acc.data[S.DEC0], tokenYDecimals: acc.data[S.DEC1],
    activeBinId: i32(acc.data, S.TICK_CUR), binStep: null,
    programOwner: CLMM_PROGRAM.toBase58(),
  };
  let positions = [];
  if (ownerAddress) {
    positions = (await scanWalletRaydiumClmm(ownerAddress)).filter((p) => p.poolAddress === String(poolAddress));
  }
  return {
    success: true, poolFound: true, protocol: "raydium-clmm",
    poolProtocolName: "RAYDIUM CLMM",
    indexerVisibility: "indexed",   // Raydium's app lists CLMM pools normally
    pool, positions,
  };
}

module.exports = { scanWalletRaydiumClmm, scanPoolRaydiumClmm };
