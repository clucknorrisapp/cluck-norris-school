...
// 10. Birdeye — needs a key; unread until set.
const birdeye = {
  ...
  async read(mint, deps) {
    ...
    const j = await getJson(deps, `https://public-api.birdeye.so/defi/token_overview?address=${mint}&ui_amount_mode=scaled`, { timeoutMs: 15000, headers: { "X-API-KEY": key, "x-chain": "solana" } });
    const d = j && j.data;
    if (!d || !d.address) return { found: false };
    const e = d.extensions || {};
    const shown = { name: pick(d, "name"), symbol: pick(d, "symbol"), logo: pick(d, "logoURI"), website: pick(e, "website"), x: pick(e, "twitter"), telegram: pick(e, "telegram"), discord: pick(e, "discord"), description: pick(e, "description") };
