...
const onchain = {
  ...
  async read(mint, deps) {
    assertMint(mint);
    // heliusRpcCall returns the JSON-RPC envelope ({ result } / { error }); a bare asset is accepted
    // too so fixtures and other RPC helpers keep working. Seen live 2026-09-06: without this unwrap
    // every real token read as "not found" on-chain.
    const rpc = await deps.rpcCall("listing-asset", "getAsset", [mint]);
    if (rpc && rpc.error) throw new Error("rpc: " + String(rpc.error.message || rpc.error).slice(0, 80));
    const asset = rpc && rpc.result !== undefined ? rpc.result : rpc;
    if (!asset || !asset.content) return { found: false };
    ...
    const auth = Array.isArray(asset.authorities) ? asset.authorities.find((a) => a && Array.isArray(a.scopes) && a.scopes.includes("full")) : null;
    const extra = { mutable: asset.mutable === true ? true : asset.mutable === false ? false : null,
      mintAuthority: pick(ti, "mint_authority"), freezeAuthority: pick(ti, "freeze_authority"),
      supply: (ti.supply != null && ti.decimals != null) ? Number(ti.supply) / Math.pow(10, Number(ti.decimals)) : null, decimals: ti.decimals != null ? Number(ti.decimals) : null,
      tokenProgram: /Tokenz|2022/i.test(String(ti.token_program || "")) ? "Token-2022" : (ti.token_program ? "Token" : null), updateAuthority: auth ? auth.address : "" };
    return { found: true, shown, url: `https://solscan.io/token/${mint}#metadata`, mutable: asset.mutable, extra };
  },
};
...
const coingecko = {
  ...
  async read(mint, deps) {
    assertMint(mint);
    // The demo key lifts the anonymous limit that 429s from a shared egress (seen live 2026-09-06).
    const cgKey = deps.env && deps.env.COINGECKO_API_KEY;
    const j = await getJson(deps, `https://api.coingecko.com/api/v3/coins/solana/contract/${mint}`, { timeoutMs: 15000, headers: cgKey ? { "x-cg-demo-api-key": cgKey } : {} });
...
const jupiter = {
  ...
  fixUrl: () => "https://verified.jup.ag/",   // Jupiter Verify — the Catdet list is retired (2026)
  async read(mint, deps) {
    assertMint(mint);
...
const rugcheck = {
  ...
  async read(mint, deps) {
    ...
    for (const m of Array.isArray(j.markets) ? j.markets : []) { const p = m && m.lp && Number(m.lp.lpLockedPct); if (Number.isFinite(p)) lpLockedPct = lpLockedPct == null ? p : Math.max(lpLockedPct, p); }
    return { found: true, shown, url: `https://rugcheck.xyz/tokens/${mint}`, verified: !!v.jup_verified, extra: { lpLockedPct, jupVerified: !!v.jup_verified } };
  },
};
...
const birdeye = {
  ...
  async read(mint, deps) {
    ...
    // /single answers { data: { address, … } }; /multiple answers { data: { [address]: { … } } } —
    // accept both so a switch between them never reads as not-found.
    const raw = j && j.data;
    const d = raw && raw.address ? raw : (raw && raw[mint]) || null;
    if (!d || !d.address) return { found: false };
