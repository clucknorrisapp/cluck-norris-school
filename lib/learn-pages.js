...
const esc = require("./html-escape").escHtml;
...
const L = {
  en: {
    ...
    ctaBody: (n) => `Understanding ${n} is one step. The Cluck Norris school teaches wallets, DeFi, scams, and self-custody for free — with a permanent, shareable transcript when you finish.`,
    ...
    hubCtaBody: "The free school turns this knowledge into real skills — wallets, DeFi, scams, self-custody — with a permanent, shareable transcript.",
  ...
  es: {
    ...
    ctaBody: (n) => `Entender ${n} es solo un paso. La escuela de Cluck Norris enseña wallets, DeFi, estafas y autocustodia gratis — con un expediente permanente y compartible al terminar.`,
    ...
    hubCtaBody: "La escuela gratis convierte este conocimiento en habilidades reales — wallets, DeFi, estafas, autocustodia — con un expediente permanente y compartible.",
  ...
  it: {
    ...
    ctaBody: (n) => `Capire ${n} è solo un passo. La scuola di Cluck Norris insegna wallet, DeFi, truffe e autocustodia gratis — con un attestato permanente e condivisibile quando finisci.`,
    ...
    hubCtaBody: "La scuola gratuita trasforma questa conoscenza in competenze reali — wallet, DeFi, truffe, autocustodia — con un attestato permanente e condivisibile.",
  ...
  pt: {
    ...
    ctaBody: (n) => `Entender ${n} é só um passo. A escola do Cluck Norris ensina wallets, DeFi, golpes e autocustódia de graça — com um histórico permanente e compartilhável quando você conclui.`,
    ...
    hubCtaBody: "A escola grátis transforma esse conhecimento em habilidades reais — wallets, DeFi, golpes, autocustódia — com um histórico permanente e compartilhável.",
  ...
  vi: {
    ...
    ctaBody: (n) => `Hiểu ${n} mới chỉ là một bước. Trường Cluck Norris dạy ví, DeFi, lừa đảo và tự lưu ký miễn phí — kèm bảng thành tích vĩnh viễn, chia sẻ được khi bạn hoàn thành.`,
    ...
    hubCtaBody: "Trường miễn phí biến kiến thức này thành kỹ năng thật — ví, DeFi, lừa đảo, tự lưu ký — kèm bảng thành tích vĩnh viễn, chia sẻ được.",
  ...
  zh: {
    ...
    ctaBody: (n) => `理解${n}只是第一步。Cluck Norris学堂免费教你钱包、DeFi、防骗和自托管——学完还能拿到可永久分享的成绩单。`,
    ...
    hubCtaBody: "免费学堂把这些知识变成真本事——钱包、DeFi、防骗、自托管——还有可永久分享的成绩单。",
  ...
  hi: {
    ...
    ctaBody: (n) => `${n} को समझना बस एक कदम है। Cluck Norris स्कूल मुफ़्त में वॉलेट, DeFi, स्कैम और सेल्फ-कस्टडी सिखाता है — कोर्स पूरा करने पर स्थायी, शेयर करने योग्य ट्रांसक्रिप्ट भी।`,
    ...
    hubCtaBody: "मुफ़्त स्कूल इस ज्ञान को असली हुनर में बदलता है — वॉलेट, DeFi, स्कैम, सेल्फ-कस्टडी — स्थायी, शेयर करने योग्य ट्रांसक्रिप्ट के साथ।",
...
// Floating bottom-right language switcher — matches the rest of the app's i18n widget
// (public/i18n.js). Here it navigates to the per-language URLs instead of client-side
// dict-swapping, since /learn is server-rendered per language.
const LANG_FULL = { en: "English", es: "Español", it: "Italiano", pt: "Português", vi: "Tiếng Việt", zh: "中文", hi: "हिन्दी" };
function langSwitcher(lang, slug, langs) {
  langs = langs || ["en"];
  if (langs.length < 2) return ""; // nothing to switch to — hide it
  const items = langs.map((lg) =>
    lg === lang
      ? `<span class="lsw-it on">${LANG_FULL[lg] || lg} ✓</span>`
      : `<a class="lsw-it" href="${urlFor(lg, slug)}">${LANG_FULL[lg] || lg}</a>`).join("");
  return `<div class="lsw" translate="no">
    <div class="lsw-menu" id="lswMenu">${items}</div>
    <button type="button" class="lsw-btn" id="lswBtn" title="Language · 语言 · Idioma">🌐 ${LANG_LABELS[lang] || String(lang).toUpperCase()}</button>
  </div>
  <script>(function(){var b=document.getElementById('lswBtn'),m=document.getElementById('lswMenu');if(!b)return;
    b.addEventListener('click',function(e){e.stopPropagation();m.style.display=(m.style.display==='flex')?'none':'flex';});
    document.addEventListener('click',function(){m.style.display='none';});})();</script>`;
}
...
const SHARED_CSS = `
  ...
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  .coin-badge{width:66px;height:66px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-family:var(--head);font-size:20px;color:#fff;flex-shrink:0;text-shadow:0 1px 3px rgba(0,0,0,.5);background:#0d0e12;border:1px solid var(--border);overflow:hidden;}
  .coin-badge img{width:100%;height:100%;object-fit:contain;padding:2px;}
  .hero-id{flex:1;min-width:160px;}
  .hero h1{font-family:var(--head);font-size:46px;letter-spacing:.5px;line-height:.98;}
  .hero .ticker{font-family:var(--mono);font-size:13px;color:var(--sub);letter-spacing:1px;margin-top:6px;}
  .hero-px{text-align:right;margin-left:auto;}
  .hero-px .p{font-family:var(--mono);font-size:32px;font-weight:700;color:var(--text);line-height:1;}
  .hero-px .s{font-family:var(--mono);font-size:13.5px;margin-top:7px;letter-spacing:.3px;}
  .hero-px .chg{font-weight:700;}
  .hero-px .chg.green{color:var(--green);}.hero-px .chg.red{color:var(--red);}
  .hero-px .mc{color:var(--sub);}
  .hero .tagline{color:var(--body-text);margin-top:18px;font-size:17px;line-height:1.55;max-width:660px;}
  .lsw{position:fixed;bottom:calc(14px + env(safe-area-inset-bottom,0px));right:12px;z-index:2147483600;font-family:var(--body);}
  .lsw-btn{font-family:var(--body);font-size:13px;font-weight:700;letter-spacing:.5px;color:#FFD9A0;background:rgba(26,15,8,.96);border:1px solid rgba(255,122,24,.55);border-radius:999px;padding:8px 13px;cursor:pointer;box-shadow:0 4px 16px rgba(0,0,0,.5);-webkit-tap-highlight-color:transparent;}
  .lsw-menu{position:absolute;bottom:46px;right:0;display:none;flex-direction:column;gap:3px;background:rgba(20,11,6,.98);border:1px solid rgba(255,122,24,.45);border-radius:12px;padding:6px;box-shadow:0 8px 24px rgba(0,0,0,.55);min-width:132px;}
  .lsw-it{text-align:left;font-size:13px;font-weight:700;color:#FFD9A0;background:transparent;border:0;border-radius:8px;padding:8px 12px;cursor:pointer;white-space:nowrap;text-decoration:none;display:block;}
  .lsw-it:hover{color:#FFB627;background:rgba(255,122,24,.10);}
  .lsw-it.on{color:#FFB627;background:rgba(255,122,24,.14);}
  ...
  .tv-wrap{height:420px;width:100%;border-radius:10px;overflow:hidden;border:1px solid var(--border);}
  @media(max-width:600px){.tv-wrap{height:340px;}}
  ...
  .hub-card .b{width:40px;height:40px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-family:var(--head);font-size:13px;color:#fff;flex-shrink:0;background:#0d0e12;border:1px solid var(--border);overflow:hidden;}
  .hub-card .b img{width:100%;height:100%;object-fit:contain;padding:2px;}
...
// Live-chart symbols — Binance USDT pairs (deepest liquidity + reliably on TradingView).
const TV_SYMBOL = {
  btc: "BINANCE:BTCUSDT", eth: "BINANCE:ETHUSDT", sol: "BINANCE:SOLUSDT", bnb: "BINANCE:BNBUSDT",
  xrp: "BINANCE:XRPUSDT", xlm: "BINANCE:XLMUSDT", ada: "BINANCE:ADAUSDT", doge: "BINANCE:DOGEUSDT",
  trx: "BINANCE:TRXUSDT", avax: "BINANCE:AVAXUSDT", link: "BINANCE:LINKUSDT", dot: "BINANCE:DOTUSDT",
  ton: "BINANCE:TONUSDT", ltc: "BINANCE:LTCUSDT", bch: "BINANCE:BCHUSDT", near: "BINANCE:NEARUSDT",
  sui: "BINANCE:SUIUSDT", apt: "BINANCE:APTUSDT",
};
const CHART_LABEL = { en: "Live Chart", es: "Gráfico en vivo", it: "Grafico dal vivo", pt: "Gráfico ao vivo", vi: "Biểu đồ trực tiếp", zh: "实时图表", hi: "लाइव चार्ट" };
function chartCard(a, lang) {
  const sym = TV_SYMBOL[a.slug];
  if (!sym) return "";
  const cfg = JSON.stringify({
    autosize: true, symbol: sym, interval: "D", timezone: "Etc/UTC", theme: "dark", style: "1",
    locale: "en", hide_side_toolbar: true, allow_symbol_change: false, save_image: false,
    backgroundColor: "rgba(20,21,26,1)", gridColor: "rgba(255,255,255,0.06)", support_host: "https://www.tradingview.com",
  });
  return `<div class="card"><h2>${esc(CHART_LABEL[lang] || CHART_LABEL.en)}</h2>
    <div class="tv-wrap"><div class="tradingview-widget-container" style="height:100%;width:100%">
      <div class="tradingview-widget-container__widget" style="height:100%;width:100%"></div>
      <scr` + `ipt type="text/javascript" src="https://s3.tradingview.com/external-embedding/embed-widget-advanced-chart.js" async>${cfg}</scr` + `ipt>
    </div></div></div>`;
}
...
function renderAssetPage(a, lang, langs) {
  ...
  <div class="hero">
    ...
    <div class="hero-row">
      <div class="coin-badge"><img src="/learn-icons/${esc(a.slug)}.png" alt="${esc(a.ticker)} logo" loading="lazy" onerror="this.replaceWith(document.createTextNode(${esc(JSON.stringify(a.ticker))}))"></div>
      <div class="hero-id"><h1>${esc(a.name)}</h1><div class="ticker">${esc(a.ticker)} · ${esc(a.category)}</div></div>
      <div class="hero-px">
        <div class="p" id="pxPrice">—</div>
        <div class="s"><span class="chg" id="pxChange">—</span> <span class="mc">· <span id="pxMcap">—</span> ${esc(t.mcap)}</span></div>
      </div>
    </div>
  ...
  (async function(){
    try{
      ...
      const fmt=n=>n>=1e12?'$'+(n/1e12).toFixed(2)+'T':n>=1e9?'$'+(n/1e9).toFixed(1)+'B':n>=1e6?'$'+(n/1e6).toFixed(1)+'M':'$'+Number(n).toLocaleString();
      ...
      ce.textContent=(c>=0?'+':'')+c.toFixed(2)+'%'; ce.className='chg '+(c>=0?'green':'red');
...
function renderHub(assets, lang, langs) {
  ...
  const cards = assets.map((a) => `<a class="hub-card" href="${urlFor(lang, a.slug)}">
    <div class="b"><img src="/learn-icons/${esc(a.slug)}.png" alt="${esc(a.ticker)} logo" loading="lazy" onerror="this.replaceWith(document.createTextNode(${esc(JSON.stringify(a.ticker))}))"></div>
...
module.exports = { renderAssetPage, renderHub };
