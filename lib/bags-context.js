...
// Jupiter Tokens API — keyed host (higher limits) when JUPITER_API_KEY is set, else lite-api.
const _JUP_KEY = process.env.JUPITER_API_KEY || "";
const JUPITER_SEARCH_URL = (_JUP_KEY ? "https://api.jup.ag" : "https://lite-api.jup.ag") + "/tokens/v2/search?query=";
const JUPITER_HEADERS = _JUP_KEY ? { "x-api-key": _JUP_KEY } : {};
...
async function fetchBagsContext(mint) {
  ...
  try {
    const r = await fetch(`${JUPITER_SEARCH_URL}${mint}`, { headers: JUPITER_HEADERS, signal: AbortSignal.timeout(8000) });
    if (r.ok) {
