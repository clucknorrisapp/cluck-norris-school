...
function mergeLedger({ scanned, ledger, nowUnix, createdAt }) {
  ...
  for (const { escrow, account } of scanned || []) {
    ...
    if (!prev) {
      ...
      // Only ever EARLIER than now, never later, and never zero/NaN.
      const start = Number.isFinite(born) && born > 0 && born < now ? Math.floor(born) : now;
  ...
  for (const escrow of escrows || []) {
    try {
      ...
      for (let page = 0; page < maxPages; page++) {
        ...
        const sigs = await conn.getSignaturesForAddress(new PublicKey(escrow), opts);
