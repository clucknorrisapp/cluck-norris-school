...
async function buildCreateLockTx(opts) {
  ...
  // Pre-flight simulate a signed COPY so a doomed tx is caught before the user signs (sigVerify
  // false: the fee-payer signature is added on the client). The returned tx stays UNSIGNED.
  let simError = null;
  try {
    const simTx = Transaction.from(tx.serialize({ requireAllSignatures: false, verifySignatures: false }));
    simTx.partialSign(base);
    const sim = await conn().simulateTransaction(simTx, undefined, false);
    if (sim.value.err) {
