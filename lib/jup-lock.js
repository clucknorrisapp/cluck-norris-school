...
const { Connection, PublicKey, Keypair, Transaction, VersionedTransaction, TransactionMessage, TransactionInstruction } = require('@solana/web3.js');
const spl = require('@solana/spl-token');
const BN = require('bn.js');
const { isOnCurve } = require('./solana-addr');

const LOCK_PROGRAM = new PublicKey('LocpQgucEQHbqNABEYvBvwoxCPsSbG91A1QaQhQQqjn');
// On-chain attribution memo appended to every lock created through the Locker Room. Makes
// third-party use of the free locker permanently countable by ANYONE with a block explorer
// (it shows in the tx logs as `Memo (len 26): "locked via clucknorris.app"`) — the verification
// mechanism behind the grant's adoption milestone, and the source of the 🐔 badge in the
// recently-locked feed. Keep the string STABLE: counting depends on exact-match.
const LOCKER_MEMO = 'locked via clucknorris.app';
const DAY = 86400, WEEK = 604800, MONTH = 2592000;

// Failover-aware Connection (rolls to FALLBACK_RPC_URL / HELIUS_API_KEY_2 / public on a
// 429/5xx/network blip) — same as every other RPC reader in the repo. Was a bare
// `new Connection(rpcUrl())` bound to the primary Helius key only, which took the whole
// Locker Room (create/claim/recent-locks/verify) offline on a single-endpoint outage.
let _conn = null;
function conn() { return (_conn = _conn || require('./rpc').connection('confirmed')); }
...
async function buildCreateLockTx(opts) {
  ...
  // Attribution memo (no accounts, no extra signer — so it can't disturb the Phantom
  // wallet-first signing order). See LOCKER_MEMO above for why this exists.
  const memoIx = new TransactionInstruction({ keys: [], programId: MEMO_PROGRAM, data: Buffer.from(LOCKER_MEMO, 'utf8') });
  ...
  try {
    const simMsg = new TransactionMessage({
      ...
      instructions: nameIx ? [preIx, ix, nameIx, memoIx] : [preIx, ix, memoIx],
    }).compileToV0Message();
...
// The log-line regex above matches ANYWHERE in a log line — including inside a Memo
// program's own logged payload ("Program log: Memo (len N): \"Instruction: CreateVestingEscrowV2\"").
// A tx that never touches the Lock program at all could satisfy it. Require an actual
// invocation of LOCK_PROGRAM (top-level or CPI) as the real signal; the log text is kept
// only as a secondary check.
function invokesLockProgram(tx) {
  const top = (tx.transaction && tx.transaction.message && tx.transaction.message.instructions) || [];
  const inner = ((tx.meta && tx.meta.innerInstructions) || []).flatMap((g) => g.instructions || []);
  for (const ix of [...top, ...inner]) {
    if (programIdOf(ix) === LOCK_PROGRAM_B58) return true;
  }
  return false;
}

const LOCK_PROGRAM_B58 = LOCK_PROGRAM.toBase58();
function programIdOf(ix) {
  const pid = ix && ix.programId;
  return pid && typeof pid.toBase58 === 'function' ? pid.toBase58() : String(pid || '');
}
// accountKeys from getParsedTransaction are { pubkey, signer, writable, source } (pubkey is a
// PublicKey); older/raw shapes are plain base58 strings. Normalise to base58 or null.
function keyAt(keys, i) {
  const k = keys && keys[i];
  if (!k) return null;
  if (typeof k === 'string') return k;
  const pk = k.pubkey != null ? k.pubkey : k;
  return typeof pk === 'string' ? pk : (pk && typeof pk.toBase58 === 'function' ? pk.toBase58() : null);
}

// ⚠️ NEITHER the log text NOR invokesLockProgram() is proof of a lock CREATION, and neither is
// an off-curve owner:
//   • LOCK_LOG_CREATE matches raw log text, which a Memo instruction can contain verbatim
//     (`Program log: Memo (len N): "Instruction: CreateVestingEscrowV2"`).
//   • invokesLockProgram() is satisfied by ANY call into the Lock program — a claim, a cancel.
//   • an ATA created with allowOwnerOffCurve for any attacker-chosen PDA has an off-curve
//     owner, so a plain top-level transfer into it passes the off-curve test.
// The one part of the transaction an attacker cannot forge is the token-program CPI the LOCK
// PROGRAM ITSELF makes to fund the vault (sender_token -> escrow_token). So bind the deposited
// account to the destination of a transfer CPI'd from INSIDE a top-level LOCK_PROGRAM
// instruction. A claim/cancel payout is also such a destination, but it lands in the
// recipient's own on-curve-owned ATA, which bestDepositedAccount() still rejects — and the
// mint/amount then come from the vault the program funded, not from anything the sender chose.
function lockCpiDepositAccounts(tx) {
  const out = new Set();
  const top = (tx && tx.transaction && tx.transaction.message && tx.transaction.message.instructions) || [];
  const lockIdx = new Set();
  top.forEach((ix, i) => { if (programIdOf(ix) === LOCK_PROGRAM_B58) lockIdx.add(i); });
  if (!lockIdx.size) return out;
  for (const g of ((tx.meta && tx.meta.innerInstructions) || [])) {
    if (!lockIdx.has(g.index)) continue;
    for (const ix of (g.instructions || [])) {
      const parsed = ix && ix.parsed;
      const type = parsed && parsed.type;
      if (type !== 'transfer' && type !== 'transferChecked') continue;
      const dest = parsed.info && parsed.info.destination;
      if (dest) out.add(String(dest));
    }
  }
  return out;
}

// Pick the deposited-amount account from post token balances that went 0 → positive,
// restricted to accounts OWNED by an off-curve address (a program-derived escrow vault —
// never a plain wallet). Without this, a self-transfer into a fresh ATA the attacker's own
// (on-curve) wallet owns would out-rank the real escrow vault and plant a fake amount/mint.
// `onlyAccounts` (with `accountKeys` to resolve accountIndex -> address) restricts the search
// to the vaults the Lock program itself funded — see lockCpiDepositAccounts(). Callers MUST
// pass it; the two-argument form is the unrestricted legacy shape and is not proof of a lock.
function bestDepositedAccount(pre, post, { onlyAccounts = null, accountKeys = null } = {}) {
  const preMap = new Map(pre.map((b) => [b.accountIndex, Number(b.uiTokenAmount && b.uiTokenAmount.uiAmount) || 0]));
  let best = null;
  for (const b of post) {
    if (!b.owner || isOnCurve(b.owner)) continue; // only program-owned (off-curve) vaults count
    if (onlyAccounts) {
      const addr = keyAt(accountKeys, b.accountIndex);
      if (!addr || !onlyAccounts.has(addr)) continue; // not an account the Lock program funded
    }
...
async function recentLocks(limit = 12, scan = 60) {
  ...
  for (let i = 0; i < sigs.length && out.length < limit; i += CHUNK) {
    ...
    const txs = await Promise.all(batch.map((s) =>
      c.getParsedTransaction(s.signature, { maxSupportedTransactionVersion: 1 }).catch(() => null)));
    for (let j = 0; j < txs.length; j++) {
      ...
      if (!invokesLockProgram(tx)) continue; // the log text alone isn't proof — a memo can contain it
      const funded = lockCpiDepositAccounts(tx);
      if (!funded.size) continue;            // the Lock program itself never funded a vault here
      const keys = (tx.transaction && tx.transaction.message && tx.transaction.message.accountKeys) || [];
      // deposited amount = the vault the Lock program funded by CPI, 0 → positive, off-curve owner
      const best = bestDepositedAccount(tx.meta.preTokenBalances || [], tx.meta.postTokenBalances || [], { onlyAccounts: funded, accountKeys: keys });
      if (!best) continue;
      const creator = keys.length ? (typeof keys[0] === "string" ? keys[0] : keys[0].pubkey) : null;
      // Locks created through our Locker Room carry the attribution memo in their logs —
      // derive the "via" tag from the CHAIN, not from our own records.
      const via = logs.some((l) => l.includes(LOCKER_MEMO)) ? 'clucknorris' : null;
      out.push({ mint: best.mint, amount: best.amount, creator, sig: si.signature, ts: (tx.blockTime || si.blockTime || 0) * 1000, via });
    }
  }
  return out;
}
...
async function verifyLockSig(sig) {
  const c = conn();
  const tx = await c.getParsedTransaction(String(sig), { maxSupportedTransactionVersion: 1 }).catch(() => null);
  if (!tx || !tx.meta || tx.meta.err) return null;
  const logs = tx.meta.logMessages || [];
  if (!logs.some((l) => LOCK_LOG_CREATE.test(l))) return null;
  if (!invokesLockProgram(tx)) return null; // the log text alone isn't proof — a memo can contain it
  const funded = lockCpiDepositAccounts(tx);
  if (!funded.size) return null;           // the Lock program itself never funded a vault here
  const keys = (tx.transaction && tx.transaction.message && tx.transaction.message.accountKeys) || [];
  const best = bestDepositedAccount(tx.meta.preTokenBalances || [], tx.meta.postTokenBalances || [], { onlyAccounts: funded, accountKeys: keys });
  if (!best) return null;
  const creator = keys.length ? (typeof keys[0] === "string" ? keys[0] : keys[0].pubkey) : null;
  const via = logs.some((l) => l.includes(LOCKER_MEMO)) ? 'clucknorris' : null;
  return { mint: best.mint, amount: best.amount, creator, sig: String(sig), ts: (tx.blockTime || 0) * 1000, via };
}

module.exports = {
  buildCreateLockTx, listClaimable, buildClaimTx, recentLocks, verifyLockSig, LOCK_PROGRAM,
  _internal: { invokesLockProgram, bestDepositedAccount, lockCpiDepositAccounts }, // exported for dependency-free tests only
};
