...
// amount still LOCKED x the term they COMMITTED TO — held flat for the life of the lock, then zero.
//
// This used to count days REMAINING, which meant the rate decayed every day. The page sells a
// ladder ("3 months = 1x … 18 months = 6x") and that table was only true on day one: a 3-month
// locker was down to 0.77x of their own rate by day 30 and earned almost nothing in their final
// week, receiving 3.68M over their term where the advertised rate pays 6.25M. Worse at the top of
// the ladder — an 18-month lock with 3 months left weighed EXACTLY the same as a brand-new 3-month
// lock, so the longest commitment on the board finished on the entry-tier rate (owner, 2026-09-05:
// "It makes no sense to lock something for eighteen months, and then by the end, you're only
// getting paid like a three month lock. That's not fair.").
//
// So: you are paid the rate you committed to, for as long as you are committed. The term comes
// from OUR firstSeenAt and the real release end, so neither endpoint is creator-set.
//
// The AMOUNT side still moves — a drip schedule that is paying tokens out loses weight as it
// releases, because what has vested is no longer locked up. Only the rate is fixed.
//
// A matured lock is worth ZERO, never negative: a negative weight would let an expired lock
// subtract from the pool and hand everyone else more than 100%.
function weightOf(lock, nowUnix, cfg) {
  const end = Number(lock.fullyVestedAt || 0);
  const seen = Number(lock.firstSeenAt || 0);
  // No end date, or never indexed, or already finished — all worth nothing. Failing closed here
  // matters: without the !seen guard an unindexed lock would read as committed since the epoch.
  if (!end || !seen || Number(nowUnix) >= end) return 0n;
  const committedDays = Math.max(0, Math.floor((end - seen) / DAY));
  return unvestedRaw(lock, nowUnix) * BigInt(Math.min(committedDays, maxTermDaysOf(cfg)));
}
...
module.exports = {
  DAY, DEFAULT_MAX_TERM_DAYS, normalizeEscrow, poolForDay, dailyUnlockRaw, disqualify, qualifies,
  weightOf, maxTermDaysOf, unvestedRaw, splitOf,
  accrueDay,
