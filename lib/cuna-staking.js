...
function disqualify(lock, cfg) {
  ...
  const {
    ...
    cancelableAllowed = false, vesting = "any",
  } = cfg || {};
  ...
  // A project may choose to reward cancelable locks (platform terms, 2026-09-16); an UNREADABLE
  // cancel mode still fails closed either way, and a lock already cancelled never earns.
  if (!cmReadable) r.push("cancelable — the creator can take these back");
  else if (Number(cm) !== 0 && !cancelableAllowed) r.push("cancelable — the creator can take these back");
  if (Number(lock.cancelledAt || 0) > 0) r.push("already cancelled");
  // Which lock SHAPES the program admits (platform terms): a single release at the cliff ("no
  // vesting") versus a periodic schedule. `any` is the CUNA behaviour.
  let periodic = false;
  try { periodic = Number(lock.periods) > 1 && BigInt(lock.perPeriodRaw || 0) > 0n; } catch (_) { periodic = false; }
  if (vesting === "cliff-only" && periodic) r.push("vesting schedule not allowed — this program takes single-release locks only");
  if (vesting === "vesting-only" && !periodic) r.push("single-release lock not allowed — this program takes vesting schedules only");
...
// Walk the UNVESTED releases of a lock, bucketed by day, handing each bucket to `fn(raw, days)`
// where `days` is already clamped (0 below minDurationDays, capped at maxTermDays). Shared by
// weightOf (sum raw x days) and earningRawOf (sum raw where days > 0) so the two can never
// disagree about which tokens count.
function forEachRelease(lock, nowUnix, cfg, fn) {
  const seen = Number(lock.firstSeenAt || 0);
  const cliff = Number(lock.cliffTime || 0);
  const now = Number(nowUnix);
  if (!seen || !cliff || !Number.isFinite(now)) return;
  ...
  if (cliffRaw > 0n && cliff > now) fn(cliffRaw, clamp(Math.floor((cliff - seen) / DAY)));
  ...
  if (per > 0n && periods > 0) {
    ...
    if (jLo <= periods) {
      const ceilAt = seen + maxDays * DAY;
      const jCeil = Math.max(jLo, Math.ceil((ceilAt - cliff) / freq));
      if (jCeil <= periods) fn(per * BigInt(periods - jCeil + 1), maxDays);
      ...
      if (jLo <= jHi) {
        ...
        while (j <= jHi) {
          const D = dayOf(j);
          const dayEnd = seen + (D + 1) * DAY;
          const jEnd = Math.min(jHi, Math.floor((dayEnd - 1 - cliff) / freq));
          fn(per * BigInt(jEnd - j + 1), clamp(D));
          j = jEnd + 1;
        }
      }
    }
...
function weightOf(lock, nowUnix, cfg) {
  let w = 0n;
  forEachRelease(lock, nowUnix, cfg, (raw, days) => { w += raw * BigInt(days); });
  return w;
}

// The tokens that actually carry weight right now: unvested AND locked for at least the minimum
// term. For a single-cliff lock this is everything still locked; for a Jupiter-default monthly
// drip it is the tranches 90+ days out. "Earning on X of Y" on the page and in announcements comes
// from here, so it can never say a token is earning when weightOf gives it nothing.
function earningRawOf(lock, nowUnix, cfg) {
  let e = 0n;
  forEachRelease(lock, nowUnix, cfg, (raw, days) => { if (days > 0) e += raw; });
  return e;
}
...
module.exports = {
  ...
  disqualify, qualifies, weightOf, earningRawOf, anyTokenLockedFor, maxTermDaysOf, minTermDaysOf, unvestedRaw, splitOf,
  accrueDay,
