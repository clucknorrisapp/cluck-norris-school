...
// ---------------------------------------------------------------------------
function normalizeEscrow(escrowAddr, account) {
...
function disqualify(lock, cfg) {
  ...
  // vesting_start_time is on the escrow account, so a lock's terms are readable without tracing
  // its creation transaction. It is CREATOR-SET and therefore backdatable: someone can claim a
  // 1-year shape on a lock that really runs ten weeks. That is not defended here — weightOf()
  // prices a lock by the days it has LEFT, so a backdated lock earns in proportion to the little
  // time it actually has. These checks describe the shape; the weight decides the money.
  const start = Number(lock.vestingStartTime || 0);
  if (!start) r.push("unknown start time");
  else if (startAfterUnix && start < Number(startAfterUnix)) r.push("started before the programme began");

  const cliff = Number(lock.cliffTime || 0);
  const end = Number(lock.fullyVestedAt || 0);
  if (!cliff) r.push("no cliff");
  else if (start && (cliff - start) < minCliffDays * DAY) r.push(`cliff shorter than ${minCliffDays} days`);
  if (!end) r.push("no end date");
  else if (start && (end - start) < minDurationDays * DAY) r.push(`locked for less than ${minDurationDays} days`);
