...
async function tickSol({ dryRun = false } = {}) {
  ...
  const balanced = ranges.options.find((o) => o.id === "balanced");
  const float = await getFloat(conn);
  // BOTH guards, matching preSolFree above (solGasReserve was missing here — audit, BULLEN
  // production incident 2026-09-25: a tiny wallet with a stock swapSolFloor left solAvail at
  // 0 and this branch fell to "deploy 100% of the token side" with no check that the position
  // it implies is even affordable — see the guard block below buildOpenPosition needs).
  const spendableSol = Math.max(0, float.sol - cfg.swapSolFloor - (cfg.solGasReserve || 0));
  ...
  if (inputMint === tok().mint && inputAmount > 0) {
    try {
      ...
      const solNeeded = Number(fresh.clknIsA ? q2.maxB : q2.maxA);
    ...
    } catch (e) { /* quote failed — proceed with the unscaled amount; the SDK's own price-deviation
      slippage bounds are the last line of defense against an over-large deploy. */ }
  }
...
async function manualSwap({ fromSym, toSym, amountUi, slippageBps = 100, maxImpactPct = null, dryRun = false, silent = false, allowTokenSell = false }) {
  ...
  if (effectiveCeiling != null && impactPct > Number(effectiveCeiling)) {
...
async function bootstrap({ dryRun = true, maxImpactPct = 1.5, maxSlices = 6, minStepUsd = 3, reserveSol = RESERVE_SOL_DEFAULT } = {}) {
  ...
  // Total capital this bootstrap is allowed to consider = spendable SOL + all USDC + any
  // token already held (so a partial prior run — some BULLEN already bought — still nets out
  // to the same 4-way split rather than re-deriving a smaller total and stranding a leftover).
  const totalUsd = spendableSol * solUsd + float.usdc + float.clkn * tokenUsd;
  const perLegUsd = totalUsd / 4;
  const targetSolLegUi = perLegUsd / solUsd;
  const targetTokenUsd = perLegUsd * 2;   // one BULLEN leg per pool

  const solNeededUi = Math.max(0, targetSolLegUi - spendableSol);
  const solNeededUsd = solNeededUi * solUsd;
  const tokenNeededUsd = Math.max(0, targetTokenUsd - float.clkn * tokenUsd);
  ...
  const plan = {
    ...
    targets: {
      solLegUi: Number(targetSolLegUi.toFixed(4)), usdcLegUsd: Number(perLegUsd.toFixed(2)),
  ...
  if (solNeededUsd >= minStepUsd) {
    ...
    if (!dryRun && res && res.action === "swap") float.usdc = Math.max(0, float.usdc - (res.inAmt || spendUsdc));
  } else {
  ...
  if (tokenNeededUsd >= minStepUsd) {
    ...
    while (remaining >= minStepUsd && slices < maxSlices) {
      ...
      if (res && (res.action === "swap" || res.action === "would-swap")) {
        plan.steps.push({ leg: "token", sliceUsd: Number(sliceUsd.toFixed(2)), ...res });
