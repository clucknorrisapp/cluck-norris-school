...
async function tick({ dryRun = false } = {}) {
  ...
  if (!managed) {
    ...
    if (frac < 0 || frac > 1) {
      ...
      if (Math.min(stagedUsd, pairUsd) >= cfg.baseDeployThresholdUsd) { needRoll = true; forceRoll = Math.min(stagedUsd, pairUsd) >= 2 * cfg.baseDeployThresholdUsd; reason = `deploying staged USDC ($${stagedUsd.toFixed(0)} above reserve)`; }
...
async function tickJup({ dryRun = false } = {}) {
  ...
  if (!managed) {
    ...
    if (frac < 0 || frac > 1) {
      ...
      if (preClknForJup * price >= cfg.jupDeployThreshold) { needRoll = true; deployDriven = (preClknForJup * price >= 2 * cfg.jupDeployThreshold); reason = `deploying available JUP (${preJupFree})`; }   // ≥2× = meaningful refill skips pacing; leftovers stay paced (see base decay-loop note)
...
async function tickSol({ dryRun = false } = {}) {
  ...
  if (!managed) {
    ...
    if (frac < 0 || frac > 1) {
      ...
      // Pairing gate: unpaired quote must not trigger rolls (see base sleeve comment).
      if (preClknForSol * price >= cfg.solDeployThreshold) { needRoll = true; deployDriven = (preClknForSol * price >= 2 * cfg.solDeployThreshold); reason = `deploying available SOL (${preSolFree.toFixed(2)})`; }   // ≥2× = meaningful refill skips pacing; leftovers stay paced (see base decay-loop note)
...
async function manualSwap({ fromSym, toSym, amountUi, slippageBps = 100, maxImpactPct = null, dryRun = false, silent = false, allowTokenSell = false }) {
  ...
  if (from === tok().mint && !allowTokenSell) {
    const p = getProject(pid());
    if (!p || !p.tokenSellOk) throw new Error("refusing to sell the project token (brand bag) — quote↔quote only");
  }
...
async function evenPools({ dryRun = false } = {}) {
  if (!isEnabled()) return { enabled: false, action: "none", reason: "operator not set" };
  const cfg = getConfig();
  if (!cfg.swapEnabled || !cfg.jupEnabled) return { enabled: true, action: "none", reason: "needs swapEnabled + jupEnabled" };
  const st = getState();
  if (st.paused) return { enabled: true, action: "none", reason: "paused" };
  const conn = connection();
  let positions = [];
  ...
  const jup = positions.find((p) => p.positionMint === st.jup_positionMint);
  if (!base || !sol || !jup) return { enabled: true, action: "none", reason: "need base + SOL + JUP positions live to even (deploy ticks fill the gap first)" };
  const px = await jupPriceUsdMap([engine.WSOL_MINT, engine.JUP_MINT]);
  ...
  const pools = [
    ...
    { key: "jup", quote: "JUP", v: jup.clknAmount * clknUsd + jup.quoteAmount * jupUsd, mint: st.jup_positionMint, quoteUsd: jupUsd },
  ].sort((a, b) => a.v - b.v);
  const lean = pools[0], fat = pools[2];
  const mean = (pools[0].v + pools[1].v + pools[2].v) / 3;
  ...
  // the capital that actually exists — deployed value plus free quote — split three
  // ways. Caps stop being a hand-tuned input; they are just how the deploy ticks are
  // told each pool's share. Quote side of a two-sided pool ≈ half its total share.
  const float = await getFloat(conn);
  ...
  const totalUsd = pools[0].v + pools[1].v + pools[2].v + freeQuoteUsd;
  const sideTarget = Math.max(20, totalUsd / 3 / 2);
  const wantCaps = {
    maxUsd: Math.round(sideTarget),
    solMaxSol: Number((sideTarget / solUsd).toFixed(3)),
    jupMaxJup: Math.round(sideTarget / jupUsd),
  };
  const drifted = (a, b) => Math.abs(a - b) > 0.05 * Math.max(Math.abs(a), Math.abs(b), 1);
  if (drifted(cfg.maxUsd, wantCaps.maxUsd) || drifted(cfg.solMaxSol, wantCaps.solMaxSol) || drifted(cfg.jupMaxJup, wantCaps.jupMaxJup)) {
    setConfig(wantCaps);
  }
  ...
  if (fat.v - lean.v < tol * Math.max(fat.v, 1)) {
    ...
    if (cfg.scaleUpUsdPerCycle > 0 && !dryRun) {
      const today = new Date().toISOString().slice(0, 10);
  ...
  // 1. TRIM the fat pool: roll it. Close is withdraw-only; the next deploy tick
  // reopens it at its cap. Cooldown keeps this to one structural move per 10 min.
  if (fat.v > mean * (1 + tol) && Date.now() - (st.evenCloseTs || 0) > 10 * 60 * 1000) {
    if (dryRun) return { ...out, action: "would-trim", pool: fat.key };
    await closePosition({ mint: fat.mint });
    setState({ evenCloseTs: Date.now() });
    await notifyDigest(`⚖️ <b>Pool evenness — trim</b>\n${fat.key} pool $${Math.round(fat.v)} vs mean $${Math.round(mean)} — rolled; redeploys at cap next tick.`);
    return { ...out, action: "trim", pool: fat.key };
  }

  // 2. FEED the lean pool: convert free float toward its quote so its next deploy
  // tick can absorb it. Skips when the lean quote's own free float already covers
  // the gap (nothing to convert — the deploy tick will pick it up as-is).
  const today = new Date().toISOString().slice(0, 10);
  const swapsToday = st.swapDayStamp === today ? (st.swapsToday || 0) : 0;
  if (swapsToday >= cfg.maxSwapsPerDay) return { ...out, action: "capped", reason: "daily swap cap reached" };
  const freeUsdBy = {
    USDC: Math.max(0, float.usdc - cfg.usdcFloor),
    SOL: Math.max(0, float.sol - cfg.swapSolFloor) * solUsd,
    JUP: Math.max(0, float.jup) * jupUsd,
  };
  const gap = mean - lean.v;
  if (freeUsdBy[lean.quote] >= gap) return { ...out, action: "none", reason: `lean ${lean.key} pool has $${Math.round(freeUsdBy[lean.quote])} free ${lean.quote} waiting to deploy` };
  const sources = Object.entries(freeUsdBy).filter(([q]) => q !== lean.quote).sort((a, b) => b[1] - a[1]);
  const [srcQuote, srcUsd] = sources[0];
  const moveUsd = Math.min(gap - freeUsdBy[lean.quote], cfg.maxSwapUsdPerCycle, srcUsd);
  if (moveUsd < cfg.minSwapUsd) return { ...out, action: "none", reason: `lean ${lean.key} pool needs $${Math.round(gap)} but no source quote has free float` };
  const srcQuoteUsd = srcQuote === "USDC" ? 1 : srcQuote === "SOL" ? solUsd : jupUsd;
  if (dryRun) return { ...out, action: "would-feed", fromSym: srcQuote, toSym: lean.quote, moveUsd: Math.round(moveUsd) };
  const res = await manualSwap({ fromSym: srcQuote, toSym: lean.quote, amountUi: moveUsd / srcQuoteUsd, slippageBps: cfg.swapSlippageBps, dryRun: false });
  setState({ swapsToday: swapsToday + 1, swapDayStamp: today });
  await notifyDigest(`⚖️ <b>Pool evenness — feed</b>\n${lean.key} pool $${Math.round(lean.v)} vs mean $${Math.round(mean)} — swapped ~$${Math.round(moveUsd)} ${srcQuote}→${lean.quote}.`);
  return { ...out, action: "feed", fromSym: srcQuote, toSym: lean.quote, moveUsd: Math.round(moveUsd), sig: res.sig };
}
...
module.exports = {
  ...
  evenPools: (o = {}) => withProject(o.projectId, () => evenPools(o)),
