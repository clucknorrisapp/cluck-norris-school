...
async function signSend(conn, txBase64) {
  ...
  try { engine.invalidatePositions(operator().publicKey.toBase58()); } catch (_) {}
  return sig;
}
...
async function tickAskWall({ dryRun = false } = {}) {
  ...
  if (wall) {
    ...
    const { txs } = await engine.buildClosePosition({ owner: operator().publicKey.toBase58(), positionMint: wall.positionMint, slippageBps: cfg.slippageBps });
    for (const t of txs) report.steps.push({ close: await signSend(conn, t) });
    wallDayActions++;
