...
function eng() { return ENGINES[venue()] || engine; }
...
function withProject(projectId, fn) {
  ...
  const p = getProject(id);
  return als.run({ projectId: id, tok: projectTok(id), venue: (p && p.venue) || "orca" }, fn);
}
...
async function tick({ dryRun = false } = {}) {
  ...
  if (cfg.baseEnabled === false) {
    // Still stamp lastTickTs — tick() is the sole writer, and the boot-tick redeploy guard
    // (server.js) keys off it. Without this a single-sided project fires a full cold cycle
    // on every redeploy.
...
async function publicPositions() {
  ...
  const data = { enabled: true, positions: out, totalUsd, clknUsd, solUsd };
