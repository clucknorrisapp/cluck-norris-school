...
async function tickJup({ dryRun = false } = {}) {
  ...
  if (!managed) {
    ...
    if (frac < 0 || frac > 1) { needRoll = true; reason = "out of range"; }
      ...
      if (preClknForJup * price >= cfg.jupDeployThreshold) { needRoll = true; deployDriven = true; reason = `deploying available JUP (${preJupFree})`; }
...
async function tickSol({ dryRun = false } = {}) {
  ...
  if (!managed) {
    ...
    if (frac < 0 || frac > 1) { needRoll = true; reason = "out of range"; }
      ...
      // Pairing gate: unpaired quote must not trigger rolls (see base sleeve comment).
      if (preClknForSol * price >= cfg.solDeployThreshold) { needRoll = true; deployDriven = true; reason = `deploying available SOL (${preSolFree.toFixed(2)})`; }
