...
const { buybackDecision, spendableSol: spendableSolPure, rollRebalanceDecision, clampFreedToPosition, spendableForAdd } = require("./engine-decisions"); // pure gate logic, replayable by scripts/engine-sim-test.cjs
// The BULLEN project's own mint (Token-2022) — the roll-rebalance hard token guard (review
// round 2026-09-26) requires the ACTIVE project's token to be exactly this mint, not merely
// pid()==="bullen", so a future project id collision or a misregistered project can't inherit
// a rebalance budget meant for BULLEN alone. Kept local (not read from server.js) since this
// file has no dependency on server.js and shouldn't grow one for a single constant.
...
const DEFAULT_CONFIG = {
  ...
  // Roll rebalance (tick()/tickSol()/tickJup(), BULLEN production incident 2026-09-26): swaps a
  // fresh close's freed float toward ~50/50 before reopening, so a one-way move can't reopen the
  // position ~100% one-sided. 0 = OFF FOR EVERYONE (the default) — a project opts in explicitly.
  // Shares the maxSwapUsdPerCycle per-swap cap and the swapsToday/maxSwapsPerDay counter above.
  rollRebalanceUsdPerDay: 0,
  rollRebalanceMaxImpactPct: 3, // Jupiter prices unverified tokens off a frozen reference, so a real ~1.3% BULLEN buy reads 2-2.5% "impact"; 1.5 refused every JUP-pool buy (2026-09-26)
...
function registerProject(rec) {
  ...
  all[rec.id] = {
    ...
    // Declared, not probed on-chain every call (Codex review on #444): getFloat() reads this
    // to decide whether the Token-2022 balance scan is MANDATORY (throw on failure) rather than
    // best-effort — a project whose own token is Token-2022 (BULLEN) must never silently read
    // its float as 0 because that scan alone hiccuped. "legacy" (the default) keeps the existing
    // best-effort behavior for every project that predates this field.
    tokenProgram: rec.tokenProgram || prev.tokenProgram || "legacy",
...
const CONFIG_BOUNDS = {
  ...
  rollRebalanceUsdPerDay:  [0, 2000],
  rollRebalanceMaxImpactPct: [0.5, 10],
...
// Explicit-projectId, synchronous, no ALS scope needed (getState already takes an id) — used by
// the shared-wallet arm guard (server.js, bullen/cuna/dnc/rose) to check "is THIS project's vault
// paused" without going through withProject/status(). A few call sites already speculatively
// wrote `whirlpoolMM.vault.isPaused ? ... : undefined` before this existed, so it was always a
// silent no-op there; this makes it real.
function isPaused(projectId) { return !!getState(projectId).paused; }
...
async function notify(text) {
  ...
  // Room policy (lib/telegram-rooms): vault alerts never reach the OnlyRose room — the rose
  // project is muted ("off") already, and this holds even if a fallback re-routes.
  { const refused = require("./telegram-rooms").refusal(chat, "sendMessage"); if (refused) { console.warn(`[vault] ${refused}`); return; } }
  try {
...
// A single position's collected LP fees are cents to a few dollars. Anything wildly
// past that is a bad read (a ghost position from the stale Helius index, a raw base-unit
// amount treated as a UI amount), and because these counters ACCUMULATE, one bad read
// poisons the lifetime total permanently. That already happened: the ROSE project's
// realizedFeeUsdc carries 1.4658e+27, which made its earnings and netPnlUsd meaningless
// (found 2026-09-03; the CLKN/treasury project was unaffected, and no gate or sizing
// decision reads these — they are report-only, so nothing traded on the bad number).
const MAX_SANE_FEE_ACCRUAL = 1e6;
function saneFee(v, what) {
  const n = Number(v) || 0;
  if (!isFinite(n) || n < 0 || n > MAX_SANE_FEE_ACCRUAL) {
    try { console.warn("[vault] refusing absurd fee accrual (" + what + "): " + v); } catch (e) {}
    return 0;
  }
  return n;
}
function accrueRealizedFees(pos) {
  if (!pos) return;
  const st = getState();
  const clkn = saneFee(pos.pendingFeeClkn, "clkn");
  const quote = saneFee(pos.pendingFeeQuote, pos.quoteSymbol || "quote");
  const patch = {
...
async function earnings() {
  ...
  // Read side is guarded too, so a counter ALREADY poisoned before the write guard existed
  // reports honestly instead of a fantasy number — and without writing to live engine state.
  // `realizedSuspect` names the offenders so the reset is a deliberate owner action, not a
  // silent correction that would erase real earnings history along with the bad value.
  const suspect = [];
  const rd = (v, name) => {
    const n = Number(v) || 0;
    if (!isFinite(n) || n < 0 || n > MAX_SANE_FEE_ACCRUAL) { suspect.push(name + "=" + v); return 0; }
    return n;
  };
  const rClkn = rd(st.realizedFeeClkn, "realizedFeeClkn");
  const rUsdc = rd(st.realizedFeeUsdc, "realizedFeeUsdc");
  const rSol = rd(st.realizedFeeSol, "realizedFeeSol");
  ...
  return {
    ...
    // present ONLY when a stored counter was discarded as absurd — so "0 fees" from a
    // genuinely new project never looks like a data fault, and a real fault is never silent.
    realizedSuspect: suspect.length ? suspect : undefined,
  };
}
...
async function getFloat(conn) {
  ...
  // MANDATORY, not best-effort, when the project's OWN token is Token-2022 (Codex review on
  // #444, BULLEN): "best-effort" on that scan meant an RPC hiccup on the 2022 side alone would
  // read the project token's float as 0 — for bullen that's not a display glitch, it's exactly
  // the input bootstrap() and every deploy-sizing decision trusts. registerProject's
  // `tokenProgram` field (declared at registration, not probed per-call) decides this.
  const token2022Mandatory = (() => { try { return getProject(pid()).tokenProgram === "token2022"; } catch (_) { return false; } })();
  for (const programId of PROGRAMS) {
    let resp;
    try { resp = await conn.getParsedTokenAccountsByOwner(op, { programId: new PublicKey(programId) }); }
    catch (e) {
      if (programId === PROGRAMS[0]) throw e;
      if (token2022Mandatory) throw new Error(`Token-2022 balance scan failed for a Token-2022 project (${pid()}) — refusing to report a possibly-wrong float: ${e.message}`);
      continue; /* legacy-token projects: 2022 read stays best-effort */
    }
...
// ── Retained decision events (W5, docs/COLOSSEUM_ROADMAP.md) ────────────────────
// A bounded, public-safe ring buffer of what a project's ticks decided — the evidence class the
// dashboard labels "decision log (retained events)", distinct from the backfilled Helius
// transfers and the illustrative simulator replay. Written from choke points in the
// module.exports wrappers below, right where each one has its tick's own result — never from
// inside a tick function itself, so every return path (including ones added later) is covered
// without re-touching that function. One project can run several sleeves at once (the base pool,
// the ask-wall, the SOL/BTC/JUP vaults, the treasury dual-sleeve, a concentrate switch), and
// before batch 8 only the base `tick` wrapper recorded anything — the others' decisions (an
// ask-wall roll, a treasury recenter, a BTC-vault cap) were made and acted on but never reached
// this log, so the public "decision log (retained events)" undercounted what the engine actually
// decided. `source` names which sleeve made the call (`"base"`, `"askWall"`, `"sol"`, `"btc"`,
// `"jup"`, `"treasury"`, `"concentrate"`) so the page can label each row instead of implying they
// are all the same tick.
//
// Deliberately excluded: `{ action: "none", reason: "paused" }`, the result EVERY tick()
// returns while the owner's stop order holds (see the Money section of CLAUDE.md) — a paused
// engine is not "deciding" anything, and recording a no-op every scheduler cycle would just
// make an idle fleet look active in the log. The dashboard shows the fact of the pause
// (fleet-wide `ownerStopOrder`) separately from this log, which stays empty and says why.
//
// Shape is intentionally thin — `{ t, action, reason, price, source }` only. Never the operator
// pubkey, a float balance, P&L, or a telegram id: those never flow through a tick() result
// in the first place, but the field allow-list here is the second guard.
const ENGINE_LOG_MAX = 200;
function recordDecision(projectId, result, source) {
  if (!result || typeof result !== "object" || !result.action) return;
  if (result.action === "none" && result.reason === "paused") return; // paused: not a decision
  try {
    const key = `engineLog:${projectId || "clkn"}`;
    const log = kv.get(key, []) || [];
    log.push({
      t: Date.now(),
      action: String(result.action).slice(0, 40),
      reason: String(result.reason || "").slice(0, 200),
      price: typeof result.price === "number" && isFinite(result.price) ? result.price : null,
      source: source ? String(source).slice(0, 40) : null,
    });
    kv.set(key, log.slice(-ENGINE_LOG_MAX));
  } catch (_) { /* best effort — a logging failure must never affect the tick's own result */ }
}
...
async function tick({ dryRun = false } = {}) {
  ...
  // AFFORDABILITY GUARD (BULLEN production incident, 2026-09-25): never close a position this
  // wallet can't afford to reopen — a ±small-% recenter can need a BRAND NEW tick array (real
  // on-chain rent, not free), and a stranded empty pool (closed but never reopened) makes
  // evenPools refuse to run ("need all enabled pools live"). Scoped to bullen for now — its
  // tiny shared wallet is what actually hit this; other projects' reserves are deep enough
  // that it hasn't been an issue there. A failed check HOLDS rather than closing blind.
  if (managed && pid() === "bullen") {
    try {
      const preRanges = eng().suggestRanges(pool, cfg.widthPct);
      const preBalanced = preRanges.options.find((o) => o.id === "balanced");
      const est = await eng().estimateReopenLamports({ owner: operator().publicKey.toBase58(), address, lowerTick: preBalanced.lowerTick, upperTick: preBalanced.upperTick });
      const spendableSolForReopen = Math.max(0, (await getFloat(conn)).sol - cfg.swapSolFloor - (cfg.solGasReserve || 0));
      if (spendableSolForReopen * 1e9 < est.totalLamports) {
        setState({ lastPrice: price, lastTickTs: Date.now() });
        return { ...base, action: "hold", reason: `would need to close, but reopening needs ~${est.totalSol.toFixed(4)} SOL rent and only ~${spendableSolForReopen.toFixed(4)} is spendable — holding to avoid stranding the pool` };
      }
    } catch (e) {
      setState({ lastPrice: price, lastTickTs: Date.now() });
      return { ...base, action: "hold", reason: `reopen-affordability check failed (${e.message}) — holding rather than closing blind` };
    }
  }

  // 1) Close the existing position (withdraw + collect fees) if present.
  // Snapshot float right before the close so the roll-rebalance below can measure exactly
  // what THIS close freed — never another pool's idle float or the SOL gas reserve. HARD TOKEN
  // GUARD (review round 2026-09-26): the active project's token must be the BULLEN mint AND its
  // project record must have tokenSellOk — pid()==="bullen" alone isn't enough (a project-id
  // collision or a misregistered project must not inherit BULLEN's rebalance budget).
  const rollRebalProj = managed ? getProject(pid()) : null;
  const rollRebalAllowed = !!(managed && pid() === "bullen" && tok().mint === BULLEN_MINT && rollRebalProj && rollRebalProj.tokenSellOk === true);
  let preCloseFloat = null;
  if (rollRebalAllowed) { try { preCloseFloat = await getFloat(conn); } catch { /* best effort — skip the rebalance below if this failed */ } }
  if (managed) {
    ...
    // ROLL REBALANCE (production incident, 2026-09-26; hardened after adversarial review the
    // same day): a one-way move closes this pool almost entirely into one asset, so sizing the
    // reopen by the scarce side (below) opens tiny and strands the rest as idle float.
    // Rebalance THIS close's freed assets to ~50/50 value at the current price BEFORE
    // reopening — gated on rollRebalAllowed (the hard token guard above), a daily USD budget
    // (rollRebalanceUsdPerDay, OFF/0 by default — a project must opt in), the shared
    // swapsToday/maxSwapsPerDay counter, skipped under ~$5, and clamped to maxSwapUsdPerCycle.
    // Freed amounts are bounded to what THIS position itself reported before the close
    // (clampFreedToPosition) so a stray deposit or RPC lag can't inflate the swap size. A swap
    // failure logs and falls through to the reopen with whatever float exists — it must never
    // abort and leave the pool closed, EXCEPT an uncertain confirmation (timeout), which holds
    // this cycle rather than reopening on top of an unknown balance (positionMint is already
    // null, so the next tick reopens fresh from the real, settled balances).
    if (rollRebalAllowed && preCloseFloat) {
      try {
        const postCloseFloat = await getFloat(conn);
        const freedClknRaw = Math.max(0, (postCloseFloat.clkn || 0) - (preCloseFloat.clkn || 0));
        const freedQuoteRaw = Math.max(0, (postCloseFloat.usdc || 0) - (preCloseFloat.usdc || 0));
        const freedClkn = clampFreedToPosition(freedClknRaw, managed.clknAmount, managed.pendingFeeClkn);
        const freedQuote = clampFreedToPosition(freedQuoteRaw, managed.quoteAmount, managed.pendingFeeQuote);
        const rrSwapsToday = st.swapDayStamp === today ? (st.swapsToday || 0) : 0;
        const rb = rollRebalanceDecision({
          clknUi: freedClkn, quoteUi: freedQuote, price, quoteUsd: 1, maxSwapUsdPerCycle: cfg.maxSwapUsdPerCycle,
          dayBudgetUsd: cfg.rollRebalanceUsdPerDay, usedTodayUsd: st.rollRebalanceUsdToday, budgetDayStamp: st.rollRebalanceDay,
          swapsToday: rrSwapsToday, maxSwapsPerDay: cfg.maxSwapsPerDay, todayStamp: today,
        });
        if (rb.action === "swap") {
          const sw = rb.dir === "sellClkn"
            ? await manualSwap({ fromSym: "CLKN", toSym: "USDC", amountUi: rb.amountUi, slippageBps: cfg.swapSlippageBps, maxImpactPct: cfg.rollRebalanceMaxImpactPct || 3, dryRun: false, silent: true, allowTokenSell: true })
            : await manualSwap({ fromSym: "USDC", toSym: "CLKN", amountUi: rb.amountUi, slippageBps: cfg.swapSlippageBps, maxImpactPct: cfg.rollRebalanceMaxImpactPct || 3, dryRun: false, silent: true });
          report.steps.push({ rollRebalance: { dir: rb.dir, amountUi: Number(rb.amountUi.toFixed(6)), swapUsd: Math.round(rb.swapUsd), clamped: !!rb.clamped, result: sw && sw.action, sig: sw && sw.sig, reason: rb.reason } });
          setState({
            rollRebalanceUsdToday: (st.rollRebalanceDay === today ? (st.rollRebalanceUsdToday || 0) : 0) + rb.swapUsd,
            rollRebalanceDay: today,
            swapsToday: rrSwapsToday + 1, swapDayStamp: today,
          });
        }
      } catch (e) {
        const msg = String((e && e.message) || e);
        console.warn(`[vault:${pid()}] roll rebalance FAILED (base): ${msg}`);
        if (/timeout|may still land/i.test(msg)) {
          // CONFIRM-TIMEOUT SAFETY: the swap may have actually landed — reopening on top of an
          // unconfirmed balance could size the reopen wrong. Hold; positionMint is already
          // null, so the next tick opens fresh from the real, settled balances.
          setState({ lastPrice: price, lastTickTs: Date.now(), dayActions, dayStamp: today });
          report.action = "hold";
          report.reason = `roll rebalance swap confirmation uncertain (${msg}) — holding this cycle; positions closed, next tick reopens from real balances`;
          return report;
        }
        report.steps.push({ rollRebalanceError: msg });
      }
    }
  }
  ...
  // Deployed-value cap (opt-in via cfg.maxDeployedUsd — see deployCapRoomUsd's own comment).
  const capRoomUsd = await deployCapRoomUsd(cfg);

  // Size: deploy up to deployFrac of USDC (capped at maxUsd AND at any remaining cap room);
  // if that needs more CLKN than we hold, fall back to sizing by our CLKN balance (also capped).
  const usdcDeploy = Math.min(Math.max(0, float.usdc - cfg.usdcFloor) * cfg.deployFrac, cfg.maxUsd, capRoomUsd);
  ...
  if (usdcDeploy > 0) {
    ...
    if (clknNeeded > float.clkn * cfg.deployFrac) {
      inputMint = tok().mint;
      inputAmount = Math.min(float.clkn * cfg.deployFrac, price > 0 ? capRoomUsd / price : float.clkn * cfg.deployFrac);
    }
  ...
  // The deployed-value cap can clamp inputAmount to ~0 even though the float check above passed
  // (there WAS float, but no cap room left) — catch that here rather than opening a zero-size
  // position.
  if (!(inputAmount > 0)) {
    setState({ lastPrice: price, lastTickTs: Date.now(), dayActions, dayStamp: today });
    report.action = "capped"; report.reason = `deployed-value cap ($${cfg.maxDeployedUsd}) reached — no room to deploy more`;
    return report;
  }

  // Same-class affordability guard as tickSol's (Codex review on #444, "add it to the base tick()
  // leg for bullen at least"): when the token side ends up primary here, the other side is USDC —
  // quote it and fail closed (hold, never proceed blind) on an error or a non-finite result.
  // Scoped to bullen; other projects' USDC-side headroom hasn't been an issue.
  if (pid() === "bullen" && inputMint === tok().mint && inputAmount > 0) {
    let usdcNeeded;
    try {
      const q2 = await eng().quote({ address, owner: operator().publicKey.toBase58(), inputMint: tok().mint, inputAmount: String(inputAmount), lowerTick: balanced.lowerTick, upperTick: balanced.upperTick, slippageBps: cfg.slippageBps });
      usdcNeeded = Number(fresh.clknIsA ? q2.maxB : q2.maxA);
    } catch (e) {
      setState({ lastPrice: price, lastTickTs: Date.now(), dayActions, dayStamp: today });
      report.action = "no-float";
      report.reason = `could not verify the USDC side is affordable (quote failed: ${e.message}) — holding rather than deploying blind`;
      return report;
    }
    const spendableUsdc = Math.max(0, float.usdc - cfg.usdcFloor);
    if (!Number.isFinite(usdcNeeded) || usdcNeeded < 0) {
      setState({ lastPrice: price, lastTickTs: Date.now(), dayActions, dayStamp: today });
      report.action = "no-float";
      report.reason = `could not verify the USDC side is affordable (quote returned ${usdcNeeded}) — holding rather than deploying blind`;
      return report;
    }
    if (usdcNeeded > spendableUsdc) {
      if (!(spendableUsdc > 0) || !(usdcNeeded > 0)) {
        setState({ lastPrice: price, lastTickTs: Date.now(), dayActions, dayStamp: today });
        report.action = "no-float";
        report.reason = `sized position needs $${usdcNeeded.toFixed(2)} USDC but only $${spendableUsdc.toFixed(2)} is spendable — holding`;
        return report;
      }
      inputAmount = inputAmount * (spendableUsdc / usdcNeeded);
    }
  }
...
async function tickJup({ dryRun = false } = {}) {
  ...
  // Snapshot float right before the close so the roll-rebalance below can measure exactly what
  // THIS close freed — never another pool's idle float. HARD TOKEN GUARD (review round
  // 2026-09-26): see the identical block + comment in tick() above.
  const rollRebalProj = managed ? getProject(pid()) : null;
  const rollRebalAllowed = !!(managed && pid() === "bullen" && tok().mint === BULLEN_MINT && rollRebalProj && rollRebalProj.tokenSellOk === true);
  let preCloseFloat = null;
  if (rollRebalAllowed) { try { preCloseFloat = await getFloat(conn); } catch { /* best effort — skip the rebalance below if this failed */ } }
  if (managed) {
    ...
    // ROLL REBALANCE (production incident, 2026-09-26; hardened after adversarial review the
    // same day): see the identical block + comment in tick() above. This is the pool that never
    // reopened at all in today's incident — the close froze the wallet almost entirely into JUP
    // with ~no CLKN to pair, so the reopen sized by the scarce (CLKN) side and had nothing left
    // to deploy. `jupUsd` is already the live price fetched above for the external-market guard.
    if (rollRebalAllowed && preCloseFloat) {
      try {
        const postCloseFloat = await getFloat(conn);
        const freedClknRaw = Math.max(0, (postCloseFloat.clkn || 0) - (preCloseFloat.clkn || 0));
        const freedJupRaw = Math.max(0, (postCloseFloat.jup || 0) - (preCloseFloat.jup || 0));
        const freedClkn = clampFreedToPosition(freedClknRaw, managed.clknAmount, managed.pendingFeeClkn);
        const freedJup = clampFreedToPosition(freedJupRaw, managed.quoteAmount, managed.pendingFeeQuote);
        const rrSwapsToday = st.swapDayStamp === today ? (st.swapsToday || 0) : 0;
        const rb = rollRebalanceDecision({
          clknUi: freedClkn, quoteUi: freedJup, price, quoteUsd: jupUsd, maxSwapUsdPerCycle: cfg.maxSwapUsdPerCycle,
          dayBudgetUsd: cfg.rollRebalanceUsdPerDay, usedTodayUsd: st.rollRebalanceUsdToday, budgetDayStamp: st.rollRebalanceDay,
          swapsToday: rrSwapsToday, maxSwapsPerDay: cfg.maxSwapsPerDay, todayStamp: today,
        });
        if (rb.action === "swap") {
          const sw = rb.dir === "sellClkn"
            ? await manualSwap({ fromSym: "CLKN", toSym: "JUP", amountUi: rb.amountUi, slippageBps: cfg.swapSlippageBps, maxImpactPct: cfg.rollRebalanceMaxImpactPct || 3, dryRun: false, silent: true, allowTokenSell: true })
            : await manualSwap({ fromSym: "JUP", toSym: "CLKN", amountUi: rb.amountUi, slippageBps: cfg.swapSlippageBps, maxImpactPct: cfg.rollRebalanceMaxImpactPct || 3, dryRun: false, silent: true });
          report.steps.push({ rollRebalance: { dir: rb.dir, amountUi: Number(rb.amountUi.toFixed(6)), swapUsd: Math.round(rb.swapUsd), clamped: !!rb.clamped, result: sw && sw.action, sig: sw && sw.sig, reason: rb.reason } });
          setState({
            rollRebalanceUsdToday: (st.rollRebalanceDay === today ? (st.rollRebalanceUsdToday || 0) : 0) + rb.swapUsd,
            rollRebalanceDay: today,
        ...
        const msg = String((e && e.message) || e);
        console.warn(`[vault:${pid()}] roll rebalance FAILED (jup): ${msg}`);
        if (/timeout|may still land/i.test(msg)) {
...
async function tickSol({ dryRun = false } = {}) {
  ...
  try { const f = await getFloat(conn); preSolFree = spendableSolPure(cfg, f); preClknForSol = f.clkn || 0; } catch { /* best effort */ }   // BOTH guards, via the shared/tested helper (lib/engine-decisions.js)
  ...
  // AFFORDABILITY GUARD — see the identical block + comment in tick() above. Scoped to bullen.
  if (managed && pid() === "bullen") {
    try {
      const preRanges = eng().suggestRanges(pool, cfg.solWidthPct);
      const preBalanced = preRanges.options.find((o) => o.id === "balanced");
      const est = await eng().estimateReopenLamports({ owner: operator().publicKey.toBase58(), address, lowerTick: preBalanced.lowerTick, upperTick: preBalanced.upperTick });
      const spendableSolForReopen = Math.max(0, (await getFloat(conn)).sol - cfg.swapSolFloor - (cfg.solGasReserve || 0));
      if (spendableSolForReopen * 1e9 < est.totalLamports) {
  ...
  // Snapshot float right before the close so the roll-rebalance below can measure exactly what
  // THIS close freed — never another pool's idle float or the SOL gas reserve. HARD TOKEN GUARD
  // (review round 2026-09-26): see the identical block + comment in tick() above.
  const rollRebalProj = managed ? getProject(pid()) : null;
  const rollRebalAllowed = !!(managed && pid() === "bullen" && tok().mint === BULLEN_MINT && rollRebalProj && rollRebalProj.tokenSellOk === true);
  let preCloseFloat = null;
  if (rollRebalAllowed) { try { preCloseFloat = await getFloat(conn); } catch { /* best effort — skip the rebalance below if this failed */ } }
  if (managed) {
    ...
    // ROLL REBALANCE (production incident, 2026-09-26; hardened after adversarial review the
    // same day): see the identical block + comment in tick() above. The 2026-09-26 incident hit
    // THIS pool worst (the SOL pool went $167→$109 after closing almost entirely into one
    // asset). SOL's own gas reserve is never touched — freedSol is measured as this close's OWN
    // delta, not the wallet's total SOL, and is further bounded to the closed position's own
    // reported amount below.
    if (rollRebalAllowed && preCloseFloat) {
      try {
        const postCloseFloat = await getFloat(conn);
        const freedClknRaw = Math.max(0, (postCloseFloat.clkn || 0) - (preCloseFloat.clkn || 0));
        const freedSolRaw = Math.max(0, (postCloseFloat.sol || 0) - (preCloseFloat.sol || 0));
        const freedClkn = clampFreedToPosition(freedClknRaw, managed.clknAmount, managed.pendingFeeClkn);
        const freedSol = clampFreedToPosition(freedSolRaw, managed.quoteAmount, managed.pendingFeeQuote);
        const solUsdNow = (await solPriceUsd()) || 0;
        const rrSwapsToday = st.swapDayStamp === today ? (st.swapsToday || 0) : 0;
        const rb = rollRebalanceDecision({
          clknUi: freedClkn, quoteUi: freedSol, price, quoteUsd: solUsdNow, maxSwapUsdPerCycle: cfg.maxSwapUsdPerCycle,
          dayBudgetUsd: cfg.rollRebalanceUsdPerDay, usedTodayUsd: st.rollRebalanceUsdToday, budgetDayStamp: st.rollRebalanceDay,
          swapsToday: rrSwapsToday, maxSwapsPerDay: cfg.maxSwapsPerDay, todayStamp: today,
        });
        if (rb.action === "swap") {
          const sw = rb.dir === "sellClkn"
            ? await manualSwap({ fromSym: "CLKN", toSym: "SOL", amountUi: rb.amountUi, slippageBps: cfg.swapSlippageBps, maxImpactPct: cfg.rollRebalanceMaxImpactPct || 3, dryRun: false, silent: true, allowTokenSell: true })
            : await manualSwap({ fromSym: "SOL", toSym: "CLKN", amountUi: rb.amountUi, slippageBps: cfg.swapSlippageBps, maxImpactPct: cfg.rollRebalanceMaxImpactPct || 3, dryRun: false, silent: true });
          report.steps.push({ rollRebalance: { dir: rb.dir, amountUi: Number(rb.amountUi.toFixed(6)), swapUsd: Math.round(rb.swapUsd), clamped: !!rb.clamped, result: sw && sw.action, sig: sw && sw.sig, reason: rb.reason } });
          setState({
            rollRebalanceUsdToday: (st.rollRebalanceDay === today ? (st.rollRebalanceUsdToday || 0) : 0) + rb.swapUsd,
            rollRebalanceDay: today,
        ...
        const msg = String((e && e.message) || e);
        console.warn(`[vault:${pid()}] roll rebalance FAILED (sol): ${msg}`);
        if (/timeout|may still land/i.test(msg)) {
  ...
  const balanced = ranges.options.find((o) => o.id === "balanced");
  const float = await getFloat(conn);
  // BOTH guards, via the shared/tested helper (matching preSolFree above; solGasReserve was
  // missing here before — audit, BULLEN production incident 2026-09-25: a tiny wallet with a
  // stock swapSolFloor left solAvail at 0 and this branch fell to "deploy 100% of the token
  // side" with no check that the position it implies is even affordable — see the guard block
  // below buildOpenPosition needs). Extracted to lib/engine-decisions.js `spendableSol()` and
  // pinned in scripts/engine-sim-test.cjs so this can't silently drift out of sync again.
  const spendableSol = spendableSolPure(cfg, float);

  // Deployed-value cap (opt-in via cfg.maxDeployedUsd — see deployCapRoomUsd's own comment).
  // Converted to SOL/CLKN terms via the live SOL/USD price (`price` here is CLKN priced in SOL).
  const capRoomUsdSol = await deployCapRoomUsd(cfg);
  const capCapped = capRoomUsdSol < Infinity;   // a cap is actually configured
  const solUsdForCap = capCapped ? await solPriceUsd() : 0;
  // Fail CLOSED, never pass the raw USD figure through as a SOL/CLKN amount: without a live SOL
  // price there is no way to convert the $ cap into native units, so a configured cap with no
  // price available means zero room, not unlimited.
  const capRoomSol = !capCapped ? Infinity : (solUsdForCap > 0 ? capRoomUsdSol / solUsdForCap : 0);
  const capRoomClkn = !capCapped ? Infinity : (solUsdForCap > 0 && price > 0 ? capRoomUsdSol / (price * solUsdForCap) : 0);
  ...
  const solAvail = spendableSol;
  const solDeploy = Math.min(solAvail * cfg.deployFrac, cfg.solMaxSol, capRoomSol);
  ...
  if (solDeploy > 0) {
    ...
    if (clknNeeded > float.clkn * cfg.deployFrac) { inputMint = tok().mint; inputAmount = Math.min(float.clkn * cfg.deployFrac, capRoomClkn); }
  } else if (float.clkn > 0) {
    inputMint = tok().mint; inputAmount = Math.min(float.clkn * cfg.deployFrac, capRoomClkn);
  } else {
    setState({ sol_lastPrice: price, sol_dayActions: dayActions, sol_dayStamp: today });
  ...
  // The deployed-value cap can clamp inputAmount to ~0 even though the float check above passed
  // — catch that here rather than opening a zero-size position (mirrors the base tick() guard).
  if (!(inputAmount > 0)) {
    setState({ sol_lastPrice: price, sol_dayActions: dayActions, sol_dayStamp: today });
    report.action = "capped"; report.reason = `deployed-value cap ($${cfg.maxDeployedUsd}) reached — no room to deploy more`;
    return report;
  ...
  // GUARD (BULLEN production incident, 2026-09-25): whenever the TOKEN side ends up primary
  // (either branch above), the position's OTHER side is SOL — and sizing off 100% of the
  // token float with no check on that implied SOL half sent a real deploy that needed MORE
  // SOL than the wallet held after reserves ("fall back to all token" had no balance check).
  // Quote the actual pair for the chosen input and, if the implied SOL side exceeds what's
  // genuinely spendable, scale the input DOWN proportionally so the position that's about to
  // be built actually fits — a sizing must never exceed the wallet's real balance.
  // FAIL CLOSED (Codex review on #444): a quote error or a non-finite/NaN result must HOLD, not
  // silently proceed with the unscaled amount — `NaN > spendableSol` is false, so the old
  // catch-and-proceed and the bare `>` comparison both let an unverifiable size through.
  if (inputMint === tok().mint && inputAmount > 0) {
    let solNeeded;
    try {
      const q2 = await eng().quote({ address, owner: operator().publicKey.toBase58(), inputMint: tok().mint, inputAmount: String(inputAmount), lowerTick: balanced.lowerTick, upperTick: balanced.upperTick, slippageBps: cfg.slippageBps });
      solNeeded = Number(fresh.clknIsA ? q2.maxB : q2.maxA);
    } catch (e) {
      setState({ sol_lastPrice: price, sol_dayActions: dayActions, sol_dayStamp: today });
      report.action = "no-float";
      report.reason = `could not verify the SOL side is affordable (quote failed: ${e.message}) — holding rather than deploying blind`;
      return report;
    }
    if (!Number.isFinite(solNeeded) || solNeeded < 0) {
      setState({ sol_lastPrice: price, sol_dayActions: dayActions, sol_dayStamp: today });
      report.action = "no-float";
      report.reason = `could not verify the SOL side is affordable (quote returned ${solNeeded}) — holding rather than deploying blind`;
      return report;
    }
    if (solNeeded > spendableSol) {
      if (!(spendableSol > 0) || !(solNeeded > 0)) {
        setState({ sol_lastPrice: price, sol_dayActions: dayActions, sol_dayStamp: today });
        report.action = "no-float";
        report.reason = `sized position needs ${solNeeded.toFixed(4)} SOL but only ${spendableSol.toFixed(4)} is spendable after reserves — holding`;
        await notify(`📊 <b>Liquidity vault (${tok().symbol}/SOL)</b>: skipped a deploy that needed more SOL than the wallet holds after reserves.`);
        return report;
      }
      inputAmount = inputAmount * (spendableSol / solNeeded);
    }
  }
...
// Pay a fixed list of OUTSIDE wallets one SPL token — giveaway prizes, buy-comp bonuses.
//
// Every other money path in this file moves value between wallets WE control. This is the only
// one that sends to addresses we do not, so it is deliberately narrow:
//
//   • The caller supplies the exact recipients and amounts. Nothing here derives them from a
//     balance, so this cannot be pointed at a wallet and told to "empty it".
//   • It REFUSES to sign with a wallet that is the token's MINT AUTHORITY, checked on-chain every
//     call rather than against a hardcoded list so it stays true if wallets change. The reason is
//     concrete: the CLKN treasury (2zMCU…EuPy8) holds the CUNA supply AND is CUNA's mint authority
//     (see server.js). Spending from it over HTTP would put MINTING behind an admin key, not just
//     spending. Fund a plain hot wallet and pay from that — the hot/cold split this guard forces.
//   • A per-recipient cap and a whole-run total cap are BOTH required. A mistyped amount cannot
//     drain the payer.
//   • DRY RUN unless dryRun === false, like every other money path here.
//   • One transaction per recipient. Slower than batching, but a failure then costs exactly one
//     winner and is retryable without re-paying anyone who already landed.
// The recipient list as a JSON response may carry it: amountRaw is a BigInt internally (exact
// base units), and JSON.stringify throws on BigInt — the first lock-to-earn dry run failed on
// exactly that (2026-09-16). Strings out, always.
function serializableRecipients(list) {
  return (list || []).map((r) => ({ wallet: r.wallet, amountUi: r.amountUi, ...(r.amountRaw !== null && r.amountRaw !== undefined ? { amountRaw: r.amountRaw.toString() } : {}) }));
}
async function payoutSpl({ mintAddr, recipients, perRecipientMaxUi, totalMaxUi, dryRun = true, onPaid = null } = {}) {
  if (!isEnabled()) return { enabled: false, action: "none", reason: "operator not set" };
  const spl = require("@solana/spl-token");
  const conn = connection();
  const from = operator();
  const mint = new PublicKey(String(mintAddr));
  // amountRaw (base units, optional) is what actually moves when present: the lock-to-earn ledger
  // is kept in raw units, and a raw→ui→raw float round trip can be off by a unit. amountUi stays
  // the number the caps, the balance check and the journal speak in.
  const list = (recipients || []).map((r) => ({ wallet: String((r && r.wallet) || ""), amountUi: Number(r && r.amountUi),
    amountRaw: r && r.amountRaw != null ? (() => { try { return BigInt(String(r.amountRaw)); } catch (_) { return null; } })() : null }));
  if (!list.length) return { action: "none", reason: "no recipients" };
  for (const r of list) {
    if (!SOL_ADDR_RE.test(r.wallet)) throw new Error(`bad recipient address: ${r.wallet}`);
    if (!(r.amountUi > 0)) throw new Error(`bad amount for ${r.wallet}`);
    if (r.amountRaw !== null && r.amountRaw !== undefined && !(r.amountRaw > 0n)) throw new Error(`bad raw amount for ${r.wallet}`);
    // A NaN cap must not pass: `x > NaN` is false, so an unparsable limit would silently
    // disable the very check that stops a mistyped amount draining the payer.
    if (perRecipientMaxUi != null) {
      const cap = Number(perRecipientMaxUi);
      if (!Number.isFinite(cap)) throw new Error(`perRecipientMax is not a number: ${perRecipientMaxUi}`);
  ...
  if (totalMaxUi == null) throw new Error("totalMaxUi is required — refusing an uncapped payout");
  const totalCap = Number(totalMaxUi);
  if (!Number.isFinite(totalCap)) throw new Error(`totalMax is not a number: ${totalMaxUi}`);
  if (total > totalCap) throw new Error(`total ${total} exceeds totalMax ${totalCap}`);

  // THE GUARD: a mint authority must never be the wallet an HTTP endpoint spends from.
  let mintAuthority = null;
  try {
    const info = await conn.getParsedAccountInfo(mint);
    const parsed = info && info.value && info.value.data && info.value.data.parsed;
    // A SUCCESSFUL response that isn't a parsed mint must fail closed too. value:null (an RPC
    // that failed over to a lagging node) or base64 `data` both yield undefined here, and the
    // old code read that as "no mint authority — safe to proceed". This guard is the only thing
    // between &from=treasury over HTTP and a wallet this file describes as having held mint
    // authority, so "the read told us nothing" must never pass as "the authority is revoked".
    if (!parsed || parsed.type !== "mint" || !parsed.info) {
      throw new Error("mint account did not parse as a mint (refusing to sign blind)");
    }
    mintAuthority = parsed.info.mintAuthority;   // null here genuinely means revoked
  } catch (e) { throw new Error(`could not read mint authority (refusing to sign blind): ${e.message}`); }
  if (mintAuthority && mintAuthority === from.publicKey.toBase58()) {
    throw new Error("refusing to pay from the token's MINT AUTHORITY wallet — fund a separate hot wallet and pay from that");
  }

  const fromAta = spl.getAssociatedTokenAddressSync(mint, from.publicKey);
  let bal = 0, dec = 9;
  try {
    const tb = (await conn.getTokenAccountBalance(fromAta)).value;
    bal = Number(tb.uiAmount) || 0;
    if (tb.decimals != null) dec = tb.decimals;      // authoritative on-chain decimals
  } catch (e) { return { action: "none", reason: `payer holds no account for ${mintAddr}` }; }
  const out = { from: from.publicKey.toBase58(), mint: String(mintAddr), decimals: dec, balance: bal, total, count: list.length };
  if (bal < total) return { ...out, action: "none", reason: `insufficient balance: have ${bal}, need ${total}` };
  if (dryRun) return { ...out, action: "would-pay", recipients: serializableRecipients(list) };

  const loop = await runPayoutLoop({
    list,
    // prepareOne SIGNS and hands back the signature the cluster will know the transfer by, so the
    // loop can journal it BEFORE anything is broadcast (see the loop's own comment). submit() is
    // the broadcast, and it must never be called before that journal row is on disk.
    prepareOne: async (r) => {
      const destOwner = new PublicKey(r.wallet);
      const destAta = spl.getAssociatedTokenAddressSync(mint, destOwner);
      const tx = new Transaction()
        .add(spl.createAssociatedTokenAccountIdempotentInstruction(from.publicKey, destAta, destOwner, mint))
      ...
      tx.recentBlockhash = (await conn.getLatestBlockhash()).blockhash;
      tx.sign(from);
      // The signature is a function of the signed bytes — known here, before any broadcast.
      const sig = bs58.encode(tx.signature);
      const raw = tx.serialize();
      return { sig, submit: async () => {
        const got = await conn.sendRawTransaction(raw, { skipPreflight: false, maxRetries: 3 });
        if (got !== sig) throw new Error(`rpc echoed a different signature (${String(got).slice(0, 12)}…) — journaled ${sig.slice(0, 12)}…`);
        return got;
      } };
    },
    confirmOne: (sig) => confirmSig(conn, sig),
    onPaid,
  });
  return { ...out, ...loop };
}

// THE LINE BETWEEN "NOT SENT" AND "SENT" IS THE WHOLE SAFETY MODEL OF A PAYOUT.
//
// Before sendOne resolves, nothing has moved — a throw is a clean `failed` and the recipient can
// safely be retried. AFTER it resolves, the transaction is in the cluster and the money may
// already be gone, even if confirmOne then times out (~90s, roughly blockhash expiry).
//
// This loop got the ordering wrong TWICE, both times while carrying a comment claiming otherwise:
//   1. send and confirm shared one try/catch with `const sig` declared inside it, so a confirm
//      timeout threw the signature away, the row was recorded as `failed` with no sig,
//      payoutOwed put the wallet back in `owed`, and the documented retry PAID THEM AGAIN.
//   2. the fix split them and moved the record after the confirm while the comment said "Record
//      BEFORE confirming". That left a ~90s window per recipient in which the signature existed
//      only in a local variable — and a Railway redeploy (SIGTERM) or an OOM inside it lost the
//      record of money that had already been sent. Same double-pay, later.
//
// So it is extracted, exported and TESTED, rather than trusted. The invariant a test can now
// assert directly: for every recipient, `record` is called with the signature BEFORE `confirm` is
// awaited, and a record that fails to persist stops the batch instead of sending the next one.
//
// THIRD TIME (second reviewer, 2026-09-15): with sendOne the signature only exists once
// sendRawTransaction RESOLVES. An RPC that accepted the transaction but whose response timed out
// therefore threw, the row was filed as a clean `failed` with no sig, and a retry paid the winner
// again. So callers now pass `prepareOne`, which signs first and returns { sig, submit }: the sig
// is journaled as pending BEFORE submit() broadcasts, and a broadcast that throws leaves a pending
// row for the sweep / the operator — never a retryable blank. `sendOne` remains for callers that
// cannot know the signature up front; it keeps the older (weaker) ordering.
async function runPayoutLoop({ list, sendOne, prepareOne, confirmOne, onPaid }) {
  const paid = [], pending = [], failed = [];
  // A RECORD THAT DID NOT LAND IS A DOUBLE-PAY WAITING TO HAPPEN. onPaid writes to the Railway
  // volume; if /data is full, read-only, or failed to re-attach after a redeploy — which is
  // exactly when a payout gets retried — the write throws. Swallowing it means money leaves the
  // wallet with nothing durable recording it, and the next run pays everyone again. So a record
  // failure stops the batch: the recipients not yet sent keep their prizes and can be paid once
  // the store is writable, which is strictly better than paying them twice.
  let recordError = null;
  let sentCount = 0;
  const record = (row) => {
    if (typeof onPaid !== "function") return true;
    try { onPaid(row); return true; }
    catch (e) { recordError = String((e && e.message) || e).slice(0, 200); return false; }
  };
  for (const r of list) {
    if (recordError) break;
    let sig = null;
    if (typeof prepareOne === "function") {
      // JOURNAL BEFORE BROADCAST. Sign, learn the signature, write the pending row, THEN send.
      let prep;
      try {
        prep = await prepareOne(r);
        if (!prep || !prep.sig || typeof prep.submit !== "function") throw new Error("prepareOne returned no signature");
      } catch (e) {
        // Nothing was signed-and-sent — safe to retry this recipient.
        failed.push({ wallet: r.wallet, amountUi: r.amountUi, sig: null, error: String((e && e.message) || e).slice(0, 200) });
        continue;
      }
      sig = prep.sig;
      if (!record({ wallet: r.wallet, amountUi: r.amountUi, sig, pending: true })) {
        // Ledger unwritable and NOTHING broadcast: this recipient keeps their prize and the batch
        // stops here (rule above). Not `pending` — there is nothing on-chain to reconcile.
        failed.push({ wallet: r.wallet, amountUi: r.amountUi, sig: null, error: "not sent — ledger unwritable: " + recordError });
        break;
      }
      sentCount++;
      try { await prep.submit(); }
      catch (e) {
        // Broadcast threw AFTER the journal row: it may or may not have reached the cluster (the
        // accepted-but-timed-out case). It stays pending for the sweep / the operator — never a
        // clean failed, never retried blind.
        pending.push({ wallet: r.wallet, amountUi: r.amountUi, sig, pending: true,
                       error: "broadcast error — may have landed, verify on-chain: " + String((e && e.message) || e).slice(0, 200) });
        continue;
      }
    } else {
      try {
        sig = await sendOne(r);
        if (!sig) throw new Error("send returned no signature");
      } catch (e) {
        // Nothing left the wallet — safe to retry this recipient.
        failed.push({ wallet: r.wallet, amountUi: r.amountUi, sig: null, error: String((e && e.message) || e).slice(0, 200) });
        continue;
      }
      sentCount++;
      // SUBMITTED — record it NOW, before the confirm. Recorded as pending:true; recordPayout's own
      // "pending -> false" branch resolves it when the confirm lands below.
      if (!record({ wallet: r.wallet, amountUi: r.amountUi, sig, pending: true })) {
        pending.push({ wallet: r.wallet, amountUi: r.amountUi, sig, pending: true, recorded: false,
                       error: "sent but NOT recorded: " + recordError });
        break;
      }
    }
    try {
      await confirmOne(sig);
      const row = { wallet: r.wallet, amountUi: r.amountUi, sig };
      paid.push(row); record(row);
    } catch (e) {
      const row = { wallet: r.wallet, amountUi: r.amountUi, sig, pending: true,
                    error: String((e && e.message) || e).slice(0, 200) };
      pending.push(row); record(row);
    }
  }
  // Never report "paid" when nothing was. The old unconditional action:"paid" meant a run where
  // every transfer failed still read as success to anything skimming the headline field.
  const action = paid.length && !failed.length && !pending.length ? "paid"
    : (paid.length || pending.length ? "partial" : "failed");
  const unsent = recordError ? list.slice(sentCount + failed.length) : [];
  return { action, paid, pending, failed,
    ...(recordError ? { recordError, recordFailed: true, unsent,
      detail: "the payout ledger could not be written, so the run STOPPED. Verify the sigs above " +
              "on-chain and fix the store before retrying — recipients in `unsent` were not paid." } : {}) };
}
...
async function manualSwap({ fromSym, toSym, amountUi, slippageBps = 100, maxImpactPct = null, dryRun = false, silent = false, allowTokenSell = false }) {
  ...
  // Jupiter returns priceImpactPct as a decimal fraction (e.g. "0.0012" = 0.12%). A MISSING or
  // non-numeric figure is tracked separately (impactKnown) — Codex review on #444: the old
  // `Number(x) || 0` silently treated "Jupiter didn't tell us" as "zero impact", which is the
  // opposite of safe. It only refuses below when a ceiling is actually being enforced — a
  // caller that never asked for a ceiling hasn't opted into that protection either way.
  const rawImpact = q.priceImpactPct;
  const impactKnown = rawImpact !== undefined && rawImpact !== null && Number.isFinite(Number(rawImpact));
  const impactPct = impactKnown ? Math.abs(Number(rawImpact)) * 100 : 0;
  const base = { enabled: true, reason: `${inAmt.toFixed(4)} ${fromSym} → ${out.toFixed(4)} ${toSym} (impact ${impactKnown ? impactPct.toFixed(3) + "%" : "unknown"})`, inAmt, out, impactPct: Number(impactPct.toFixed(4)), impactKnown };
  ...
  //
  // PROJECT-SCOPED HARD FLOOR on top of that (Token-2022 audit, BULLEN intake 2026-09-25):
  // /api/whirlpool/vault/swap never passes maxImpactPct at all, so a manual sell of the
  // PROJECT TOKEN had no ceiling unless the caller remembered one — and BULLEN's only venue
  // is a thin ~$50k PumpSwap pool where an uncapped sell could move price hard. This table can
  // only ever TIGHTEN whatever the caller passed, never loosen it, and it's keyed on the
  // project id (pid()) so it stays scoped to BULLEN and cannot be widened by a request param.
  const PROJECT_SELL_IMPACT_CEILING = { bullen: 1.5 };
  const hardCeiling = from === tok().mint ? PROJECT_SELL_IMPACT_CEILING[pid()] : undefined;
  const effectiveCeiling = hardCeiling != null ? (maxImpactPct != null ? Math.min(Number(maxImpactPct), hardCeiling) : hardCeiling) : maxImpactPct;
  if (effectiveCeiling != null) {
    if (!impactKnown) {
      return { ...base, action: "none", reason: `Jupiter's quote had no price-impact figure — refusing rather than assuming it's safe (ceiling ${effectiveCeiling}% is enforced)` };
    }
    if (impactPct > Number(effectiveCeiling)) {
      return { ...base, action: "none", reason: `price impact ${impactPct.toFixed(3)}% > ${effectiveCeiling}% ceiling — skipped` };
    }
  }
...
// ── Bootstrap: turn a single deposited quote asset into the 4-leg float two pools need ──
// Owner ask 2026-09-25 ("the client sends one token and we take care of the rest" — BULLEN's
// wallet arrives holding USDC + whatever SOL gas it already had): convert that into
// token+USDC for the base pool and token+SOL for the sol pool, FOUR legs of equal USD value,
// sized off LIVE balances + LIVE prices on every call — never a fixed split. Re-running after
// a partial run only buys what's still missing (each leg's target minus what's already held,
// floored at 0) — it is IDEMPOTENT and never double-buys. Always dryRun by default; the caller
// (the admin route) is what requires run=1.
//
// Order matters: SOL is bought FIRST. Every tx this bootstrap, the two pool creations and the
// two position opens need (plus rent for two Whirlpool accounts + two positions + the
// Token-2022 BULLEN ATA + the wSOL ATA) is paid in SOL, and the wallet may start near-zero.
const RESERVE_SOL_DEFAULT = 0.1;   // owner's own figure (2026-09-25): 2 pool creations (~0.03 SOL
// rent each, confirmed by the read-only create-pool simulation) + 2 position/tick-array rents +
// the Token-2022 BULLEN ATA (slightly larger than legacy — carries ImmutableOwner) + the wSOL
// ATA + fee/priority-fee headroom for ~10-15 transactions. Generous on purpose: running short
// mid-sequence (after SOL is already spent on BULLEN) is far worse than an idle few cents.
async function bootstrap({ dryRun = true, maxImpactPct = 1.5, maxSlices = 6, minStepUsd = 3, reserveSol = RESERVE_SOL_DEFAULT } = {}) {
  if (!isEnabled()) return { enabled: false, action: "none", reason: "operator not set" };
  const conn = connection();
  const solUsd = await solPriceUsd();
  if (!(solUsd > 0)) return { enabled: true, action: "none", reason: "no live SOL price — refusing to size blind" };
  const market = await marketPriceUsd();
  if (!(market.usd > 0)) return { enabled: true, action: "none", reason: "no live token price — refusing to size blind" };
  const tokenUsd = market.usd;

  const float = await getFloat(conn);
  const spendableSol = Math.max(0, float.sol - reserveSol);

  // Already-deployed position value COUNTS toward the total and toward each leg's target
  // (Codex review on #444): once real pools/positions exist (as they do in production), a
  // re-run must recognize what's already correctly deployed, not just what's sitting in the
  // float — otherwise it would keep trying to build a full second 4-leg set from scratch. Best
  // effort: a read failure treats deployed value as 0, which only ever makes bootstrap think
  // MORE is still needed (never less) — the safe direction for a sizing decision.
  let deployedUsd = 0, deployedTokenUsd = 0, deployedSolLegUsd = 0, deployedUsdcLegUsd = 0;
  try {
    const pv = await publicPositions();
    deployedUsd = Number(pv && pv.totalUsd) || 0;
    const pvClknUsd = Number(pv && pv.clknUsd) || tokenUsd;
    const pvSolUsd = Number(pv && pv.solUsd) || solUsd;
    for (const p of (pv && pv.positions) || []) {
      deployedTokenUsd += (Number(p.clknAmount) || 0) * pvClknUsd;
      if (p.quoteSymbol === "USDC") deployedUsdcLegUsd += Number(p.quoteAmount) || 0;
      else if (p.quoteSymbol === "SOL") deployedSolLegUsd += (Number(p.quoteAmount) || 0) * pvSolUsd;
    }
  } catch (_) { /* best effort — see comment above */ }

  // Total capital this bootstrap is allowed to consider = already-deployed value + spendable
  // SOL + all USDC + any token already held in the FLOAT (so a partial prior run nets out to
  // the same 4-way split rather than re-deriving a smaller total and stranding a leftover).
  const totalUsd = deployedUsd + spendableSol * solUsd + float.usdc + float.clkn * tokenUsd;
  const perLegUsd = totalUsd / 4;
  const targetSolLegUi = perLegUsd / solUsd;
  const targetTokenUsd = perLegUsd * 2;   // one BULLEN leg per pool
  const targetUsdcLegUsd = perLegUsd;

  // "Still needed" nets out what a leg's target ALREADY has covered by deployed positions,
  // then by float, before deciding what to buy — this is what makes a re-run after positions
  // already exist buy only the genuine remainder instead of a second full set.
  const solLegRemainingUsd = Math.max(0, targetSolLegUi * solUsd - deployedSolLegUsd);
  const solNeededUi = Math.max(0, solLegRemainingUsd / solUsd - spendableSol);
  const solNeededUsd = solNeededUi * solUsd;
  const tokenLegRemainingUsd = Math.max(0, targetTokenUsd - deployedTokenUsd);
  const tokenNeededUsd = Math.max(0, tokenLegRemainingUsd - float.clkn * tokenUsd);

  const plan = {
    enabled: true, project: pid(), tokenSymbol: tok().symbol, dryRun,
    livePrices: { solUsd, tokenUsd, tokenSource: market.source },
    reserveSol, float,
    deployed: { totalUsd: Number(deployedUsd.toFixed(2)), tokenUsd: Number(deployedTokenUsd.toFixed(2)), solLegUsd: Number(deployedSolLegUsd.toFixed(2)), usdcLegUsd: Number(deployedUsdcLegUsd.toFixed(2)) },
    totalUsd: Number(totalUsd.toFixed(2)), perLegUsd: Number(perLegUsd.toFixed(2)),
    targets: {
      solLegUi: Number(targetSolLegUi.toFixed(4)), usdcLegUsd: Number(targetUsdcLegUsd.toFixed(2)),
      tokenUsd: Number(targetTokenUsd.toFixed(2)), tokenUi: Math.round(targetTokenUsd / tokenUsd),
    },
    needed: { solUi: Number(solNeededUi.toFixed(4)), solUsd: Number(solNeededUsd.toFixed(2)), tokenUsd: Number(tokenNeededUsd.toFixed(2)) },
    steps: [],
  };

  if (totalUsd < 20) return { ...plan, action: "none", reason: `not enough capital to bootstrap two pools ($${totalUsd.toFixed(2)} total — need at least $20)` };

  // KV LOCK around a REAL run only (Codex review on #444): two overlapping POST ?run=1 calls
  // would each read the same pre-run float and could double-buy before either finishes. A
  // dry-run never signs anything, so it never needs the lock. 5-minute TTL — long enough to
  // cover the slowest realistic run (up to maxSlices Jupiter round trips), short enough that a
  // crashed run doesn't wedge the lock forever.
  const lockKey = `bootstrapLock:${pid()}`;
  if (!dryRun) {
    const heldSince = kv.get(lockKey, null);
    if (heldSince && Date.now() - heldSince < 5 * 60 * 1000) {
      return { ...plan, action: "none", reason: `a bootstrap run is already in progress for ${pid()} (started ${Math.round((Date.now() - heldSince) / 1000)}s ago) — refusing to run concurrently` };
    }
    kv.set(lockKey, Date.now());
  }
  try {
    // STEP 1 — USDC → SOL. SOL/USDC is deep, so a tight ceiling (never above the caller's own) is fine.
    if (solNeededUsd >= minStepUsd) {
      const spendUsdc = Math.min(solNeededUsd, float.usdc);
      const res = await manualSwap({ fromSym: "USDC", toSym: "SOL", amountUi: spendUsdc, maxImpactPct: Math.min(Number(maxImpactPct) || 1.5, 0.5), dryRun, silent: true });
      plan.steps.push({ leg: "sol", targetUsd: Number(solNeededUsd.toFixed(2)), ...res });
      // Subtract step 1's spend in BOTH real and dry runs (Codex review on #444) — a dry-run
      // plan that doesn't account for its own step 1 overstates what step 2 has to work with.
      if (res && (res.action === "swap" || res.action === "would-swap")) float.usdc = Math.max(0, float.usdc - (res.inAmt || spendUsdc));
    } else {
      plan.steps.push({ leg: "sol", action: "none", reason: solNeededUsd > 0 ? `below the $${minStepUsd} step floor` : "already holds enough SOL for its leg + reserve" });
    }

    // STEP 2 — USDC → BULLEN, SLICED so no single swap exceeds maxImpactPct (the only venue is a
    // thin ~$50k PumpSwap pool). Halves the slice and re-quotes on refusal; reports whatever it
    // could not place rather than erroring, so a partial bootstrap is still visible and re-runnable.
    // Also tracks CUMULATIVE impact across all slices combined (Codex review on #444) — each
    // slice can individually pass the per-slice ceiling while the total price move across the
    // whole sequence still degrades badly; stop the sequence, not just each slice, once the
    // cumulative move (vs the FIRST slice's own rate) crosses the same ceiling.
    if (tokenNeededUsd >= minStepUsd) {
      let usdcAvailable = float.usdc;
      if (!dryRun) { try { usdcAvailable = (await getFloat(conn)).usdc; } catch (_) { /* keep the pre-step estimate */ } }
      let remaining = Math.min(tokenNeededUsd, usdcAvailable);
      let sliceUsd = remaining;
      let slices = 0;
      let firstRate = null;   // out/in from the first executed slice — the baseline for cumulative impact
      while (remaining >= minStepUsd && slices < maxSlices) {
        const res = await manualSwap({ fromSym: "USDC", toSym: "CLKN", amountUi: sliceUsd, maxImpactPct, dryRun, silent: true, allowTokenSell: false });
        if (res && (res.action === "swap" || res.action === "would-swap")) {
          const rate = res.inAmt > 0 ? res.out / res.inAmt : null;
          let cumulativeImpactPct = 0;
          if (rate != null) {
            if (firstRate == null) firstRate = rate;
            else if (firstRate > 0) cumulativeImpactPct = Math.max(0, (1 - rate / firstRate) * 100);
          }
          if (cumulativeImpactPct > Number(maxImpactPct)) {
            plan.steps.push({ leg: "token", action: "stuck", reason: `cumulative price impact across slices reached ${cumulativeImpactPct.toFixed(2)}% (> ${maxImpactPct}%) — stopping the sequence rather than degrading the fill further`, remainingUsd: Number(remaining.toFixed(2)) });
            break;
          }
          plan.steps.push({ leg: "token", sliceUsd: Number(sliceUsd.toFixed(2)), cumulativeImpactPct: Number(cumulativeImpactPct.toFixed(3)), ...res });
          remaining -= sliceUsd; slices++;
          sliceUsd = Math.min(sliceUsd, remaining);
        } else {
          sliceUsd = sliceUsd / 2;
          if (sliceUsd < minStepUsd) {
            plan.steps.push({ leg: "token", action: "stuck", reason: `price impact stayed above ${maxImpactPct}% even at the smallest slice — $${remaining.toFixed(2)} left unbought`, remainingUsd: Number(remaining.toFixed(2)) });
            break;
          }
        }
      }
      if (slices >= maxSlices && remaining >= minStepUsd) plan.steps.push({ leg: "token", action: "stuck", reason: `hit the ${maxSlices}-slice cap with $${remaining.toFixed(2)} still unbought — re-run to continue`, remainingUsd: Number(remaining.toFixed(2)) });
    } else {
      plan.steps.push({ leg: "token", action: "none", reason: tokenNeededUsd > 0 ? `below the $${minStepUsd} step floor` : "already holds enough of the project token" });
    }
  } finally {
    if (!dryRun) kv.set(lockKey, null);
  }

  const stuck = plan.steps.some((s) => s.action === "stuck");
  plan.action = stuck ? "partial" : (dryRun ? "would-bootstrap" : "bootstrap");
  return plan;
}
...
async function buyback({ dryRun = false } = {}) {
  ...
  const res = await manualSwap({ fromSym, toSym: "CLKN", amountUi: spendUi, slippageBps: cfg.buybackSlippageBps, dryRun: false, silent: true });
...
async function evenPools({ dryRun = false } = {}) {
  ...
  if (fat.v - lean.v < tol * Math.max(fat.v, 1)) {
    ...
    if (cfg.scaleUpUsdPerCycle > 0 && scaleProj && scaleProj.tokenSellOk === true && !dryRun) {
      ...
      if (clip >= 5 && idleTokUsd > 100) {
        ...
        } catch (e) { /* impact guard or route failure — skip quietly, retry next cycle */ }
      }
    }
  ...
  try {
    ...
    setState({ evenShiftTs: Date.now() });   // start the cooldown the instant we remove
...
async function addLiquidity({ mint, fromSym, amountUi, dryRun = false } = {}) {
  ...
  // `spendableForAdd` (lib/engine-decisions.js) is this exact formula, pulled out
  // (behavior-identical) so it's pinned by scripts/engine-sim-test.cjs.
  ...
  const maxIn = spendableForAdd(fl[balKey], reserve, slip);
...
// ── Deployed-value cap headroom (owner ask, BULLEN, 2026-09-25) ───────────────────────────────
// cfg.maxDeployedUsd is OPT-IN (undefined/0 for every project unless one sets it) — it bounds how
// much TOTAL position value (summed across all of this project's pools) the engine will ever grow
// to, so stray SOL/USDC sent to a SHARED operator wallet isn't silently absorbed without bound.
// Reuses publicPositions()'s own $ valuation (30s-cached) rather than a second read path — that
// cache can read up to 30s STALE right after this same tick's own close, which only ever makes
// this cap MORE conservative (a just-freed position still counts as "deployed" for a few more
// seconds), never less — worst case a legitimate redeploy waits one more 90s cycle. A
// verification failure returns 0 room, not unlimited — can't confirm what's deployed, don't grow.
async function deployCapRoomUsd(cfg) {
  if (!cfg.maxDeployedUsd) return Infinity;
  try {
    const pv = await publicPositions();
    const deployedNow = Number(pv && pv.totalUsd) || 0;
    return Math.max(0, cfg.maxDeployedUsd - deployedNow);
  } catch (_) { return 0; }
}
...
module.exports = {
  // The send/record/confirm loop, exported PURE so the ordering invariant on the money path can
  // be tested without a chain. See scripts/giveaway-holdcheck-test.cjs.
  runPayoutLoop, serializableRecipients,
  // Explicit-projectId helpers (default to "clkn" outside a scope).
  isEnabled, operatorPubkey, operatorPubkeys, routeAlertsToMain, isPaused,
  getConfig, setConfig,
  ...
  // Each per-project tick is its own choke point that records retained decision events (W5/batch
  // 8): every caller of vault.tick()/tickAskWall()/etc — the per-project schedulers and the
  // manual admin routes alike — goes through here, so wrapping the caller covers each tick
  // function's several internal return statements without touching any of them. `source` names
  // which sleeve decided (see recordDecision's own doc above).
  tick: (o = {}) => withProject(o.projectId, async () => {
    const r = await tick(o);
    recordDecision(o.projectId, r, "base");
    return r;
  }),
  tickAskWall: (o = {}) => withProject(o.projectId, async () => {
    const r = await tickAskWall(o);
    recordDecision(o.projectId, r, "askWall");
    return r;
  }),
  tickSol: (o = {}) => withProject(o.projectId, async () => {
    const r = await tickSol(o);
    recordDecision(o.projectId, r, "sol");
    return r;
  }),
  tickBtc: (o = {}) => withProject(o.projectId, async () => {
    const r = await tickBtc(o);
    recordDecision(o.projectId, r, "btc");
    return r;
  }),
  tickJup: (o = {}) => withProject(o.projectId, async () => {
    const r = await tickJup(o);
    recordDecision(o.projectId, r, "jup");
    return r;
  }),
  tickTreasury: (o = {}) => withProject(o.projectId, () => withTickLock(o.projectId || "clkn", o.dryRun, async () => {
    const r = await tickTreasury(o);
    recordDecision(o.projectId, r, "treasury");
    return r;
  })),
  ...
  bootstrap: (o = {}) => withProject(o.projectId, () => bootstrap(o)),
  ...
  payoutSpl: (o = {}) => withProject(o.projectId, () => payoutSpl(o)),
  ...
  concentrate: (o = {}) => withProject(o.projectId, () => withTickLock(o.projectId || "clkn", o.dryRun, async () => {
    const r = await concentrate(o);
    recordDecision(o.projectId, r, "concentrate");
    return r;
  })),
  ...
  // Exported for scripts/jvp-dashboard-test.cjs (the ring buffer bound + the paused no-op
  // exclusion) — the real writer is the `tick` wrapper above, not a test-only path.
  recordDecision,
};
