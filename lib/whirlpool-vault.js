...
async function evenPools({ dryRun = false } = {}) {
  ...
  setState({ evenVals: { ts: Date.now(), vals: Object.fromEntries(pools.map((p) => [p.key, Math.round(p.v)])) } });
  ...
  if (drifted(cfg.maxUsd, wantCaps.maxUsd) || drifted(cfg.solMaxSol, wantCaps.solMaxSol) || (jup && drifted(cfg.jupMaxJup, wantCaps.jupMaxJup))) {
    setConfig(wantCaps);
  }
