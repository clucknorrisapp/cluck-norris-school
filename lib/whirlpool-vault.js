...
const DEFAULT_PROJECT = {
  ...
  tokenMint: engine.CLKN_MINT, quoteMints: [engine.USDC_MINT, engine.WSOL_MINT, engine.CBBTC_MINT],
...
async function resolvePoolAddress(cfg) {
  ...
  const quoteMint = quoteSym === "SOL" ? engine.WSOL_MINT : quoteSym === "BTC" ? engine.CBBTC_MINT : engine.USDC_MINT;
  try {
...
async function tickBtc({ dryRun = false } = {}) {
  ...
  let preBtcFree = 0;
  try { preBtcFree = (await getFloat(conn)).btc; } catch { /* best effort */ }
  ...
  if (!managed) {
    ...
    else if (preBtcFree > cfg.btcDeployThreshold) { needRoll = true; reason = `deploying available cbBTC (${preBtcFree})`; }
    else { reason = `in range (${(frac * 100).toFixed(0)}% across)`; }
  }
...
async function tickTreasury({ dryRun = false } = {}) {
  ...
  if (!wide && !tight) {
    ...
    if (concF > 0) {
      ...
      if (t) { dayActions++; steps.push({ tightOpen: t.sig }); setState({ tg_mint: t.mint, tg_lower: t.lower, tg_upper: t.upper, tg_oorSince: null }); }
    }
  ...
  if (!tight && concF > 0) {
    ...
    if (t) { dayActions++; steps.push({ tightOpen: t.sig }); setState({ tg_mint: t.mint, tg_lower: t.lower, tg_upper: t.upper, tg_oorSince: null, tw_dayActions: dayActions, tw_dayStamp: today }); await notify(`🟢 <b>Treasury — concentrated sleeve opened</b>\n${pair} ±${concW}% · ${t.lower.toPrecision(4)}→${t.upper.toPrecision(4)}`); }
  ...
  if (tightAction === "roll") {
    ...
    if (t) { dayActions++; steps.push({ tightRoll: t.sig }); setState({ tg_mint: t.mint, tg_lower: t.lower, tg_upper: t.upper, tg_oorSince: null, tw_dayActions: dayActions, tw_dayStamp: today }); }
...
async function manualSwap({ fromSym, toSym, amountUi, slippageBps = 100, dryRun = false, silent = false }) {
  if (!isEnabled()) return { enabled: false, action: "none", reason: "operator not set" };
  const M = { SOL: engine.WSOL_MINT, USDC: engine.USDC_MINT, CLKN: tok().mint, BTC: engine.CBBTC_MINT };
  const D = { SOL: 9, USDC: 6, CLKN: 9, BTC: 8 }; // cbBTC is 8 decimals
  const from = M[fromSym], to = M[toSym];
  if (!from || !to || from === to) throw new Error("from/to must be SOL, USDC, CLKN, or BTC (and differ)");
  ...
  if (!silent) await notify(`🔁 <b>Vault swap</b>: ${inAmt.toFixed(3)} ${fromSym} → ${out.toFixed(2)} ${toSym}\n<code>${sig.slice(0, 8)}…</code>`);
...
async function dislocation() {
  ...
  try { const px = await jupPriceUsdMap([engine.WSOL_MINT, engine.CBBTC_MINT]); solUsd = Number(px[engine.WSOL_MINT]?.usdPrice) || 0; btcUsd = Number(px[engine.CBBTC_MINT]?.usdPrice) || 0; } catch { /* */ }
...
async function createPool({ quoteSym = "SOL", feeTierPct = 0.02, dryRun = false } = {}) {
  ...
  const quoteMint = quote === "USDC" ? engine.USDC_MINT : quote === "BTC" ? engine.CBBTC_MINT : engine.WSOL_MINT;
  // Live market price of the token in the quote, via Jupiter (venue-agnostic).
  const px = await jupPriceUsdMap([t.mint, engine.WSOL_MINT, engine.CBBTC_MINT]);
  ...
  const quoteUsd = quote === "USDC" ? 1 : quote === "BTC" ? btcUsd : solUsd;
...
// Deposit `amountUi` of `fromSym` (SOL/USDC/CLKN/BTC) into an existing position's range
...
async function addLiquidity({ mint, fromSym, amountUi, dryRun = false } = {}) {
  ...
  if (!inputMint) throw new Error("from must be SOL, USDC, CLKN, or BTC");
...
// Sets concMode, closes both sleeves, rebalances the freed float to ~50/50, and lets the
// next tick re-seed at the new mode's width + allocation. wide = backbone only (safe);
// tight = ±tightWidthPct; mega = ±megaWidthPct concentrated up to concMegaFrac (aggressive).
async function concentrate({ mode, dryRun = false } = {}) {
  ...
  const conn = connection();
  if (dryRun) {
    return { enabled: true, action: "would-switch", mode, concWidthPct: concW, concFrac: concF,
      plan: `close both sleeves → rebalance float ~50/50 → re-seed wide ±${cfg.solWidthPct}%${concF > 0 ? ` (${Math.round((1 - concF) * 100)}%) + ±${concW}% (${Math.round(concF * 100)}%)` : " only (100%)"}` };
  }
  setConfig({ concMode: mode });
  // close both sleeves
  const sigs = [];
  for (const m of [st.tw_wideMint, st.tg_mint].filter(Boolean)) {
    try {
      const ps = await eng().listPositions(operator().publicKey.toBase58(), tok());
      const pp = ps.find((x) => x.positionMint === m); if (pp) accrueRealizedFees(pp);
      const { txs } = await eng().buildClosePosition({ owner: operator().publicKey.toBase58(), positionMint: m, slippageBps: cfg.slippageBps });
      for (const t of txs) sigs.push(await signSend(conn, t));
    } catch (_) { /* may already be closed */ }
  }
  setState({ tw_wideMint: null, tg_mint: null, tg_oorSince: null });
  // rebalance freed float to ~50/50 SOL/cbBTC by value (so the re-seed deposits cleanly)
  try {
    const f = await getFloat(conn);
    const px = await jupPriceUsdMap([engine.WSOL_MINT, tok().mint]);
    const pS = Number(px[engine.WSOL_MINT]?.usdPrice) || 0, pB = Number(px[tok().mint]?.usdPrice) || 0;
    if (pS > 0 && pB > 0) {
      const diffUsd = (f.clkn * pB) - ((f.sol - cfg.swapSolFloor) * pS); // >0 = cbBTC heavy
      const swapUsd = Math.abs(diffUsd) / 2;
      if (swapUsd > 1) {
        if (diffUsd > 0) await manualSwap({ fromSym: "BTC", toSym: "SOL", amountUi: swapUsd / pB, silent: true });
        else await manualSwap({ fromSym: "SOL", toSym: "BTC", amountUi: swapUsd / pS, silent: true });
      }
    }
  } catch (_) { /* best effort; the re-seed still deposits what balances */ }
  await notify(`🔧 <b>Treasury → ${mode.toUpperCase()} mode</b>\nClosed sleeves + rebalanced; re-seeding wide ±${cfg.solWidthPct}%${concF > 0 ? ` + ±${concW}% (${Math.round(concF * 100)}% concentrated)` : " only"} on the next tick (≤5 min).`);
  return { enabled: true, action: "switched", mode, concWidthPct: concW, concFrac: concF, closed: sigs.length, note: "re-seeds at the new mode on the next tick" };
}
  ...
  const out = {
    ...
    state: {
      ...
      dualSleeve: cfg.dualSleeveEnabled
        ? { wideMint: st.tw_wideMint || null, tightMint: st.tg_mint || null }
        : "disabled",
...
async function publicPositions() {
  ...
  if (clknUsd <= 0 || solUsd <= 0 || btcUsd <= 0) {
    try {
      const px = await jupPriceUsdMap([tok().mint, engine.WSOL_MINT, engine.CBBTC_MINT]);
  ...
  const out = positions.map((p) => {
    const quoteUsd = p.quoteSymbol === "USDC" ? p.quoteAmount : p.quoteSymbol === "BTC" ? p.quoteAmount * btcUsd : p.quoteAmount * solUsd;
  ...
  const data = { enabled: true, positions: out, totalUsd, clknUsd, solUsd, btcUsd };
