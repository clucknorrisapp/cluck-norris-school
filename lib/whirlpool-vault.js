...
async function openAnchor({ quoteSym, usd = 10, downPct = 85, upPct = 400, dryRun = false } = {}) {
  ...
  if (!poolCfgFn) return { enabled: true, action: "error", reason: `unknown quote "${q}" — use USDC, SOL, or JUP` };
  const cfg = getConfig();
...
async function manualSwap({ fromSym, toSym, amountUi, slippageBps = 100, maxImpactPct = null, dryRun = false, silent = false }) {
  if (!isEnabled()) return { enabled: false, action: "none", reason: "operator not set" };
...
async function marketPriceUsd(t = tok()) {
  ...
  // DexScreener can be POISONED by a single mispriced pair — the recurring CLKN/JUP
  // artifact has read CLKN at ~$1.19 vs the real ~$0.0002, which then falsely trips the
  // external-market guards (e.g. JUP "100% off market"). If the DEX price is a wild
  // outlier vs Jupiter's aggregate, distrust the DEX read and use Jupiter instead.
  if (dex.usd > 0 && jupU > 0 && (dex.usd > jupU * 5 || dex.usd < jupU / 5)) {
    return { usd: jupU, source: "jupiter (DEX outlier rejected)", liqUsd: 0 };
  }
  if (dex.usd > 0) return dex;
  if (jupU > 0) return { usd: jupU, source: "jupiter (fallback)", liqUsd: 0 };
