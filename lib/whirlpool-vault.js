...
async function tickTreasury({ dryRun = false } = {}) {
  ...
  const wide = positions.find((p) => p.positionMint === st.tw_wideMint);
  const tight = positions.find((p) => p.positionMint === st.tg_mint);
  ...
  async function openSleeve(widthPct, solBudget) {
    const fresh = await eng().getPoolState(address, operator().publicKey, tok());
    ...
    if (solDep > 0) {
      ...
      if (tokNeeded > float.clkn) { inputMint = tok().mint; inputAmount = float.clkn; }
    } else if (float.clkn > 0) { inputMint = tok().mint; inputAmount = float.clkn; }
    else return null;
  ...
  if (!tight) { tightAction = "open"; tightReason = st.tg_mint ? "tight gone — reopening" : "no tight sleeve yet"; tgOorSince = null; }
  else {
    const frac = rangeFraction(price, tight.lowerPriceClkn, tight.upperPriceClkn);
    if (frac >= 0 && frac <= 1) { tgOorSince = null; tightReason = `tight in range (${(frac * 100).toFixed(0)}%)`; }
    else {
  ...
  if (!wide && !tight) {
    const eff = deployableSol * cfg.deployFrac;
    const w = await openSleeve(cfg.solWidthPct, eff * 0.5);
    if (w) { dayActions++; steps.push({ wideOpen: w.sig }); setState({ tw_wideMint: w.mint, tw_wideLower: w.lower, tw_wideUpper: w.upper }); }
    const t = await openSleeve(cfg.tightWidthPct, eff * 0.5);
    ...
    await notify(`🏦 <b>Treasury seeded</b> — ${pair} @ ${cfg.solFeeTierPct}%\nWide ±${cfg.solWidthPct}% + Tight ±${cfg.tightWidthPct}% (50/50)\nprice ${price.toPrecision(5)}`);
  ...
  if (!tight) {
    const t = await openSleeve(cfg.tightWidthPct, deployableSol * cfg.deployFrac);
    if (t) { dayActions++; steps.push({ tightOpen: t.sig }); setState({ tg_mint: t.mint, tg_lower: t.lower, tg_upper: t.upper, tg_oorSince: null, tw_dayActions: dayActions, tw_dayStamp: today }); await notify(`🟢 <b>Treasury — tight sleeve redeployed</b>\n${pair} ±${cfg.tightWidthPct}% · ${t.lower.toPrecision(4)}→${t.upper.toPrecision(4)}`); }
  ...
  if (tightAction === "roll") {
    ...
    const t = await openSleeve(cfg.tightWidthPct, ((await getFloat(conn)).sol - cfg.swapSolFloor) * cfg.deployFrac);
    ...
    await notify(`🟢 <b>Treasury — tight sleeve redeployed</b>\n${pair} ±${cfg.tightWidthPct}% · ${tightReason}`);
...
async function publicPositions() {
  ...
  const btcUsd = (btcPos && btcPos.currentPriceClkn > 0 && clknUsd > 0) ? clknUsd / btcPos.currentPriceClkn : 0;
  const st = getState();
  let totalUsd = 0;
...
module.exports = {
  ...
  tickTreasury: (o = {}) => withProject(o.projectId, () => tickTreasury(o)),
