...
async function tickTreasury({ dryRun = false } = {}) {
  ...
  if (!wide && !tight) {
    ...
    const clknHalf = f0.clkn * 0.5; // reserve half the token for each sleeve so the first doesn't eat it all
    const w = await openSleeve(cfg.solWidthPct, eff * 0.5, clknHalf);
    if (w) { dayActions++; steps.push({ wideOpen: w.sig }); setState({ tw_wideMint: w.mint, tw_wideLower: w.lower, tw_wideUpper: w.upper }); }
    const t = await openSleeve(cfg.tightWidthPct, eff * 0.5, clknHalf);
