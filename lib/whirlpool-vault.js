...
const { buybackDecision, spendableSol: spendableSolPure, rollRebalanceDecision, spendableForAdd } = require("./engine-decisions"); // pure gate logic, replayable by scripts/engine-sim-test.cjs
...
async function tick({ dryRun = false } = {}) {
  ...
  // Snapshot float right before the close so the bullen-only rebalance below can measure
  // exactly what THIS close freed — never another pool's idle float or the SOL gas reserve.
  let preCloseFloat = null;
  if (managed && pid() === "bullen") { try { preCloseFloat = await getFloat(conn); } catch { /* best effort — skip the rebalance below if this failed */ } }
  if (managed) {
    ...
    // BULLEN-ONLY REBALANCE (production incident, 2026-09-26): a one-way move closes this
    // pool almost entirely into one asset, so sizing the reopen by the scarce side (below)
    // opens tiny and strands the rest as idle float. Rebalance THIS close's freed assets to
    // ~50/50 value at the current price BEFORE reopening — scoped to bullen (same carve-out
    // pattern as the affordability guards above), sized only from what THIS close just freed,
    // skipped under ~$5 and clamped to maxSwapUsdPerCycle. A swap failure logs and falls
    // through to the reopen with whatever float exists — it must never abort and leave the
    // pool closed.
    if (pid() === "bullen" && preCloseFloat) {
      try {
        const postCloseFloat = await getFloat(conn);
        const freedClkn = Math.max(0, (postCloseFloat.clkn || 0) - (preCloseFloat.clkn || 0));
        const freedQuote = Math.max(0, (postCloseFloat.usdc || 0) - (preCloseFloat.usdc || 0));
        const rb = rollRebalanceDecision({ clknUi: freedClkn, quoteUi: freedQuote, price, quoteUsd: 1, maxSwapUsdPerCycle: cfg.maxSwapUsdPerCycle });
        if (rb.action === "swap") {
        ...
        console.warn(`[vault:${pid()}] roll rebalance FAILED (base): ${e.message}`);
        report.steps.push({ rollRebalanceError: e.message });
      }
    }
  }
...
async function tickJup({ dryRun = false } = {}) {
  ...
  // Snapshot float right before the close so the bullen-only rebalance below can measure
  // exactly what THIS close freed — never another pool's idle float.
  let preCloseFloat = null;
  if (managed && pid() === "bullen") { try { preCloseFloat = await getFloat(conn); } catch { /* best effort — skip the rebalance below if this failed */ } }
  if (managed) {
    ...
    // BULLEN-ONLY REBALANCE (production incident, 2026-09-26): see the identical block +
    // comment in tick() above. This is the pool that never reopened at all in today's
    // incident — the close froze the wallet almost entirely into JUP with ~no CLKN to pair,
    // so the reopen sized by the scarce (CLKN) side and had nothing left to deploy. `jupUsd`
    // is already the live price fetched above for the external-market guard.
    if (pid() === "bullen" && preCloseFloat) {
      try {
        const postCloseFloat = await getFloat(conn);
        const freedClkn = Math.max(0, (postCloseFloat.clkn || 0) - (preCloseFloat.clkn || 0));
        const freedJup = Math.max(0, (postCloseFloat.jup || 0) - (preCloseFloat.jup || 0));
        const rb = rollRebalanceDecision({ clknUi: freedClkn, quoteUi: freedJup, price, quoteUsd: jupUsd, maxSwapUsdPerCycle: cfg.maxSwapUsdPerCycle });
        if (rb.action === "swap") {
          const sw = rb.dir === "sellClkn"
        ...
        console.warn(`[vault:${pid()}] roll rebalance FAILED (jup): ${e.message}`);
        report.steps.push({ rollRebalanceError: e.message });
      }
    }
  }
...
async function tickSol({ dryRun = false } = {}) {
  ...
  if (managed) {
    ...
    // BULLEN-ONLY REBALANCE (production incident, 2026-09-26): see the identical block +
    // comment in tick() above. The 2026-09-26 incident hit THIS pool worst (the SOL pool went
    // $167→$109 after closing almost entirely into one asset). SOL's own gas reserve is never
    // touched — freedSol is measured as this close's OWN delta, not the wallet's total SOL.
    if (pid() === "bullen" && preCloseFloat) {
      try {
        const postCloseFloat = await getFloat(conn);
        const freedClkn = Math.max(0, (postCloseFloat.clkn || 0) - (preCloseFloat.clkn || 0));
        const freedSol = Math.max(0, (postCloseFloat.sol || 0) - (preCloseFloat.sol || 0));
        const solUsdNow = (await solPriceUsd()) || 0;
        const rb = rollRebalanceDecision({ clknUi: freedClkn, quoteUi: freedSol, price, quoteUsd: solUsdNow, maxSwapUsdPerCycle: cfg.maxSwapUsdPerCycle });
        if (rb.action === "swap") {
          const sw = rb.dir === "sellClkn"
        ...
        console.warn(`[vault:${pid()}] roll rebalance FAILED (sol): ${e.message}`);
        report.steps.push({ rollRebalanceError: e.message });
      }
    }
  }
  ...
  try {
    ...
    if (addAmt * usdOf[lean.quote] >= cfg.minSwapUsd) {
      try {
        ...
        // ADD-FAILURE RETRY + REVERT (BULLEN production incident, 2026-09-26): the add into
        // the lean pool failed with "insufficient funds" (custom program error 0x1) — the
        // wallet's real balance had already moved below what `addAmt` assumed (RPC lag /
        // slippage headroom between the reads above and the tx landing) — and 621 JUP sat
        // idle, never landing anywhere. Retry ONCE with the amount clamped to a FRESH read of
        // the wallet's actual balance, using the SAME formula addLiquidity's own clamp uses
        // (`spendableForAdd`, lib/engine-decisions.js) so this can't silently drift out of sync
        // with it — plus the evenness floor, so a retry can only ever be smaller, never trade
        // more than the original plan. If that also fails, put the removed value BACK into the
        // pool it came from (fat) so nothing is stranded, instead of hoping a later deploy tick
        // finds it (today's incident: it didn't).
        steps.addError = addErr.message;
        let retried = false;
        try {
          const retryFloat = await getFloat(conn);
          const gasReserve = lean.quote === "SOL" ? (cfg.solGasReserve || 0.05) : 0;
          const reserve = Math.max(floorFor[lean.quote] || 0, gasReserve);
          const retryAmt = Math.min(addAmt, spendableForAdd(retryFloat[lean.quote.toLowerCase()], reserve, cfg.slippageBps || 100));
          if (retryAmt * usdOf[lean.quote] >= cfg.minSwapUsd) {
            const add2 = await addLiquidity({ mint: lean.mint, fromSym: lean.quote, amountUi: retryAmt, dryRun: false });
            steps.added = { pool: lean.key, amount: Number(retryAmt.toFixed(4)), sigs: add2.sigs, retried: true };
            retried = true;
          }
        } catch (retryErr) { steps.addRetryError = retryErr.message; }
        if (!retried) {
          try {
            // Reverse step 2's conversion (whatever lean-quote is now idle, back to fat-quote),
            // then add what came back into the FAT pool — the SDK pairs the CLKN the remove
            // freed automatically, same as the normal add does for the lean pool.
            if (fat.quote !== lean.quote) {
              const revFloat = await getFloat(conn);
              const leanIdle = Math.max(0, (revFloat[lean.quote.toLowerCase()] || 0) - (floorFor[lean.quote] || 0));
              if (leanIdle * usdOf[lean.quote] >= cfg.minSwapUsd) {
                const back = await manualSwap({ fromSym: lean.quote, toSym: fat.quote, amountUi: leanIdle, slippageBps: cfg.swapSlippageBps, dryRun: false });
                steps.revertedSwap = { from: lean.quote, to: fat.quote, sig: back && back.sig };
              }
            }
            const postRev = await getFloat(conn);
            const fatQuoteFree = Math.max(0, (postRev[fat.quote.toLowerCase()] || 0) - (floorFor[fat.quote] || 0));
            if (fatQuoteFree * usdOf[fat.quote] >= cfg.minSwapUsd) {
              const readd = await addLiquidity({ mint: fat.mint, fromSym: fat.quote, amountUi: fatQuoteFree, dryRun: false });
              steps.revertedAdd = { pool: fat.key, amount: Number(fatQuoteFree.toFixed(4)), sigs: readd.sigs };
            }
            setState({ swapsToday: swapsToday + 1, swapDayStamp: today, evenShiftTs: Date.now() });
            console.warn(`[vault:${pid()}] evenPools add into ${lean.key} failed twice — reverted the removed value back into ${fat.key}`);
            await notifyDigest(`⚠️ <b>Pool evenness — shift reverted</b>\nAdd into ${lean.key} failed twice (insufficient funds); returned the removed value to ${fat.key} so nothing was stranded.`);
            return { ...out, action: "shift-reverted", from: fat.key, to: lean.key, shiftUsd: Math.round(shiftUsd), steps };
          } catch (revertErr) {
            console.warn(`[vault:${pid()}] evenPools revert FAILED: ${revertErr.message}`);
            steps.revertError = revertErr.message;
            return { ...out, action: "shift-partial", reason: `add: ${addErr.message}; revert failed: ${revertErr.message}`, steps };
          }
        }
      }
    }
...
async function addLiquidity({ mint, fromSym, amountUi, dryRun = false } = {}) {
  ...
  // `spendableForAdd` (lib/engine-decisions.js) is this exact formula, pulled out after
  // evenPools' own add-retry (BULLEN production incident, 2026-09-26) needed the SAME
  // clamp on a fresh balance read — one tested copy instead of two that can drift.
