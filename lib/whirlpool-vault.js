...
async function payoutSpl({ mintAddr, recipients, perRecipientMaxUi, totalMaxUi, dryRun = true, onPaid = null } = {}) {
  ...
  // THE LINE BETWEEN "NOT SENT" AND "SENT" IS THE WHOLE SAFETY MODEL HERE.
  //
  // Before sendRawTransaction returns, nothing has moved — a throw is a clean `failed` and the
  // recipient can safely be retried. AFTER it returns, the transaction is in the cluster and the
  // money may already be gone, even if confirmSig then times out (it throws after ~90s, and says
  // so: "may still land; VERIFY on-chain before retrying").
  //
  // The first version of this collapsed both into one try/catch, and declared `sig` with const
  // INSIDE it — so a confirm timeout threw away the signature, the row was recorded as `failed`
  // with no sig, recordPayout skipped it, payoutOwed put the wallet straight back in `owed`, and
  // the documented retry PAID THEM AGAIN. The commit that shipped it claimed "a retry after a
  // timeout cannot pay anyone twice", which was wrong in exactly the case it named.
  //
  // So: send and confirm are separate steps, `sig` outlives both, and a sent-but-unconfirmed
  // transfer is reported as PENDING and recorded immediately — because the one thing that must
  // never happen is re-sending money that may already be on its way.
  const paid = [], pending = [], failed = [];
  const record = (row) => { if (typeof onPaid === "function") { try { onPaid(row); } catch (_) {} } };
  for (const r of list) {
    let sig = null;
    try {
    ...
    // Submitted. Record BEFORE confirming, so a crash between here and the confirm still leaves a
    // durable trace of a transfer that may have landed.
    try {
      await confirmSig(conn, sig);
  ...
  return { ...out, action, paid, pending, failed };
}
