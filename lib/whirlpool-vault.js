...
async function payoutSpl({ mintAddr, recipients, perRecipientMaxUi, totalMaxUi, dryRun = true } = {}) {
  if (!isEnabled()) return { enabled: false, action: "none", reason: "operator not set" };
  const spl = require("@solana/spl-token");
  const conn = connection();
  const from = operator();
  ...
  for (const r of list) {
    ...
    if (perRecipientMaxUi != null && r.amountUi > Number(perRecipientMaxUi)) {
      throw new Error(`amount ${r.amountUi} for ${r.wallet} exceeds perRecipientMax ${perRecipientMaxUi}`);
    }
  }
  ...
  if (total > Number(totalMaxUi)) throw new Error(`total ${total} exceeds totalMax ${totalMaxUi}`);
  ...
  try {
    ...
    mintAuthority = info && info.value && info.value.data && info.value.data.parsed
      && info.value.data.parsed.info && info.value.data.parsed.info.mintAuthority;
  ...
  for (const r of list) {
    try {
      ...
      const sig = await conn.sendRawTransaction(tx.serialize(), { skipPreflight: false, maxRetries: 3 });
      await confirmSig(conn, sig);
      paid.push({ wallet: r.wallet, amountUi: r.amountUi, sig });
    } catch (e) {
      failed.push({ wallet: r.wallet, amountUi: r.amountUi, error: String((e && e.message) || e).slice(0, 200) });
    }
  }
  return { ...out, action: "paid", paid, failed };
}
