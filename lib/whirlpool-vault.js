...
async function tick({ dryRun = false } = {}) {
  ...
  if (cfg.notifyRolls !== false) await notify(
...
async function manualSwap({ fromSym, toSym, amountUi, slippageBps = 100, maxImpactPct = null, dryRun = false, silent = false, allowTokenSell = false }) {
  ...
  // HARD INVARIANT: never sell the project token (the brand bag). Every legit internal swap is
  // quote↔quote or a USDC→token buyback; the public /vault/swap endpoint never sets the override.
  // A deliberate token sale must set allowTokenSell:true in code, not via a request param.
  if (from === tok().mint && !allowTokenSell) throw new Error("refusing to sell the project token (brand bag) — quote↔quote only");
...
async function buyback({ dryRun = false } = {}) {
  ...
  // MULTI-QUOTE (owner, 2026-08-28: "why can't the SOL buy some DNC"). The USDC-only buyback
  // was written for the single-quote CLKN flywheel; on a three-quote engine it stalled the
  // moment USDC ran dry while free SOL/JUP sat idle and a sleeve stayed underweight for hours.
  // Spend USDC first (already USD, no price lookup); else free SOL above BOTH gas guards;
  // else JUP above a dust reserve. Never touches the project token.
  ...
  if (spendUsd < cfg.minBuybackUsd) {
    try {
      ...
      const solUsd = Number((px[engine.WSOL_MINT] || {}).usdPrice) || 0;
      const jupUsd = Number((px[engine.JUP_MINT] || {}).usdPrice) || 0;
      const freeSol = Math.max(0, float.sol - (cfg.swapSolFloor || 0) - (cfg.solGasReserve || 0));
    ...
    } catch (_) { /* price feed down → USDC-only behavior */ }
  }
  const out = { enabled: true, sym: tok().symbol, fromSym, usableUsdc: Math.round(usable * 100) / 100 };
  if (spendUsd < cfg.minBuybackUsd) return { ...out, action: "none", reason: `no spendable quote (USDC $${usable.toFixed(2)} above floor; free SOL/JUP under guards or < $${cfg.minBuybackUsd} min)` };
  const today = new Date().toISOString().slice(0, 10);
  ...
  // DEMAND OVERRIDE (owner, 2026-08-28: "why can't buybacks fire immediately when needed").
  // The interval is an anti-thrash guard, not a metronome: when the sleeves are STARVED —
  // quote staged for pairing while (nearly) no idle token exists — waiting out the clock just
  // leaves pools one-sided. Thrash stays bounded without it: a fresh unspent buyback kills
  // the demand signal until a sleeve deploys it AND the market strips it again, and the daily
  // count + per-cycle size caps bound total spend either way.
  ...
  if (sinceLast < cfg.buybackMinIntervalSec && !starved) return { ...out, action: "deferred", reason: `buyback anti-thrash (${Math.round(sinceLast)}s < ${cfg.buybackMinIntervalSec}s)` };
