...
async function evenPools({ dryRun = false } = {}) {
  ...
  const helpers = { usdc: "USDC", sol: "SOL", jup: "JUP" };
  ...
  if (swapsToday >= cfg.maxSwapsPerDay) return { ...out, action: "capped", reason: "daily swap cap reached" };
  const shiftUsd = Math.min((fat.v - lean.v) / 2, cfg.maxSwapUsdPerCycle);
  ...
  const steps = {};
  try {
    // 1. Remove the excess from the fat pool → frees ROSE + fat-quote to the wallet.
    ...
    if (fat.quote !== lean.quote) {
      ...
      const convUi = Math.min(fatFree, shiftUsd / usdOf[fat.quote]);
    ...
    // 3. Add the now-available lean-quote into the lean pool; the SDK pairs the ROSE side
    //    from wallet inventory. This also sweeps any idle lean-quote float into the pool,
    //    which fixes the "capital sitting undeployed" drift in the same pass.
    const post = await getFloat(conn);
    const addAmt = Math.max(0, post[lean.quote.toLowerCase()] - (floorFor[lean.quote] || 0));
    ...
    return { ...out, action: "shift-partial", reason: e.message, steps };
  }
}
