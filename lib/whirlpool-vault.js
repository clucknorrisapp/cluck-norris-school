...
async function status() {
  ...
  if (isEnabled()) {
    ...
    // Per-pool deployed $ value (for the dashboard) — from the cached positions read,
    // keyed by quote symbol (USDC/SOL/JUP), plus the total. Best-effort; never blocks.
    try {
      const pp = await publicPositions();
