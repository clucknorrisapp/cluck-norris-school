...
async function tickJup({ dryRun = false } = {}) {
  ...
  let preJupFree = 0;
  try { preJupFree = (await getFloat(conn)).jup; } catch { /* best effort */ }
  ...
  if (!managed) {
    ...
    else if (preJupFree > cfg.jupDeployThreshold) { needRoll = true; reason = `deploying available JUP (${preJupFree})`; }
    else { reason = `in range (${(frac * 100).toFixed(0)}% across)`; }
  }
