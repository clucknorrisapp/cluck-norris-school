...
async function evenPools({ dryRun = false } = {}) {
  ...
  const trackedMints = new Set([st.positionMint, st.sol_positionMint, st.jup_positionMint].filter(Boolean));
  const orphan = positions.find((p) => !trackedMints.has(p.positionMint));
  if (orphan) {
