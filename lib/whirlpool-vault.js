...
const DEFAULT_PROJECT = {
  id: "clkn", label: "Cluck Norris (CLKN)",
...
function registerProject(rec) {
  ...
  all[rec.id] = {
    id: rec.id, label: rec.label || rec.id, tokenMint: rec.tokenMint,
...
function operator(projectId = "clkn") {
...
function isEnabled(projectId = "clkn") { return !!operator(projectId); }
...
function operatorPubkey(projectId = "clkn") { const o = operator(projectId); return o ? o.publicKey.toBase58() : null; }
...
function getConfig(projectId = "clkn") { return { ...DEFAULT_CONFIG, ...(kv.get(cfgKey(projectId), {}) || {}) }; }
function setConfig(patch, projectId = "clkn") {
...
function getState(projectId = "clkn") { return kv.get(stateKey(projectId), {}) || {}; }
function setState(patch, projectId = "clkn") { kv.set(stateKey(projectId), { ...getState(projectId), ...patch }); }
...
async function tickSol({ dryRun = false } = {}) {
  ...
  if (!fetchOk) return { enabled: true, pair: "CLKN/SOL", feeTierPct: cfg.solFeeTierPct, price, action: "skip", reason: "position lookup failed — retrying next tick" };
...
module.exports = { isEnabled, operatorPubkey, operatorPubkeys, tick, tickAskWall, tickSol, rebalanceInventory, rebalancePools, manualSwap, status, costs, earnings, publicPositions, getConfig, setConfig, listModes, previewMode, applyMode, pause, resume, listProjects, getProject, registerProject, removeProject, DEFAULT_CONFIG };
