...
// A bounded, public-safe ring buffer of what the base tick decided, per project — the
// evidence class the dashboard labels "decision log (retained events)", distinct from the
// backfilled Helius transfers and the illustrative simulator replay. Written from ONE choke
// point: the module.exports `tick` wrapper below, right where it has the caller's tick()
// result — never from inside tick() itself, so every return path (including ones added
// later) is covered without re-touching this function.
//
...
//
// Shape is intentionally thin — `{ t, action, reason, price }` only. Never the operator
...
function recordDecision(projectId, result) {
...
module.exports = {
  ...
  // The base tick is the ONE choke point that records retained decision events (W5): every
  // caller of vault.tick() — the per-project schedulers and the manual admin route alike —
  // goes through here, so wrapping the caller covers tick()'s several internal return
  // statements without touching each one.
  tick: (o = {}) => withProject(o.projectId, async () => {
    const r = await tick(o);
    recordDecision(o.projectId, r);
    return r;
  }),
