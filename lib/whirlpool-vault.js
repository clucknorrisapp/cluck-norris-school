      ...
      try {
        ...
        if (rb.action === "swap") {
          const sw = rb.dir === "sellClkn"
            ? await manualSwap({ fromSym: "CLKN", toSym: "USDC", amountUi: rb.amountUi, slippageBps: cfg.swapSlippageBps, maxImpactPct: 1.5, dryRun: false, silent: true, allowTokenSell: true })
            : await manualSwap({ fromSym: "USDC", toSym: "CLKN", amountUi: rb.amountUi, slippageBps: cfg.swapSlippageBps, maxImpactPct: 1.5, dryRun: false, silent: true });
  ...
  if (managed) {
    ...
    if (rollRebalAllowed && preCloseFloat) {
      try {
        ...
        if (rb.action === "swap") {
          const sw = rb.dir === "sellClkn"
            ? await manualSwap({ fromSym: "CLKN", toSym: "JUP", amountUi: rb.amountUi, slippageBps: cfg.swapSlippageBps, maxImpactPct: 1.5, dryRun: false, silent: true, allowTokenSell: true })
            : await manualSwap({ fromSym: "JUP", toSym: "CLKN", amountUi: rb.amountUi, slippageBps: cfg.swapSlippageBps, maxImpactPct: 1.5, dryRun: false, silent: true });
          report.steps.push({ rollRebalance: { dir: rb.dir, amountUi: Number(rb.amountUi.toFixed(6)), swapUsd: Math.round(rb.swapUsd), clamped: !!rb.clamped, result: sw && sw.action, sig: sw && sw.sig, reason: rb.reason } });
          setState({
            rollRebalanceUsdToday: (st.rollRebalanceDay === today ? (st.rollRebalanceUsdToday || 0) : 0) + rb.swapUsd,
            rollRebalanceDay: today,
  ...
  if (managed) {
    ...
    if (rollRebalAllowed && preCloseFloat) {
      try {
        ...
        if (rb.action === "swap") {
          const sw = rb.dir === "sellClkn"
            ? await manualSwap({ fromSym: "CLKN", toSym: "SOL", amountUi: rb.amountUi, slippageBps: cfg.swapSlippageBps, maxImpactPct: 1.5, dryRun: false, silent: true, allowTokenSell: true })
            : await manualSwap({ fromSym: "SOL", toSym: "CLKN", amountUi: rb.amountUi, slippageBps: cfg.swapSlippageBps, maxImpactPct: 1.5, dryRun: false, silent: true });
          report.steps.push({ rollRebalance: { dir: rb.dir, amountUi: Number(rb.amountUi.toFixed(6)), swapUsd: Math.round(rb.swapUsd), clamped: !!rb.clamped, result: sw && sw.action, sig: sw && sw.sig, reason: rb.reason } });
          setState({
            rollRebalanceUsdToday: (st.rollRebalanceDay === today ? (st.rollRebalanceUsdToday || 0) : 0) + rb.swapUsd,
            rollRebalanceDay: today,
