...
async function tickAskWall({ dryRun = false } = {}) {
  ...
  if (!wall) {
    ...
    // adopt an existing untracked single-sided ask in THIS pool instead of stacking a
    // duplicate. A single-sided ask = all token, ~no quote, in the ask-wall pool.
    const stray = positions
      .filter((p) => p.pool === address && p.quoteAmount < 1e-9 && p.tokenAmount > 0)
