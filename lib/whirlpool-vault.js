...
async function payoutSpl({ mintAddr, recipients, perRecipientMaxUi, totalMaxUi, dryRun = true, onPaid = null } = {}) {
  ...
  const loop = await runPayoutLoop({
    list,
    // sendOne must resolve with a signature ONLY once the transaction is in the cluster, and
    // throw if nothing left the wallet. That distinction is the entire safety model below.
    sendOne: async (r) => {
      ...
      tx.recentBlockhash = (await conn.getLatestBlockhash()).blockhash;
      tx.sign(from);
      return await conn.sendRawTransaction(tx.serialize(), { skipPreflight: false, maxRetries: 3 });
    },
...
async function runPayoutLoop({ list, sendOne, confirmOne, onPaid }) {
