...
async function tick({ dryRun = false } = {}) {
  ...
  if (!managed) {
    ...
    else if (widthOff > 0.2) { needRoll = true; widthDriven = true; reason = `width reconfig → ±${cfg.widthPct}%`; }
  ...
  if (managed && !widthDriven && sinceLast < cfg.minRebalanceIntervalSec) {
    setState({ lastPrice: price, lastTickTs: Date.now() });
...
async function tickAskWall({ dryRun = false } = {}) {
  ...
  if (wall && sinceLast < cfg.minRebalanceIntervalSec) return { ...base, action: "deferred", reason: `${reason} (anti-thrash)` };
...
async function publicPositions() {
  ...
  return {
    ...
    positions: positions.map((p) => ({
      pair: p.pair,
      lower: p.lowerPriceClkn,
      upper: p.upperPriceClkn,
      current: p.currentPriceClkn,
      inRange: p.inRange,
    })),
  };
}

module.exports = { isEnabled, tick, tickAskWall, tickSol, status, publicPositions, getConfig, setConfig, pause, resume, DEFAULT_CONFIG };
