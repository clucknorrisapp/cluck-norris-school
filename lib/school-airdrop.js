...
// Funding wallet secret: AIRDROP_SECRET ONLY (a dedicated isolated float wallet). It used to
// silently fall back to MM_OPERATOR_SECRET_TREASURY — which contradicts this file's own rule
// ("never the treasury") and would ship graduate rewards from the treasury (a bug or bad amount
// then hits the treasury, with no daily cap). Now unset = feature OFF (safe no-op), as documented.
function secretBytes() {
  const raw = (process.env.AIRDROP_SECRET || "").trim();
  if (!raw) return null;
...
function canonKey(wallet) { try { return new PublicKey(wallet).toBase58(); } catch { return null; } }
function isPaid(wallet) { const k = canonKey(wallet); if (!k) return false; const p = kv.get(PAID_KV, {}) || {}; return !!p[k]; }

// In-process per-wallet mutex — two concurrent sends for the same wallet must never both broadcast.
const inFlight = new Set();
async function sigConfirmed(conn, sig) {
  try {
...
async function sendReward(wallet, amount, { force = false } = {}) {
  ...
  const key = recip.toBase58(); // canonical — a non-canonical representation can't slip past idempotency
  ...
  if (rawAmount <= 0n) return { ok: false, wallet, error: "amount rounds to 0 tokens" }; // dust can't mark a wallet paid

  if (inFlight.has(key)) return { ok: false, wallet, busy: true, error: "a send for this wallet is already in flight" };
  inFlight.add(key);
  try {
    const conn = connection();
    let paid = kv.get(PAID_KV, {}) || {};
    const rec = paid[key];
    if (rec && !force) {
      if (rec.confirmed) return { ok: true, wallet, already: true, sig: rec.sig, amount: rec.amount };
      // A prior attempt broadcast but was never marked confirmed (confirm-timeout). Verify the
      // sig ON-CHAIN before re-sending, so a landed-but-unconfirmed tx isn't paid twice.
      if (rec.sig && await sigConfirmed(conn, rec.sig)) {
        paid = kv.get(PAID_KV, {}) || {}; paid[key] = { ...rec, confirmed: true }; kv.set(PAID_KV, paid);
        return { ok: true, wallet, already: true, sig: rec.sig, amount: rec.amount };
      }
      // else: the prior tx did not land — fall through and re-send.
    }
    ...
    // Persist a PENDING marker BEFORE confirm — so a confirm-timeout retry finds this sig and
    // verifies it on-chain (above) rather than broadcasting a second payment.
    paid = kv.get(PAID_KV, {}) || {}; paid[key] = { sig, amount: amt, at: Date.now(), pending: true }; kv.set(PAID_KV, paid);
    await conn.confirmTransaction({ signature: sig, blockhash, lastValidBlockHeight }, "confirmed");
    paid = kv.get(PAID_KV, {}) || {}; paid[key] = { sig, amount: amt, at: Date.now(), confirmed: true }; kv.set(PAID_KV, paid);
