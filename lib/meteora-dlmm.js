...
async function status({ solUsd = 0, btcUsd = 0 } = {}) {
  ...
  const out = [];
  let totalUsd = 0, totalFeeUsd = 0, totalClaimedUsd = 0;
  ...
  // Lifetime = pending + claimed-on-live-positions + the banked ledger from positions we've
  // CLOSED (a close claims fees, then the position account — and its claimed history — is
  // gone; without the ledger every re-center would erase earned fees from reporting).
  ...
  return {
    ...
    lifetimeFeeUsd: Number((totalFeeUsd + totalClaimedUsd + ledgerUsd).toFixed(4)),
    positions: out,
  };
}
...
async function removeLiquidity({ positionPubkey, pct, close = false, dryRun = false } = {}) {
  if (!isEnabled()) return { enabled: false, reason: `${OPERATOR_ENV} not set` };
  ...
  // close=true on a full pull → claim fees + close the position (reclaims rent). Used for re-center/migrate.
  const shouldClaimAndClose = !!close && p >= 0.999;
  const out = { enabled: true, action: dryRun ? "would-remove" : "remove", position: found.lbPos.publicKey.toBase58(), pct: p, fromBin: d.lowerBinId, toBin: d.upperBinId, close: shouldClaimAndClose };
  ...
  // Bank this position's full fee history (claimed-so-far + the pending the close just
  // claimed) into the persistent ledger BEFORE its account disappears.
  if (shouldClaimAndClose) {
    try {
      ...
      const led = kv.get(FEE_LEDGER_KEY, { cbbtc: 0, sol: 0 }) || { cbbtc: 0, sol: 0 };
      if (X.mint === CBBTC_MINT) led.cbbtc = (led.cbbtc || 0) + fx; else if (X.mint === WSOL_MINT) led.sol = (led.sol || 0) + fx;
      if (Y.mint === CBBTC_MINT) led.cbbtc = (led.cbbtc || 0) + fy; else if (Y.mint === WSOL_MINT) led.sol = (led.sol || 0) + fy;
      kv.set(FEE_LEDGER_KEY, led);
    } catch (e) { console.warn("[meteora] fee-ledger bank failed:", e.message); }
  }
  return { ...out, sigs };
}
...
async function addLiquidity({ positionPubkey, cbbtcUi = 0, solUi = 0, distribution = "spot", dryRun = false } = {}) {
  if (!isEnabled()) return { enabled: false, reason: `${OPERATOR_ENV} not set` };
  const conn = connection();
  const DLMM = sdk();
  const found = await findPosition(conn, positionPubkey);
  ...
  const tx = await dlmm.addLiquidityByStrategy({
    ...
    strategy: { minBinId: d.lowerBinId, maxBinId: d.upperBinId, strategyType: strategyTypeOf(distribution) },
  });
...
module.exports = { status, isEnabled, operatorPubkey, removeLiquidity, addLiquidity, openPosition, getCfg, setCfg, OPERATOR_ENV, CBBTC_MINT, WSOL_MINT, METEORA_POOL };
