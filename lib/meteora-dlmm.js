...
async function rebalanceQuote({ positionPubkey, strategy = "curve", topUpX = 0, topUpY = 0 } = {}) {
  if (!isEnabled()) return { enabled: false, reason: `${OPERATOR_ENV} not set` };
  const conn = connection();
  const DLMM = sdk();
  ...
  // withdrawBps = how much to KEEP OUT of the redeposit (the SDK always withdraws 100%;
  // redeposit = total × (10000−bps)/10000 + topUp). 0 = redeposit everything → a recenter.
...
async function _rebalanceInPlace({ positionPubkey, strategy = "curve", topUpX = 0, topUpY = 0, maxActiveBinSlippage = 10, dryRun = false } = {}) {
  ...
  const sim = await dlmm.simulateRebalancePositionWithBalancedStrategy(
    ...
    new BN(Math.round((Number(topUpX) || 0) * Math.pow(10, decX))),
    new BN(Math.round((Number(topUpY) || 0) * Math.pow(10, decY))),
    new BN(0), new BN(0));   // withdrawBps 0 = redeposit everything (recenter)
