...
// POOL ALLOWLIST (audit 2026-09-05 #6). /api/meteora/open-position?pool= reached DLMM.create with any
// address the request named — the only pool-derived check was the DECIMALS table, which rejects
// unknown mints, not unknown pools, so a permissionless SOL/USDC LB pair passed it. That made
// &pool= the one admin lever that could seed treasury float into a venue chosen by the caller
// (post-auth, dry-run by default — but it turned a key compromise into unrecoverable loss, where
// every neighbouring lever only shuffles our own wallets). Allowed: the canonical pool, plus any
// address in METEORA_POOL_ALLOW (comma-separated, Railway). Checked before anything touches the
// chain, dry run included, so a typo shows on the dry run.
const POOL_ALLOW = new Set([METEORA_POOL, ...String(process.env.METEORA_POOL_ALLOW || "").split(",").map((s) => s.trim()).filter(Boolean)]);
function assertPoolAllowed(poolAddress) {
  const p = String(poolAddress || "");
  if (!POOL_ALLOW.has(p)) throw new Error(`pool ${p.slice(0, 8)}… is not on the Meteora allowlist (METEORA_POOL + METEORA_POOL_ALLOW) — refusing to open a position there`);
  return p;
}
...
const JUP_MINT = "JUPyiwrYJFskUPiHa7hkeR8VUtAeFoSYbKedZNsDvCN"; // Jupiter (6 decimals)
const SYM = { [CBBTC_MINT]: "cbBTC", [WSOL_MINT]: "SOL", [USDC_MINT]: "USDC", [JUP_MINT]: "JUP" };
const DEC = { [CBBTC_MINT]: 8, [WSOL_MINT]: 9, [USDC_MINT]: 6, [JUP_MINT]: 6 };
...
async function status({ solUsd = 0, btcUsd = 0, jupUsd = 0 } = {}) {
  ...
  const usdOf = (amt, mint) => mint === CBBTC_MINT ? amt * btcUsd : mint === WSOL_MINT ? amt * solUsd : mint === USDC_MINT ? amt : mint === JUP_MINT ? amt * jupUsd : 0;
  const out = [];
...
// ── In-place rebalance (the Meteora UI "Rebalance" primitive) ────────────────
// One atomic tx: withdraw + redeposit around the CURRENT active bin in the SAME
// position (pubkey unchanged), pending fees claimed in-op. SDK semantics (verified
// against source + live sims):
//   • withdrawBps = how much of that side to KEEP OUT of the redeposit (land in the
//     wallet). 0 = redeposit everything. The withdraw itself is always 100%.
//   • topUpX/topUpY = extra wallet funds to deposit on top of the redeposit.
//   • simulationResult amounts are NET wallet flows (withdrawn-to-wallet / deposited-
//     from-wallet), so an all-zero result for bps 0 + no top-up is CORRECT.
// Balancing therefore takes the caller doing: keep out the excess side → swap it →
// top it up in a second rebalance. READ-ONLY quote below; decimals must be in DEC.
async function rebalanceQuote({ positionPubkey, strategy = "curve", topUpX = 0, topUpY = 0, xWithdrawBps = 0, yWithdrawBps = 0 } = {}) {
  if (!isEnabled()) return { enabled: false, reason: `${OPERATOR_ENV} not set` };
  const conn = connection();
  const DLMM = sdk();
  ...
  const posPk = found.lbPos.publicKey;
  const mintX = dlmm.lbPair.tokenXMint.toString(), mintY = dlmm.lbPair.tokenYMint.toString();
  const decX = DEC[mintX], decY = DEC[mintY];
  if (decX == null || decY == null) throw new Error(`unknown token decimals for pool ${String(found.poolPk).slice(0, 8)}… — add the mint to DEC`);
  const strat = strategyTypeOf(strategy);
  const sim = await dlmm.simulateRebalancePositionWithBalancedStrategy(
    posPk, d, strat,
    new BN(Math.round((Number(topUpX) || 0) * Math.pow(10, decX))),
    new BN(Math.round((Number(topUpY) || 0) * Math.pow(10, decY))),
    new BN(Math.round(Number(xWithdrawBps) || 0)), new BN(Math.round(Number(yWithdrawBps) || 0)));
  const toUi = (bn, dec) => Number((bn && bn.toString ? bn.toString() : bn) || 0) / Math.pow(10, dec);
  const sr = sim.simulationResult || {};
  // AUTOFILL — the exact "Swaps Required" the Meteora UI shows: how much of which token
  // to add (acquired via a Jupiter swap) so the balanced redeposit consumes everything.
  // isBidSide=true → short the QUOTE (Y) side, add Y. targetX/Y = the balanced deposit
  // the UI calls "Rebalanced Position". Valid only with withdrawBps 0 (full redeposit).
  const fill = sdkMod().getAutoFillAmountByRebalancedPosition(sim.rebalancePosition, strat);
  const autofillDec = fill.isBidSide ? decY : decX;
  let targetX = 0, targetY = 0;
  try { const t = sim.rebalancePosition.totalAmounts(); targetX = toUi(t[0], decX); targetY = toUi(t[1], decY); } catch (_) {}
  return {
    enabled: true, position: posPk.toBase58(), pool: String(found.poolPk),
    mintX, mintY, symX: SYM[mintX] || mintX.slice(0, 4), symY: SYM[mintY] || mintY.slice(0, 4), decX, decY,
    // NET wallet flows for this plan:
    toWalletX: toUi(sr.actualAmountXWithdrawn, decX), toWalletY: toUi(sr.actualAmountYWithdrawn, decY),
    fromWalletX: toUi(sr.actualAmountXDeposited, decX), fromWalletY: toUi(sr.actualAmountYDeposited, decY),
    // autofill (the swap the UI does) + the balanced target it deposits into:
    autofillToken: fill.isBidSide ? (SYM[mintY] || "Y") : (SYM[mintX] || "X"),
    autofillAmount: toUi(fill.amount, autofillDec), isBidSide: !!fill.isBidSide,
    targetX, targetY,
    binArrayCount: sim.binArrayCount, binArrayCostSol: sim.binArrayCost,
  };
}
async function _rebalanceInPlace({ positionPubkey, strategy = "curve", topUpX = 0, topUpY = 0, xWithdrawBps = 0, yWithdrawBps = 0, maxActiveBinSlippage = 10, dryRun = false } = {}) {
  if (!isEnabled()) return { enabled: false, reason: `${OPERATOR_ENV} not set` };
  if (dryRun) return { enabled: true, action: "would-rebalance", ...(await rebalanceQuote({ positionPubkey, strategy, topUpX, topUpY, xWithdrawBps, yWithdrawBps })) };
  const conn = connection();
  const DLMM = sdk();
  const found = await findPosition(conn, positionPubkey);
  if (!found) throw new Error("position not found");
  ...
  const mintX = dlmm.lbPair.tokenXMint.toString(), mintY = dlmm.lbPair.tokenYMint.toString();
  const decX = DEC[mintX], decY = DEC[mintY];
  if (decX == null || decY == null) throw new Error(`unknown token decimals for pool ${String(found.poolPk).slice(0, 8)}…`);
  const sim = await dlmm.simulateRebalancePositionWithBalancedStrategy(
    posPk, d, strategyTypeOf(strategy),
    new BN(Math.round((Number(topUpX) || 0) * Math.pow(10, decX))),
    new BN(Math.round((Number(topUpY) || 0) * Math.pow(10, decY))),
    new BN(Math.round(Number(xWithdrawBps) || 0)), new BN(Math.round(Number(yWithdrawBps) || 0)));
  const { initBinArrayInstructions, rebalancePositionInstruction } =
    await dlmm.rebalancePosition(sim, new BN(maxActiveBinSlippage), operator().publicKey);
  const sigs = [];
  if (initBinArrayInstructions && initBinArrayInstructions.length) sigs.push(await signSendTx(conn, new Transaction().add(...initBinArrayInstructions)));
  sigs.push(await signSendTx(conn, new Transaction().add(...rebalancePositionInstruction)));
  return { enabled: true, action: "rebalanced", position: posPk.toBase58(), sigs };
}
async function rebalanceInPlace(a = {}) { return a.dryRun ? _rebalanceInPlace(a) : withLock(() => _rebalanceInPlace(a)); }
...
// xUi/yUi: generic per-side amounts (in the pool's OWN tokenX/tokenY order) for
// non-cbBTC/SOL pools — when either is set, they take precedence over the legacy
// cbbtcUi/solUi mapping. Check the pool's X/Y mints (e.g. via status) before sizing.
async function _openPosition({ poolAddress = METEORA_POOL, cbbtcUi = 0, solUi = 0, xUi = null, yUi = null, halfWidthPct = 0.6, distribution = "spot", dryRun = false } = {}) {
  assertPoolAllowed(poolAddress);   // before isEnabled(), so the refusal is visible even where no operator is loaded
  if (!isEnabled()) return { enabled: false, reason: `${OPERATOR_ENV} not set` };
  const conn = connection();
  const DLMM = sdk();
  ...
  const mintY = dlmm.lbPair.tokenYMint ? dlmm.lbPair.tokenYMint.toString() : null;
  const decX = DEC[mintX];
  const decY = DEC[mintY];
  // Refuse to guess decimals for an unknown mint — a wrong guess sizes the deposit
  // 10-1000x off. Add the mint to DEC before opening on a new pair.
  if (decX == null || decY == null) throw new Error(`unknown token decimals for pool ${poolAddress.slice(0, 8)}… (X ${mintX && mintX.slice(0, 6)}…, Y ${mintY && mintY.slice(0, 6)}…) — add the mint to DEC in meteora-dlmm`);
  const xIsBtc = mintX === CBBTC_MINT;
  const useGeneric = xUi != null || yUi != null;
  const totalX = new BN(Math.round((useGeneric ? Number(xUi) || 0 : (xIsBtc ? cbbtcUi : solUi)) * Math.pow(10, decX)));
  const totalY = new BN(Math.round((useGeneric ? Number(yUi) || 0 : (xIsBtc ? solUi : cbbtcUi)) * Math.pow(10, decY)));
  const out = {
...
module.exports = { status, isEnabled, operatorPubkey, removeLiquidity, addLiquidity, openPosition, rebalanceQuote, rebalanceInPlace, unwrapWsol, getCfg, setCfg, getLedger, setLedger, assertPoolAllowed, OPERATOR_ENV, CBBTC_MINT, WSOL_MINT, METEORA_POOL };
