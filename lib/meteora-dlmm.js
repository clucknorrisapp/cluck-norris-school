...
// The balancing SWAP lives in the CALLER (the vault's Jupiter swap), not here — so this
// is two parts. rebalanceQuote() simulates a balanced rebalance (withdraw 100% of both
// sides, redeposit per strategy around the active bin) and reports the AUTOFILL: how
// much of which token to add so the redeposit consumes the whole withdrawn balance.
// rebalanceInPlace() then executes WITH that token already topped-up in the wallet —
// same position pubkey throughout, pending fees claimed in-op, no close/reopen.
// READ-ONLY. Decimals must be known (DEC) — refuses to guess.
async function rebalanceQuote({ positionPubkey, strategy = "curve", topUpX = 0, topUpY = 0, debug = false } = {}) {
  if (!isEnabled()) return { enabled: false, reason: `${OPERATOR_ENV} not set` };
  const conn = connection();
  const DLMM = sdk();
  ...
  const sim = await dlmm.simulateRebalancePositionWithBalancedStrategy(
    ...
    new BN(10000), new BN(10000));
  const toUi = (bn, dec) => Number((bn && bn.toString ? bn.toString() : bn) || 0) / Math.pow(10, dec);
  // Amounts live on sim.simulationResult (RebalancePositionResponse); bin-array fields are
  // top-level (the RebalancePositionBinArrayRentalCostQuote intersection).
  const sr = sim.simulationResult || {};
  const fill = sdkMod().getAutoFillAmountByRebalancedPosition(sim.rebalancePosition, strat);
  if (debug) {
    const dump = (o) => { const r = {}; for (const k of Object.keys(o || {})) { const v = o[k]; r[k] = (v && v.toString && (v._bn || typeof v === "object")) ? String(v) : v; } return r; };
    let totals = null; try { totals = (sim.rebalancePosition.totalAmounts() || []).map(String); } catch (e) { totals = "err:" + e.message; }
    return { debug: true, simKeys: Object.keys(sim), simResultKeys: Object.keys(sr), sim: dump(sim), simResult: dump(sr), fill: dump(fill), rebalanceTotals: totals };
  }
  // isBidSide=true → short the QUOTE (Y) side, fill is in Y; else short BASE (X), fill in X.
  ...
  return {
    ...
    withdrawX: toUi(sr.actualAmountXWithdrawn, decX), withdrawY: toUi(sr.actualAmountYWithdrawn, decY),
    depositX: toUi(sr.actualAmountXDeposited, decX), depositY: toUi(sr.actualAmountYDeposited, decY),
...
async function _rebalanceInPlace({ positionPubkey, strategy = "curve", topUpX = 0, topUpY = 0, maxActiveBinSlippage = 10, dryRun = false } = {}) {
  if (!isEnabled()) return { enabled: false, reason: `${OPERATOR_ENV} not set` };
  if (dryRun) return { enabled: true, action: "would-rebalance", ...(await rebalanceQuote({ positionPubkey, strategy, topUpX, topUpY })) };
  const conn = connection();
  const DLMM = sdk();
  const found = await findPosition(conn, positionPubkey);
  if (!found) throw new Error("position not found");
