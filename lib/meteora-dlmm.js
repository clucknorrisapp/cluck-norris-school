...
// Native in-place re-center — the SDK 1.9 "rebalance" primitive (what Meteora's UI
// Rebalance tool uses): withdraw 100% of both sides and redeposit around the CURRENT
// active bin in the SAME position (same pubkey, same bin count), claiming pending fees
// in the same operation. No external swap leg — the token mix stays as-is, so depth may
// land asymmetric after a trend; it self-corrects as price oscillates. Cheaper and
// simpler than close→swap→reopen, but the position must be a single position.
async function _rebalanceInPlace({ positionPubkey, strategy = "curve", maxActiveBinSlippage = 10, dryRun = false } = {}) {
  if (!isEnabled()) return { enabled: false, reason: `${OPERATOR_ENV} not set` };
  const conn = connection();
  const DLMM = sdk();
  ...
  // Balanced-strategy simulate: no top-ups, withdraw 100% of X and Y (10000 bps each),
  // redeposit per the strategy around the active bin at the position's existing width.
  const sim = await dlmm.simulateRebalancePositionWithBalancedStrategy(
    posPk, d, strategyTypeOf(strategy), new BN(0), new BN(0), new BN(10000), new BN(10000));
  const out = {
    enabled: true, action: dryRun ? "would-rebalance" : "rebalance",
    position: posPk.toBase58(), strategy,
  ...
  const sigs = [];
  if (initBinArrayInstructions && initBinArrayInstructions.length) {
    sigs.push(await signSendTx(conn, new Transaction().add(...initBinArrayInstructions)));
  }
...
module.exports = { status, isEnabled, operatorPubkey, removeLiquidity, addLiquidity, openPosition, rebalanceInPlace, unwrapWsol, getCfg, setCfg, getLedger, setLedger, OPERATOR_ENV, CBBTC_MINT, WSOL_MINT, METEORA_POOL };
