...
// bound the per-day caller map too: for non-batch bodies the label is callerFromId(id), and the
// public /api/helius-rpc proxy forwards the client's own JSON-RPC id — an anonymous client could
// otherwise grow this map (persisted into the shared kvstore blob) without limit.
const MAX_CALLERS = 400;
...
function note(method, caller, host) {
  try {
    ...
    caller = caller || "sdk";
    if (!b.callers[caller] && Object.keys(b.callers).length >= MAX_CALLERS) caller = "other";
    bump(b.callers, caller, 1);
...
module.exports = { note, noteBody, noteProxy, summary, flush };
