...
const COLLECTION_KV = "diplomaCollectionV1"; // { collection, createdAt } — verified "Cluck Norris Diplomas" set
const MINTED_KV = "diplomaMintedV1"; // wallet -> { sig, slug, at }
const ORIGIN = "https://clucknorris.app";
// Never mint below this treasury SOL. Was 0.15 to reserve gas for the autonomous
// rebalancer — but that loop is HARD-KILLED (JUP_AUTO_REBALANCE_KILLED in server.js) and
// the vault is paused, so it was reserving gas for something that does not run, while
// silently blocking every graduation diploma (owner, 2026-07-29: "we don't need gas for
// rebalance as we aren't doing that right now").
// 0.02 still refuses to mint on a near-empty wallet, so a manual pool op always has gas.
// The merkle tree already exists, so a mint costs TX FEES ONLY (~0.000005 SOL) — the
// ~0.3 SOL tree creation was a one-time cost, already paid.
// ⚠️ If the autonomous rebalancer is ever re-enabled, put this back to 0.15.
const GAS_FLOOR_SOL = 0.02;
...
function umi() {
  ...
  const u = createUmi(primaryRpcUrl()).use(mplBubblegum()).use(mplTokenMetadata());
...
// Create the verified "Cluck Norris Diplomas" collection NFT once (idempotent). Owned by
// the treasury (= collection authority), so every diploma minted into it is auto-verified.
async function ensureCollection() {
  const existing = kv.get(COLLECTION_KV, null);
  if (existing && existing.collection) return existing.collection;
  const u = umi();
  ...
  await createNft(u, {
    mint, name: "Cluck Norris Diplomas", symbol: "CLKNDIP",
    uri: `${ORIGIN}/api/diploma-collection`, sellerFeeBasisPoints: percentAmount(0), isCollection: true,
  }).sendAndConfirm(u);
  const collection = mint.publicKey.toString();
  kv.set(COLLECTION_KV, { collection, createdAt: Date.now() });
  return collection;
}
...
async function mintDiploma(wallet, slug, { force = false } = {}) {
  ...
  const { mintToCollectionV1 } = require("@metaplex-foundation/mpl-bubblegum");
  const r = await mintToCollectionV1(u, {
    ...
    metadata: {
      ...
      collection: { key: publicKey(collection), verified: false }, // verified on-mint via the treasury authority
      creators: [],
    },
  }).sendAndConfirm(u);
...
function status() {
  ...
  const coll = kv.get(COLLECTION_KV, null);
  const minted = kv.get(MINTED_KV, {});
  return { enabled: isEnabled(), payer: payerPubkey(), tree: tree && tree.tree, collection: coll && coll.collection, mintedCount: Object.keys(minted).length };
}

module.exports = { isEnabled, payerPubkey, ensureTree, ensureCollection, mintDiploma, status, MINTED_KV };
