...
function suggestRanges(st, widthPct) {
  ...
  // Guard against a degenerate (sub-spacing) balanced range at tight widths — Raydium's
  // alignTick uses Math.round, so at the floor lower/upper could collapse to the same tick.
  // Keep >=1 tick-spacing each side of price (matches the Orca adapter; same-interface contract).
  const curAligned = Math.floor(curTick / sp) * sp;
