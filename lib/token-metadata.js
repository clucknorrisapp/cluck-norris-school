...
/**
 * Build the unsigned lock transaction.
 *
 ...
 * Returns { tx, ix, preserved, changes } so a UI can show exactly what will and will not change
 * before a human signs something irreversible.
 */
function buildLockIx({ current, newUri = null }) {
  if (!current || current.key !== 4) throw new Error("no metadata account to update");
  if (!current.isMutable) throw new Error("metadata is already immutable — nothing to do");
  ...
  const ix = new TransactionInstruction({
    ...
    // isMutable: false is the point. newUpdateAuthority stays null — we are NOT reassigning
    // ownership, only closing edits, and conflating the two on an irreversible call is asking for
    // an accident.
    data: encodeUpdateV2({ data, isMutable: false }),
  });
  return {
    ix,
    ...
    changes: { uri: { from: current.uri, to: uri, changed: uri !== current.uri },
               isMutable: { from: true, to: false } },
  };
}

async function buildLockTx({ connection, current, newUri = null }) {
  const { ix, preserved, changes } = buildLockIx({ current, newUri });
  ...
  return { unsignedBase64, preserved, changes, metadataPda: metadataPda(current.mint).toBase58() };
}

module.exports = {
  TOKEN_METADATA_PROGRAM_ID, IX_UPDATE_METADATA_ACCOUNT_V2,
  metadataPda, decodeMetadata, encodeDataV2, encodeUpdateV2, buildLockIx, buildLockTx,
};
