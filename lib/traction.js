...
function recordHubLessonRead(kv, { project, sid, nowMs = Date.now() } = {}) {
  if (!project || !sid || !kv || typeof kv.set !== "function") return;
  const day = dayKeyOf(nowMs);
  const h = hashOf("hublesson", project, sid);
...
function lessonReadsForProject(kv, projectId, { from, to } = {}) {
  ...
  try {
    const raw = kv.get("analytics_v1", null);
    const path = "/hub/" + projectId;
    if (raw && raw.days) {
      ...
      if (found) denominator = views;
    }
  } catch (_) { /* denominator stays null — "unavailable", never a guessed zero */ }
...
module.exports = {
  WALLETS_KEY, RECEIPT_OPENS_KEY, HUB_LESSON_READS_KEY, FOUNDER_IDS, DRYRUN_IDS,
  dayKeyOf, isoWeekKeyOf, labelFor,
  recordWalletConnect, recordReceiptOpen, recordHubLessonRead,
  readWalletConnects, readReceiptOpens, readHubLessonReads, lessonReadsForProject,
  compute,
};
