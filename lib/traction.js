...
//
//   1. lib/hub/ledger.js's settlement journal (`hub:settle`) and lib/hub/attempts.js's self-sign
//      state machine are BUILT AND TESTED (scripts/hub-core-test.cjs, hub-attempts-test.cjs) but
//      NOT YET MOUNTED in any route of lib/hub/routes.js. Every live Hub payout today — including
//      CUNA's — still goes through the older lib/cuna-payout.js `batch.sent[wallet]` mechanism via
//      POST /api/hub/:project/payout?sent=. The journal/attempts counters below will read 0 until
//      that flow ships; they stay because they are the design's end state and start moving the
//      moment it does, and reporting a real 0 here is itself a checkable number.
...
// P1-04 (docs/HUB_PUBLIC_SURFACES_VERIFY_2026-09-18.md): `lesson`/`from`/`to` above are all bounded
// against a fixed/known set before they can become a stored key — `project` here had no such
// bound, so an unauthenticated POST /api/track with a made-up `hub_lesson_read:<anything>` id
// minted a brand-new kv key per id (each holding up to MAX_PER_BUCKET hashes/day) AND fed the
// platform-wide "school -> Hub lesson reads" total (recordHubLessonRead is summed across every
// project key), so a script could inflate that number and even flip its provenance label between
// founder-operated/independent/mixed. Bound the same way: only a project id already present in
// hubStore's registry, or one of the built-in ids server.js's hubProjects() always seeds before
// merging it (kept as a literal set — this file must not require server.js, which requires this
// file), is accepted; anything else is silently dropped, never stored. This file has no live `kv`
// at require time, so the check is a function taking `kv`, not a value computed once here.
const BUILT_IN_HUB_PROJECT_IDS = new Set(["clkn", "cuna", "rose"]);
function knownHubProjectId(kv, projectId) {
  if (!projectId) return false;
  if (BUILT_IN_HUB_PROJECT_IDS.has(projectId)) return true;
  try { return Object.prototype.hasOwnProperty.call(hubStore.readRegistry(kv) || {}, projectId); }
...
function recordHubLessonRead(kv, { project, sid, lesson, nowMs = Date.now() } = {}) {
  ...
  // P1-04: reject a project id that isn't a registered Hub project — same bound `lesson`/`from`/
  // `to` already get. Dropped silently, never stored, so a script can't mint a new kv key (and a
  // new slice of the platform-wide total) just by inventing an id.
  if (!knownHubProjectId(kv, project)) return;
  const day = dayKeyOf(nowMs);
  const h = hashOf(kv, "hublesson", project, sid);
  // P1-04: pruneByDay/KEEP_DAYS applied here now, same as recordReceiptOpen/recordWalletConnect —
  // this key never had it, so it grew forever regardless of the id bound above.
  const all = pruneByDay(kv.get(HUB_LESSON_READS_KEY, {}) || {}, nowMs);
  const byProject = { ...(all[day] || {}) };
  const list = new Set(byProject[project] || []);
  ...
  if (lesson && KNOWN_LOCK_LESSON_IDS.has(lesson)) {
    ...
    const allL = pruneByDay(kv.get(HUB_LESSON_READS_BY_LESSON_KEY, {}) || {}, nowMs);
...
function recordHubDoorClick(kv, { source, sid, nowMs = Date.now() } = {}) {
  ...
  // P1-04: pruneByDay applied — this key never had it either (docs/HUB_PUBLIC_SURFACES_VERIFY_2026-09-18.md).
  const all = pruneByDay(kv.get(HUB_DOOR_CLICKS_KEY, {}) || {}, nowMs);
...
function recordHubBridgeClick(kv, { from, to, sid, nowMs = Date.now() } = {}) {
  ...
  // P1-04: pruneByDay applied — this key never had it either (docs/HUB_PUBLIC_SURFACES_VERIFY_2026-09-18.md).
  const all = pruneByDay(kv.get(HUB_BRIDGE_CLICKS_KEY, {}) || {}, nowMs);
...
// lib/hub/attempts.js's self-sign state machine (see file header — not yet mounted anywhere).
// Read anyway: the moment it ships, this starts reporting real numbers with no code change here.
...
function compute({ kv, from, to } = {}) {
  ...
  counters.hubReceiptsIssuedJournal = {
    ...
    source: "lib/hub/store.js JOURNAL_KEY (`hub:settle`) via lib/hub/ledger.js settle() — the design's settlement journal",
    label: labelFor(journalIds),
    note: journalLifetime === 0 ? "0 because lib/hub/attempts.js's self-sign flow is not yet mounted (see this file's header) — every live payout today writes lib/cuna-payout.js `batch.sent`, counted below instead." : undefined,
  };
  ...
  counters.hubBatchesSigned = {
    ...
    source: "every project's `batches[].attempts[wallet]` (lib/hub/attempts.js registerAttempt/recordSignature) — a signature recorded the moment a wallet signs, before broadcast",
    label: labelFor(attemptIds),
    note: attemptsLifetime === 0 ? "0 for the same reason as hubReceiptsIssuedJournal — lib/hub/attempts.js is not yet mounted in a route." : undefined,
  };
  if (journalLifetime === 0 && attemptsLifetime === 0) caveats.push("the settlement-journal and self-sign counters (hubReceiptsIssuedJournal, hubBatchesSigned) read 0 because lib/hub/attempts.js is built and tested but not yet wired into a route — see lib/traction.js's header.");
