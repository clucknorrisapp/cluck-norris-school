...
function recordHubDoorClick(kv, { source, sid, nowMs = Date.now() } = {}) {
  ...
  const day = dayKeyOf(nowMs);
  const h = hashOf("doorclick", source, sid);
...
module.exports = {
  WALLETS_KEY, RECEIPT_OPENS_KEY, HUB_LESSON_READS_KEY, HUB_DOOR_CLICKS_KEY, FOUNDER_IDS, DRYRUN_IDS,
  dayKeyOf, isoWeekKeyOf, labelFor,
  recordWalletConnect, recordReceiptOpen, recordHubLessonRead, recordHubDoorClick,
  readWalletConnects, readReceiptOpens, readHubLessonReads, readHubDoorClicks,
