...
// Memoised per (endpoint, commitment): the engine ticks and the pool/position readers called this
// dozens of times a minute and each call built a fresh client (simplifier pass, 2026-09-17). The
// endpoint is part of the key, so a primary that fails over gets a new client; rpcFetch keeps doing
// the per-request failover exactly as before.
const CONNECTIONS = new Map();
function connection(commitment = "confirmed") {
  const url = primaryRpcUrl();
  const key = url + "|" + commitment;
  let c = CONNECTIONS.get(key);
  if (!c) { c = new Connection(url, { commitment, fetch: rpcFetch }); CONNECTIONS.set(key, c); }
  return c;
}
