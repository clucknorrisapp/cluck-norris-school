...
// First-party call attribution (best-effort; must NEVER break an RPC call).
// Lazily required so a load-order or dependency hiccup can't take down RPC.
let _usage = null;
function usage() { try { return (_usage = _usage || require("./helius-usage")); } catch { return null; } }

const PUBLIC_FALLBACK = "https://api.mainnet-beta.solana.com";

// A hung primary (socket accepted, response never arrives — the common overload mode)
// otherwise never triggers failover: only a retriable HTTP status or a thrown error
// advances to the next endpoint, and rpcFetch had no signal of its own. Env-tunable so
// a slow-but-legitimate call site can override it via init.signal.
const RPC_TIMEOUT_MS = Number(process.env.RPC_TIMEOUT_MS) > 0 ? Number(process.env.RPC_TIMEOUT_MS) : 20000;

function heliusUrl(key) {
  return `https://mainnet.helius-rpc.com/?api-key=${key}`;
}

// Ordered, de-duplicated list of RPC endpoints to try. Derived from env, which is
// fixed at boot, so it's computed once and cached (this runs on every RPC call).
...
async function rpcFetch(url, init) {
  ...
  for (let i = 0; i < attempts.length; i++) {
    ...
    try {
      // A caller-supplied signal is respected as-is; otherwise bound this attempt so a
      // stalled socket throws (and, for reads, fails over) instead of hanging forever.
      const attemptInit = init.signal ? init : { ...init, signal: AbortSignal.timeout(RPC_TIMEOUT_MS) };
      ...
      // Attribute the served call (method + caller-label from the JSON-RPC id).
      try { usage()?.noteBody(init && init.body, hostOf(target)); } catch {}
      return res;
    } catch (e) {
      lastErr = e;
