...
function createEngine(deps) {
  ...
  function start({ mint, requestedBy = null, force = false, cooldownMs = 6 * 3600 * 1000 }) {
    ...
    if (active) return { ok: true, job: jobPublic(active), reused: true };
    if (!force) {
  ...
  async function runJob(job) {
    ...
    for (const t of targets) {
      ...
      if (newSigs.size) {
        ...
        // newest signature across the accounts becomes the incremental watermark
        const newest = [...newSigs.entries()].sort((a, b) => b[1] - a[1])[0];
        c.lastSig = newest ? newest[0] : c.lastSig;
      }
