...
function createEngine(deps) {
  ...
  async function runJob(job) {
    ...
    const cpSet = new Set();
    for (const a of analysed.values()) for (const r of a.rows) if (r.cp && (r.action === "send" || r.action === "receive") && !deps.isOnCurve(r.cp)) cpSet.add(r.cp);
    let cpTypes = new Map();
    try { cpTypes = await deps.classifyAddressTypes([...cpSet], rpc); } catch {}
    for (const [w, a] of analysed) {
      let changed = false;
      for (const r of a.rows) {
        if (!r.cp || (r.action !== "send" && r.action !== "receive")) continue;
        if (deps.isOnCurve(r.cp)) continue;
        ...
        const next = isLock ? (r.action === "send" ? "lock" : "unlock") : (r.action === "send" ? "add_lp" : "withdraw_lp");
