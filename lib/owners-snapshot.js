...
const holdersSnapshot = require("./holders-snapshot");
...
function createEngine(deps) {
  ...
  async function runJob(job) {
    ...
    // 4b. Where did "send"s go? A plain transfer to a person is a link; a transfer into a lock
    //     escrow or a pool is NOT an exit. Classify every off-curve counterparty once (one
    //     getMultipleAccounts per 100) and relabel: send→locker = lock, send→pool/contract =
    //     add_lp, receive from locker = unlock. The first real crawl (ROSE) had every community
    //     lock reading as "transferred 100% of peak out — OUT", which is the opposite of the truth.
    setStage("classifying counterparties", { done: 0, total: 1 });
    // Idempotent: cached rows that were labelled on an earlier run (or mislabelled by an older
    // classifier) are re-derived from the fresh classification, not skipped.
    const RELABEL = new Set(["send", "receive", "add_lp", "withdraw_lp", "lock", "unlock"]);
    const cpSet = new Set();
    for (const a of analysed.values()) for (const r of a.rows) if (r.cp && RELABEL.has(r.action) && !deps.isOnCurve(r.cp)) cpSet.add(r.cp);
    let cpTypes = new Map();
    try { cpTypes = await deps.classifyAddressTypes([...cpSet], rpc); } catch {}
    for (const [w, a] of analysed) {
      let changed = false;
      for (const r of a.rows) {
        if (!r.cp || !RELABEL.has(r.action) || deps.isOnCurve(r.cp)) continue;
        // liqHint rows (Helius typed the tx as liquidity) keep their direction; only the lock/LP
        // choice is re-derived. outbound = token left the wallet.
        const outbound = r.tok < 0;
        const t = cpTypes.get(r.cp);
        // "locker" by category, or by label when the classifier only knew the program name
        // (Jupiter Lock / Streamflow / vesting escrows all carry it in the label).
        const isLock = !!t && (t.category === "locker" || /lock|vest|escrow/i.test(t.label || ""));
        const next = isLock ? (outbound ? "lock" : "unlock") : (outbound ? "add_lp" : "withdraw_lp");
        if (next !== r.action) { r.action = next; r.cpLabel = t ? t.label : "Program account"; changed = true; }
      }
      if (changed) {
        const t = targets.find((x) => x.wallet === w);
        const m = metricsFor(a.rows, t ? t.balance : a.balance, price, a.historyTruncated, nowTs, cfg.minUsd);
        analysed.set(w, { ...a, ...m });
      }
    }
    saveCache();
    ...
    // Biggest holders first for half the cap (the owner's "who is in" list), then the sellers
    // (the "who is draining me" list) — the first real crawl spent the whole cap on dust exits.
    const byUsd = [...analysed.values()].sort((a, b) => b.usd - a.usd);
    const half = Math.ceil(cfg.funderCap / 2);
    const pick = new Map();
    for (const a of byUsd.slice(0, half)) pick.set(a.wallet, a);
    for (const a of [...analysed.values()].filter((x) => x.flags.dumper).sort((a, b) => (b.soldUsd7d - a.soldUsd7d) || (b.peakUsd - a.peakUsd))) { if (pick.size >= cfg.funderCap) break; pick.set(a.wallet, a); }
    for (const a of byUsd) { if (pick.size >= cfg.funderCap) break; pick.set(a.wallet, a); }
    const wantFunder = [...pick.values()];
    done = 0;
    ...
    const distributions = (links.distributions || []).map((d) => {
      const recs = d.wallets.map((x) => analysed.get(x.wallet)).filter(Boolean);
      const bal = recs.reduce((s, a) => s + a.balance, 0);
      return {
        from: d.from, kind: d.kind, recipients: d.recipients, label: (deps.KNOWN_CEX_WALLETS && deps.KNOWN_CEX_WALLETS[d.from]) || null,
        isHolder: analysed.has(d.from), holderStatus: analysed.has(d.from) ? analysed.get(d.from).status : null,
        balance: Math.round(bal), usd: r2(bal * price), supplyPct: totalSupply ? r4(100 * bal / totalSupply) : 0,
        statuses: recs.reduce((m, a) => { m[a.status] = (m[a.status] || 0) + 1; return m; }, {}),
        soldUsd7d: r2(recs.reduce((s, a) => s + (a.soldUsd7d || 0), 0)),
    ...
    const summary = {
      ...
      distributionCount: distributions.length, distributedSupplyPct: r4(distributions.filter((d) => d.kind === "transfer").reduce((s, d) => s + d.supplyPct, 0)),
    ...
    const resultObj = { version: 1, summary, watch, clusters, distributions, wallets: walletsOut, nonWallets: nonWallets.sort((a, b) => b.balance - a.balance).slice(0, 200), links: links.slice(0, 2000),
      notes: NOTES };
    ...
    // X7 (Colosseum roadmap §8): a dated, hashed record of THIS run's full holder list —
    // append-only, kv-backed, served read-only with no gate (history of a public on-chain fact,
    // not the crawl itself). `people` is every non-contract owner with balance > 0 from step 3 —
    // the full list this run produced, before the $5+ analysis filter narrows it down. Never
    // fails the run: this is an index over what was already computed, not part of the crawl.
    try {
      const { ok } = holdersSnapshot.appendSnapshot(kv, {
        mint, at: ts, symbol,
        holderCount: people.length,
        top: people.map((p) => ({ wallet: p.wallet, amount: p.balance })),
        totalSupplyRaw: supplyRes && supplyRes.result && supplyRes.result.value && supplyRes.result.value.amount != null ? String(supplyRes.result.value.amount) : null,
        fullList: people.map((p) => ({ wallet: p.wallet, amount: p.balance })),
        // P2-06: this run already read the mint's own decimals (step 3, above) — carry it onto
        // the record so a later diff can convert `totalSupplyRaw` into the same UI unit `top`'s
        // amounts are already in, without a second, gated crawl.
        decimals: Number.isInteger(decimals) ? decimals : null,
      });
      if (!ok) log(`job ${job.id}: holder snapshot record for ${mint} did not verify on kv`);
    } catch (e) { log(`job ${job.id}: holder snapshot record failed: ${e.message}`); }
...
function metricsFor(rows, liveBalance, price, truncated, nowTs, minUsd) {
  let bought = 0, sold = 0, recv = 0, sent = 0, lpIn = 0, lpOut = 0, lockIn = 0, lockOut = 0;
  ...
  for (const r of rows) {
    ...
    switch (r.action) {
      ...
      case "lock": lockIn += a; break;
      case "unlock": lockOut += a; break;
    }
  }
  const lockedNet = Math.max(0, lockIn - lockOut);
  ...
  if (!meaningful) { status = "DUST"; why = "never held a meaningful position"; }
    ...
    // Tokens sitting in a lock escrow or a pool are not gone — the wallet just doesn't hold them
    // directly. Those come first so a community locker never reads as an exit.
    if (peak > 0 && lockedNet >= peak * 0.5) { status = "HOLDING"; why = `position is locked (${Math.round(100 * lockedNet / peak)}% of peak in a lock escrow)`; }
    else if (peak > 0 && (lpIn - lpOut) >= peak * 0.5) { status = "HOLDING"; why = "position moved into a liquidity pool"; }
    else if (soldOfPeak >= 0.5) { status = "OUT"; why = `sold ${Math.round(soldOfPeak * 100)}% of peak and holds under $${minUsd}`; }
  ...
  return {
    status, why, rows,
    bought: Math.round(bought), sold: Math.round(sold), received: Math.round(recv), sent: Math.round(sent), lpIn: Math.round(lpIn), lpOut: Math.round(lpOut), locked: Math.round(lockedNet),
...
// A DISTRIBUTOR is a wallet that handed this token to many holders (an airdrop / presale /
// team-distribution wallet — on ROSE the deployer seeded 79 holders). Linking every recipient
// to every other through it would fold half the holder base into one meaningless "cluster", so
// distributors (≥ DISTRIBUTOR_MIN recipients) are reported on their own and their transfer edges
// do not join the union-find. Same rule for a funder that seeded that many wallets: that is an
// exchange hot wallet or a distribution wallet we don't have in the table, not a ring.
const DISTRIBUTOR_MIN = 8;
function buildLinks(analysed, deps) {
  ...
  for (const a of analysed.values()) {
    for (const d of a.sendDests || []) if (d.to && deps.isOnCurve(d.to) && !isCexOrSvc(d.to)) pending.push({ kind: "transfer", from: a.wallet, to: d.to, amount: d.amount });
    for (const d of a.recvFrom || []) {
      if (!d.from || !deps.isOnCurve(d.from) || isCexOrSvc(d.from)) continue;
      pending.push({ kind: "transfer", from: d.from, to: a.wallet, amount: d.amount });
      const l = byDistributor.get(d.from) || []; l.push({ wallet: a.wallet, amount: d.amount }); byDistributor.set(d.from, l);
    }
  ...
  const distributors = new Set([...byDistributor.entries()].filter(([, l]) => l.length >= DISTRIBUTOR_MIN).map(([f]) => f));
  const massFunders = new Set([...byFunder.entries()].filter(([, l]) => l.length >= DISTRIBUTOR_MIN).map(([f]) => f));
  for (const l of pending) if (!distributors.has(l.from)) links.push(l);
  for (const [f, ws] of byFunder) if (ws.length >= 2 && !massFunders.has(f)) for (const w of ws) links.push({ kind: "funder", from: f, to: w });
  ...
  out.distributions = [...byDistributor.entries()].filter(([f]) => distributors.has(f)).map(([from, l]) => ({ from, kind: "transfer", recipients: l.length, wallets: l.sort((a, b) => b.amount - a.amount) }))
    .concat([...byFunder.entries()].filter(([f]) => massFunders.has(f)).map(([from, ws]) => ({ from, kind: "funder", recipients: ws.length, wallets: ws.map((w) => ({ wallet: w, amount: 0 })) })));
  return out;
}
...
function buildClusters(analysed, links, totalSupply, price, deps) {
  ...
  for (const nodes of groups.values()) {
    ...
    const freshRecs = recs.filter((a) => a.flags.fresh);
    const fresh = freshRecs.length;
    const freeBagRecs = recs.filter((a) => a.flags.freeBag);
    const freeBag = freeBagRecs.length;
    const supplyPct = totalSupply ? 100 * balance / totalSupply : 0;
    const freshPct = totalSupply ? 100 * freshRecs.reduce((s, a) => s + a.balance, 0) / totalSupply : 0;
    const freeBagPct = totalSupply ? 100 * freeBagRecs.reduce((s, a) => s + a.balance, 0) / totalSupply : 0;
    const funderMembers = new Set(); for (const l of links) if (l.kind === "funder" && nodes.has(l.to)) funderMembers.add(l.to);
    const funderPct = totalSupply ? 100 * recs.filter((a) => funderMembers.has(a.wallet)).reduce((s, a) => s + a.balance, 0) / totalSupply : 0;
    const soldUsd7d = recs.reduce((s, a) => s + (a.soldUsd7d || 0), 0);
    const flags = [];
    // Each flag states the share held by the wallets it is actually about — the first real crawl
    // printed the whole group's 47% next to "16 wallets under 14 days old", which was misleading.
    if (kinds.has("funder") && funderMembers.size >= 3 && funderPct >= 1) flags.push(`${funderMembers.size} wallets share a first funder and hold ${funderPct.toFixed(2)}% of supply between them`);
    if (freeBag >= 2 && kinds.has("transfer")) flags.push(`${freeBag} wallets were handed their bags by transfer rather than buying (${freeBagPct.toFixed(2)}% of supply)`);
    if (fresh >= 2 && freshPct >= 0.5) flags.push(`${fresh} wallets under 14 days old hold ${freshPct.toFixed(2)}% of supply between them`);
...
const NOTES = {
  statuses: {
    ...
    OUT: "Held a meaningful position and now holds under the minimum — sold or transferred it away. Tokens moved into a lock escrow or a liquidity pool are NOT counted as gone.",
  },
  honesty: [
