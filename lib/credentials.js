...
function record(wallet, { kind, score, total, pct, verified, isHolder, balance } = {}) {
  const now = new Date().toISOString();
  let rec = byWallet[wallet];
  if (!rec) {
    const slug = slugFor(wallet);
    rec = { wallet, slug, createdAt: now, updatedAt: now, diploma: null, graduation: null, holder: null };
    byWallet[wallet] = rec;
    bySlug[slug] = wallet;
  }
  rec.updatedAt = now;
