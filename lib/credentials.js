...
// The on-chain graduation NFT, once minted: signature (+ tree) so the transcript can link an
// explorer. Idempotent — a re-mint returns the same signature and this is a no-op.
function setNft(wallet, { sig, tree } = {}) {
  const rec = byWallet[wallet];
  if (!rec || !sig) return null;
  if (rec.nft && rec.nft.sig === sig) return rec;
  rec.nft = { sig: String(sig), tree: tree || null, at: new Date().toISOString() };
  rec.updatedAt = rec.nft.at;
  persist();
  return rec;
}
...
module.exports = { record, setOwnership, setNft, getBySlug, resolve, stats, all: () => Object.values(byWallet) };
