...
// ── The draw itself (added 2026-09-15, the 0.25 SOL drawing's pick) ─────────────────────────
// Every address that looks like a wallet in a block of free text — a pasted X thread, a file of
// one address per line, a reply body. Canonical, deduped, first-seen order; programs and PDAs out.
function extractAddresses(text) {
  const out = [];
  const seen = new Set();
  for (const m of String(text || "").match(/[1-9A-HJ-NP-Za-km-z]{32,44}/g) || []) {
    const a = canonicalAddress(m);
    if (!a || seen.has(a) || walletReason(a)) continue;
    seen.add(a); out.push(a);
  }
  return out;
}

// Deterministic weighted draw — the same SHA256 walk the Buy Special raffle publishes, so the
// verification recipe is one the community has already seen. `entries` = [{address, chances}];
// they are sorted by address first so the input order can never move the result. Pick k = the
// address at slot (SHA256("<seed>:<k>") mod slots-remaining); a picked address's whole block is
// removed so nobody is drawn twice. Returns the picks in order: winner first, then alternates.
function drawFromSeed(entries, seed, n = 1) {
  const rows = (entries || []).filter((e) => e && e.address).map((e) => ({ address: e.address, chances: Math.max(0, Math.floor(Number(e.chances) || 0)) }))
    .sort((a, b) => a.address.localeCompare(b.address));
  let pool = [];
  for (const e of rows) for (let i = 0; i < e.chances; i++) pool.push(e.address);
  const picks = [];
  let k = 0;
  while (picks.length < n && pool.length > 0 && k < 100000) {
    const h = createHash("sha256").update(`${seed}:${k}`).digest("hex");
    const idx = Number(BigInt("0x" + h) % BigInt(pool.length));
    const w = pool[idx];
    k++;
    picks.push({ address: w, round: k - 1, slot: idx, of: pool.length });
    pool = pool.filter((x) => x !== w);
  }
  return picks;
}

// Hash of the exact list the draw ran over, for the announcement — anyone holding the list can
// recompute it, and a list edited after the fact cannot match.
function entriesHash(entries) {
  const rows = (entries || []).map((e) => ({ address: e.address, chances: Math.max(0, Math.floor(Number(e.chances) || 0)) }))
    .sort((a, b) => a.address.localeCompare(b.address));
...
module.exports = { KEY, DEFAULT_WINDOW, DEFAULT_CAP, NOT_A_WALLET, canonicalAddress, walletReason, windowState, enter, check, deleteEntry, exportRows, windowFromEnv, extractAddresses, drawFromSeed, entriesHash };
