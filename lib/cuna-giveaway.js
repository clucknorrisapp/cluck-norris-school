...
// SETTLE DELAY (2026-09-24, after wallet 8w3JXv…HCuNt lost 15 of 35 qualifying buys on
// 2026-09-23). getTradeTapeHelius only reports reachedWindowStart:false when a signature is
// MISSING from its batch — but Helius's enhanced parse for very fresh transactions can come back
// with the signature present and NO usable tokenTransfers yet (the enrichment lags a little
// behind the signature index). That reads as a fully-covered, empty slice, so the incremental
// scanner (never looks back) retired it and the buys inside it were gone forever — both the
// 12:31:17–12:31:26 UTC burst (scanned by the 12:31:30 tick, nine seconds later) and the whole
// 21:21:38–21:22:35 burst. So: never scan a slice whose end is within SETTLE_MS of now. The board
// runs 5 minutes behind live, which is fine — it already refreshes every 15–20 minutes.
...
function blank() {
  return {
    ...
    // GENERATION COUNTER (Codex round 32 P1) — bumped by rewindCursor() and resetLedger(), the two
    // things that move cursorMs from OUTSIDE the running scan. scanOnce() captures it before each
    // slice's tape await and checks it after: a change means a rewind landed mid-flight, and the
    // stale slice's `to` must never be written over the rewound cursor — see the big comment on
    // scanOnce() below.
    gen: 0,
...
function configure(patch) {
  const s = load();
  const prev = s.config || {};
  const c = Object.assign({ minUsd: 5, displayUsd: 5, symbol: 'CUNA' }, prev, patch || {});
  ...
  // GENERATION BUMP (Codex round 32 "second lens" P2): reconfiguring to a different mint/pool/
  // window/threshold/scoring-rule out from under an in-flight scan is the same hazard
  // rewindCursor()/resetLedger() already guard `s.gen` against — a scan that captured `gen`
  // against the OLD promo must not credit trades scored against a bar/threshold that no longer
  // applies, and must not overwrite the cursor this reconfigure is about to set. Bump whenever a
  // field that changes WHAT is being scanned or HOW changes; cosmetic fields (symbol, displayUsd,
  // exclude, bonusPct, chatId, mode) don't need to — they don't change what an already-scanned
  // slice means.
  const identityChanged = c.mint !== prev.mint || c.pool !== prev.pool ||
    c.startMs !== prev.startMs || c.endMs !== prev.endMs ||
    c.minUsd !== prev.minUsd || c.entryMode !== prev.entryMode;
  s.config = c;
  if (!s.cursorMs || s.cursorMs < c.startMs) s.cursorMs = c.startMs;
  if (identityChanged) s.gen = (s.gen || 0) + 1;
  save();
  return c;
}
...
function resetLedger() {
  ...
  s.gen = (s.gen || 0) + 1;   // supersede any scan already in flight — see blank()'s comment
  save();
  return { ok: true, cursorMs: s.cursorMs };
}

// Accepts an ISO string, a unix-ms integer, or a unix-seconds integer (< 1e11 is read as seconds
// and scaled up — the admin route's other time fields use the same >1e11 threshold to tell them
// apart, see server.js's `ms()` helper on /api/cuna-giveaway/admin).
...
// REWIND — the operator lever for reopening a stretch of already-scanned tape (2026-09-24, the
// wallet 8w3JXv…HCuNt incident above). The incremental scanner never looks back on its own; this
// is the deliberate, auditable exception. Moving cursorMs backward and letting the next scanOnce()
// re-walk that stretch is safe by construction:
//   - every credited buy is deduped by signature (rec.buys.some(b => b.sig === t.sig)) before a
//     re-scan can add it again, so an already-counted buy is never double-counted;
//   - a sell already recorded as a dq stays a dq — nothing here un-marks a wallet, it only gives
//     the scanner another pass at signatures it may have missed the first time.
// It touches ONLY s.cursorMs. Wallets, dq marks, payouts and the draw are all untouched — this is
// not an undo, it is "look at this stretch again."
function rewindCursor(to) {
  const s = load();
  const c = s.config;
  if (!c) return { ok: false, error: 'not_configured' };
  const toMs = parseTimeArg(to);
  if (!Number.isFinite(toMs)) return { ok: false, error: 'bad_time' };
  const before = s.cursorMs;
  const startMs = Number(c.startMs) || 0;
  // Clamp both ends: never before the promo started (nothing to rescan there), and never FORWARD
  // past where the cursor already is — rewind only ever re-opens tape, it must never be used to
  // skip a stretch that hasn't been scanned yet.
  const clamped = Math.max(startMs, Math.min(toMs, before));
  s.cursorMs = clamped;
  if (clamped !== before) s.gen = (s.gen || 0) + 1;   // supersede any scan already in flight
  save();
  return { ok: true, cursorMs: s.cursorMs, rewoundMs: Math.max(0, before - s.cursorMs) };
}
...
// Races a promise against a wall-clock timer. Used only when scanOnce() is given a deadline
// (see catchUp()'s P2 fix below) — never on the plain 5-minute tick, which has no timer at all
// and behaves exactly as before. Rejects with `message` on timeout; always clears the timer so a
// resolved tape request never leaves a dangling handle.
function withTimeout(promise, ms, message) {
  if (!(ms > 0)) return Promise.reject(new Error(message));
  let timer;
  const timeout = new Promise((_, reject) => { timer = setTimeout(() => reject(new Error(message)), ms); });
  return Promise.race([promise, timeout]).finally(() => { try { clearTimeout(timer); } catch (_) {} });
}

// opts.deadlineMs (Codex round 32 P2, "nothing should take 100 minutes ever" pt.2): catchUp()'s
// own budget was only checked BETWEEN complete scanOnce() calls, so one call's up-to-8-slice loop
// could itself run far past the budget if each tape request was slow (measured: a mocked 20s tape
// request took a 75s budget to 160s). When given, scanOnce() (a) stops STARTING a new slice once
// the deadline has passed — the cursor is left wherever it got to, same shape as the
// MAX_SLICES_PER_RUN cap — and (b) bounds each individual tape request with a timeout derived
// from the remaining time to that deadline; a timed-out request is treated exactly like an
// incomplete slice (does not advance the cursor past it, retried next tick) rather than a hard
// error, since the tape itself may well complete on a later attempt.
async function scanOnce(deps, opts) {
  const s = load();
  const c = s.config;
  ...
  const deadlineMs = (opts && Number.isFinite(opts.deadlineMs)) ? opts.deadlineMs : null;

  // The settle delay only matters while the window is still (or recently) open — once
  // now >= endMs + SETTLE_MS the subtraction no longer wins the min() and the ceiling is exactly
  // endMs, so a closed window still gets scanned all the way to its true end.
  const nowV = nowMs();
  const ceiling = Math.min(nowV - SETTLE_MS, c.endMs || nowV);
  if (s.cursorMs >= ceiling) return { ok: true, upToDate: true, cursorMs: s.cursorMs };

  // GENERATION CHECK, part 0 (Codex round 33 P1). `loadBars` is itself an await a concurrent
  // configure()/rewindCursor()/resetLedger() can outlive — captured BEFORE it, same pattern as
  // "GENERATION CHECK, part 1" below (which only guards the per-slice TAPE await). Missing this
  // one meant a reconfigure landing during the PRICE lookup went unnoticed: the local `c` still
  // pointed at the OLD config object (configure() replaces `s.config`, never mutates the object a
  // running scan is holding), so the scanner went on to query the old mint's tape, price it off
  // whatever bars this call loaded, and credit it into the ledger configure() had just reset or
  // repointed — advancing the cursor with no supersede ever reported.
  const genBeforePrices = s.gen || 0;
  await loadBars(c.pool);
  if ((s.gen || 0) !== genBeforePrices) {
    return {
      ok: true, superseded: true, cursorMs: s.cursorMs, slices: 0, trades: 0, newEntries: 0, newDq: 0,
      incompleteSlices: s.incompleteSlices || 0, stalled: false,
      deadlineHits: s.deadlineHits || 0, deadline: false,
  ...
  // Codex round 32 "second lens" P3: a deadline cutoff is an OPERATOR BUDGET decision, not a tape
  // fault — it must never be folded into incompleteSlices/stalled (which tell an operator "the
  // tape itself would not cooperate, go look at Helius"). Its own counter/flag keeps that message
  // honest; the cursor still doesn't advance past the cut-off slice either way.
  let deadlineHitThisRun = false;
  while (s.cursorMs < ceiling && slices < MAX_SLICES_PER_RUN) {
    if (deadlineMs != null && nowMs() >= deadlineMs) break;   // out of time — stop starting slices
    const from = s.cursorMs;
    const to = Math.min(from + SLICE_MS, ceiling);
    // GENERATION CHECK, part 1 (Codex round 32 P1). Captured before the await that can be
    // outlived by a concurrent rewindCursor()/resetLedger() call — see the check right after the
    // await, and the big comment on `gen` in blank().
    const genAtStart = s.gen || 0;
    let tape = null;
    try {
      const tapePromise = getTradeTapeHelius(c.mint, from, to, {
      ...
      if (deadlineMs != null) {
        tape = await withTimeout(tapePromise, deadlineMs - nowMs(), 'slice_timeout');
      } else {
        tape = await tapePromise;
      }
    } catch (e) {
      if (e && e.message === 'slice_timeout') {
        // Cursor treatment matches a slice the tape itself couldn't fully cover — keep whatever
        // was already credited, do NOT advance the cursor past this slice, retry it next tick —
        // but the REPORTING must not: this is a budget cutoff, not a tape fault (see the
        // `deadlineHitThisRun` comment above).
        deadlineHitThisRun = true;
        s.deadlineHits = (s.deadlineHits || 0) + 1;
        s.lastError = 'slice_timeout: ' + JSON.stringify({ from, to });
        slices++;
        break;
      }
    ...
    // GENERATION CHECK, part 2. A rewind/reset landed on the ledger while THIS slice's tape
    // request was in flight — the cursor it set must win. Crediting trades from a slice we did
    // manage to see would still be safe (every credit is deduped by signature), but this run's
    // own `to` must never be written over the rewound cursor, so stop here without touching it —
    // stale writes racing a supersede are simplest as "the whole slice never happened"; the next
    // tick re-walks it from wherever the rewind/reset left the cursor.
    //
    // Codex round 32 "second lens" P3: `gen` lives in `_mem`, this PROCESS's in-memory copy —
    // it only protects a rewind/reset against a scan racing it IN THE SAME PROCESS. Railway runs
    // more than one process; a scan's `save()` on process A can still overwrite a rewind another
    // process B wrote to disk moments earlier (an overlapping deploy is the likeliest window),
    // because A's `s`/`gen` were loaded before B's write landed and `load()`'s mtime check only
    // catches a re-read that happens to occur between them. That cross-process race is NOT closed
    // here; only the single-process one this comment is attached to is.
    if ((s.gen || 0) !== genAtStart) {
      s.scans++; s.lastScanAt = nowMs();
      if (!incompleteThisRun && !deadlineHitThisRun) s.lastError = null;
      save();
      return {
        ok: true, superseded: true, cursorMs: s.cursorMs, slices, trades, newEntries, newDq,
        incompleteSlices: s.incompleteSlices || 0, stalled: false,
        deadlineHits: s.deadlineHits || 0, deadline: false,
        behindMs: Math.max(0, ceiling - s.cursorMs),
      };
  ...
  s.scans++; s.lastScanAt = nowMs();
  // Clear lastError unless THIS run hit an incomplete slice or a deadline cutoff. Testing the
  // stored string instead left a recovered scanner reporting itself stalled forever, off the
  ...
  return {
    ...
    deadlineHits: s.deadlineHits || 0,
    deadline: deadlineHitThisRun,
    behindMs: Math.max(0, ceiling - s.cursorMs),
  };
}

// CATCH UP — the operator/scheduler lever for a multi-hour backlog (owner, 2026-09-24: "nothing
// should take 100 minutes ever"). scanOnce() deliberately caps itself at MAX_SLICES_PER_RUN so one
// call can't run for minutes; that is right for the steady-state 5-minute tick, but it means a
// &rewind= or &reset=1 that reopens a 14h stretch takes ~100 minutes to re-scan at the scheduler's
// own pace (8 slices x 5-minute tick), while looping scanOnce() directly caught the same stretch up
// in ~3 minutes on the night this was found. This is that loop, made a first-class function so both
// the admin route and the scheduler's self-heal can call it instead of hand-rolling one each.
//
// Stops on the first of three things:
//   - a scanOnce() call reports upToDate:true — done;
//   - the wall-clock budget (opts.budgetMs) runs out — the cursor is wherever it got to, and the
//     next call (another catchUp, or the plain 5-minute tick) picks up from there;
//   - a scanOnce() call returns ok:false — a real error (bad config, tape/RPC failure, no price
//     bars), surfaced rather than swallowed. The one exception is a slice the tape could not fully
//     cover (scanOnce() reports this as ok:true, stalled:true, and does NOT advance the cursor past
//     it — see the big comment in scanOnce()): that is worth retrying a few times in case it clears
//     on its own, but not worth burning the whole budget on if it never does, so three CONSECUTIVE
//     stalls stop the loop too.
// Never sleeps between calls — the tape is deduped by signature, so back-to-back calls are safe and
// free of the risk a delay would only slow down a real backlog.
async function catchUp(deps, opts) {
  const budgetMs = (opts && Number(opts.budgetMs)) || 4 * 60 * 1000;
  const startedAt = nowMs();
  // Codex round 32 P2: pass the SAME absolute deadline into every scanOnce() call so a slow tape
  // (or several) can't let one call's own up-to-8-slice loop blow through the whole budget before
  // catchUp() ever gets to check it again — scanOnce() now self-enforces it slice-by-slice.
  const deadlineMs = startedAt + budgetMs;
  let calls = 0, slices = 0, newEntries = 0, newDq = 0, consecutiveStalls = 0;
  let lastBehindMs = null;
  while (true) {
    if (nowMs() - startedAt >= budgetMs) {
      return { ok: true, calls, slices, newEntries, newDq, behindMs: lastBehindMs, upToDate: false, ms: nowMs() - startedAt };
    }
    const r = await scanOnce(deps, { deadlineMs });
    calls++;
    if (!r || !r.ok) {
      return { ok: false, error: (r && r.error) || 'no_result', detail: r && r.detail,
        calls, slices, newEntries, newDq, behindMs: (r && r.behindMs != null) ? r.behindMs : lastBehindMs,
        upToDate: false, ms: nowMs() - startedAt };
    }
    slices += r.slices || 0;
    newEntries += r.newEntries || 0;
    newDq += r.newDq || 0;
    lastBehindMs = r.behindMs != null ? r.behindMs : lastBehindMs;
    if (r.upToDate) {
      return { ok: true, calls, slices, newEntries, newDq, behindMs: 0, upToDate: true, ms: nowMs() - startedAt };
    }
    if (r.stalled) {
      consecutiveStalls++;
      if (consecutiveStalls >= 3) {
        return { ok: false, error: 'slice_incomplete', calls, slices, newEntries, newDq,
          behindMs: lastBehindMs, upToDate: false, ms: nowMs() - startedAt };
      }
    } else {
      consecutiveStalls = 0;
    }
...
async function postBoard(opts) {
  ...
  try {
    // POST, not GET. /api/tg-test has been POST-only since 2026-09-17 (#335: every form of it
    // sends, so the whole GET method answers 405) — and this hop kept sending a GET, so every
    // board refresh since then answered 405 → `send_failed`, silently, on the 5-minute tick. The
    // previous promo had already closed, so nothing showed it until the owner's 2026-09-22
    // birthday special opened with a board that never appeared. scripts/cuna-board-post-test.cjs
    // pins the method against a stub of the route.
    const r = await fetch(url, { method: 'POST', headers: { 'x-premium-key': key }, signal: AbortSignal.timeout(20000) });
    const j = await r.json();
...
async function payoutSweepPending({ rpcUrl } = {}) {
  ...
  rows.forEach(([wallet, rec], i) => {
    ...
    if (st && !st.err) { rec.pending = false; rec.confirmedAt = nowMs(); confirmed.push({ wallet, sig: rec.sig }); }
      ...
      // Landed as an error: the chain holds a definitive record that no tokens moved. Archive it.
      s.payoutsVoid = s.payoutsVoid || [];
      s.payoutsVoid.push({ round, wallet, ...rec, voidedAt: nowMs(), reason: 'tx_error' });
      delete bucket[wallet];
      cleared.push({ wallet, sig: rec.sig, reason: 'tx_error' });
    } else {
      // NOT FOUND stays pending, however old (second reviewer, 2026-09-15). The earlier rule —
      // "null and older than five minutes = never landed, clear it" — trusted one RPC node's
      // history: a lagging node, or one with a gap, answers null for a transfer that DID land, and
      // clearing it put a paid winner back in `owed`. Absence of evidence is not evidence of
      // absence on money. The operator clears a row they have checked on an explorer with
      // &unpay=<wallet>&sig=<sig>; nothing here does it for them.
      stillPending.push({ wallet, sig: rec.sig, ageMs: age,
        note: 'not found by this RPC — NOT cleared. Check the signature on an explorer; only if it truly never landed, &unpay=<wallet>&sig=<sig>.' });
    }
  });
...
module.exports = { manualDq, undoDq, checkWinnersStillHolding,
  configure, config, resetLedger, rewindCursor, scanOnce, catchUp, traceOutbound, standings,
