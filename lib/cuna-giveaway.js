...
function configure(patch) {
  ...
  // 'contest' = buy-and-hold, ranked and paid in TOKENS; 'giveaway' = the entry raffle.
  if (c.mode !== 'contest') c.mode = 'giveaway';
...
function boardText() {
  ...
  // CONTEST MODE (owner, 2026-08-26): ranked by CUMULATIVE TOKENS BOUGHT, bonus paid in tokens.
  if (c.mode === 'contest') return contestBoardText(st, c, sym, closed);
...
function contestBoardText(st, c, sym, closed) {
  const pct = Number(c.bonusPct) || 0;
  const L = [];
  L.push(closed ? '\uD83C\uDFC6 CUNA BUY CONTEST \u2014 BUY WINDOW CLOSED' : '\uD83C\uDFC6 CUNA BUY CONTEST \u2014 LIVE');
  L.push('');
  ...
  if (!st.top.length) {
    ...
    st.top.forEach((r, i) => {
      ...
      L.push((medal[i] || ('  ' + (i + 1) + '.')) + ' ' + short(r.wallet)
             + ' \u2014 ' + tk(r.tokens) + ' ' + sym
             + '  (+' + tk(bonus) + ' bonus)');
    });
  }
  ...
  L.push(closed
    ...
    : 'Every buy earns ' + pct + '% back in ' + sym + '. Hold 48h after close. Selling or moving tokens out voids it.');
  L.push('Updated every 10 min \u00B7 counted straight off the chain');
  return L.join('\n');
}
