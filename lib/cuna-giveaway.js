...
async function postBoard(opts) {
  ...
  // Pinning: tg-test can only pin the message it just sent, so each refresh re-pins the new
  // board. Deleting the old message also drops its pin, so without this a pinned board would
  // survive exactly one refresh. Pins go out silently (the send's own notification mode).
  ...
  const url = base + '/api/tg-test?key=' + encodeURIComponent(key)
    ...
    + (pin ? '&pin=1' : '')
    + (prev ? '&replaceMsg=' + encodeURIComponent(String(prev)) : '');
  try {
