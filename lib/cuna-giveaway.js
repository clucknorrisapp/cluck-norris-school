...
let _mem = null;
let _memMtime = 0;
// True once an EXISTING file failed to parse. Distinct from "file doesn't exist yet" (a
// genuinely fresh ledger, where blank() is correct) — a corrupt file must never be silently
// treated the same way, because the next save() would then overwrite it with a near-empty
// ledger and permanently erase every wallet's entries with no other copy anywhere. While
// true, save() refuses to touch disk (fail closed) until an operator restores the file.
let _corrupt = false;
function load() {
  ...
  try {
    ...
    if (e && e.code === 'ENOENT') {
      // No file yet — a genuinely fresh ledger, not corruption.
      if (!_mem) _mem = blank();
      return _mem;
    }
    // The file EXISTS but failed to read/parse. Quarantine a copy once, keep serving the
    // last good in-memory snapshot if this (warm) process has one, and disable persistence
    // until an operator restores the file — never fall back to blank() and let the next
    // save() overwrite a real ledger with an empty one.
    if (!_corrupt) {
      _corrupt = true;
      try { fs.copyFileSync(STATE_FILE(), STATE_FILE() + '.corrupt-' + Date.now()); } catch (_) {}
      console.error(`[cuna-giveaway] STATE FILE CORRUPT (${e.message}) — persistence DISABLED (fail-closed). Restore ${STATE_FILE()} from a good copy, then restart.`);
    }
    if (_mem) return _mem; // warm process — keep serving the last good in-memory copy
    _mem = blank();
    _mem.lastError = 'state_file_corrupt';
    return _mem;
  }
}
// Serializes the HANDLE the caller mutated, not the module-level `_mem` cache — those can
// diverge. load() swaps `_mem` for a fresh parse whenever the on-disk mtime has moved (another
// process/tick wrote in between), so a mutator that holds its `s = load()` handle across an
// await (runDraw's getSlot/getBlock, scanOnce/traceOutbound's Helius calls) could have an
// intervening load() (its own re-entry, or a concurrent standings()/postBoard() call in this
// same process) leave `s !== _mem` by the time it calls save() — silently dropping everything
// it just computed (a draw, a scan slice, a dq mark) while `_mem` (and the file) stay on the
// older snapshot. Always require the exact object that was mutated, and adopt it as the cache.
function save(s) {
  if (!s) { console.warn('[cuna-giveaway] save() called with no handle — refusing to write'); return; }
  _mem = s;
  if (_corrupt) {
    console.error('[cuna-giveaway] refusing to persist — state file is quarantined as corrupt; restore it and restart to resume saving.');
    return;
  }
  try {
...
async function checkWinnersStillHolding(deps) {
  ...
  for (const w of s.draw.winners) {
    ...
    const px = priceAt(nowMs()) || priceAt((s.config || {}).endMs) || 0;
    const holdUsd = bal * px;
    ...
    rows.push({
      ...
      balance: bal, holdUsd: Math.round(holdUsd * 100) / 100,
      heldPct: boughtUsd > 0 ? Math.round((holdUsd / boughtUsd) * 1000) / 10 : null,
      emptied: bal === 0,
      error: err,
    });
  }
  const flagged = rows.filter((r) => r.emptied || (r.heldPct !== null && r.heldPct < 50));
  return { ok: true, at: nowMs(), rows, flagged: flagged.length, flaggedWallets: flagged.map((r) => r.wallet) };
}
...
async function scanOnce(deps) {
  ...
  if (!_bars.list.length) {
    s.lastError = 'no_price_bars'; save(s);
  ...
  while (s.cursorMs < ceiling && slices < MAX_SLICES_PER_RUN) {
    ...
    try {
      ...
      s.lastError = 'tape:' + e.message; save(s);
      return { ok: false, error: 'tape_failed', detail: e.message, cursorMs: s.cursorMs };
    }
    if (!tape) { s.lastError = 'tape_null'; save(s); return { ok: false, error: 'tape_null', cursorMs: s.cursorMs }; }
    ...
    for (const t of (tape.trades || [])) {
      ...
      if (t.side === 'sell') {
        ...
        if (!rec.buys.some((b) => b.sig === t.sig)) {
          ...
          rec.buys.push({ ts: t.ts, usd: Math.round(usd * 100) / 100, tok, sig: t.sig });
...
async function traceOutbound(deps, opts) {
  ...
  for (const w of cands) {
    ...
    // A wallet whose scan the API couldn't complete is NOT proven clean — it must be visible
    // as coverage-unknown, not silently folded into "checked N" as if it passed.
  ...
  s.traced = { at: nowMs(), checked, dq, unknown, hops };
  save(s);
  return { ok: true, checked, dq, unknown, candidates: cands.length };
}
...
function setBoardMsgId(id) { const s = load(); s.boardMsgId = id || null; save(s); return s.boardMsgId; }
