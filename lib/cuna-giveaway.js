...
// round bucket is a map of wallet records and never does. They are folded into the CURRENT draw's
// round, which is correct because the flat shape only ever existed for one draw.
function migratePayouts(s) {
  const p = s.payouts;
  if (!p || typeof p !== 'object') return false;
  const legacy = Object.entries(p).filter(([, v]) => v && typeof v === 'object' && typeof v.sig === 'string');
  if (!legacy.length) return false;
  const round = String((s.draw && s.draw.seedHash) || 'legacy');
  p[round] = p[round] || {};
  for (const [wallet, rec] of legacy) {
    if (!p[round][wallet]) p[round][wallet] = rec;
    delete p[wallet];
  }
  return true;
}
...
function payoutOwed() {
  ...
  const owed = s.draw.winners
    ...
    .filter((w) => {
      ...
      const rec = (s.wallets || {})[w.wallet];
      if (rec && rec.dq) { dqd.push({ wallet: w.wallet, prize: Number(w.prize), reason: rec.dq.reason }); return false; }
      return true;
    })
