...
function configure(patch) {
  const s = load();
  const c = Object.assign({ minUsd: 5, symbol: 'CUNA' }, s.config || {}, patch || {});
...
function standings(n) {
  ...
  return {
    ...
    disqualified: out.length, disqualifiedList: out.slice(0, 50),
...
function boardText() {
  ...
  L.push('');
  if (!st.top.length) {
    L.push('No qualifying buys yet \u2014 every $' + (c.minUsd || 5) + '+ buy is one entry.');
  } else {
    const medal = ['\uD83E\uDD47', '\uD83E\uDD48', '\uD83E\uDD49'];
    st.top.forEach((r, i) => {
  ...
  L.push(closed
    ...
    : 'Every $' + (c.minUsd || 5) + '+ buy = 1 entry, no cap, they stack. Selling voids every entry you have.');
...
async function postBoard(opts) {
  ...
  // DROP AS A NEW MESSAGE (owner, 2026-08-22, after watching it live). The first build deleted
  // the previous board on each refresh. In a QUIET room that is visually identical to editing in
  // place — the old message vanishes, the new one lands in the same spot, and nothing reads as
  // having happened. The owner wants each refresh to land as its own visible drop, so the delete
  // is off by default now. Set boardReplace:true to go back to one-board-at-a-time.
  const pin = c.boardPin === true;
  const replace = c.boardReplace === true;
