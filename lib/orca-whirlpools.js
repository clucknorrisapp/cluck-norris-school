...
async function discoverPools() {
  const j = await orcaApi(`/pools?token=${CLKN_MINT}`);
  const pools = Array.isArray(j.data) ? j.data : [];
  const out = [];
  for (const p of pools) {
    const a = p.tokenA, b = p.tokenB;
    const clknIsA = a.address === CLKN_MINT;
    const quoteTok = clknIsA ? b : a;
    if (!QUOTE_TOKENS[quoteTok.address]) continue; // only CLKN/USDC + CLKN/SOL

    const priceBPerA = Number(p.price); // tokenB per tokenA, straight from Orca
    // CLKN price expressed in the quote token. If CLKN is tokenA, that's just
    // the raw price (quote per CLKN). If CLKN is tokenB, invert.
    const clknPriceInQuote = clknIsA ? priceBPerA : (priceBPerA > 0 ? 1 / priceBPerA : 0);

    out.push({
      address: p.address,
      config: p.whirlpoolsConfig,
      pair: `CLKN/${QUOTE_TOKENS[quoteTok.address]}`,
      quoteSymbol: QUOTE_TOKENS[quoteTok.address],
...
async function getPoolState(address, ownerPubkey) {
  ...
  const clknIsA = ta.mint.toBase58() === CLKN_MINT;
  const quoteMint = (clknIsA ? tb.mint : ta.mint).toBase58();
  const quoteSymbol = QUOTE_TOKENS[quoteMint] || "?";
...
function suggestRanges(st, widthPct) {
  ...
  // Map A/B-only to CLKN/quote-only based on which side CLKN sits on.
  const clknOnly = st.clknIsA
    ? { lowerTick: aLower, upperTick: aUpper, depositMint: CLKN_MINT, depositSymbol: "CLKN" }
    : { lowerTick: bLower, upperTick: bUpper, depositMint: CLKN_MINT, depositSymbol: "CLKN" };
  ...
  const balanced = { lowerTick: lowerWide, upperTick: upperWide, depositMint: null, depositSymbol: "CLKN + " + st.quoteSymbol };
  ...
  return {
    ...
    widthPct: w * 100,
    options: [
      decorate(clknOnly, "clkn", `Sell-side wall — deposit CLKN only (asks above ${fmt(st.clknPriceInQuote)})`),
...
async function quote({ address, owner, inputMint, inputAmount, lowerTick, upperTick, slippageBps }) {
  ...
  const q = increaseLiquidityQuoteByInputToken(
...
async function buildOpenPosition({ owner, address, lowerTick, upperTick, inputMint, inputAmount, slippageBps }) {
  ...
  const slippage = Percentage.fromFraction(Math.max(0, Math.min(5000, Number(slippageBps) || 100)), 10000);
  const liqInput = increaseLiquidityQuoteByInputToken(
    new PublicKey(inputMint),
    new Decimal(inputAmount),
    Number(lowerTick),
    Number(upperTick),
...
async function listPositions(owner) {
  const ownerPk = new PublicKey(owner);
  const ctx = getCtx(ownerPk);
  const client = buildWhirlpoolClient(ctx);

  const clknPools = new Set((await discoverPools()).map((p) => p.address));
  ...
  for (const { account } of resp.value) {
    ...
    if (!st) { st = await getPoolState(poolAddr, ownerPk); poolStateCache.set(poolAddr, st); }
