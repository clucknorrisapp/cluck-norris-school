...
async function poolWalls(poolAddress, tokenMint, spotUsd = null) {
  ...
  for (const a of accts) {
    ...
    if (sellUsd != null && buyUsd != null) {
      if (buyUsd < 0.02 * sellUsd) side = "sell";
      else if (sellUsd < 0.02 * buyUsd) side = "buy";
    } else if (tokenAmt > 0 && quoteAmt === 0) side = "sell";
    else if (quoteAmt > 0 && tokenAmt === 0) side = "buy";
    walls.push({
      ...
      tickLower, tickUpper, inRange: tickLower <= st.tickCurrentIndex && st.tickCurrentIndex < tickUpper,
      owner: null,
    });
  }
