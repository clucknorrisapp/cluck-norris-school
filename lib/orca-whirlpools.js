...
async function buildRepriceStep({ owner, poolAddress, targetTick, dustAmount = 10000 }) {
  ...
  // TOKEN-2022 GUARD (BULLEN Token-2022 audit, 2026-09-25): this function hand-builds the
  // LEGACY swapIx and derives both ATAs with splToken's default (legacy) program id — neither
  // is Token-2022-aware (no swapV2Ix, no transfer-hook extra accounts, wrong ATA for a
  // Token-2022 mint). Every other builder in this file resolves the token program from the
  // mint's own on-chain owner (see buildOpenPosition/buildClosePosition/etc, which delegate to
  // the SDK's TokenExtensionUtil); this one never did. Rather than guess at a partial fix for a
  // path nothing currently exercises (BULLEN's pools are created fresh, never repriced), fail
  // loudly instead of silently building a transaction that would error on submit (or worse,
  // succeed against the wrong accounts) — fix this properly (swapV2Ix + resolved ATAs) before
  // ever repricing a Token-2022 pool.
  const [ownerA, ownerB] = await Promise.all([
    ctx.connection.getAccountInfo(d.tokenMintA), ctx.connection.getAccountInfo(d.tokenMintB),
  ]);
  const TOKEN_2022 = "TokenzQdBNbLqP5VEhdkAS6EPFLC1PHnBqCXEpPxuEb";
  if ((ownerA && ownerA.owner.toBase58() === TOKEN_2022) || (ownerB && ownerB.owner.toBase58() === TOKEN_2022)) {
    throw new Error("buildRepriceStep does not support Token-2022 pools yet (needs swapV2Ix + resolved ATAs) — refusing rather than building a broken transaction");
  }
