...
const {
  ...
  decreaseLiquidityQuoteByLiquidity: decreaseLiquidityQuote,
  NO_TOKEN_EXTENSION_CONTEXT,
  WhirlpoolIx,
  SwapUtils,
} = require("@orca-so/whirlpools-sdk");
const { Percentage, DecimalUtil, TransactionBuilder } = require("@orca-so/common-sdk");
...
const CBBTC_MINT = "cbbtcf3aa214zXHbiAZQwf4122FBYbraNdFqgw4iMij"; // Coinbase wrapped BTC (8 decimals) — the deepest BTC on Orca
...
// CLKN/USDC, CLKN/SOL, CLKN/cbBTC and CLKN/JUP; everything else (XPX/FCKN pairs) is hidden by default.
const QUOTE_TOKENS = {
  [USDC_MINT]: "USDC",
  [WSOL_MINT]: "SOL",
  [CBBTC_MINT]: "BTC",
...
const DEFAULT_TOKEN = { mint: CLKN_MINT, symbol: "CLKN", quoteMints: [USDC_MINT, WSOL_MINT, CBBTC_MINT, JUP_MINT] };
...
// Mainnet RPC — resilient: the project's Helius key with automatic failover to
// any configured backup RPC and the public node (see lib/rpc.js). So a primary
// 429 / outage no longer blinds the reads/builds.
...
function getCtx(ownerPubkey) {
  const connection = rpcConnection("confirmed");
...
async function orcaApi(path) {
  const r = await fetch(ORCA_API + path, { headers: { accept: "application/json" }, signal: AbortSignal.timeout(10000) });
...
function suggestRanges(st, widthPct) {
  const w = Math.max(0.05, Math.min(95, Number(widthPct) || 15)) / 100;
  const cur = st.priceBPerA;
  const { decimalsA: dA, decimalsB: dB, tickSpacing: sp, tickCurrentIndex: curTick } = st;

  let lowerWide = alignTick(cur * (1 - w), dA, dB, sp);
  let upperWide = alignTick(cur * (1 + w), dA, dB, sp);
  // Guard against a degenerate (sub-spacing) balanced range at tight widths: keep at least
  // one tick-spacing on each side of price, so tight ranges work on fine-spacing pools
  // (0.02% → spacing 2) without ever collapsing to zero width on coarse-spacing pools.
  const curAligned = alignTick(cur, dA, dB, sp);
  if (lowerWide >= curAligned) lowerWide = curAligned - sp;
  if (upperWide <= curAligned) upperWide = curAligned + sp;
  ...
  // Honesty: the REQUESTED width (w) and the REALIZED width can differ — alignTick
  // snaps to the pool's tick spacing and the tight-width guards force ≥1 spacing per
  // side. Report the realized band (from the actual balanced ticks) so the label and
  // the "±X%" headline match what actually gets deposited, not the slider request.
  const curP = st.clknPriceInQuote;
  const bLowP = tickToClknPrice(lowerWide, st), bUpP = tickToClknPrice(upperWide, st);
  const lo = Math.min(bLowP, bUpP), hi = Math.max(bLowP, bUpP);
  const realizedWidthPct = curP > 0 ? ((hi - lo) / 2 / curP) * 100 : w * 100;
  const wlabel = realizedWidthPct < 1 ? realizedWidthPct.toFixed(2) : realizedWidthPct.toFixed(1);

  return {
    currentPriceClkn: st.clknPriceInQuote,
    quoteSymbol: st.quoteSymbol,
    widthPct: w * 100,               // the requested width (slider echo)
    realizedWidthPct,                // the actual band after tick-alignment + guards
    options: [
      ...
      decorate(balanced, "balanced", `Balanced — both sides, ±${wlabel}% around price`),
    ],
  };
}

// Asymmetric ultra-wide "anchor" range: a band from downPct below to upPct above the
// current CLKN price (e.g. 85, 400 → −85% / +400%). Mirrors suggestRanges' tick math
// (alignTick handles decimals + the clknIsA inversion) but allows an ASYMMETRIC band,
// and forces the band to straddle the current price so the position is two-sided and
// stays continuously quotable. For a tiny, never-closed "keep the pool alive" backstop.
function suggestAnchorRange(st, downPct, upPct) {
  const { decimalsA: dA, decimalsB: dB, tickSpacing: sp } = st;
  const down = Math.max(1, Math.min(99, Number(downPct) || 80)) / 100;   // fraction below price
  const up = Math.max(0.1, Number(upPct) || 400) / 100;                  // fraction above price
  const cur = st.clknPriceInQuote;
  const toBPerA = (clknP) => (st.clknIsA ? clknP : (clknP > 0 ? 1 / clknP : 0));
  let t1 = alignTick(toBPerA(cur * (1 - down)), dA, dB, sp);
  let t2 = alignTick(toBPerA(cur * (1 + up)), dA, dB, sp);
  let lowerTick = Math.min(t1, t2), upperTick = Math.max(t1, t2);
  // Clamp to Orca's tick bounds, aligned to spacing (defensive — not hit at sane widths).
  const MIN = Math.ceil(-443636 / sp) * sp, MAX = Math.floor(443636 / sp) * sp;
  if (lowerTick < MIN) lowerTick = MIN;
  if (upperTick > MAX) upperTick = MAX;
  // Force a straddle of the current price so it's a two-sided (quotable) position.
  const curAligned = alignTick(st.priceBPerA, dA, dB, sp);
  if (lowerTick >= curAligned) lowerTick = curAligned - sp;
  if (upperTick <= curAligned) upperTick = curAligned + sp;
  const p1 = tickToClknPrice(lowerTick, st), p2 = tickToClknPrice(upperTick, st);
  return { lowerTick, upperTick, lowerPriceClkn: Math.min(p1, p2), upperPriceClkn: Math.max(p1, p2) };
}

// Convert an explicit CLKN-price band to aligned tick bounds (NO straddle forcing) — for a
// single-sided wall. A band ENTIRELY above the current CLKN price = a CLKN-only sell wall
// (deposit CLKN; sells into the quote as price rises through it); entirely below = quote-only
// buy wall. Caller validates which side; this just maps prices → ticks honouring clknIsA.
function ticksForClknRange(st, loClkn, hiClkn) {
  const { decimalsA: dA, decimalsB: dB, tickSpacing: sp } = st;
  const toBPerA = (clknP) => (st.clknIsA ? clknP : (clknP > 0 ? 1 / clknP : 0));
  let t1 = alignTick(toBPerA(loClkn), dA, dB, sp);
  let t2 = alignTick(toBPerA(hiClkn), dA, dB, sp);
  let lowerTick = Math.min(t1, t2), upperTick = Math.max(t1, t2);
  if (upperTick <= lowerTick) upperTick = lowerTick + sp;
  const p1 = tickToClknPrice(lowerTick, st), p2 = tickToClknPrice(upperTick, st);
  return { lowerTick, upperTick, lowerPriceClkn: Math.min(p1, p2), upperPriceClkn: Math.max(p1, p2) };
}
...
async function buildOpenPosition({ owner, address, lowerTick, upperTick, inputMint, inputAmount, slippageBps, forWallet }) {
  ...
  if (initBuilder) txs.push(await serializeUnsigned(ctx, initBuilder, ownerPk, forWallet));
  ...
  // vault (forWallet unset): openSer is a base64 STRING → txBase64/txs identical to before.
  // browser (forWallet true): openSer is { txBase64, signerSecrets } and txs carries those objects
  // so the client can sign wallet-FIRST then partialSign the ephemerals.
  const openSer = await serializeUnsigned(ctx, tx, ownerPk, forWallet);
  txs.push(openSer);
  return {
    txBase64: forWallet ? openSer.txBase64 : openSer,   // the open tx (kept for callers that don't need tick-array init)
...
// ── Estimate the ON-CHAIN RENT COST of REOPENING a position at a given range ───────────────
// Real numbers read live from the cluster, never hardcoded constants — used by the vault
// (lib/whirlpool-vault.js tick/tickSol) BEFORE closing a position, so a roll never closes a
// position it then can't afford to reopen (a ±1.5% recenter at tickSpacing 1 can need a BRAND
// NEW tick array, ~0.05 SOL rent, that a shallow reserve doesn't budget for — production
// incident, BULLEN, 2026-09-25: a stranded empty pool + evenPools refusing to run with a pool
// down). Counts: tick arrays the target range doesn't already have initialized (checked
// on-chain via the same TickArrayUtil path openPosition itself uses — never assumed 0), one
// fresh position account + its position-NFT token account (a reopen always mints a new NFT),
// and each pool side's ATA if the owner doesn't already hold one.
async function estimateReopenLamports({ owner, address, lowerTick, upperTick }) {
  const ownerPk = new PublicKey(owner);
  const ctx = getCtx(ownerPk);
  const client = buildWhirlpoolClient(ctx);
  const pool = await client.getPool(new PublicKey(address), IGNORE_CACHE);
  const d = pool.getData();
  const conn = ctx.connection;

  // Tick arrays: reuse the SDK's own gap-check (the same one buildOpenPosition/serializeUnsigned
  // uses) so "how many are missing" is never a guess — it returns one addInstruction() call per
  // missing array (0, 1 or 2 for a two-tick range), or null if both already exist.
  const initBuilder = await pool.initTickArrayForTicks([Number(lowerTick), Number(upperTick)], ownerPk);
  const tickArraysNeeded = initBuilder ? initBuilder.instructions.length : 0;

  const [tickArrayRent, positionRent, positionAtaRent] = await Promise.all([
    conn.getMinimumBalanceForRentExemption(ctx.program.account.dynamicTickArray.size),
    conn.getMinimumBalanceForRentExemption(ctx.program.account.position.size),
    // The position-NFT token account is Token-2022 on current Orca (openPositionWithTokenExtensionsIx)
    // — a hair bigger than a legacy ATA (carries ImmutableOwner). Use the larger figure so this
    // never under-budgets.
    conn.getMinimumBalanceForRentExemption(170),
  ]);

  // Each pool side's ATA — only costs rent if the owner doesn't already hold one. Token program
  // is read from the mint's own on-chain owner, never assumed legacy (Token-2022 audit, 2026-09-25).
  const [mintAInfo, mintBInfo] = await Promise.all([conn.getAccountInfo(d.tokenMintA), conn.getAccountInfo(d.tokenMintB)]);
  const ataFor = (mint, programId) => splToken.getAssociatedTokenAddressSync(mint, ownerPk, false, programId);
  const ataA = ataFor(d.tokenMintA, mintAInfo.owner);
  const ataB = ataFor(d.tokenMintB, mintBInfo.owner);
  const [ataAInfo, ataBInfo] = await Promise.all([conn.getAccountInfo(ataA), conn.getAccountInfo(ataB)]);
  const ataRentEach = await conn.getMinimumBalanceForRentExemption(170);   // worst case (Token-2022); fine if actually legacy (smaller)
  const ataANeeded = !ataAInfo, ataBNeeded = !ataBInfo;

  const totalLamports = tickArraysNeeded * tickArrayRent + positionRent + positionAtaRent
    + (ataANeeded ? ataRentEach : 0) + (ataBNeeded ? ataRentEach : 0);
  return {
    tickArraysNeeded, tickArrayRentLamports: tickArrayRent,
    positionRentLamports: positionRent, positionAtaRentLamports: positionAtaRent,
    ataANeeded, ataBNeeded, ataRentEachLamports: ataRentEach,
    totalLamports, totalSol: totalLamports / 1e9,
  };
}
...
const FEE_TIER_SPACING = { 0.01: 1, 0.02: 2, 0.05: 8, 0.08: 8, 0.16: 16, 0.3: 64, 0.65: 96, 1: 128 };
...
// ── Reprice an EMPTY pool: walk its stale tick to market with price-limited swaps ──
// A pool with ZERO active liquidity can't fill anything — a swap with a sqrt-price
// limit just moves tickCurrentIndex to the limit at the cost of a tx fee. That safely
// re-anchors a stale-priced empty pool to the live market so the vault can manage it
// (the price-gap guard skips pools quoting far off market). Two hard constraints:
//   1. REFUSED outright if the pool has any active liquidity — the same walk through a
//      live book would actually trade against it.
//   2. One swap can only traverse the 3 tick-arrays it's given (88*tickSpacing ticks
//      each), so a long walk is a LOOP of steps; the caller sends one tx per step.
// The pool's tick arrays are typically uninitialized (never LP'd) — fine: the deployed
// whirlpool program's sparse-swap path treats them as empty spans.
async function buildRepricePlan({ tokenMint, quoteMint, feeTierPct, tokenPriceInQuote }) {
  const poolAddress = poolAddressFor({ tokenMint, quoteMint, feeTierPct });
  const ctx = getCtx();
  const client = buildWhirlpoolClient(ctx);
  const pool = await client.getPool(new PublicKey(poolAddress), IGNORE_CACHE);
  const d = pool.getData();
  const tokenIsA = PoolUtil.compareMints(new PublicKey(tokenMint), new PublicKey(quoteMint)) < 0;
  const decA = await getMintDecimals(ctx.connection, d.tokenMintA.toBase58());
  const decB = await getMintDecimals(ctx.connection, d.tokenMintB.toBase58());
  const priceBPerA = tokenIsA ? Number(tokenPriceInQuote) : 1 / Number(tokenPriceInQuote);
  const targetTick = PriceMath.priceToTickIndex(new Decimal(priceBPerA), decA, decB);
  const arraySpan = 88 * d.tickSpacing;
  return {
    poolAddress, currentTick: d.tickCurrentIndex, targetTick, tickSpacing: d.tickSpacing,
    liquidityZero: d.liquidity.isZero(), liquidity: d.liquidity.toString(),
    estSteps: Math.max(1, Math.ceil(Math.abs(targetTick - d.tickCurrentIndex) / (arraySpan * 3))),
  };
}
async function buildRepriceStep({ owner, poolAddress, targetTick, dustAmount = 10000 }) {
  const ownerPk = new PublicKey(owner);
  const ctx = getCtx(ownerPk);
  const client = buildWhirlpoolClient(ctx);
  const poolPk = new PublicKey(poolAddress);
  const pool = await client.getPool(poolPk, IGNORE_CACHE);
  const d = pool.getData();
  if (!d.liquidity.isZero()) throw new Error("pool has active liquidity — a price-walk would trade against it; refusing");
  // TOKEN-2022 (BULLEN, 2026-09-25): the token program is read from each mint's own on-chain
  // owner, never assumed legacy. A Token-2022 side takes swapV2Ix with the resolved programs and
  // Token-2022 ATAs; a pure-legacy pair keeps the original swapIx path byte for byte. A mint with
  // a transfer hook is refused — the dust walk passes no extra hook accounts. Needed live: BULLEN's
  // pools sold out above their range and sat pinned at the edge while the market ran 7% higher,
  // and an empty pool can only be walked back to market with this tool.
  const [infoA, infoB] = await Promise.all([
    ctx.connection.getAccountInfo(d.tokenMintA), ctx.connection.getAccountInfo(d.tokenMintB),
  ]);
  if (!infoA || !infoB) throw new Error("could not read a pool mint's account — refusing to build blind");
  const TOKEN_2022 = splToken.TOKEN_2022_PROGRAM_ID;
  const progA = infoA.owner, progB = infoB.owner;
  const isT22 = progA.equals(TOKEN_2022) || progB.equals(TOKEN_2022);
  if (isT22) {
    for (const [mint, info] of [[d.tokenMintA, infoA], [d.tokenMintB, infoB]]) {
      if (!info.owner.equals(TOKEN_2022)) continue;
      const m = splToken.unpackMint(mint, info, TOKEN_2022);
      if (splToken.getTransferHook(m)) throw new Error(`mint ${mint.toBase58().slice(0, 8)}… has a transfer hook — the reprice walk passes no hook accounts; refusing`);
    }
  }
  const cur = d.tickCurrentIndex;
  const target = Math.trunc(Number(targetTick));
  if (cur === target) return { done: true, tick: cur };
  const aToB = target < cur;   // price down = swapping A in; price up = swapping B in
  // Clamp this step to the far edge of the 3-array window the swap ix can traverse.
  const arraySpan = 88 * d.tickSpacing;
  const windowStart = Math.floor(cur / arraySpan) * arraySpan;
  const stepTick = aToB
    ? Math.max(target, windowStart - 2 * arraySpan)
    : Math.min(target, windowStart + 3 * arraySpan - 1);
  const tickArrays = SwapUtils.getTickArrayPublicKeys(cur, d.tickSpacing, aToB, ORCA_WHIRLPOOL_PROGRAM_ID, poolPk);
  if (isT22) {
    // Both owner token accounts must exist for the swap to validate, even though an empty pool
    // moves nothing. Create any that are missing (idempotent), and close a WSOL one created here
    // in the same tx so the walk leaves no stray rent behind (it holds 0 after an empty swap).
    const ataA = splToken.getAssociatedTokenAddressSync(d.tokenMintA, ownerPk, false, progA);
    const ataB = splToken.getAssociatedTokenAddressSync(d.tokenMintB, ownerPk, false, progB);
    const [haveA, haveB] = await Promise.all([ctx.connection.getAccountInfo(ataA), ctx.connection.getAccountInfo(ataB)]);
    const builder = new TransactionBuilder(ctx.connection, readonlyWallet(ownerPk));
    const cleanup = [];
    for (const [have, ata, mint, prog] of [[haveA, ataA, d.tokenMintA, progA], [haveB, ataB, d.tokenMintB, progB]]) {
      if (have) continue;
      builder.addInstruction({ instructions: [splToken.createAssociatedTokenAccountIdempotentInstruction(ownerPk, ata, ownerPk, mint, prog)], cleanupInstructions: [], signers: [] });
      if (mint.equals(splToken.NATIVE_MINT)) cleanup.push(splToken.createCloseAccountInstruction(ata, ownerPk, ownerPk, [], prog));
    }
    const ixV2 = WhirlpoolIx.swapV2Ix(ctx.program, {
      amount: new BN(dustAmount), otherAmountThreshold: new BN(0),
      sqrtPriceLimit: PriceMath.tickIndexToSqrtPriceX64(stepTick),
      amountSpecifiedIsInput: true, aToB,
      whirlpool: poolPk, tokenAuthority: ownerPk,
      tokenMintA: d.tokenMintA, tokenMintB: d.tokenMintB,
      tokenOwnerAccountA: ataA, tokenVaultA: d.tokenVaultA,
      tokenOwnerAccountB: ataB, tokenVaultB: d.tokenVaultB,
      tokenProgramA: progA, tokenProgramB: progB,
      tickArray0: tickArrays[0], tickArray1: tickArrays[1], tickArray2: tickArrays[2],
      oracle: PDAUtil.getOracle(ORCA_WHIRLPOOL_PROGRAM_ID, poolPk).publicKey,
    });
    builder.addInstruction(ixV2);
    if (cleanup.length) builder.addInstruction({ instructions: cleanup, cleanupInstructions: [], signers: [] });
    const txBase64 = await serializeUnsigned(ctx, builder, ownerPk);
    return { done: false, fromTick: cur, stepTick, targetTick: target, txBase64 };
  }
  const ix = WhirlpoolIx.swapIx(ctx.program, {
    ...
    amountSpecifiedIsInput: true, aToB,
    whirlpool: poolPk, tokenAuthority: ownerPk,
    tokenOwnerAccountA: splToken.getAssociatedTokenAddressSync(d.tokenMintA, ownerPk),
    tokenVaultA: d.tokenVaultA,
    tokenOwnerAccountB: splToken.getAssociatedTokenAddressSync(d.tokenMintB, ownerPk),
    tokenVaultB: d.tokenVaultB,
    tickArray0: tickArrays[0], tickArray1: tickArrays[1], tickArray2: tickArrays[2],
    oracle: PDAUtil.getOracle(ORCA_WHIRLPOOL_PROGRAM_ID, poolPk).publicKey,
  });
  const builder = new TransactionBuilder(ctx.connection, readonlyWallet(ownerPk)).addInstruction(ix);
  const txBase64 = await serializeUnsigned(ctx, builder, ownerPk);
  return { done: false, fromTick: cur, stepTick, targetTick: target, txBase64 };
}
...
async function buildClosePosition({ owner, positionMint, slippageBps, forWallet }) {
  const ownerPk = new PublicKey(owner);
  const ctx = getCtx(ownerPk);
  const client = buildWhirlpoolClient(ctx);
  ...
  const txs = [];
  for (const b of builders) txs.push(await serializeUnsigned(ctx, b, ownerPk, forWallet));
  return { txs };
}

// ── Add liquidity to an EXISTING position (no close/reopen) ───────────────────
// Deposits more into a position's SAME range via increase_liquidity — used to top up
// a sleeve with fresh funds without burning + re-minting the NFT (cheaper, no IL
// realization). inputMint is one side; the SDK pulls the matching amount of the other.
async function buildIncreaseLiquidity({ owner, positionMint, inputMint, inputAmount, slippageBps }) {
  const ownerPk = new PublicKey(owner);
  const ctx = getCtx(ownerPk);
  const client = buildWhirlpoolClient(ctx);
  const positionPda = PDAUtil.getPosition(ORCA_WHIRLPOOL_PROGRAM_ID, new PublicKey(positionMint)).publicKey;
  ...
  const pool = await client.getPool(pd.whirlpool);
  const decA = pool.getTokenAInfo().decimals;
  const decB = pool.getTokenBInfo().decimals;
  const slippage = Percentage.fromFraction(Math.max(0, Math.min(5000, Number(slippageBps) || 100)), 10000);
  const liqInput = increaseLiquidityQuote(
  ...
  const builder = await position.increaseLiquidity(liqInput, true, ownerPk, ownerPk, ownerPk);
  const txBase64 = await serializeUnsigned(ctx, builder, ownerPk);
  return {
    txs: [txBase64], txBase64,
    estA: DecimalUtil.fromBN(liqInput.tokenEstA, decA).toString(),
    estB: DecimalUtil.fromBN(liqInput.tokenEstB, decB).toString(),
    maxA: DecimalUtil.fromBN(liqInput.tokenMaxA, decA).toString(),
    maxB: DecimalUtil.fromBN(liqInput.tokenMaxB, decB).toString(),
...
// Withdraws `liquidityPct` (0..1) of a position's liquidity back to the wallet via
// decrease_liquidity — keeps the position open (same range). Used to shift capital
// between sleeves (e.g. pull from the wide backbone into the tight sleeve when calm)
// and to free treasury reserves, without burning the NFT.
async function buildDecreaseLiquidity({ owner, positionMint, liquidityPct, slippageBps }) {
  const ownerPk = new PublicKey(owner);
  const ctx = getCtx(ownerPk);
  const client = buildWhirlpoolClient(ctx);
  const positionPda = PDAUtil.getPosition(ORCA_WHIRLPOOL_PROGRAM_ID, new PublicKey(positionMint)).publicKey;
  ...
  const decA = pool.getTokenAInfo().decimals;
  const decB = pool.getTokenBInfo().decimals;
  const pct = Math.max(0, Math.min(1, Number(liquidityPct) || 0));
  const liqToRemove = pd.liquidity.mul(new BN(Math.round(pct * 1e6))).div(new BN(1e6));
  if (liqToRemove.lten(0)) throw new Error("nothing to remove (zero liquidity)");
  const slippage = Percentage.fromFraction(Math.max(0, Math.min(5000, Number(slippageBps) || 100)), 10000);
  const quote = decreaseLiquidityQuote(liqToRemove, slippage, position, pool, NO_TOKEN_EXTENSION_CONTEXT);
  const builder = await position.decreaseLiquidity(quote, true, ownerPk, ownerPk, ownerPk);
  const txBase64 = await serializeUnsigned(ctx, builder, ownerPk);
  return {
    txs: [txBase64], txBase64, liquidityPct: pct,
    estA: DecimalUtil.fromBN(quote.tokenEstA, decA).toString(),
    estB: DecimalUtil.fromBN(quote.tokenEstB, decB).toString(),
  };
}
...
async function serializeUnsigned(ctx, txBuilder, ownerPk, forWallet) {
  ...
  if (forWallet) {
    // BROWSER (wallet) path: do NOT pre-sign the ephemeral signers here. Phantom's Lighthouse
    // flags a multi-signer tx whose ephemeral signature sits AHEAD of the connected wallet's
    // (see public/locker-room.html + lib/jup-lock.js — the reference implementation). Instead we
    // hand the ephemeral secrets to the client, which signs wallet-FIRST then partialSigns the
    // ephemerals. These ephemerals are throwaway position/mint keypairs that hold no funds and no
    // authority, so exposing their secret to the client is safe (same rationale as jup-lock.js).
    return {
      txBase64: transaction.serialize({ requireAllSignatures: false, verifySignatures: false }).toString("base64"),
      signerSecrets: ephemeral.map((k) => Buffer.from(k.secretKey).toString("base64")),
    };
  }
  // SERVER (vault / operator) path — UNCHANGED: no browser wallet is involved, the operator
  // keypair signs server-side (whirlpool-vault.js), so pre-signing the ephemerals here is correct
  // and required. Returns a bare base64 string, exactly as before, so the vault path is untouched.
...
async function listPositions(owner, tok = DEFAULT_TOKEN) {
  ...
  // Allowed quote mints for this token (its configured pools). We identify which
  // positions belong to the token by reading each position's pool ON-CHAIN (its mints),
  // not via the Orca API — so a brand-new pool not yet API-indexed is recognised at once.
  const allow = new Set((tok.quoteMints && tok.quoteMints.length) ? tok.quoteMints : Object.keys(QUOTE_TOKENS));
  // Position NFTs can be minted under the legacy Token program OR Token-2022 — Orca's
  // newer (concentrated) positions use Token-2022, so scanning only the legacy program
  // silently drops them. Query both and merge.
  const TOKEN_PROGRAM_ID = new PublicKey("TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA");
  const TOKEN_2022_PROGRAM_ID = new PublicKey("TokenzQdBNbLqP5VEhdkAS6EPFLC1PHnBqCXEpPxuEb");
  const [respLegacy, resp2022] = await Promise.all([
    ctx.connection.getParsedTokenAccountsByOwner(ownerPk, { programId: TOKEN_PROGRAM_ID }),
    ctx.connection.getParsedTokenAccountsByOwner(ownerPk, { programId: TOKEN_2022_PROGRAM_ID }).catch(() => ({ value: [] })),
  ]);
  const resp = { value: [...respLegacy.value, ...resp2022.value] };
  ...
  for (const { account } of resp.value) {
    ...
    if (!st) {
      try { st = await getPoolState(poolAddr, ownerPk, tok); } catch { continue; }
      poolStateCache.set(poolAddr, st);
    }
    // Keep only positions in a pool pairing THIS token with one of its allowed quotes,
    // checked on-chain via the pool's mints (works even before Orca's API indexes the pool).
    const involvesToken = st.mintA === tok.mint || st.mintB === tok.mint;
    const quoteM = st.mintA === tok.mint ? st.mintB : st.mintA;
    if (!involvesToken || !allow.has(quoteM)) continue;
...
// Read EVERY position in a pool (any owner) and classify single-sided ones as
// sell/buy "walls" — Orca's limit orders are single-sided range positions. A
// position holding only the target token = sell-side; only quote = buy-side;
// both = two-sided LP. Composition is read at the live price via the SDK math,
// so it's orientation-agnostic. spotUsd (optional) adds USD sizing.
async function poolWalls(poolAddress, tokenMint, spotUsd = null) {
  const st = await getPoolState(poolAddress, null, { mint: tokenMint, symbol: "TKN" });
  const quoteUsd = (spotUsd && st.tokenPriceInQuote > 0) ? spotUsd / st.tokenPriceInQuote : null;
  const conn = rpcConnection("confirmed");
  let accts = [];
  try {
    accts = await conn.getProgramAccounts(ORCA_WHIRLPOOL_PROGRAM_ID, {
      commitment: "confirmed",
      filters: [{ dataSize: 216 }, { memcmp: { offset: 8, bytes: poolAddress } }], // Position accts, this pool
    });
  } catch (_) {}
  const curSqrt = new BN(st.sqrtPrice);
  const walls = [];
  for (const a of accts) {
    ...
    if (!buf || buf.length < 96) continue;
    const liquidity = new BN(buf.subarray(72, 88), "le");
    if (liquidity.isZero()) continue;
    const tickLower = buf.readInt32LE(88), tickUpper = buf.readInt32LE(92);
    if (tickLower >= tickUpper) continue;
    let aUi = 0, bUi = 0;
    try {
      const amts = PoolUtil.getTokenAmountsFromLiquidity(liquidity, curSqrt, PriceMath.tickIndexToSqrtPriceX64(tickLower), PriceMath.tickIndexToSqrtPriceX64(tickUpper), false);
      aUi = Number(DecimalUtil.fromBN(amts.tokenA, st.decimalsA).toString());
      bUi = Number(DecimalUtil.fromBN(amts.tokenB, st.decimalsB).toString());
    } catch { continue; }
    const tokenAmt = st.tokenIsA ? aUi : bUi;
    const quoteAmt = st.tokenIsA ? bUi : aUi;
    const sellUsd = spotUsd != null ? tokenAmt * spotUsd : null;
    const buyUsd = quoteUsd != null ? quoteAmt * quoteUsd : null;
    // A position is a resting "wall" (limit-order-like) ONLY when price has fully
    // exited its range (single-sided). While price is in-range it's active two-sided
    // LP — classifying by tick position (not raw composition) avoids flip-flop right
    // at the edge that would otherwise fire spurious appear/fill alerts.
    const inRange = tickLower <= st.tickCurrentIndex && st.tickCurrentIndex < tickUpper;
    const side = inRange ? "two-sided" : ((sellUsd || 0) >= (buyUsd || 0) ? "sell" : "buy");
    walls.push({
      positionPubkey: a.pubkey.toBase58(), positionMint: new PublicKey(buf.subarray(40, 72)).toBase58(), side,
      tokenAmount: tokenAmt, quoteAmount: quoteAmt,
      sizeUsd: side === "sell" ? sellUsd : side === "buy" ? buyUsd : ((sellUsd || 0) + (buyUsd || 0)) || null,
      priceLow: tickToClknPrice(tickLower, st), priceHigh: tickToClknPrice(tickUpper, st),
      tickLower, tickUpper, inRange,
      owner: null,
    });
  }
  // Resolve the owner (position-NFT holder) of each position — so a third-party
  // wall can be told apart from the project's own LP, and two-sided LP is
  // attributable too. Capped to keep RPC light.
  for (const w of walls.slice(0, 40)) {
    try {
      const la = await conn.getTokenLargestAccounts(new PublicKey(w.positionMint));
      const acct = (la.value || []).find(x => x.amount === "1") || (la.value || [])[0];
      if (acct) { const ai = await conn.getParsedAccountInfo(acct.address); w.owner = ai?.value?.data?.parsed?.info?.owner || null; }
    } catch (_) {}
  }
  return { pool: poolAddress, quoteSymbol: st.quoteSymbol, quoteMint: st.quoteMint, tokenPriceInQuote: st.tokenPriceInQuote, feeTierPct: st.feeTierPct, tickCurrentIndex: st.tickCurrentIndex, count: walls.length, walls };
}

module.exports = {
  CLKN_MINT, USDC_MINT, WSOL_MINT, CBBTC_MINT, JUP_MINT,
  DEFAULT_TOKEN,
  discoverPools,
  getPoolState,
  poolWalls,
