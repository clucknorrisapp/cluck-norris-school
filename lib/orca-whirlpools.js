...
async function poolWalls(poolAddress, tokenMint, spotUsd = null) {
  ...
  // Resolve the owner (position-NFT holder) of each single-sided wall so a
  // third-party position can be told apart from the project's own LP. Capped to
  // keep RPC light (two-sided LP is skipped — it's not a "limit order").
  const single = walls.filter(w => w.side === "sell" || w.side === "buy").slice(0, 25);
  ...
  return { pool: poolAddress, quoteSymbol: st.quoteSymbol, tokenPriceInQuote: st.tokenPriceInQuote, tickCurrentIndex: st.tickCurrentIndex, count: walls.length, walls };
}
