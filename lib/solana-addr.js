...
// Split into its own Node-free module (lib/sol-addr-re.js) so a consumer that needs only the
// address-shape regex — not the PDA/address math below, which needs Buffer/crypto — never pulls
// this whole file's Node builtins in with it. Re-exported here unchanged, not redefined, so
// there is exactly one regex, not two that could drift.
const { SOL_ADDR_RE } = require("./sol-addr-re");
