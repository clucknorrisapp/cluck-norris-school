...
async function scanOrcaWalls(mint, spotUsd) {
  ...
  for (const pool of pools.slice(0, 6)) {
    ...
    poolPrices.push({ pool, quote: res.quoteSymbol, tokenPriceInQuote: poolSpotQ, priceUsd: (poolSpotQ > 0 && quoteUsd) ? poolSpotQ * quoteUsd : null });
  ...
  // (e.g. CLKN priced differently on its SOL vs USDC vs JUP pool when a quote moves).
  const priced = poolPrices.filter(p => p.priceUsd > 0);
  let dislocation = null;
  if (priced.length >= 2) {
    const mx = Math.max(...priced.map(p => p.priceUsd)), mn = Math.min(...priced.map(p => p.priceUsd));
    dislocation = { spreadPct: (mx / mn - 1) * 100, high: priced.find(p => p.priceUsd === mx), low: priced.find(p => p.priceUsd === mn) };
  }
  return { source: "orca-walls", pools: pools.length, walls, poolPrices, dislocation };
}
