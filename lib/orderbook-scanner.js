...
async function jupQuote(inputMint, outputMint, amountRaw) {
  const u = `https://lite-api.jup.ag/swap/v1/quote?inputMint=${inputMint}&outputMint=${outputMint}&amount=${amountRaw}&slippageBps=5000&restrictIntermediateTokens=false`;
...
async function scanAmmDepth(mint, decimals, spotUsd) {
  ...
  for (const usd of DEPTH_SIZES_USD) {
    ...
    try {
      const res = await jupQuote(USDC_MINT, mint, Math.round(usd * 1e6));
      if (res && res.outAmount > 0) {
        const tokens = res.outAmount / 10 ** decimals;
        const avg = usd / tokens;
        ask.push({ usd, tokens, avgPriceUsd: avg, distPct: (avg / spotUsd - 1) * 100, impactPct: res.impactPct });
      }
    } catch (_) {}
    // BID side — sell `usd`-worth of `mint` for USDC (pushes price DOWN).
    try {
      const tokRaw = Math.round((usd / spotUsd) * 10 ** decimals);
      const res = await jupQuote(mint, USDC_MINT, tokRaw);
      if (res && res.outAmount > 0) {
        const tokens = tokRaw / 10 ** decimals;
        const avg = (res.outAmount / 1e6) / tokens;
        bid.push({ usd, tokens, avgPriceUsd: avg, distPct: (avg / spotUsd - 1) * 100, impactPct: res.impactPct });
      }
    } catch (_) {}
  }
  return { source: "amm-depth", quote: "USDC", ask, bid };
}
...
async function scan(mint, { nocache = false } = {}) {
  ...
  const [loRes, ammRes] = await Promise.allSettled([
  ...
  let ammDepth = null;
  if (ammRes.status === "fulfilled") { ammDepth = ammRes.value; sources["amm-depth"] = { askPts: (ammRes.value.ask || []).length, bidPts: (ammRes.value.bid || []).length, skipped: ammRes.value.skipped }; }
...
module.exports = { scan, scanJupiterLimitOrders, scanAmmDepth, getDecimals, getUsdPrice, debugJupSample, JUP_LIMIT_PROGRAMS };
