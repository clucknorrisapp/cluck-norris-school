...
async function monitorScan(mint) {
  const [decimals, spotUsd] = await Promise.all([getDecimals(mint), getUsdPrice(mint)]);
  const lo = await scanJupiterLimitOrders(mint, decimals, spotUsd);
  return { spotUsd, decimals, orders: lo.orders };
}

module.exports = { scan, scanJupiterLimitOrders, scanAmmDepth, monitorScan, getDecimals, getUsdPrice, debugJupSample, JUP_LIMIT_PROGRAMS };
