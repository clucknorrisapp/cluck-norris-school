...
const _metaCache = new Map();     // mint -> { name, symbol, at }
const _scanCache = new Map();     // mint -> { data, at }
const PRICE_TTL = 30_000, SCAN_TTL = 30_000, META_TTL = 3_600_000; // name/symbol are ~immutable
...
async function getUsdPrice(mint) {
  ...
  // Fallback: derive spot from a small Jupiter buy quote ($10 USDC → token).
  // Reuses the same routing infra that already works server-side, so a
  // GeckoTerminal throttle never leaves a (big) token with no price.
  if (usd == null && mint !== USDC_MINT && mint !== USDT_MINT) {
    try {
      const q = await jupQuote(USDC_MINT, mint, 10_000_000);
      if (q && q.outAmount > 0) { const dec = await getDecimals(mint); const tokens = q.outAmount / 10 ** dec; if (tokens > 0) usd = 10 / tokens; }
    } catch (_) {}
  }
  if (usd != null) _priceCache.set(mint, { usd, at: Date.now() }); // never cache a failure — let it self-heal
  return usd;
}

// Token name + ticker for the header (so we show "Cluck Norris (CLKN)", not a
// truncated mint). GeckoTerminal first (already indexes every Solana token);
// Jupiter Tokens V2 fallback. Cached ~1h — names don't change.
async function getTokenMeta(mint) {
  const hit = _metaCache.get(mint);
  if (hit && Date.now() - hit.at < META_TTL) return { name: hit.name, symbol: hit.symbol };
  let name = null, symbol = null;
  try {
    const r = await fetch(`https://api.geckoterminal.com/api/v2/networks/solana/tokens/${mint}`, {
      headers: { accept: "application/json" }, signal: AbortSignal.timeout(12_000),
    });
    if (r.ok) { const j = await r.json(); const a = j?.data?.attributes; if (a) { name = a.name || null; symbol = a.symbol || null; } }
  } catch (_) {}
  if (!symbol) {
    try {
      const r = await fetch(`https://lite-api.jup.ag/tokens/v2/search?query=${mint}`, { headers: { accept: "application/json" }, signal: AbortSignal.timeout(12_000) });
      if (r.ok) { const j = await r.json(); const t = Array.isArray(j) ? j.find(x => x?.id === mint) || j[0] : null; if (t) { name = name || t.name || null; symbol = symbol || t.symbol || null; } }
    } catch (_) {}
  }
  if (name || symbol) _metaCache.set(mint, { name, symbol, at: Date.now() }); // never cache a total miss
  return { name, symbol };
}
...
async function scanOrcaWalls(mint, spotUsd) {
  ...
  for (const pool of pools.slice(0, 6)) {
    ...
    const quoteUsd = await getUsdPrice(res.quoteMint).catch(() => null);
    poolPrices.push({ pool, quote: res.quoteSymbol, tokenPriceInQuote: poolSpotQ, valueUsd: (res.walls || []).reduce((s, w) => s + (w.sizeUsd || 0), 0), priceUsd: (poolSpotQ > 0 && quoteUsd) ? poolSpotQ * quoteUsd : null });
    for (const w of res.walls) {
      const midQ = ((w.priceLow || 0) + (w.priceHigh || 0)) / 2;
      const toUsd = q => (spotUsd && poolSpotQ > 0) ? spotUsd * (q / poolSpotQ) : null;
      ...
      walls.push({
        ...
        priceUsd: toUsd(midQ), priceLowUsd: toUsd(Math.min(w.priceLow || 0, w.priceHigh || 0)), priceHighUsd: toUsd(Math.max(w.priceLow || 0, w.priceHigh || 0)),
        distPct, orderPubkey: w.positionPubkey, owner: w.owner || null, quoteSymbol: res.quoteSymbol, inRange: w.inRange,
      });
    }
  }
  // Cross-pool dislocation = the live arbitrage spread between this token's pools.
  // Only count pools holding >= $5 of liquidity (owner's call) — stale shell-pool
  // prices can't fake a dislocation, and dust pools are dropped from the list.
  const active = poolPrices.filter(p => p.priceUsd > 0 && (p.valueUsd || 0) >= 5);
  let dislocation = null;
  if (active.length >= 2) {
    const mx = Math.max(...active.map(p => p.priceUsd)), mn = Math.min(...active.map(p => p.priceUsd));
    dislocation = { spreadPct: (mx / mn - 1) * 100, high: active.find(p => p.priceUsd === mx), low: active.find(p => p.priceUsd === mn) };
  }
  return { source: "orca-walls", pools: pools.length, walls, poolPrices: active, dislocation };
}

// ── Aggregation: split asks/bids, bucket by distance-from-spot, cumulative depth ─
function summarize(orders, spotUsd) {
  // Trust only sanely-priced orders: within ±100% of spot (the actionable walls)
  // and under a $1B per-order cap — guards against mispriced obscure counter
  // tokens inflating totals. Orders beyond the ±100% window are a "far" count.
  const SANE = o => o.priceUsd != null && o.distPct != null && (o.sizeUsd == null || o.sizeUsd < 1e9);
  const priced = orders.filter(SANE);
  const asksAll = priced.filter(o => o.side === "sell" && o.distPct >= -0.5);
  const bidsAll = priced.filter(o => o.side === "buy" && o.distPct <= 0.5);
  const asks = asksAll.filter(o => o.distPct <= 100).sort((a, b) => a.priceUsd - b.priceUsd);
  const bids = bidsAll.filter(o => o.distPct >= -100).sort((a, b) => b.priceUsd - a.priceUsd);
  ...
  const sum = l => l.reduce((s, o) => s + (o.sizeUsd || 0), 0);
  return {
    asks: { count: asks.length, usd: sum(asks), top: asks.slice(0, 15), cumulative: cum(asks, +1), far: asksAll.length - asks.length },
    bids: { count: bids.length, usd: sum(bids), top: bids.slice(0, 15), cumulative: cum(bids, -1), far: bidsAll.length - bids.length },
  };
}
...
async function scan(mint, { nocache = false } = {}) {
  ...
  const [decimals, spotUsd, meta] = await Promise.all([getDecimals(mint), getUsdPrice(mint), getTokenMeta(mint).catch(() => ({}))]);
  ...
  if (orcaRes.status === "fulfilled") {
    ...
    const lp = w.filter(x => x.side === "two-sided").sort((a, b) => (b.sizeUsd || 0) - (a.sizeUsd || 0)); // in-range positions = the full picture
    orcaWalls = { pools: orcaRes.value.pools, sells, buys, twoSided: lp.length, lp, poolPrices: orcaRes.value.poolPrices || [], dislocation: orcaRes.value.dislocation || null };
  ...
  const data = {
    ...
    name: (meta && meta.name) || null, symbol: (meta && meta.symbol) || null,
...
async function monitorScan(mint) {
  ...
  if (orca.status === "fulfilled") orders = orders.concat((orca.value.walls || []).filter(w => w.single).map(w => ({ venue: "orca-" + w.side, side: w.side, orderPubkey: w.orderPubkey, sizeUsd: w.sizeUsd, priceUsd: w.priceUsd, distPct: w.distPct, owner: w.owner })));
  return { spotUsd, decimals, orders };
}

module.exports = { scan, scanJupiterLimitOrders, scanAmmDepth, monitorScan, getDecimals, getUsdPrice, getTokenMeta, debugJupSample, debugOrcaPositions, JUP_LIMIT_PROGRAMS };
