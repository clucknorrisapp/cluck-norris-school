...
(function () {
  ...
  // ---- 🔥 THE BURN SHOP — trade $NORMIE for one Item Reserve boost ---------------------------
  //
  ...
  //   2. It is mostly words. A burn is irreversible, so the confirm step has to say plainly what
  ...
  //
  // GUARDRAILS AGAINST A DOUBLE BURN, in layers:
  //   • the BURN button is inert until an explicit "I understand" tick,
  ...
  //   • the confirm screen is REPLACED by a progress screen the moment a burn is in flight —
  ...
  //     the claim instead of losing a burn that already landed,
  //   • and server-side the session is single-use: one burn, one item, replay-guarded on disk.
  ...
  function shopErr(status) {
    return ({
      ...
      no_session: 'The shop session expired. Nothing was burned — start again.',
      expired: 'The shop session expired. Nothing was burned — start again.',
      slow_down: 'Too many attempts — wait a minute and try again.',
      insufficient_funds: 'Not enough $NORMIE (or not enough SOL for the network fee).',
  ...
  function renderShop() {
    ...
    host.innerHTML =
      '<div class="nqp-sub" style="margin-bottom:2px">Burn $NORMIE, keep one boost. It lands in your Item Reserve — the three slots at the top of the HUD.</div>'
      + '<div class="nqp-warn">🔥 A burn is permanent. The tokens are destroyed on-chain and the supply drops — they are not sent to us or to anyone. No refunds, no undo. Testing only; nothing here is final.</div>'
      + (shopCfg.items || []).map(function (it) {
          ...
          return '<div class="nqp-shoprow"><span class="ic">' + m.emoji + '</span>'
            ...
            + '<span class="pr">' + fmtAmt(it.price) + '<u>$NORMIE</u></span>'
            + '<button class="nqp-b sm" data-buy="' + esc(it.id) + '">BURN</button></div>';
        }).join('')
  ...
  // Guardrails before power: restate the exact amount, the exact item, and what cannot be undone,
  // then make them tick a box before the button is even live.
  function confirmBurn(id) {
    ...
    host.innerHTML = '<div class="nqp-confirm">'
      + '<div class="hd">🔥 CONFIRM A BURN</div>'
      + '<div class="ln">You will burn <b>' + fmtAmt(row.price) + ' $NORMIE</b>.</div>'
      + '<div class="ln">You will get <b>' + m.emoji + ' ' + esc(m.label) + '</b> — ' + esc(m.desc) + '. One item, banked to your Item Reserve.</div>'
      + '<div class="ln warn">This cannot be undone. The tokens are destroyed on-chain; nobody receives them and there is no refund. Your wallet will show you the exact amount before you approve — read it there too.</div>'
      + '<div class="ln dim">Paid from ' + esc(pk.slice(0, 4) + '…' + pk.slice(-4)) + ' · you also pay the usual Solana network fee in SOL.</div>'
      + '<label class="nqp-ack"><input type="checkbox" id="nqp-ack"> I understand this burn is permanent and cannot be undone.</label>'
      + '<button class="nqp-b danger wide" id="nqp-doburn" disabled>BURN ' + fmtAmt(row.price) + ' $NORMIE</button>'
  ...
  async function startBurn(id) {
    ...
    var failed = function (msgText) {
      shopBusy = false;
      var host = shopHost(); if (!host) return;
      host.innerHTML = '<div class="nqp-confirm"><div class="hd" style="color:#ff9db8">BURN NOT COMPLETED</div>'
    ...
    try {
      ...
      var ses = await (await fetch('/api/nq/shop/session', { method: 'POST', headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ wallet: walletState.pubkey, token: walletState.token, item: id }) })).json();
      if (!ses.ok) return failed(shopErr(ses.status));
      shopStatus('Building the burn transaction…');
      ...
      // Park the session BEFORE broadcasting. From this point a burn can be on-chain, and losing
      ...
      if (!snd.ok) {
        ...
        // parked session and say honestly that nothing was burned.
      ...
      failed(/reject|denied|cancel|User rejected/i.test(t) ? 'You cancelled the approval — nothing was burned.' : t);
    }
  }
  ...
  function watchBurn(p) {
    ...
    if (host) {
      host.innerHTML = '<div class="nqp-confirm"><div class="hd">🔥 BURN SENT</div>'
  ...
  // ONE poller, ever. Runs with or without the shop pane on screen, so a burn is still claimed if
  ...
  function pollClaim(p) {
    ...
    var slow = function (why) {
      ...
      host.innerHTML = '<div class="nqp-confirm"><div class="hd" style="color:#ffc98a">STILL CONFIRMING</div>'
        + '<div class="ln">' + esc(why || 'The chain has not reported your burn yet.') + '</div>'
        + '<div class="ln">If the burn went through, your ' + m.emoji + ' <b>' + esc(m.label) + '</b> is still owed to you — press Check again.</div>'
    ...
    (function step() {
      ...
      fetch('/api/nq/shop/claim', { method: 'POST', headers: { 'content-type': 'application/json' },
        ...
        .then(function (j) {
          ...
          if (tries >= MAX) return slow('The chain has not reported your burn after two minutes.');
  ...
  function burnDone(item, pending) {
    ...
    host.innerHTML = '<div class="nqp-confirm ok"><div class="hd">✅ ' + m.emoji + ' ' + esc(m.label) + ' IS YOURS</div>'
      + '<div class="ln">Burned and confirmed on-chain. It is banked to your wallet and drops into your Item Reserve — tap a slot, or press 1 / 2 / 3 in a level, to spend it.</div>'
  ...
  function ready() {
    ...
    restoreWallet(function (ok) {
      ...
      // A burn that was already broadcast when the tab reloaded still has to be claimed. Resume
