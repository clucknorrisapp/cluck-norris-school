...
// DEV BENCH — a SECRET, dev-only unlock (owner, not testers). Reveals the HIDDEN/secret rooms in
// the level picker and turns the picker on by itself. Set once via ?nqdev=clkn-bench-7 (?nqdev=off
// clears it); persisted to localStorage so the bare game URL keeps it on THIS device only. Testers
// using ?test=1 never see the hidden rooms. ⚠ Obscurity, not real security: the game JS is
// client-side so the token lives in the shipped bundle — it just keeps casual testers out.
var DEV = (function(){ try{
  var q=(location.search||'')+(location.hash||'');
  if(/[?&#]nqdev=clkn-bench-7(\b|$)/i.test(q)){ try{ localStorage.setItem('nqDev','1'); }catch(e){} return true; }
  if(/[?&#]nqdev=off(\b|$)/i.test(q)){ try{ localStorage.removeItem('nqDev'); }catch(e){} return false; }
  return (typeof localStorage!=='undefined' && localStorage.getItem('nqDev')==='1');
}catch(e){ return false; } })();
// TEST BUILD — playtester dashboard. ?test=1 (query or hash) unlocks a level-picker screen
...
// The dev bench implies the picker too, so one secret URL gives the owner picker + hidden rooms.
var TEST_MODE = (function(){ try{
  var qh=(location.search||'')+(location.hash||'');
  // Kill switch: ?test=0 clears the persisted tester flags for this install.
  try{ if(/[?&#]test=0(\b|$)/.test(qh)){ localStorage.removeItem('nq_test'); localStorage.removeItem('nq_tester'); } }catch(e){}
  var on = DEV || /[?&#]test=1(\b|$)/.test(qh) || /[?&#]tester=[A-Za-z0-9_-]{1,16}/.test(qh);
  // Beta testers reach the game through a FIXED wrapper URL (TestFlight/Seeker) that can't carry a
  // query param across relaunches — so a one-time ?test=1 or ?tester= link would light the feedback
  // widget once and lose it on the next launch. Persist activation for this install so the bug-report
  // button stays. Normal players never set the flag (no ?test / no ?tester link), so they never see it.
  try{ if(on) localStorage.setItem('nq_test','1'); }catch(e){}
  try{ if(localStorage.getItem('nq_test')==='1' || localStorage.getItem('nq_tester')) on=true; }catch(e){}
  return on;
}catch(e){ return false; } })();
...
// Scary world (hidden ?room=scary): AI-illustrated cutouts loaded under the SAME texture keys the
// procedural draw fns use — those fns each `return` early if the key already exists, so preloading
// these here makes the art win with zero rewiring, and the enemies' setScale(px/height) keeps every
// sprite at its tuned on-screen size. Drop a key to fall straight back to the code-drawn version.
var SCARY_ART={ eyeball:'__SCARY_EYEBALL__', longneck:'__SCARY_LONGNECK__', gravemite:'__SCARY_GRAVEMITE__', ghostship:'__SCARY_GHOSTSHIP__' };
// ── Playable characters (cosmetic, FREE — no $NORMIE gate) ───────────────────────────────────────
// Each skin re-keys the whole pose set by a prefix, exactly like the moon suit ('m'): its five frames
// load as <prefix>+{normie,nrun1,nrun2,njump,nduck}, all 144x216 so a setTexture swap keeps the
// player's scale + body constant. '' = Normie (default). The chosen character id lives in localStorage
// 'nqChar'; the picker in the EXTRAS panel writes it. Missing art falls back to Normie (see setup).
var CHARACTERS = {
  normie:    { prefix:'',   name:'Normie',          emoji:'🐸' },
  princess:  { prefix:'pr', name:'Normie Princess', emoji:'👑' },
  lilnormie: { prefix:'kd', name:"Lil' Normie",     emoji:'🧒' }
};
var CHAR_ART = {
  prnormie:'__PR_IDLE__', prnrun1:'__PR_RUN1__', prnrun2:'__PR_RUN2__', prnjump:'__PR_JUMP__', prnduck:'__PR_DUCK__',
  kdnormie:'__KD_IDLE__', kdnrun1:'__KD_RUN1__', kdnrun2:'__KD_RUN2__', kdnjump:'__KD_JUMP__', kdnduck:'__KD_DUCK__'
};
function nqChosenChar(){ try{ var c=localStorage.getItem('nqChar'); return (c && CHARACTERS[c]) ? c : 'normie'; }catch(e){ return 'normie'; } }
function nqCharPrefix(){ return (CHARACTERS[nqChosenChar()]||CHARACTERS.normie).prefix; }
// LAZY skin loading (perf/memory): the skins are big detailed textures. Preloading ALL of them at
// boot — for every player, even one who only ever plays as Normie — piles decoded texture memory onto
// the iPad, where iOS Safari answers memory pressure by DOWNSCALING the whole WebGL drawing buffer, so
// the entire game (Normie included) renders softer. Fix: a Normie player loads NO skin; only the
// chosen skin's five frames are decoded, and only when it's chosen (boot reads the saved pick; the
...
try{ if(typeof window!=='undefined'){ window.__NQ_CHARS = {
  list: [ { id:'normie', name:'Normie', icon:SPRITES.normie },
          { id:'princess', name:'Normie Princess', icon:CHAR_ART.prnormie },
          { id:'lilnormie', name:"Lil' Normie", icon:CHAR_ART.kdnormie } ],
  get: nqChosenChar,
  // save the pick, then eagerly decode that skin (while the player is still in the menu) so it's ready
  // by the time they start their next run. __NQ_LOADSKIN is published by Boot once textures exist.
  set: function(id){ try{ if(CHARACTERS[id]){ localStorage.setItem('nqChar', id);
    if(typeof window!=='undefined' && window.__NQ_LOADSKIN) window.__NQ_LOADSKIN((CHARACTERS[id]||{}).prefix); } }catch(e){} return nqChosenChar(); }
}; } }catch(e){}
...
function _acHealth(){ try{
  ...
  _acCheckTimer=setTimeout(function(){ _acCheckTimer=null; try{
    ...
    if(zombie || stuckDead){
      // close() returns a PROMISE — swallow its rejection or a zombie that refuses to close leaves
      // an unhandled rejection AND a live corpse counting against iOS's ~4-concurrent-context cap.
      // Hit that cap and new AudioContext() starts throwing: silence forever after a few
      // background cycles (owner's exact 2026-08-16 pattern — fine on early levels, dead later).
      try{ var cp2=c.close(); if(cp2&&cp2.catch) cp2.catch(function(){}); }catch(e){}
      _AC=null; _sfxGain=null;
...
try{ if(typeof document!=='undefined') document.addEventListener('visibilitychange',function(){ try{ if(document.hidden) MUSIC.suspend(); else if(!window.__NQ_PAUSED) MUSIC.resume(); }catch(e){} }); }catch(e){}
// ABANDON BEACON — the single most useful funnel signal and the one nothing else captures. A
// player who dies is a 'death'; one who finishes is a 'clear'; one who simply CLOSES THE TAB
// mid-level is currently invisible, and that is exactly the person we most want to know about.
// window.__NQ_LIVE is set by the Game scene while a level is genuinely in progress and cleared
// the moment it ends, so this can never fire for a menu, a finished level, or twice for one
// attempt. pagehide is the reliable mobile signal (iOS often never fires beforeunload); the
// hidden-branch of visibilitychange covers app-switching, which on a phone IS quitting.
try{
  if(typeof window!=='undefined'){
    var nqSendQuit=function(){
      try{
        var L=window.__NQ_LIVE;
        if(!L||!L.world) return;
        window.__NQ_LIVE=null;   // one beacon per attempt, whichever event wins the race
        nqTele('quit',{world:L.world,x:Math.round(L.x()||0),t:Math.round(((Date.now()-(L.at||Date.now()))-(L.paused()||0))/1000),score:L.score()||0});
      }catch(e){}
    };
    window.addEventListener('pagehide',nqSendQuit);
    document.addEventListener('visibilitychange',function(){ if(document.hidden) nqSendQuit(); });
  }
}catch(e){}   // paused game keeps its silence — resumeGame() owns the restart (audit #20)
...
var THEMES=[
  ...
  { sky1:0x2aa4e0, sky2:0x9fe6f2, hill:0x38b6cc },    // 29 THE SHALLOWS (prize room — tropical daytime: bright sky-blue up top → pale aqua horizon → turquoise sea behind golden sand)
  { sky1:0x08060e, sky2:0x000000, hill:0x150e22 }     // 30 THE SCARY WORLD (kids' hidden world — pitch-black night, barely-there witching-purple hills)
];
...
var LEVELS=[
  ...
  // ===== PRIVATE DEV WORLD — "MYSPACE" (owner + the Normie dev). A full 3-level world reachable
  //   ONLY via ?room=myspace. All three are hidden:true (out of progression + level-select) and
  //   private:true, with NO warp pointing at them — the unlisted URL is the only door. They CHAIN
  //   via def.next (a hidden level with def.next advances instead of returning to the surface);
  //   the finale is TOM (his own generated sprite 'tom'), who re-uses the KOL mechanic via the
  //   boss* overrides — bigger (bossScale) and tougher (bossHits/bossShillMs). The backdrop is a
  //   painted plate (bgArt:'wMyspace' → worlds/w-myspace.webp). Geometry on each is lifted from a
  //   proven world-3 level (3-1 / 3-2 / 3-3) so reachability is already vetted; harder via diff +
  //   denser enemies. Only the theming + the stripped speakeasy warp are new. =====
  { name:'MYSPACE', sub:'THE PROFILE PAGE', time:150, theme:28, bgArt:'wMyspace', width:5800,
    hidden:true, bonus:true, private:true, next:83, diff:1.6,
    gaps:[[820,892],[1640,1760],[2480,2600],[3300,3372],[4200,4296],[5000,5076]],
    walls:[[460,3,'steel',4],[1300,3,'stone',4],[2200,4,'steel',3],[3000,3,'brick',4],[3900,3,'stone',4],[4800,3,'steel',4],[5400,2,'brick',2]],
    plats:[[840,2,150,'crate'],[1600,2,H-130,'brick'],[1660,2,150,'stone'],[2140,2,H-120,'steel'],[2500,2,150,'steel'],[3320,2,150,'brick'],[4220,2,150,'stone']],
    spikes:[[1000],[1024],[4560],[4584]],
    firepits:[[1980,64],[3600,56]],
    powerups:[['moon',3200,230],['omegachad',4090,230],['bull',2850,230],['candle',1600,230]],
    airdrops:[[1200,H-118],[3500,H-72]],
    coins:[[472,158],[520,158],[700,196],[1312,158],[1360,158],[1500,196],[2212,134],[2260,134],[2680,196],[3012,158],[3060,158],[3600,196],[3912,158],[3960,158],[4500,196],[4812,158],[4860,158],[5300,196]],
    enemies:[['jeet',340,230,80],['bot',680,230,60],['ghost',1050,212,80],['fudster',1150,230,60],['sniper',1450,230,30],['sniper',1900,230,30],['bitmaxi',2380,230,70],['bot',2800,230,70],['ghost',2820,212,60],['sniper',3150,230,30],['fudster',3600,230,70],['ghost',3720,212,80],['bitmaxi',4100,230,70],['bot',4500,230,70],['sniper',4650,230,30],['ghost',4700,212,90],['fudster',4900,230,60]],
    pumpdumps:[[1500,174],[3700,174]], honeypots:[[2680,232],[4500,232]], npcs:[[2900,90]],
    bonusblocks:[[1280,5],[2950,'moon'],[4760,8]],
    key:[1624,H-150], door:5680 },

  { name:'TOP8', sub:'TOP 8 OR NOTHING', time:160, theme:28, bgArt:'wMyspace', width:6400,
    hidden:true, bonus:true, private:true, next:84, diff:1.75,
    gaps:[[780,852],[1620,1692],[2440,2560],[3260,3380],[4120,4240],[4980,5076],[5900,5976]],
    walls:[[440,3,'steel',4],[1240,4,'stone',4],[2100,3,'crate',5],[2960,4,'steel',4],[3820,3,'brick',5],[4680,4,'stone',4],[5250,2,'stone',2],[5500,3,'crate',4],[6180,2,'crate',2]],
    plats:[[800,2,150,'stone'],[1140,2,H-130,'stone'],[1640,2,150,'crate'],[2460,2,150,'steel'],[3280,2,150,'brick'],[3500,2,H-140,'steel'],[4140,2,150,'stone'],[5000,2,150,'crate']],
    spikes:[[1000],[1024],[3600],[3624],[4400],[4424]],
    firepits:[[1850,56],[2700,64],[4560,48]],
    powerups:[['supergeek',1400,230],['omegachad',4480,230],['bull',4100,230],['candle',3150,230]],
    airdrops:[[2300,H-118],[4200,H-72]],
    coins:[[452,158],[500,158],[700,196],[1252,134],[1300,134],[1400,196],[2112,158],[2160,158],[2208,158],[2680,196],[2972,134],[3020,134],[3200,196],[3832,158],[3880,158],[3928,158],[4692,134],[4740,134],[5512,158],[5560,158],[6100,196]],
    enemies:[['bot',340,230,70],['jeet',600,230,60],['ghost',1000,212,80],['fudster',1400,230,70],['sniper',1900,230,30],['bitmaxi',2250,230,70],['sniper',2350,230,30],['bot',2700,230,70],['ghost',3200,212,70],['sniper',3450,230,30],['fudster',3650,230,70],['ghost',3900,212,80],['bitmaxi',4000,230,70],['bot',4400,230,70],['sniper',4560,230,30],['ghost',4820,212,80],['sniper',5250,230,30],['bitmaxi',5650,230,60]],
    pumpdumps:[[1000,174],[3500,174],[5300,174]], honeypots:[[2350,232],[4300,232]], npcs:[[3150,90]],
    bonusblocks:[[1120,5],[2080,8],[3220,5],[4490,8],[5480,8]],
    key:[1164,H-150], door:6300 },

  { name:'TOMSTURF', sub:"TOM'S TURF", time:180, theme:28, bgArt:'wMyspace', width:4800,
    hidden:true, bonus:true, private:true, boss:true, bossType:'kol', diff:1.7,
    bossName:'TOM', bossSub:'YOUR FIRST FRIEND — STOMP x5 TO CRACK HIS TOP 8',
    bossTex:'tom', bossScale:78, bossHits:5, bossShillMs:1450, bossAttack:'tom',
    bossColor:'#8aa0d8', bossGlowHex:0x8aa0d8,
    bossTaunts:['a/s/l?','THANKS FOR THE ADD!','NEW COMMENT ON YOUR WALL','CHECK YOUR BULLETINS','YOU + ME = TOP 8','TOM APPROVES THIS','ADD ME BACK!'],
    gaps:[[700,772],[1380,1500],[2060,2156],[2720,2816],[3380,3500]],
  ...
  // ===== OCEAN / BEACH PRIZE ROOM — "THE SHALLOWS" (entry #1 in the rotating prize-level catalogue).
  //   Reachable ONLY via ?room=beach (private:true, hidden:true — out of progression + level-select,
  //   no warp points here). The boss is a SHARK: it reuses the proven KOL charge+shill mechanic via
  //   the boss* overrides (bossTex:'shark' → drawSharkTex()), tuned to 5 hearts per the boss cap.
  ...
  { name:'BEACH', sub:'THE SHALLOWS', time:170, theme:29, width:4800, ground:'sandground', music:'desert',
    ...
    bossTex:'shark', bossScale:96, bossHits:5, bossShillMs:1500,
  ...
  // ===== THE SCARY WORLD — the kids' hidden monster world (owner ask 2026-08-13: "a scary world,
  //   3 levels with boss at the end… background black, floating eyeballs as a bad guy, little
  //   monsters out of the ground as turrets with radial fire, roaming monsters with long necks
  //   that spray poisonous gas into the pits that takes a heart, haunted pirate ship with big big
  //   monster for the end boss"). Reachable ONLY via ?room=scary — private:true + hidden:true like
  //   MYSPACE, chained via def.next, no warps here, the unlisted URL is the only door.
  //   Pitch-black theme 30; the two run levels are dark:true (the mines' torchlight) so the
  //   monsters loom out of the dark — the candle powerup widens the torch, and the boss arena is
  //   lit so you can see the whole ship. NEW MONSTERS (procedural canvas, like the shark/sandlord):
  //   'eyeball' re-skins the proven flash-drone hover+dive; 'longneck' patrols and sprays POISON
  //   into pits — its acid-bright gas TAKES A HEART (poisonTouch), unlike the FUDster's slow-gas;
  //   the ground turrets are the proven miniworm/sentry radial-fan mechanic wearing the
  //   'gravemite' skin (def.mwTex). The finale is THE GHOST GALLEON — the KOL charge mechanic
  //   under a haunted-ship texture with its own three-attack rotation (bossAttack:'ship').
  //   Geometry is lifted VERBATIM from the vetted MYSPACE / TOP8 / TOMSTURF rooms so reachability
  //   is proven; theme, enemies, darkness and the boss are the only changes. =====
  { name:'SCARY', sub:'THE HAUNTED HOLLOW', time:150, theme:30, width:5800, music:'mines',
    hidden:true, bonus:true, private:true, next:88, dark:true, darkAlpha:0.84, diff:1.4, mwTex:'gravemite',
    ...
    airdrops:[[1200,H-118],[3500,H-72]],
    coins:[[472,158],[520,158],[700,196],[1312,158],[1360,158],[1500,196],[2212,134],[2260,134],[2680,196],[3012,158],[3060,158],[3600,196],[3912,158],[3960,158],[4500,196],[4812,158],[4860,158],[5300,196]],
    // longnecks sit right before each pit so they lean over and spray poison INTO it; eyeballs
    // patrol the air. No ghosts (owner call). Gravemite goblins glow up out of the dark ground.
    enemies:[['longneck',720,230,70],['eyeball',1150,150,60],['longneck',1540,230,70],['eyeball',1900,140,60],['longneck',2380,230,70],['eyeball',2820,150,60],['eyeball',3600,140,60],['longneck',4100,230,70],['eyeball',4700,150,50],['longneck',4900,230,60]],
    miniworms:[1100,2700,4400,5300],
    pumpdumps:[[1500,174],[3700,174]], honeypots:[[2680,232],[4500,232]], npcs:[[2900,90]],
    bonusblocks:[[1280,5],[2950,'moon'],[4760,8]],
    key:[1624,H-150], door:5680 },

  { name:'GRAVEYARD', sub:'THE MONSTER PITS', time:160, theme:30, width:6400, music:'mines',
    hidden:true, bonus:true, private:true, next:89, dark:true, darkAlpha:0.84, diff:1.5, mwTex:'gravemite',
    roomLabel:'THE SCARY WORLD',
    gaps:[[780,852],[1620,1692],[2440,2560],[3260,3380],[4120,4240],[4980,5076],[5900,5976]],
    walls:[[440,3,'steel',4],[1240,4,'stone',4],[2100,3,'crate',5],[2960,4,'steel',4],[3820,3,'brick',5],[4680,4,'stone',4],[5250,2,'stone',2],[5500,3,'crate',4],[6180,2,'crate',2]],
    plats:[[800,2,150,'stone'],[1140,2,H-130,'stone'],[1640,2,150,'crate'],[2460,2,150,'steel'],[3280,2,150,'brick'],[3500,2,H-140,'steel'],[4140,2,150,'stone'],[5000,2,150,'crate']],
    ...
    airdrops:[[2300,H-118],[4200,H-72]],
    coins:[[452,158],[500,158],[700,196],[1252,134],[1300,134],[1400,196],[2112,158],[2160,158],[2208,158],[2680,196],[2972,134],[3020,134],[3200,196],[3832,158],[3880,158],[3928,158],[4692,134],[4740,134],[5512,158],[5560,158],[6100,196]],
    // "THE MONSTER PITS" — a longneck guards nearly every gap, spraying poison down into it; the
    // pits are the theme, so lean into them. Eyeballs above, a den of glowing gravemite goblins below.
    enemies:[['longneck',700,230,70],['eyeball',1000,150,60],['longneck',1540,230,70],['eyeball',1900,140,60],['longneck',2360,230,70],['eyeball',2900,150,60],['longneck',3180,230,70],['eyeball',3600,140,60],['longneck',4040,230,70],['eyeball',4560,150,50],['longneck',4900,230,70],['eyeball',5400,140,60],['longneck',5820,230,60]],
    miniworms:[1400,2850,4400,5700],
    pumpdumps:[[1000,174],[3500,174],[5300,174]], honeypots:[[2350,232],[4300,232]], npcs:[[3150,90]],
    bonusblocks:[[1120,5],[2080,8],[3220,5],[4490,8],[5480,8]],
    key:[1164,H-150], door:6300 },

  { name:'GHOSTSHIP', sub:'THE HAUNTED GALLEON', time:190, theme:30, width:4800, music:'mines',
    hidden:true, bonus:true, private:true, boss:true, bossType:'kol', diff:1.45, mwTex:'gravemite', darkAlpha:0.84,
    roomLabel:'THE SCARY WORLD',
    ...
    airdrops:[[1500,H-118],[3130,H-72]],
    coins:[[300,196],[560,150],[620,196],[900,196],[1160,140],[1420,130],[1620,196],[1950,150],[2200,196],[2500,150],[2760,130],[2900,196],[3150,150],[3400,130],[3760,150],[4000,196],[4200,150]],
    // Enemies stay ahead of the boss arena (door-460 cull). Longnecks work the gaps, eyeballs the air.
    enemies:[['longneck',640,230,60],['eyeball',900,150,60],['longneck',1300,230,60],['eyeball',1850,140,50],['longneck',2000,230,60],['eyeball',2400,150,60],['longneck',2660,230,60],['eyeball',3000,140,60],['longneck',3300,230,60],['eyeball',3560,150,50]],
    miniworms:[1200,1900,3000],
    pumpdumps:[[1380,174],[2720,174]], honeypots:[[1250,232],[3760,232]], npcs:[[2450,90]],
    bonusblocks:[[1040,5],[1950,8],[2950,8],[3840,8]],
    key:[4200,H-150], door:4600 },
];
...
// Is a full-screen DOM overlay currently the thing the player is interacting with? Controller
// presses then belong to the OVERLAY, not the game behind it.
//   #nqremap   controller button remap      #nqhow-wrap  how-to-play card
//   #nqfb-wrap tester feedback form         #nqp-ov      leaderboard / wallet panel
function nqModalUp(){
  try{ return !!document.querySelector('#nqremap, #nqhow-wrap.on, #nqfb-wrap.on, #nqp-ov.on'); }
  catch(e){ return false; }
}
function padAdvance(scene, go){
  ...
  scene.time.addEvent({ delay:55, loop:true, callback:function(){
    if(fired) return;
    // FIELD BUG (owner, 2026-08-22): players who remapped their controller could not then start
    // the game with it — only a touch or a mouse click worked. Cause: this watcher never knew the
    // remap card was open, so the very presses the player made to BIND their buttons were read as
    // "any button starts" — walking the title screen forward underneath the overlay and spending
    // this one-shot latch. By the time the card closed there was nothing left to fire.
    // While an overlay owns the screen: stay armed, hold `prev` down so the press that dismisses
    // it is not read as fresh, and restart the 500ms grace so a still-held button on close cannot
    // instantly trigger either.
    if(nqModalUp()){ prev=true; t0=scene.time.now; return; }
    var press=false;
...
var Boot=new Phaser.Class({ Extends:Phaser.Scene,
  ...
  create:function(){
    ...
    // 🐙 waterlurker — the sea-beast that waits in the water on beach levels. A dark half-submerged
    // silhouette with glowing eyes: reads menacing at any size and never looks cheap. The surface band
    // gets drawn OVER its lower half in create() to sell the "lurking just under the water" look.
    g.clear();
    g.fillStyle(0x0c2e38); g.fillRoundedRect(1,16,9,28,4); g.fillRoundedRect(0,12,8,11,3);      // left tentacle
    g.fillRoundedRect(54,16,9,28,4); g.fillRoundedRect(56,12,8,11,3);                             // right tentacle
    g.fillStyle(0x123c48); g.fillEllipse(32,30,46,34);                                            // head
    g.fillStyle(0x1c5666); g.fillEllipse(32,22,30,12);                                            // forehead highlight
    g.fillStyle(0xffcf3f); g.fillCircle(23,27,6); g.fillCircle(41,27,6);                          // glowing eyes
    g.fillStyle(0x0a0a0a); g.fillCircle(24,28,2.6); g.fillCircle(42,28,2.6);                      // pupils
    g.fillStyle(0xffffff); g.fillCircle(22,25,1.5); g.fillCircle(40,25,1.5);                      // glints
    ...
    // Publish the on-demand skin loader now that the texture manager exists (the DOM picker calls it).
    try{ if(typeof window!=='undefined'){ var _game=this; window.__NQ_LOADSKIN=function(pfx){ try{ nqLoadSkin(_game.textures, pfx); }catch(e){} }; } }catch(e){}
    // load the base64 sprites, start Title once all registered. Boot loads the base game + ONLY the
    // currently-chosen skin (a Normie player loads no skin frames at all) — see nqLoadSkin for why.
    var _bootPfx=nqCharPrefix(), _bootSkin={};
    if(_bootPfx){ ['normie','nrun1','nrun2','njump','nduck'].forEach(function(b){ var k=_bootPfx+b; if(CHAR_ART[k]) _bootSkin[k]=CHAR_ART[k]; }); _nqSkinLoaded[_bootPfx]=1; }
    var self=this, all=Object.assign({},SPRITES,POWERUPS,EXTRA,SCARY_ART,_bootSkin), keys=Object.keys(all), need=keys.length, started=false;
    var go=function(){ if(started) return; started=true;
      // PRIVATE ROOM DOOR: ?room=<name> boots straight into a level flagged private:true (e.g.
      // ?room=myspace → the MySpace/Tom dev room) — routed HERE, before the tappable Title ever
      // shows, so there's no title-screen flash and no race with a habitual "Play" tap landing on
      // 1-1. No warp points at these levels and they're hidden from level-select, so the unlisted
      // URL is the only door. lab:1 bypasses the tier gate; no setup lane needed.
      try{ var _src=(location.search||'')+(location.hash||''); var _rm=_src.match(/[?&#]room=([A-Za-z0-9_-]{1,24})/);
        if(_rm){ var _rn=_rm[1].toLowerCase();
          // optional &at=<x> deep-links to a spawn position inside the room (0 = default spawn)
          var _atm=_src.match(/[?&#]at=(\d{1,5})/), _at=_atm?parseInt(_atm[1],10):0;
          for(var _i=0;_i<LEVELS.length;_i++){ if(LEVELS[_i]&&LEVELS[_i].private && String(LEVELS[_i].name||'').toLowerCase()===_rn){ self.scene.start('Game',{level:_i,score:0,lives:3,lab:1,spawnX:(_at>0?_at:undefined)}); return; } }
        }
      }catch(e){}
...
var LevelSelect=new Phaser.Class({ Extends:Phaser.Scene,
  ...
  create:function(){
    ...
    var worlds={}, order=[];
    // Hidden bonus/private rooms show in the picker ONLY on the secret dev bench (DEV) — testers on
    // ?test=1 see just the normal worlds; real players (no picker) see the normal progression.
    LEVELS.forEach(function(l,i){ if(l.hidden && !DEV) return; var w=(l.name||'').split('-')[0]||'?'; if(!worlds[w]){ worlds[w]=[]; order.push(w); } worlds[w].push({l:l,i:i}); });
...
var Title=new Phaser.Class({ Extends:Phaser.Scene,
  ...
  create:function(){
    this.cameras.main.setZoom(RES).centerOn(W/2,H/2); this.cameras.main.setBackgroundColor(0x120c2e);
    ...
    // TIER BANDS — how far the game runs and which worlds sit in which band. Bands come from
    // the LIVE gate so the front door can never advertise a structure the gate isn't actually
    // handing out. Terms (owner, 2026-08-22: $5 of NORMIE → 4-12, $50 → 13-21) render from
    // nqTerms() — live server config — and NEVER from a literal here; if the config hasn't
    // arrived the chips fall back to naming the bands without amounts. While the launch cap is
    // armed the honest statement is "these worlds now, the rest soon" — no terms, because
    // nothing above the cap is obtainable by anyone yet.
    ...
    if(!nqGateOn()){
      ...
      _bands=[['FREE','W1-'+FREE_MAX,'#3dff6e'],
              [(_T&&_T.usdPriced)?('$'+_T.tier1Usd+' NORMIE'):'HOLDERS','W'+(FREE_MAX+1)+'-'+(_vipMin-1),'#ffd23f'],
              [(_T&&_T.usdPriced)?('$'+_T.tier2Usd+' NORMIE'):'BIG HOLDERS','W'+_vipMin+'-'+_maxWorld,'#7fdcff']];
    }
    ...
    // FIRST-VISIT RACE (found live on launch day): with no localStorage cache the chips above can
    // draw before the config fetch lands, and a fresh visitor's title said "ALL WORLDS OPEN" while
    // world 4 was really a $5 wall. The boot shim raises __NQ_GATE_READY + 'nq-gate-ready' when the
    // truth arrives — if this scene is still up and the bands it drew no longer match, redraw once
    // via a scene restart (the title is static; a restart is invisible beyond the chips changing).
    try{ if(typeof window!=='undefined' && !window.__NQ_GATE_READY){
      var _sig=JSON.stringify(_bands), _tScene=this;
      var _onReady=function(){ try{
        window.removeEventListener('nq-gate-ready',_onReady);
        if(!_tScene.scene || !_tScene.scene.isActive || !_tScene.scene.isActive('Title')) return;
        var f2=nqGateOn()?nqFreeMax():_maxWorld, c2=nqGateCap(), b2;
        if(!nqGateOn()) b2=[['ALL WORLDS OPEN','W1-'+_maxWorld]];
        else if(c2) b2=[['PLAY NOW','W1-'+Math.min(f2,c2)],['W'+(Math.min(f2,c2)+1)+'-'+_maxWorld,'OPENING SOON']];
        else { var T2=nqTerms(); b2=[['FREE','W1-'+f2],[(T2&&T2.usdPriced)?('$'+T2.tier1Usd+' NORMIE'):'HOLDERS','W'+(f2+1)+'-'+(_vipMin-1)],[(T2&&T2.usdPriced)?('$'+T2.tier2Usd+' NORMIE'):'BIG HOLDERS','W'+_vipMin+'-'+_maxWorld]]; }
        if(JSON.stringify(b2.map(function(x){return [x[0],x[1]];}))!==JSON.stringify(JSON.parse(_sig).map(function(x){return [x[0],x[1]];}))) _tScene.scene.restart();
      }catch(e){} };
...
// The published holder terms (owner, 2026-08-22): {tier1Usd,tier2Usd,usdPriced,tier1Normie,
...
// ANONYMOUS SESSION ID — a random per-browser tag so two events can be known to be the same
// person. NOT a wallet, name, IP, or anything derived from one; it is generated locally, never
// leaves localStorage except on our own beacons, and identifies nobody. It exists purely so the
// funnel can answer "how many PEOPLE played" and "where did this one give up", which raw
// death counts cannot. Distinct from nq_tester (the ?tester= panel tag), which still rides along.
function nqSid(){
  try{
    var v=localStorage.getItem('nq_sid');
    if(!v||!/^[A-Za-z0-9_-]{6,32}$/.test(v)){
      v='';
      try{
        var a=new Uint8Array(9); crypto.getRandomValues(a);
        for(var i=0;i<a.length;i++) v+='0123456789abcdefghijklmnopqrstuvwxyz'[a[i]%36];
      }catch(e){ v=Math.random().toString(36).slice(2,11)+Math.random().toString(36).slice(2,5); }
      v=('s'+v).slice(0,16);
      localStorage.setItem('nq_sid',v);
    }
    return v;
  }catch(e){ return ''; }   // private mode / storage blocked → no sid, difficulty telemetry still flows
}
function nqTele(ev,d){
  try{
    ...
    var b=JSON.stringify(Object.assign({ev:ev,who:who||undefined,sid:nqSid()||undefined,token:tok||undefined},d||{}));
...
// F7: WHICH MUSIC LOOP A LEVEL PLAYS. This used to read def.name.charAt(0), so every 10-x..19-x
// level answered '1' and played the World-1 tutorial theme (36 levels), worlds 20/21 fell through
// to the same default, world 9 never matched its own track, and world 8 sent 8-1 and 8-2 into the
// BOSS theme before a boss existed. Parse the world NUMBER instead. Only eight loops exist
// (world1/desert/casino/skyline/exchange/sacred/mines/boss), so the later worlds deliberately
// share the closest existing mood rather than inventing a track that isn't there. Hidden rooms
// have non-numeric names (parseInt -> NaN) and keep the default, as before.
// Pure and top-level on purpose: normie-quest/test/nq-music-routing.cjs asserts the whole table
// against LEVELS with no browser.
function nqTrackFor(def){
  if(!def) return 'world1';
  if(def.music) return def.music;                        // per-level override (the hidden rooms use this)
  if(def.boss && def.bossType) return 'boss';            // boss arenas; 1-3 has no bossType and switches in startBoss
  if(def.name==='2-1') return 'desert';                  // named cases kept exactly as they were
  if(def.name==='2-2') return 'casino';
  var WORLD_TRACK={
    2:'desert',    3:'skyline',  4:'exchange', 5:'sacred',   6:'mines',
    7:'exchange',  8:'mines',    9:'mines',   10:'casino',  11:'exchange',
    12:'skyline', 13:'mines',   14:'sacred',  15:'desert',  16:'exchange',
    17:'casino',  18:'sacred',  19:'desert',  20:'skyline', 21:'desert'
  };
  var w=parseInt(def.name,10);
  return (w>=2 && WORLD_TRACK[w]) ? WORLD_TRACK[w] : 'world1';
}
...
var RESERVE_ITEMS = {
  ...
  vial:   { icon:'heart',     emoji:'❤️', label:'FULL HEAL',    desc:'refill all your hearts',    use:function(g){ while(g.lives<(g.maxLives||3)){ g.lives++; if(g.hearts[g.lives-1]) g.hearts[g.lives-1].setAlpha(1); } } },
...
// ❤️ EXTRA HEART buff levers (same-origin localStorage; the lounge wheel sets this on a win). The
// setter is also the test hook: __NQ_HEARTBUFF(24) arms 24h, __NQ_HEARTBUFF(0) clears. It only takes
// effect on the NEXT level start (init reads it), matching how a won buff applies from the next run.
try{ if(typeof window!=='undefined'){
  window.__NQ_HEARTBUFF=function(h){ try{ if(h===0){ localStorage.removeItem('nqHeartBuffUntil'); return 0; } var t=Date.now()+((h||24)*3600000); localStorage.setItem('nqHeartBuffUntil', String(t)); return t; }catch(e){ return 0; } };
  window.__NQ_HEARTBUFF_LEFT=function(){ try{ var t=(parseInt(localStorage.getItem('nqHeartBuffUntil'),10)||0)-Date.now(); return t>0?t:0; }catch(e){ return 0; } };
} }catch(e){}

var Game=new Phaser.Class({ Extends:Phaser.Scene,
  initialize:function(){ Phaser.Scene.call(this,{key:'Game'}); },
  // ❤️ EXTRA HEART buff (VIP wheel prize): while active, the heart cap is 4 instead of 3, on EVERY
  // level, for 24h. Read from same-origin localStorage (the lounge sets nqHeartBuffUntil when you win
  // it — game and lounge share an origin). Default (no buff) → 3, byte-identical to the old behaviour.
  // window.__NQ_HEARTBUFF(hours) is a test lever; the real grant comes from the wheel.
  _heartBuffActive:function(){ try{ return (parseInt(localStorage.getItem('nqHeartBuffUntil'),10)||0) > Date.now(); }catch(e){ return false; } },
  // levelStartScore = coins banked BEFORE this level; honeypots never drain below it (they only take THIS level's coins).
  // maxLives is the heart cap for THIS level (3, or 4 with the buff). '3' was always "full", so any
  // incoming full/over-full request refills to the cap; a hurt carry (1-2) keeps its count, capped.
  init:function(d){ d=d||{}; this.levelIdx=Phaser.Math.Clamp(d.level||0,0,LEVELS.length-1); this.score=d.score||0;
    this.maxLives=(this._heartBuffActive()?4:3);
    var reqLives=(d.lives!=null?d.lives:3);
    this.lives=(reqLives>=3?this.maxLives:reqLives);
    this.levelStartScore=this.score; this._spawnX=(d.spawnX!=null?d.spawnX:null); this._labWarp=!!d.lab; },
  ...
  create:function(){
    ...
    try{ var _sc=this; var _live=function(){ try{ return _sc.scene&&_sc.scene.isActive&&_sc.scene.isActive()&&!_sc.over; }catch(e){ return false; } };   // page-hide must not run pauseGame on a STOPPED scene instance (polish: same guard __NQ_SYNCREWARDS has)
         ...
         // Screen-fraction bounding box of a live object, for the visual-regression harness
         // (nq-visual.cjs): it crops each surface to the thing under test instead of a guessed
         // rectangle, so a baseline can't silently frame empty background. Fractions are relative to
         // the camera's worldView (0..1 across the visible game), which maps straight onto the
         // harness's canvas-relative clip regardless of zoom/Scale.FIT/DPR. Inert unless called.
         //   __NQ_RECT('player')  -> {x,y,w,h,cx,cy,tex,vis} | null
         //   __NQ_RECT('boss')    -> same, for the active Rug King / worm boss | null
         //   __NQ_RECT('grave')   -> {player, hits:[...]} : every display object whose texture key
         //                           matches /grave/i, each with the same frac fields (nearest to the
         //                           player first) so the caller can pick the creature it means.
         window.__NQ_RECT=function(target){ try{
           var cam=_sc.cameras&&_sc.cameras.main; if(!cam) return null; var wv=cam.worldView;
           function frac(o){ if(!o||!o.getBounds) return null; var b=o.getBounds(); if(!wv.width||!wv.height) return null;
             return { x:+((b.x-wv.x)/wv.width).toFixed(4), y:+((b.y-wv.y)/wv.height).toFixed(4),
               w:+(b.width/wv.width).toFixed(4), h:+(b.height/wv.height).toFixed(4),
               cx:+((b.centerX-wv.x)/wv.width).toFixed(4), cy:+((b.centerY-wv.y)/wv.height).toFixed(4),
               tex:(o.texture&&o.texture.key)||null, vis:!!o.visible }; }
           if(!target||target==='player') return frac(_sc.player);
           if(target==='boss'){ var k=(_sc.rugking&&_sc.rugking.active)?_sc.rugking:((_sc.worm&&_sc.worm.active)?_sc.worm:null); return frac(k); }
           var re=null; try{ re=new RegExp(target,'i'); }catch(e){ re=null; }
           var pc=frac(_sc.player)||{cx:0.5,cy:0.5}, hits=[], seen=[];
           function scan(o){ try{ if(!o||o.active===false||!o.texture||!o.texture.key) return; if(seen.indexOf(o)>=0) return; if(re&&!re.test(o.texture.key)) return; var f=frac(o); if(f){ seen.push(o); hits.push(f); } }catch(e){} }
           try{ if(_sc.enemies&&_sc.enemies.children) _sc.enemies.children.iterate(scan); }catch(e){}
           try{ if(_sc.children&&_sc.children.list) _sc.children.list.forEach(scan); }catch(e){}
           hits.sort(function(a,b){ return (Math.abs(a.cx-pc.cx)+Math.abs(a.cy-pc.cy))-(Math.abs(b.cx-pc.cx)+Math.abs(b.cy-pc.cy)); });
           return { player:pc, hits:hits };
         ...
         if(window.__NQ_SETUP){
           ...
           // Frame the boss fight for an eyeball/screenshot check: seat the player on the floor beside
           // the boss and lock the camera on the pair. Boss-render bugs (feet vs floor, shadow) can't
           // be judged when the camera is stranded back at the level start after __NQ_FORCEBOSS.
           window.__NQ_BOSSVIEW=function(){ try{ var k=(_sc.rugking&&_sc.rugking.active)?_sc.rugking:null; if(!k) return false;
             if(_sc.player){ _sc.player.body.reset(k.x-62, GY-30); _sc.player.setVelocity(0,0); }
             _sc.cameras.main.stopFollow(); _sc.cameras.main.centerOn(k.x-31, 135);
             return {kx:Math.round(k.x)}; }catch(e){ return {err:String(e)}; } };
         } }catch(e){}
    var self=this, i;
    ...
    // Chosen playable character (cosmetic skin). Falls back to Normie if its art hasn't loaded, so a
    // missing/failed asset can never spawn an invisible player. A chosen skin takes precedence over
    // the moon suit (there's no per-character moon art).
    this.charPrefix = nqCharPrefix();
    nqLoadSkin(this.textures, this.charPrefix);   // defensive: decode the skin if boot/picker didn't (idempotent)
    if(this.charPrefix && !this.textures.exists(this.charPrefix+'normie')) this.charPrefix='';
    ...
    if(!this._labWarp && !nqWorldAllowed(def)){
      var _wm=/^(\d+)-/.exec((def&&def.name)||'');
      this.scene.start('VipPitch',{locked:true,score:this.score||0,world:_wm?+_wm[1]:0});
      return;
    }
    ...
    try{ MUSIC.forWorld(nqTrackFor(def)); }catch(e){}   // F7: see nqTrackFor — charAt(0) sent 36 levels to the World-1 theme
    ...
    this.cameras.main.setBounds(0,0,LW,H); this.cameras.main.setZoom(RES); this.cameras.main.setBackgroundColor(th.sky1);
    ...
    // 🌊 WATER PITS (beach theme, 29): the gaps aren't empty void — they're WATER, with a sea-beast
    // lurking in each one, waiting. Falling in still costs a life (the pit-death below), but themed as
    // being dragged under. Purely cosmetic hazards layered into the gap; the kill logic is unchanged.
    this.isWater = (def.theme===29);
    if(this.isWater){
      this.waterMonsters=[];
      var wSurf=GY+3, wBot=H+150;
      gaps.forEach(function(gp){
        var gx0=gp[0], gx1=gp[1], gw=gx1-gx0, gcx=(gx0+gx1)/2;
        self.add.rectangle(gcx, (wSurf+wBot)/2, gw+6, wBot-wSurf, 0x0e5a80, 0.86).setDepth(1);   // deep water
        self.add.rectangle(gcx, wSurf+22, gw+6, 40, 0x1a86b0, 0.45).setDepth(1);                  // lighter upper column
        if(gw>=64){                                                                               // a beast fits in a wide-enough pit
          var mon=self.add.image(gcx, wSurf+12, 'waterlurker').setDepth(2);
          mon.setScale(Math.min(1, (gw-14)/64));
          // lunge state (driven in waterTick — NOT a looping tween, so it can strike). It rises out of
          // the water to swipe at a player crossing the gap: a low jump gets caught, a high one clears
        ...
        self.tweens.add({ targets:surf, alpha:0.26, duration:950, yoyo:true, repeat:-1, ease:'Sine.easeInOut' });
        self.add.rectangle(gcx, wSurf, gw+6, 3, 0xbdf0fb, 0.9).setDepth(3);                        // bright surface line
      });
    }
    ...
    // enemies
    // Scary-world procedural sprites (eyeball / long-neck / gravemite turret) are canvas-drawn on
    // demand like the shark & sandlord — exists-guarded, so this is free after the first level.
    this.drawEyeballTex(); this.drawLongneckTex(); this.drawGravemiteTex();
    ...
    (def.miniworms||[]).forEach(function(mx,i){
      ...
      var _mwKey = /^2-/.test(def.name||'') ? 'miniworm' : (def.mwTex||'sentry');   // def.mwTex re-skins the turret (scary world: 'gravemite'); world 2 stays worms — see above
    ...
    this.poisonClouds=this.physics.add.group({allowGravity:false});// LONG-NECK's pit poison — HURTS (see poisonTouch), not a slow
    ...
    this.player.setCollideWorldBounds(true); this.player.setMaxVelocity(192,600);   // 240→192, owner 2026-08-16: overall speed down ~20% (keep in sync with the caps in update())
    ...
    this.physics.add.overlap(this.player,this.poisonClouds,this.poisonTouch,null,this);
    ...
    // TEST BUILD: expose the current level + a "back to level picker" hook to the DOM overlay
  ...
  makeEnemy:function(kind,x,y,range){
    ...
    var spd={jeet:50,paper:66,bot:88,ghost:52,bitmaxi:72,sniper:34,fudster:44,gasgoblin:46,sandwich:58,laserbot:34,mevdrone:64,flashbot:40,drillbit:60,flyfish:44,jumpfish:60,rugpuller:130,eyeball:44,longneck:38}[kind]||55;
    ...
    if(kind==='flashbot'||kind==='flyfish'||kind==='eyeball'){ e.fbState='hover'; e.fbT=this.time.now; e.fbNext=this.time.now+800+Phaser.Math.RND.between(0,600); var _fg=this.addGlow(e,kind==='flyfish'?0x7ed6f0:(kind==='eyeball'?0xff5b6e:0x66ddff),3); if(_fg) this.tweens.add({targets:_fg,outerStrength:8,duration:480,yoyo:true,repeat:-1,ease:'Sine.inOut'}); }   // FLASH DRONE / FLYING FISH / FLOATING EYEBALL: hovers then DIVE-BOMBS you (telegraphed)
    ...
    if(kind==='longneck'){ e.nextSpray=this.time.now+1600+Phaser.Math.RND.between(0,900); var _lg=this.addGlow(e,0x8cdc6e,3); if(_lg) this.tweens.add({targets:_lg,outerStrength:7,duration:700,yoyo:true,repeat:-1,ease:'Sine.inOut'}); }   // LONG-NECK (scary world): roams; sprays heart-taking poison into pits (see the longneck block in update)
    ...
    if(kind==='ghost'||kind==='flashbot'||kind==='rugpuller'||kind==='flyfish'||kind==='eyeball') e.body.setAllowGravity(false); else e.setCollideWorldBounds(true);   // flyfish/eyeball hover (dive-bombers); jumpfish keeps gravity (erupts from the sand)
    return e;
  },
  ...
  grabPowerup:function(player,pu){
    ...
    try{ nqTele('powerup',{world:this.def&&this.def.name,item:type,x:Math.round(px)}); }catch(e){}
    ...
    else if(type==='candle'){ if(this.lives<this.maxLives){ this.lives++; if(this.hearts[this.lives-1]) this.hearts[this.lives-1].setAlpha(1); } this.candleUntil=now+6000; this.addScore(100); this.powerBanner('GREEN CANDLE',(this._dark?'LIGHT IT UP — wider torch 6s, ':'PUMPING — ')+'+1 heart, +100','#3dff6e'); }
  ...
  popBlock:function(block){
    ...
    if(this.lives<=1 && this.lives<this.maxLives){ this.lives++; if(this.hearts[this.lives-1]) this.hearts[this.lives-1].setAlpha(1); SFX.power(); this.cameras.main.flash(160,255,120,160); this.flash('CLUTCH! +1 HEART','#ff5a8a'); }
  ...
  setupDarkness:function(){   // build the moving "torchlight" overlay for dark (mines) levels
    ...
    // def.darkAlpha < 1 = "mostly dark, not pitch black" (owner ask for the scary world): the level +
    // the glowing monsters + the key show through faintly outside the torch, instead of pure black.
    // Mines levels set no darkAlpha and stay fully dark. R already lights the area you're standing in.
    if(this.def && this.def.darkAlpha!=null) this.darkOverlay.setAlpha(this.def.darkAlpha);
    this.darkScale=1;
    // KEY BEACON — in the scary den, lift the key + its glow ABOVE the darkness (depth 61, under the
    // HUD's 1000) and pulse it bright, so it reads as a findable gold beacon in the dark. (owner:
    // "make finding the key easier"). Its glow was being hidden behind the full-black overlay before.
    if(this.def && this.def.darkAlpha!=null && this.key){
      this.key.setDepth(61);
      var kb=this.addGlow(this.key,0xffe45a,6); if(kb) this.tweens.add({targets:kb,outerStrength:16,duration:600,yoyo:true,repeat:-1,ease:'Sine.inOut'});
    }
  },
  ...
  useReserve:function(slot){   // spend a specific slot (default: the SELECTED one)
    ...
    if(id==='vial' && this.lives>=this.maxLives){ this.flash('HEARTS ALREADY FULL','#ff9ec0'); return; }   // don't waste a full-heal at full HP
  ...
  manualThrow:function(){   // MANUAL THROW: hurl a Solana disc from the counted ammo (F/X or THROW button)
    ...
    // small muzzle puff at the player — NOT a full-screen camera flash. The flash strobed the whole
    // screen on every throw (tester on iPhone Safari: "when I shoot the coins my screen flashes").
    try{ var p=this.player, dir=p.flipX?-1:1; this.burst(p.x+dir*16, (p.body?p.body.bottom-16:p.y-4), 0x14f195, 5); }catch(e){}
  },
  ...
  grabAirdrop:function(player,a){
    ...
    if(this.lives<this.maxLives){ this.lives++; if(this.hearts[this.lives-1]) this.hearts[this.lives-1].setAlpha(1); this.powerBanner('HEALING AIRDROP','BAGS SAVED — +1 HEART','#3dff6e'); }
  ...
  tryDoor:function(player,door){
    ...
    if(this.def.hidden && this.def.next==null){ this.returnFromHidden(); return; }   // non-boss bonus room exit → back to the surface (a hidden level WITH def.next chains onward instead — e.g. the private MySpace world)
  ...
  penBoss:function(){
    ...
    // FLOOR CLAMP (gravity bosses only). The wormhole BURROWS below the ground on purpose and has
    // allowGravity=false, so it is excluded — as is any never-falling flyer. A gravity boss's arena
    // floor is flat at GY, so its feet must never cross below it. Two ways they did:
    //   • a hard LEAP-landing at a low frame-rate punched the boss a whole tile through the floor —
    //     Arcade separates a body that crossed a floor tile's CENTRE out the FAR side, so it dropped
    //     onto the SECOND floor row at GY+TILE and WEDGED there permanently ("not jumping, going into
    //     the ground"), body still overlapping and KILLING the player (game-over: BOSS TOUCH);
    //   • or it was shoved / fell into a pit and kept dropping.
    // Correct ANY penetration the instant it happens: set the feet exactly on the surface and kill
    // only the DOWNWARD velocity (an upward leap is untouched). penBoss runs in update() BEFORE the
    // frame is drawn, so the boss is never RENDERED below the floor — there is no dip at any frame
    // rate. x is already re-clamped just above. This subsumes the old deep-fall snap-back.
    if(b.body.allowGravity && b.body.bottom > GY+1){
  ...
  // Prize-room SHARK texture — drawn once, on demand (same canvas technique as swanfeather/sigorb),
  // so the ocean prize level needs no new asset in the build pipeline. Side view, nose to the LEFT;
  // the shared KOL boss sprite is built from this key when a level sets bossTex:'shark'.
  drawSharkTex:function(){
    if(this.textures.exists('shark')) return;
    var cv=this.textures.createCanvas('shark',132,72), c=cv.getContext();
    c.clearRect(0,0,132,72); c.translate(66,36);
    // caudal (tail) fin — crescent at the RIGHT
    c.fillStyle='#586772';
    c.beginPath(); c.moveTo(40,-4); c.lineTo(64,-26); c.lineTo(56,-2); c.lineTo(64,22); c.lineTo(40,6); c.closePath(); c.fill();
    // pectoral fin (sits behind the body)
    c.fillStyle='#4d5c67';
    c.beginPath(); c.moveTo(-16,10); c.lineTo(-30,30); c.lineTo(-2,14); c.closePath(); c.fill();
    // body — grey torpedo, nose to the LEFT
    c.fillStyle='#6c7c88';
    c.beginPath();
    c.moveTo(-54,2);
    c.quadraticCurveTo(-40,-20,6,-16);
    c.quadraticCurveTo(30,-14,44,-4);
    c.quadraticCurveTo(28,0,44,4);
    c.quadraticCurveTo(24,16,-6,16);
    c.quadraticCurveTo(-40,16,-54,2);
    c.closePath(); c.fill();
    // white belly overlay
    c.fillStyle='#dfe7ea';
    c.beginPath();
    c.moveTo(-50,6); c.quadraticCurveTo(-30,17,-6,16); c.quadraticCurveTo(20,15,40,5);
    c.quadraticCurveTo(16,12,-6,11); c.quadraticCurveTo(-34,11,-50,6); c.closePath(); c.fill();
    // dorsal fin
    c.fillStyle='#586772';
    c.beginPath(); c.moveTo(-2,-15); c.lineTo(16,-40); c.lineTo(22,-13); c.closePath(); c.fill();
    // gill slits
    c.strokeStyle='#3f4c56'; c.lineWidth=1.6; c.lineCap='round';
    for(var g=0; g<3; g++){ c.beginPath(); c.moveTo(-30+g*5,-8); c.lineTo(-32+g*5,8); c.stroke(); }
    // mouth
    c.strokeStyle='#3a2530'; c.lineWidth=2.4;
    c.beginPath(); c.moveTo(-54,6); c.quadraticCurveTo(-44,12,-30,11); c.stroke();
    // teeth
    c.fillStyle='#ffffff';
    for(var t=0;t<5;t++){ var tx=-50+t*4; c.beginPath(); c.moveTo(tx,8); c.lineTo(tx+2,8); c.lineTo(tx+1,12); c.closePath(); c.fill(); }
    // eye
    c.fillStyle='#0a0e12'; c.beginPath(); c.arc(-40,-4,3.1,0,6.283); c.fill();
  ...
  // ===== SCARY WORLD sprites (?room=scary) — all procedural like the shark/sandlord above =====
  // FLOATING EYEBALL — a bloodshot eye that hovers and dive-bombs (re-skins the flash-drone AI).
  // Pupil glances LEFT (the enemy "nose" convention — setFlipX turns it).
  drawEyeballTex:function(){
    if(this.textures.exists('eyeball')) return;
    var cv=this.textures.createCanvas('eyeball',40,40), c=cv.getContext();
    c.clearRect(0,0,40,40); c.translate(20,20);
    // trailing optic-nerve strands (behind, to the right)
    c.strokeStyle='#c94f6d'; c.lineWidth=2; c.lineCap='round';
    c.beginPath(); c.moveTo(12,4); c.quadraticCurveTo(18,8,17,14); c.stroke();
    c.beginPath(); c.moveTo(13,0); c.quadraticCurveTo(19,2,18,8); c.stroke();
    // sclera
    c.fillStyle='#f2efe6'; c.beginPath(); c.arc(0,0,14,0,6.283); c.fill();
    // bloodshot veins
    c.strokeStyle='rgba(201,79,90,0.75)'; c.lineWidth=1.1;
    c.beginPath(); c.moveTo(-13,-3); c.quadraticCurveTo(-8,-2,-5,-4); c.stroke();
    c.beginPath(); c.moveTo(-11,6); c.quadraticCurveTo(-6,4,-3,6); c.stroke();
    c.beginPath(); c.moveTo(9,-9); c.quadraticCurveTo(6,-6,7,-3); c.stroke();
    // iris + pupil
    c.fillStyle='#b8452f'; c.beginPath(); c.arc(-4,0,7.2,0,6.283); c.fill();
    c.fillStyle='#ff9b3d'; c.beginPath(); c.arc(-4,0,5.4,0,6.283); c.fill();
    c.fillStyle='#120a0a'; c.beginPath(); c.arc(-4,0,3.1,0,6.283); c.fill();
    // wet highlight
    c.fillStyle='rgba(255,255,255,0.85)'; c.beginPath(); c.arc(-8,-6,2.6,0,6.283); c.fill();
    cv.refresh();
  },
  // LONG-NECK — a squat shadow-beast whose neck arcs up-left to a small head that drools the
  // poison it sprays into pits. Head LEFT per the enemy convention.
  drawLongneckTex:function(){
    if(this.textures.exists('longneck')) return;
    var cv=this.textures.createCanvas('longneck',56,52), c=cv.getContext();
    c.clearRect(0,0,56,52); c.translate(28,26);
    var hide='#2c2438', hideD='#1b1526', belly='#4a3f5e';
    // legs
    c.fillStyle=hideD; c.fillRect(-14,16,6,9); c.fillRect(2,16,6,9); c.fillRect(10,15,6,10);
    // body blob (right side)
    c.fillStyle=hide; c.beginPath(); c.ellipse(2,10,18,11,0,0,6.283); c.fill();
    c.fillStyle=belly; c.beginPath(); c.ellipse(3,14,13,6,0,0,6.283); c.fill();
    // back spines
    c.fillStyle=hideD; for(var s2=0;s2<4;s2++){ var sx=12-s2*7; c.beginPath(); c.moveTo(sx,1); c.lineTo(sx-3,-5+(s2%2)); c.lineTo(sx-6,1); c.closePath(); c.fill(); }
    // the LONG neck, arcing up-left
    c.strokeStyle=hide; c.lineWidth=7; c.lineCap='round';
    c.beginPath(); c.moveTo(-8,8); c.quadraticCurveTo(-20,-4,-16,-16); c.stroke();
    // head + open snout
    c.fillStyle=hide; c.beginPath(); c.ellipse(-16,-19,8,5.5,-0.35,0,6.283); c.fill();
    c.fillStyle='#0e0a14'; c.beginPath(); c.moveTo(-23,-18); c.lineTo(-15,-16.5); c.lineTo(-22,-14); c.closePath(); c.fill();
    // poison drool
    c.fillStyle='rgba(140,220,110,0.85)'; c.beginPath(); c.arc(-23,-13,2.2,0,6.283); c.fill();
    c.fillStyle='rgba(140,220,110,0.5)'; c.beginPath(); c.arc(-24,-9,1.6,0,6.283); c.fill();
    // glowing eye
    c.fillStyle='#9dff5a'; c.beginPath(); c.arc(-17,-21,1.8,0,6.283); c.fill();
    cv.refresh();
  },
  // GRAVEMITE — the scary world's ground turret skin: a little horned cyclops that pops out of
  // the dirt (same burrow/rise/radial-fan mechanic as the sentry — only the sprite differs).
  drawGravemiteTex:function(){
    if(this.textures.exists('gravemite')) return;
    var cv=this.textures.createCanvas('gravemite',36,44), c=cv.getContext();
    c.clearRect(0,0,36,44); c.translate(18,22);
    var skin='#3a2d4e', skinD='#241a33';
    // horns
    c.fillStyle=skinD;
    c.beginPath(); c.moveTo(-10,-14); c.lineTo(-14,-21); c.lineTo(-6,-16); c.closePath(); c.fill();
    c.beginPath(); c.moveTo(10,-14); c.lineTo(14,-21); c.lineTo(6,-16); c.closePath(); c.fill();
    // squat body
    c.fillStyle=skin; c.beginPath(); c.ellipse(0,-2,13,16,0,0,6.283); c.fill();
    // claws gripping the dirt
    c.fillStyle=skinD; c.fillRect(-15,10,7,5); c.fillRect(8,10,7,5);
    // big cyclops eye
    c.fillStyle='#efe9da'; c.beginPath(); c.arc(0,-7,6,0,6.283); c.fill();
    c.fillStyle='#c9303e'; c.beginPath(); c.arc(0,-7,3.4,0,6.283); c.fill();
    c.fillStyle='#0c0810'; c.beginPath(); c.arc(0,-7,1.7,0,6.283); c.fill();
    // jagged grin
    c.fillStyle='#0e0a14'; c.fillRect(-8,3,16,5);
    c.fillStyle='#efe9da'; for(var t2=0;t2<4;t2++){ c.beginPath(); c.moveTo(-7+t2*4,3); c.lineTo(-5+t2*4,3); c.lineTo(-6+t2*4,6.6); c.closePath(); c.fill(); }
    cv.refresh();
  },
  // THE GHOST GALLEON — scary-world finale boss: a haunted pirate ship with the BIG BIG monster
  // rising out of the deck (tentacles, one huge eye). Drawn wide so it reads huge at bossScale.
  drawGhostShipTex:function(){
    if(this.textures.exists('ghostship')) return;
    var cv=this.textures.createCanvas('ghostship',170,130), c=cv.getContext();
    c.clearRect(0,0,170,130); c.translate(85,65);
    var hull='#241a12', hullD='#140d08', sail='#9fb3a8', mast='#3a2a1a', glow='#7af0a0';
    // ---- the BIG BIG monster first, so it looms BEHIND the ship ----
    // tentacles
    c.strokeStyle='#274232'; c.lineWidth=7; c.lineCap='round';
    c.beginPath(); c.moveTo(-52,10); c.quadraticCurveTo(-64,-18,-52,-34); c.stroke();
    c.beginPath(); c.moveTo(56,12); c.quadraticCurveTo(70,-14,58,-30); c.stroke();
    c.beginPath(); c.moveTo(40,6); c.quadraticCurveTo(52,-30,38,-44); c.stroke();
    // head looming over the deck
    c.fillStyle='#2f5140'; c.beginPath(); c.ellipse(24,-38,26,20,0,0,6.283); c.fill();
    // one huge eye
    c.fillStyle='#e8f0d8'; c.beginPath(); c.arc(18,-40,9,0,6.283); c.fill();
    c.fillStyle='#c9303e'; c.beginPath(); c.arc(18,-40,5,0,6.283); c.fill();
    c.fillStyle='#0c0810'; c.beginPath(); c.arc(18,-40,2.4,0,6.283); c.fill();
    // fangs
    c.fillStyle='#e8f0d8';
    c.beginPath(); c.moveTo(10,-24); c.lineTo(14,-24); c.lineTo(12,-16); c.closePath(); c.fill();
    c.beginPath(); c.moveTo(28,-23); c.lineTo(32,-23); c.lineTo(30,-15); c.closePath(); c.fill();
    // ---- masts + tattered sails ----
    c.fillStyle=mast; c.fillRect(-30,-58,4,66); c.fillRect(6,-50,4,58);
    c.fillStyle=sail; c.globalAlpha=0.85;
    c.beginPath(); c.moveTo(-28,-54); c.quadraticCurveTo(-6,-46,-28,-30); c.lineTo(-28,-38); c.lineTo(-24,-34); c.lineTo(-28,-26); c.closePath(); c.fill();
    c.beginPath(); c.moveTo(8,-46); c.quadraticCurveTo(28,-38,8,-22); c.lineTo(8,-30); c.lineTo(12,-26); c.lineTo(8,-18); c.closePath(); c.fill();
    c.globalAlpha=1;
    // torn black flag with a skull dot
    c.fillStyle='#141414'; c.beginPath(); c.moveTo(-30,-58); c.lineTo(-12,-54); c.lineTo(-18,-51); c.lineTo(-12,-48); c.lineTo(-30,-46); c.closePath(); c.fill();
    c.fillStyle='#e8f0d8'; c.beginPath(); c.arc(-24,-52,2.2,0,6.283); c.fill();
    // ---- hull ----
    c.fillStyle=hull;
    c.beginPath(); c.moveTo(-62,6); c.lineTo(64,6); c.lineTo(50,40); c.quadraticCurveTo(0,52,-46,40); c.closePath(); c.fill();
    // bow + stern rails
    c.fillStyle=hullD; c.fillRect(-66,-2,14,10); c.fillRect(54,-6,16,14);
    // deck line
    c.strokeStyle=hullD; c.lineWidth=3; c.beginPath(); c.moveTo(-58,12); c.lineTo(58,12); c.stroke();
    // glowing portholes — the ghost crew's lights
    c.fillStyle=glow; for(var p2=0;p2<4;p2++){ c.beginPath(); c.arc(-36+p2*24,24,3.4,0,6.283); c.fill(); }
    // hull cracks
    c.strokeStyle='#0c0805'; c.lineWidth=1.5;
    c.beginPath(); c.moveTo(-30,18); c.lineTo(-22,34); c.stroke();
    c.beginPath(); c.moveTo(20,20); c.lineTo(28,36); c.stroke();
    cv.refresh();
  },
  ...
  startKolBoss:function(){
    ...
    if(bTex==='ghostship') this.drawGhostShipTex(); // scary world: the haunted galleon + its big big monster
    this.bossHP=bHits;
    ...
    this.physics.add.overlap(this.player, k, this.bossTouch, null, this);
    this.door.setVisible(false);
    this.bossPips=[]; for(var i=0;i<bHits;i++) this.bossPips.push(this.hb(this.add.image(W/2-(bHits-1)*12+i*24, 22, 'heart').setScale(1.4).setDepth(40).setTint(bPip)));
  ...
  kolTick:function(now,p){
    ...
    if(now>this.kolNextShill){ this.kolNextShill=now+(this.def.bossShillMs||2200); if(this.def.bossAttack==='tom') this.tomAttack(k,p); else if(this.def.bossAttack==='ship') this.shipAttack(k,p); else this.kolShill(k,p); }
  ...
  // TOM's custom fight — three MySpace attacks in rotation, escalating on his last two hits.
  // Same enemyShots conventions as kolShill (no gravity, dieAt, srcCause) + the stomp guard.
  tomAttack:function(k,p){
    if(this.over||!this.enemyShots) return; var self=this;
    if(this.stompImminent(p,k)) return;   // never punish a committed stomp point-blank
    var low=this.bossHP<=2;
    var shot=function(x,y,vx,vy,tint,tex,cause){ var s=self.enemyShots.create(x,y,tex||'bullet').setDepth(6); if(!s||!s.body) return null;
      s.srcCause=cause; s.body.setAllowGravity(false); s.body.setSize(s.width*0.85,s.height*0.85); s.setVelocity(vx,vy); s.setTint(tint); s.dieAt=self.time.now+3000; return s; };
    var base=Math.atan2((p.y-8)-k.y, p.x-k.x);
    this._tomPhase=((this._tomPhase||0)+1)%3;
    if(this._tomPhase===0){
      // FRIEND REQUESTS — a fan of hearts aimed at you (one extra when he's low)
      this.flash('FRIEND REQUEST — DODGE THE ADD!','#8aa0d8');
      var n=low?4:3, sp=195; for(var i=0;i<n;i++){ var ang=base+(i-(n-1)/2)*0.20; shot(k.x,k.y-8,Math.cos(ang)*sp,Math.sin(ang)*sp,0x9fb6ee,'heart','FRIEND REQUEST'); }
      _tone(560,240,0.10,'square',0.05);
    } else if(this._tomPhase===1){
      // TOP 8 — a glitter ring bursting outward (8 shots, 10 when low)
      this.flash('TOP 8 INCOMING!','#ffcf5a');
      var m=low?10:8, sp2=165; for(var j=0;j<m;j++){ var a2=(j/m)*Math.PI*2+(low?0.3:0); shot(k.x,k.y-6,Math.cos(a2)*sp2,Math.sin(a2)*sp2,0xffcf5a,'bullet','TOP 8 GLITTER'); }
      _tone(720,220,0.12,'square',0.05);
    } else {
      // BULLETIN BLAST — a fast flat triple toward you
      this.flash('NEW BULLETIN!','#ff7ad9');
      var sp3=low?250:225; [-0.10,0,0.10].forEach(function(off){ var a3=base+off; shot(k.x,k.y-6,Math.cos(a3)*sp3,Math.sin(a3)*sp3,0xff7ad9,'bullet','BULLETIN BLAST'); });
      _tone(640,200,0.10,'sawtooth',0.05);
    }
  },
  // THE GHOST GALLEON's fight (scary-world finale) — three pirate attacks in rotation, escalating
  // on its last two hearts. Same enemyShots conventions as tomAttack + the stomp guard.
  shipAttack:function(k,p){
    if(this.over||!this.enemyShots) return; var self=this;
    if(this.stompImminent(p,k)) return;   // never punish a committed stomp point-blank
    var low=this.bossHP<=2;
    var shot=function(x,y,vx,vy,tint,tex,cause){ var s=self.enemyShots.create(x,y,tex||'bullet').setDepth(6); if(!s||!s.body) return null;
      s.srcCause=cause; if(tex==='gas') s.setScale(0.6); s.body.setAllowGravity(false); s.body.setSize(s.width*0.85,s.height*0.85); s.setVelocity(vx,vy); s.setTint(tint); s.dieAt=self.time.now+3000; return s; };
    var base=Math.atan2((p.y-8)-k.y, p.x-k.x);
    this._shipPhase=((this._shipPhase||0)+1)%3;
    if(this._shipPhase===0){
      // CANNON VOLLEY — a fast flat triple of cannonballs at you (faster when low)
      this.flash('CANNON VOLLEY — HIT THE DECK!','#9aa4b2');
      var sp=low?245:220; [-0.09,0,0.09].forEach(function(off){ var a=base+off; shot(k.x,k.y-10,Math.cos(a)*sp,Math.sin(a)*sp,0x9aa4b2,'bullet','CANNON VOLLEY'); });
      _tone(140,300,0.14,'square',0.06);
    } else if(this._shipPhase===1){
      // GHOST BROADSIDE — a ring of spirit-fire bursting outward (10 shots when low)
      this.flash('GHOST BROADSIDE!','#7af0a0');
      var m=low?10:8, sp2=160; for(var j=0;j<m;j++){ var a2=(j/m)*Math.PI*2+(low?0.3:0); shot(k.x,k.y-8,Math.cos(a2)*sp2,Math.sin(a2)*sp2,0x7af0a0,'bullet','GHOST BROADSIDE'); }
      _tone(520,260,0.12,'sine',0.05);
    } else {
      // KRAKEN SPIT — an aimed fan of poison blobs (one extra when low; same acid green as the long-necks)
      this.flash('KRAKEN SPIT!','#8ce85a');
      var n=low?4:3, sp3=185; for(var i=0;i<n;i++){ var a3=base+(i-(n-1)/2)*0.18; shot(k.x,k.y-12,Math.cos(a3)*sp3,Math.sin(a3)*sp3,0xd8ff70,'gas','KRAKEN SPIT'); }
      _tone(320,220,0.12,'sawtooth',0.05);
    }
  },
  ...
  bossDefeat:function(){
    ...
    try{ window.__NQ_LIVE=null; }catch(e){}   // level ended legitimately — the abandon beacon must not also fire
    ...
    if(this.def.final){
      // F1: the World-12 kill is only an ENDING for a player who cannot enter the wing. This branch
      // ran BEFORE the normal routing, so it never banked a checkpoint at the wing's first level and
      // worlds 13-21 had no in-game door for anyone. Use the SAME predicate advanceLevel uses —
      // nqWorldAllowed, not nqIsVipOrAll (which ignores the launch cap) — and only fall through to
      // the Win screen when the next world really is closed (tier-1 keeps today's finale exactly).
      var _fnx=(this.def&&this.def.next!=null)?this.def.next:this.levelIdx+1;
      var _fOpen=false;
      try{ _fOpen=(_fnx<LEVELS.length) && (!!this._labWarp || nqWorldAllowed(LEVELS[_fnx])); }catch(e){ _fOpen=false; }
      if(_fOpen){
        this.bankCheckpoint();
        this.powerBanner('THE CARTEL IS PURGED!','RELAUNCH COMPLETE — THE ABYSS OPENS','#ffd23f');
    ...
    } else if(this.def.bossName==='TOM'){
      // PRIVATE MySpace dev world (owner + Normie dev): Tom is the final boss and the last LEVELS
      // entry, so the generic KOL branch below was wrong twice over — it showed "WORLD 3 CLEARED",
      // then advanceLevel() fell off the end into the game-complete Win screen AND submitted a
      // full-clear run to the real leaderboard. Give Tom his own MySpace win, flagged private so
      // the Win scene skips the leaderboard submit; "play again" replays MySpace, not the main game.
      var _msIdx = LEVELS.findIndex(function(l){ return l && l.name==='MYSPACE'; });
      this.addScore(2000);
      this.powerBanner('TOM DEFEATED!',"YOU'RE #1 IN HIS TOP 8 — MYSPACE CLEARED",'#8aa0d8');
      this.time.delayedCall(2200, function(){ this.scene.start('Win',{score:this.score, boss:'tom', private:true, replayLevel:(_msIdx>=0?_msIdx:0), casino:this.registry.get('nqCasino')||0}); },[],this);
  ...
  // F4/F5: carry the world actually REACHED. worldCount() answers 21 for every non-private win, so a
  // real 1-1 -> 12-3 clear submitted world=21, which the server's anti-forgery bound marks suspect
  // (filtered from every board), and the Win screen congratulated a 12-world finisher on "ALL 21
  // WORLDS CLEARED". Hidden/private rooms have non-numeric names — they fall back to worldCount().
  winData:function(){ var _wc=/^(\d+)-/.exec((this.def&&this.def.name)||'');
    return {worldsCleared:(_wc?+_wc[1]:0), score:this.score, boss:(this.def&&this.def.bossType==='wenmoon')?'wenmoon':(this.def&&this.def.bossType==='saylor')?'saylor':(this.def&&this.def.vip)?'vip':this.def.bossType==='dirtywhale'?'dirtywhale':this.def.bossType==='mevdragon'?'mevdragon':this.def.bossType==='blackswan'?'blackswan':this.def.final?'51attack':(this.def.bossType||(this.def.boss?'rugking':'wormhole')), casino:this.registry.get('nqCasino')||0}; },
  advanceLevel:function(){ var next=(this.def&&this.def.next!=null)?this.def.next:this.levelIdx+1;   // def.next skips the hidden block (9-3 → 10-1)
    ...
    if(!lab && !nqWorldAllowed(LEVELS[next])){ var _wm2=/^(\d+)-/.exec((LEVELS[next]&&LEVELS[next].name)||''); this.scene.start('VipPitch',{locked:true,score:this.score,world:_wm2?+_wm2[1]:0}); return; }
  ...
  emitPoison:function(e,pitX){   // LONG-NECK rears up and sprays a spray of poison puffs INTO the pit
    if(this.over||!this.poisonClouds) return; var self=this;
    // In the scary den the gas is drawn ABOVE the darkness (depth 62 < HUD's 1000) so it GLOWS as a
    // green pool in the pit — you could barely see it under full dark before ("where's the gas?").
    var gd=(this.def&&this.def.darkAlpha!=null)?62:5, dir=(pitX>e.x)?1:-1;
    this.burst(e.x+dir*8, e.y-24, 0x9dff5a, 12);   // acid spit at the mouth
    try{ _tone(200,460,0.15,'sawtooth',0.05); }catch(err){}
    for(var i=0;i<5;i++){
      var g=this.poisonClouds.create(e.x+dir*10, e.y-26, 'gas');
      if(!g||!g.body) continue;
      g.setDepth(gd).setTint(0xcaff4a).setAlpha(0.92).setScale(0.7);   // bigger + acid-bright + glowing
      g.body.setAllowGravity(false); g.body.setSize(g.width*0.72,g.height*0.72);
      g.setVelocity(dir*(70+i*26), 26+i*12);   // arcs OUT from the mouth toward the pit and SINKS in
      (function(gg){ self.tweens.add({targets:gg, scaleX:1.9, scaleY:1.9, alpha:0, duration:3200, ease:'Sine.out', onComplete:function(){ if(gg.active) gg.destroy(); }}); })(g);
    }
  },
  poisonTouch:function(player,cloud){   // LONG-NECK poison gas — takes a heart (same guard set as spikeHit)
    var now=this.time.now;
    if(this.over||player.invuln||now<this.shieldUntil||now<this.moonUntil||now<this.omegaUntil||now<this.whaleUntil||now<this.coldUntil) return;
  ...
  waterTick:function(now){
    ...
    for(var i=0;i<this.waterMonsters.length;i++){
      ...
      if(m.st==='idle'){
        ...
        if(rt>=1){ m.st='idle'; m.next=now + 640 + (i*170)%620; }       // staggered cooldown
      }
    }
  },
  ...
  // Belt-and-braces for the respawn itself: even a banked spot can go bad (a platform destroyed by
  // a mechanic we haven't written yet), and respawning into thin air over a pit is the one outcome
  // that must never happen. Cheap, so it runs on every pit death.
  footingUnder:function(x,y){
    if(!this.platforms) return false;
    var found=false;
    this.platforms.children.iterate(function(t){
      if(found||!t||!t.body||!t.body.enable) return; var s=t.body;
      if(x>s.x-4 && x<s.x+s.width+4 && s.y>=y-8 && s.y<y+90) found=true;
    });
    return found;
  },
  ...
  resumeGame:function(){ if(!this.paused) return; this.paused=false;
    ...
    // (where a thumb naturally rests, and the card only says "TAP TO RESUME") spent a counted disc
    // — 10 per level — with no player intent; resuming on JUMP burned a jump charge the same way.
    // Latching both to `true` means the held button must be RELEASED and pressed again to count,
    // which is what the player means by "unpause, then throw". Pad edges get drained for the same
    // reason (anyBtnEdge is already handled by the caller; jumpEdge was not).
    this.prevThrow=true; this.prevJump=true; this.jumpBufferAt=-9999;
    try{ if(window.__NQ_PAD){ window.__NQ_PAD.jumpEdge=false; window.__NQ_PAD.selEdge=false; } }catch(e){}
  ...
  update:function(){
    ...
    { var _pe=(typeof window!=='undefined')?window.__NQ_PAD:null;
      ...
      // Shared with padAdvance — this list used to miss the remap card and the how-to card, so a
      // controller press during either still reached the running game.
      var _modalUp=nqModalUp();
    ...
    if(this.over||this.paused){
      ...
      if(this.paused && this._pauseAudioLine && (!this._paLast || Date.now()-this._paLast>600)){
        ...
        try{ var st=MUSIC.state(), c=st.ctx, adv=null;
          ...
          // fps here is the REAL render loop rate — the number to read off an iPad when "it's laggy"
          this._pauseAudioLine.setText('audio: '+(c||'none')+'@'+adv+(st.streaming?' · track':' · synth')+' · fps '+_fps);
        }catch(e){} }
      return;
    }
    ...
    var fast=now<this.caffeineUntil||now<this.moonUntil||now<this.omegaUntil;   // caffeine/moon/GIGA CHAD = super speed
    // GIGA CHAD joins the fast tier (owner 2026-08-15: "in giga chad mode he gets slow"). Omega already
    // GROWS Normie ~1.4x (anyPower), same as caffeine — but caffeine is 340 and omega was stuck at 240,
    // so a big chad lumbered at pedestrian speed, the opposite of "UNSTOPPABLE". Matching caffeine's cap
    // makes him big AND fast; it also frees him from FUD-gas slow (fast disables it) — fitting the theme.
    ...
    // SPEED RETUNE (owner, 2026-08-16, after playing the 3x build): everything ~20% down —
    // base 240→192, slow 120→96 — and the boost tier deliberately much flatter: 340→225
    // (1.17x base, was 1.42x; "not too much extra speed on the boost"). Accels scaled to match
    // so time-to-top-speed feels the same. Reachability holds: the widest gap in any level is
    // 120px vs a ~184px single-jump reach at 192 (all 217 gaps measured; the geometry checker's
    // caps were re-derived for the new speed in the same commit).
    p.setMaxVelocity(fast?225:(slow?96:192),600);
    var accel=fast?1080:(slow?345:720);
    ...
    // ONE pass over this.platforms answers both questions — "am I wedged inside solid?" and "is my
    // footing permanent?" (below). They were two separate full iterations over every ground tile,
    // every frame, which on a 7800px level is ~1300 body tests per frame for nothing; a phone/iPad
    // pays for that twice.
    var _pcx=b.center.x, _pcy=b.center.y, _inSolid=false, _onSolid=false;
    var _fx1=b.left+2, _fx2=b.right-2, _fby=b.bottom;
    this.platforms.children.iterate(function(t){ if(!t||!t.body||!t.body.enable) return; var s=t.body;
      if(!_inSolid && _pcx>s.x+2 && _pcx<s.x+s.width-2 && _pcy>s.y+3 && _pcy<s.y+s.height-3) _inSolid=true;
      if(!_onSolid && _fx2>s.x && _fx1<s.x+s.width && _fby>=s.y-6 && _fby<=s.y+10) _onSolid=true; });
    if(_inSolid){ if(this._lastSafe){ b.reset(this._lastSafe.x, this._lastSafe.y); } b.velocity.set(0,0); this.isJumping=false; }
    // ...but bank the spot ONLY when the footing is PERMANENT. `onGround` is true on a pump-dump,
    // a bridge plank, a depeg peg, a rug plat, a puller rug, a yield lift and a mover — every one
    // of which is *designed* to vanish or move away under you. Banking those was a guaranteed
    // death loop: the plank you are standing on drops (it is anchored over a pit by design), you
    // fall, the pit death respawns you at the banked spot — which is now 1.6s of empty air over
    // the same pit — and you fall again, burning every remaining life in about two seconds with
    // no input. The depeg is worse: it fires on a global rhythm, so it is not even player-triggered.
    // loseLife() already carries this exact fix for the Bridge Drainer BOSS deck; this generalises
    // it. Positive test against this.platforms (ground + plats + walls, all static) rather than a
    // blocklist of the temporary groups, so a mechanic added later cannot silently re-break it.
    else if(onGround && !this.crouching && _onSolid){ this._lastSafe={ x:p.x, y:p.y }; }
    ...
    // Powerup grow targets, dialed DOWN (owner 2026-08-15: "powerups were too big even before").
    // Was regular 1.55 / whale 2.05 — too clumsy. Now +40% regular, +85% whale. Easy to retune.
    var pt=whale?1.85:(mw?1:(anyPower?1.4:1)); this.pscale.v+=(pt-this.pscale.v)*0.16;
    ...
    // F2: only the MOON pair is a real two-frame cycle. cut_normie_run1 and cut_normie_run2 are two
    // renders of the SAME pose (meanAbsDiff 8.9/255 vs 80.9 for the moon pair), so swapping them
    // 10x/sec was not animation — it was texture boil, and it is the "blurry when running" report.
    // Skins (pr/kd) already held a single pose for their own reason: two detailed illustrations
    // flipping that fast read as a bounce/blur while moving (owner playtest 2026-08-15). Everyone
    // but the moon suit now holds nrun1; restore the cycle here if run2 is ever re-cut as a real
    // opposite-leg pose.
    var baseKey = this.crouching ? 'nduck'
                : !onGround ? 'njump'
                : (Math.abs(b.velocity.x)>25 ? ((!this.charPrefix && this.moonSuit) ? ((Math.floor(now/(fast?70:100))%2) ? 'nrun2':'nrun1') : 'nrun1')
                : 'normie');
    // SKIN the pose set: the chosen character (this.charPrefix) OR, for default Normie on lunar
    // levels, the moon suit ('m'). Each skin's five frames share Normie's 144x216 canvas, so a
    // setTexture swap keeps scale + body identical. Every step falls back so a missing frame — or a
    // skin with no crouch — can never blank the player or break movement.
    var pfx = this.charPrefix || (this.moonSuit ? 'm' : '');
    var animKey;
    if(pfx && this.textures.exists(pfx+baseKey)) animKey = pfx+baseKey;                          // skinned pose
    else if(pfx && baseKey==='nduck' && this.textures.exists(pfx+'normie')) animKey = pfx+'normie'; // skin has no duck → its idle
    else animKey = (baseKey==='nduck') ? (this.textures.exists('nduck')?'nduck':'normie') : baseKey; // bare Normie
    ...
    if(mw){
      ...
      p.setRotation(0);
      // Any dedicated duck frame — nduck / mnduck (moon) / prnduck / kdnduck (skins) — renders
      // UNSQUASHED at base scale. The old regex only matched m?nduck, so the skins fell to the wide
      // squash below: squashing scaleY to 0.52 shrinks the crouch body too, so the sprite drops ~8px
      // and on stand-up the standing body is embedded in the ground and wedges (owner: "stuck in the
      // ground" when hitting DOWN). Match every skin's duck frame so they behave like Normie's.
      if(/nduck$/.test(p.texture.key)) p.setScale(bs);
      else p.setScale(bs*1.28, bs*0.52);   // fallback only when a skin has no duck art yet (squash its idle)
    } else if(!onGround){
      ...
      // F3: steady clean run for EVERY character. The 2026-08-15 fix was gated on charPrefix, which
      // is falsy for base Normie, so he alone kept a +/-0.05 rad wobble and a non-uniform scale on
      // top of the frame swap — three incommensurate periods drifting his exact 2:1 NEAREST
      // minification, which is why he blurs while moving and is crisp standing still.
      p.setRotation(0); p.setScale(bs);   // clarity over bounce (owner 2026-08-15)
    } else {
    ...
    this.enemies.children.iterate(function(e){
      ...
      if(e.kind==='flashbot'||e.kind==='flyfish'||e.kind==='eyeball'){   // FLASH DRONE / FLYING FISH / FLOATING EYEBALL: hovers + tracks you → TELEGRAPHS (shudder + flash) → DIVE-BOMBS straight at where you were → peels back up. A clear, fast aerial threat you dodge then punish.
        ...
        if(e.fbState==='hover'){
          ...
          if(now>e.fbT+420){ e.fbState='dive'; e.fbT=now; e.clearTint(); e.fbTX=p.x; var ang=Math.atan2(p.y-e.y,p.x-e.x); e.setVelocity(Math.cos(ang)*320,Math.sin(ang)*320); self.burst(e.x,e.y,(e.kind==='eyeball'?0xff5b6e:0x8fe6ff),10); }
      ...
      // poison down into it. Its acid-green gas TAKES A HEART (poisonTouch) — the FUDster's
      // softer slow-gas never does — so the cloud sinks into the pit you were about to cross.
      if(e.kind==='longneck'){
        if(e.sprayUntil&&now<e.sprayUntil){ e.setVelocityX(0); e.setRotation(0); }
        else if(now>e.nextSpray){
          // Scan BOTH directions for a pit (up to 150px), then TURN to face it and spray in. The old
          // code only checked the way it happened to be walking, so a longneck at the near edge of a
          // gap was usually facing away and never sprayed — "where's the gas?". Now it always finds
          // the pit it's guarding and leans over it.
          var _pitX=0,_pdir=e.dir;
          for(var _q=24;_q<=150&&!_pitX;_q+=13){
            if(self.overPit(e.x+_q)){ _pitX=e.x+_q; _pdir=1; }
            else if(self.overPit(e.x-_q)){ _pitX=e.x-_q; _pdir=-1; }
          }
          if(_pitX){ e.dir=_pdir; e.setFlipX(e.dir>0); e.sprayUntil=now+680; e.nextSpray=now+2200+Phaser.Math.RND.between(0,500); self.emitPoison(e,_pitX); }
          else e.nextSpray=now+400;   // no pit in reach — try again shortly
        }
      }
    });
    ...
    // 🐙 water lurkers (beach worlds) — lunge out of the pits at a crossing player
    if(this.isWater) this.waterTick(now);

    // healing airdrops drop in only while you're hurt (lives<max); hide again at full health
    var hurt=this.lives<this.maxLives;
    ...
    if(p.y>H+80){
      ...
      this.loseLife(this.isWater ? 'DRAGGED UNDER' : 'FELL IN A PIT'); if(this.over) return;
      p.setVelocity(0,0);
      ...
      // Re-verify the banked spot still has solid ground under it before using it. Without this a
      // stale bank drops you straight back down the same pit, death-looping your remaining lives.
      if(this._lastSafe && this.footingUnder(this._lastSafe.x, this._lastSafe.y)){ p.setPosition(this._lastSafe.x, this._lastSafe.y-20); if(p.body) p.body.reset(this._lastSafe.x, this._lastSafe.y-20); }
      else { var _fx=Math.max(this.spawn.x,p.x-260), _fy=H-100;
        // walk back toward spawn until we find real floor, so the fallback can't land in a pit either
        for(var _i=0;_i<40 && _fx>this.spawn.x && !this.footingUnder(_fx,GY);_i++) _fx-=TILE*2;
        p.setPosition(Math.max(this.spawn.x,_fx),_fy); if(p.body) p.body.reset(Math.max(this.spawn.x,_fx),_fy); }
...
var Over=new Phaser.Class({ Extends:Phaser.Scene,
  ...
  create:function(){
    ...
    // in the handle field — must not also restart the run. Skip and RE-ARM while the panel is up.
...
var Win=new Phaser.Class({ Extends:Phaser.Scene,
  ...
  init:function(d){ d=d||{}; this.finalScore=d.score||0; this.boss=d.boss||'wormhole'; this.casino=d.casino||0; this.private=!!d.private; this.replayLevel=(d.replayLevel!=null)?d.replayLevel:0;
    this.worldsCleared=(d.worldsCleared>0)?d.worldsCleared:worldCount(); },   // F4/F5: the world REACHED (winData), not "every world that exists"
  create:function(){
    // SETUP LANE: run WON (full clear) — submit the final score at the top world tier.
    // A PRIVATE win (the MySpace dev room) is NOT a real full clear, so it never touches the board.
    if(window.NQLB && !this.private){ try{ window.NQLB.submitRun(this.worldsCleared, this.finalScore); }catch(e){} }   // F4: the reached world — submitting 21 for a 12-world clear got the run filtered as suspect
    SFX.win();
    ...
    var V = this.boss==='wenmoon'   ? {img:'wenmoon',  title:'WEN MOON? NOW. NORMIE MADE IT!', color:'#9fd8ff', tint:0, scale:98, bg:0x05060f, princess:true}
          ...
          : this.boss==='tom'      ? {img:'tom',        title:"TOM DEFEATED — #1 IN HIS TOP 8!", color:'#8aa0d8', tint:0, scale:80, bg:0x101d33}
    ...
    this.cameras.main.setZoom(RES).centerOn(W/2,H/2); this.cameras.main.setBackgroundColor(V.bg);
    ...
    var nw=this.worldsCleared;   // F5: the worlds THIS run cleared (worldCount() still owns the Over screen's "all N worlds" total)
    var _pd = (typeof LEVELS!=='undefined' && LEVELS[this.replayLevel]) || null;   // per-room clear label (was hardcoded to MySpace, wrong for other private rooms)
    var wtext = this.private ? ((_pd&&_pd.roomLabel)||'MYSPACE — PRIVATE DEV ROOM') : (nw<=1?'WORLD 1 CLEARED':(nw===2?'WORLDS 1 & 2 CLEARED':'ALL '+nw+' WORLDS CLEARED'));
...
var WORLD_CLEARS = {
  ...
  // F-flow-2: the whale beat belongs to the WORLD 12 boss (the Dirty Whale Cartel), and 43 — the
  // first level of World 13 — had no card at all, so the clear that F1 now routes through would
  // have skipped straight past the celebration. 46 keeps the burn-shrine destination but is the
  // WORLD 13 card, so it gets World 13's own boss: THE LEVIATHAN of the Trench Throne.
  43: { world:12, title:'THE WHALE CARTEL, PURGED!', color:'#ffd23f', bg:0x241206,
        boss:{img:'dirtywhale',scale:92,tint:0xffffff,angle:10},
        theme:18, dest:'DOWN INTO THE ABYSS…',
        story:'The cartel is broken and the relaunch stands. But the pools they drained ran somewhere — down, into the trenches, where the whales that never sell are still waiting.',
        term:{name:'WHALE',def:'A holder big enough to move the price alone. Watch what they DO on-chain, never what they say.'},
        nostalgia:'YOU HAVE REACHED THE DEEP END. THE WATER IS FINE.' },
  46: { world:13, title:'THE LEVIATHAN, DETHRONED!', color:'#7fdcff', bg:0x041220,
        boss:{img:'leviathan',scale:92,tint:0,angle:10},
        theme:19, dest:'ASCENDING TO THE BURN SHRINE…',
        story:'The abyssal whale-king is off his throne and the pod bows to a new apex. Above the trench burns the shrine where supply goes to die — every token burned makes the rest more scarce.',
...
var WorldClear=new Phaser.Class({ Extends:Phaser.Scene,
  ...
  update:function(){
    ...
    if(this.phase===1 && this.runner){
      // F3: same duplicate-frame boil as the player's run — nrun1/nrun2 are one pose, so this card
      // (the literal "between two run pictures" screen) holds nrun1 and bobs instead of cycling.
...
var Controls=new Phaser.Class({ Extends:Phaser.Scene,
  ...
  create:function(){
    ...
    // ===== PAGE 1 — how to move (its own screen so nothing is crammed) =====
    this.p0=[];
    this.p0.push(this.add.text(cx,42,'① CONTROLS',{fontFamily:'"Press Start 2P"',fontSize:'9px',color:'#3dff6e'}).setOrigin(.5));
    var div=this.add.graphics(); div.lineStyle(1,0x33305a,1); div.beginPath(); div.moveTo(cx,66); div.lineTo(cx,126); div.strokePath(); this.p0.push(div);
    this.p0.push(this.add.text(colL,62,'MOBILE',{fontFamily:'"Press Start 2P"',fontSize:'9px',color:isTouch?'#3dff6e':'#b6bfe0'}).setOrigin(.5));
    this.p0.push(this.add.text(colL,80,'MOVE:  drag the JOYSTICK\n(left side — down to duck)\nJUMP: tap right · ◎ throw',
      {fontFamily:UIFONT,resolution:UIRES,fontSize:'12px',color:'#eef2ff',align:'center',lineSpacing:2}).setOrigin(.5,0));
    this.p0.push(this.add.text(colR,62,'KEYBOARD',{fontFamily:'"Press Start 2P"',fontSize:'9px',color:isTouch?'#b6bfe0':'#3dff6e'}).setOrigin(.5));
    this.p0.push(this.add.text(colR,80,'MOVE:  arrows / A D\nJUMP:  UP / W / SPACE\nDUCK:  DOWN / S',
      {fontFamily:UIFONT,resolution:UIRES,fontSize:'12px',color:'#eef2ff',align:'center',lineSpacing:2}).setOrigin(.5,0));
    this.padLine=this.add.text(cx,140,'🎮 USB CONTROLLER — plug in & press a button\nD-pad move · A = jump · B = throw · Down = duck\nSELECT: tap = use item, hold = pick which · L2/R2 also use',
      {fontFamily:UIFONT,resolution:UIRES,fontSize:'12px',color:'#b6bfe0',align:'center',lineSpacing:2,wordWrap:{width:W-40}}).setOrigin(.5,0);
    this.p0.push(this.padLine);

    // ===== PAGE 2 — the mission + how to change character (was colliding on one screen) =====
    this.p1=[];
    this.p1.push(this.add.text(cx,44,'② THE GOAL',{fontFamily:'"Press Start 2P"',fontSize:'9px',color:'#ffd23f'}).setOrigin(.5));
    this.p1.push(this.add.text(cx,62,'JUMP on enemies to STOMP them\ngrab the KEY, then reach the DOOR\ngrab POWER-UPS · dodge spikes & hazards',
      {fontFamily:UIFONT,resolution:UIRES,fontSize:'12px',color:'#d6d1ec',align:'center',lineSpacing:3,wordWrap:{width:W-40}}).setOrigin(.5,0));
    this.p1.push(this.add.text(cx,120,'⚠ HONEYPOTS look like treasure but DRAIN the\ncoins you earned that level (your total stays safe)',
      {fontFamily:UIFONT,resolution:UIRES,fontSize:'12px',color:'#ff9500',align:'center',lineSpacing:2,wordWrap:{width:W-36}}).setOrigin(.5,0));
    this.p1.push(this.add.text(cx,166,'🎭 CHANGE YOUR CHARACTER',{fontFamily:'"Press Start 2P"',fontSize:'9px',color:'#ff9be0'}).setOrigin(.5));
    this.p1.push(this.add.text(cx,184,'tap 🎮 EXTRAS → 🎭 Character to play as\nNormie · Princess · Lil’ Normie',
      {fontFamily:UIFONT,resolution:UIRES,fontSize:'12px',color:'#e7c9f2',align:'center',lineSpacing:2,wordWrap:{width:W-40}}).setOrigin(.5,0));

    this.cont=this.add.text(cx,H-11,'',{fontFamily:'"Press Start 2P"',fontSize:'10px',color:'#3dff6e'}).setOrigin(.5);
    this.input.on('pointerdown',function(){ self.advance(); });
  ...
  showPage:function(n){ this.page=n; this.t0=0;   // t0=0 → update() re-stamps it, giving each page its own idle timer
    ...
    for(var j=0;j<this.p1.length;j++) this.p1[j].setVisible(n===1);
    if(this.cont) this.cont.setText((this.isTouch?'TAP':'PRESS')+(n===0?' TO CONTINUE  ▶':' TO START  ▶')); },
  advance:function(){ if(this.done) return; var now=this.time.now;
    if(now-this.lastAdv<350) return;                 // debounce: one tap must not skip a whole page
    this.lastAdv=now;
    if(this.page===0){ try{ if(SFX&&SFX.blip) SFX.blip(); }catch(e){} this.showPage(1); }
    else this.go(); },
  go:function(){ if(this.done) return; this.done=true; try{ SFX.power(); }catch(e){} /* a sound can never block the next scene — a closed iOS audio context throws on node creation */ this.scene.start('Game',{level:this.nextLevel,score:this.score,lives:3}); },
  update:function(){ if(!this.t0){ this.t0=this.time.now; return; }
    if(this.cont) this.cont.setAlpha(0.4+0.6*Math.abs(Math.sin(this.time.now/300)));
    // PAD READ — EVERY PAGE, ANY BUTTON.
    // FIELD BUG (owner, 2026-08-22, reported twice): a player could remap their controller and
    // reach this screen, then had to touch the glass or grab a mouse to actually start — the pad
    // worked perfectly once the game began. Two reasons, both here:
    //   1. this whole block used to be wrapped in `if(this.page===0)`, so on page 1 — the START
    //      page, the one you are staring at when you want to go — the pad was never read at all.
    //   2. it only watched buttons 0/1/8/9. A remapped or off-standard pad presses something else
    //      and nothing happens.
    // The one-shot padAdvance() this scene also had could not cover it: it fires once getting you
    // from page 0 to page 1 and is spent from then on. It has been removed in create() so this
    // loop is the single owner of pad-advance here — no double-fire, no dead second page.
    ...
    try{ var ps=(navigator.getGamepads?navigator.getGamepads():[]); for(var i=0;i<ps.length;i++){ var gp=ps[i]; if(!gp) continue; on=true;
      var B=gp.buttons||[]; for(var bi=0;bi<B.length;bi++){ if(B[bi]&&(B[bi].pressed||B[bi].value>0.5)){ press=true; break; } } } }catch(e){}
    // live USB-controller status only matters on the controls page (that's where the pad line lives)
    if(this.page===0 && this.padLine){
      var want=on?'🎮 CONTROLLER CONNECTED ✓\nD-pad move · A = jump · B = throw · Down = duck\nSELECT: tap = use item, hold = pick which · L2/R2 also use'
                 :'🎮 USB CONTROLLER — plug in & press a button\nD-pad move · A = jump · B = throw · Down = duck\nSELECT: tap = use item, hold = pick which · L2/R2 also use';
      if(this.padLine.text!==want) this.padLine.setText(want).setColor(on?'#3dff6e':'#b6bfe0');
    }
    // While the remap card (or any full-screen overlay) is up, its presses belong to IT — hold the
    // edge down so dismissing it cannot fall straight through into an advance.
    if(nqModalUp()){ this._pp=true; }
    else {
      if(press && !this._pp && this.time.now-this.t0>1200) this.advance();   // fresh press, after grace
      this._pp=press;
    }
    if(this.time.now-this.t0>20000) this.advance(); }   // per-page safety auto-advance (never soft-locks)
});
...
var Briefing=new Phaser.Class({ Extends:Phaser.Scene,
  ...
  create:function(){
    var self=this, cx=W/2; this.cameras.main.setZoom(RES).centerOn(W/2,H/2);
  ...
  go:function(){ if(this.done) return; this.done=true; try{ SFX.power(); }catch(e){} /* a sound can never block the next scene — a closed iOS audio context throws on node creation */ this.scene.start('Game',{level:this.nextLevel,score:this.score,lives:3,lab:this.lab}); },
  update:function(){ if(!this.t0){ this.t0=this.time.now; return; }
    if(this.cont) this.cont.setAlpha(0.4+0.6*Math.abs(Math.sin(this.time.now/300)));
...
// F6: a run finished with no handle is PARKED by NQLB.submitRun and posts the moment a handle is
// picked. Say so on the end screens and open the leaderboard panel on tap. Returns null when there
// is nothing parked, so the line only ever appears when it is true.
function nqParkedPrompt(scene, y){
  var pk=null; try{ pk=(window.NQLB && window.NQLB.parkedRun && window.NQLB.parkedRun()) || null; }catch(e){ pk=null; }
  if(!pk) return null;
  var t=scene.add.text(W/2,y,'NAME YOURSELF TO SAVE THIS SCORE →',{fontFamily:UIFONT,resolution:UIRES,fontSize:'15px',color:'#ffd23f',align:'center'}).setOrigin(.5).setInteractive({useHandCursor:true});
  t.on('pointerdown',function(){ try{ if(window.__NQ_OPENBOARD) window.__NQ_OPENBOARD(); else if(window.__NQ_OPENPREMIUM) window.__NQ_OPENPREMIUM(); }catch(e){} });
...
var LevelClear=new Phaser.Class({ Extends:Phaser.Scene,
  ...
  create:function(){
    ...
    // Phaser REUSES the scene object between runs, so anything create() does not set is whatever
    // the previous beat left behind. `runner` is only made by the nation beat; a later beat found
    // the destroyed sprite still on `this`, update() called setTexture on it, the throw escaped
    // Phaser's step and the whole frame loop stopped — the game froze on the card with the music
    // playing and every tap dead (found 2026-09-06 by the beat test; CI hit it one seed in six).
    this.runner=null; this.cont=null; this._adv=null; this._buyOpen=false;
    this.cameras.main.setZoom(RES).centerOn(W/2,H/2);
  ...
  go:function(){ if(this.done) return; this.done=true; try{ SFX.power(); }catch(e){} /* a sound can never block the next scene — a closed iOS audio context throws on node creation */
  ...
  update:function(){
    ...
    // Cosmetics can never take the loop down: a throw out of update() ends Phaser's rAF chain.
    try{
      // F3: nrun1/nrun2 are the same pose — cycling them here boiled the clear card's marching
      // Normie. Hold the frame it was created with; the bob still carries the motion.
      if(this.runner&&this.runner.scene){ this.runner.y=96+Math.sin(this.time.now/90)*2; }
      if(this.cont&&this.cont.scene) this.cont.setAlpha(0.4+0.6*Math.abs(Math.sin(this.time.now/300)));
    }catch(e){}
...
function nqShouldPitch(idx){
  ...
  if(!nqGateOn()) return false;   // gate off = nothing is locked = nothing to pitch
  // A tier-1 holder ([1,12]) is NOT a free player — pitching "you're in the free worlds" at
  // someone who already paid their way past them is both wrong and insulting. Only pitch when
  // the player's live access ends at the free band.
  try{ var _a=(typeof window.__NQ_ACCESS==='function')?window.__NQ_ACCESS():null;
       if(Array.isArray(_a)&&_a.length===2&&_a[1]>nqFreeMax()) return false; }catch(e){}
...
var VipPitch=new Phaser.Class({ Extends:Phaser.Scene,
  ...
  init:function(d){ d=d||{}; this.next=d.next||0; this.score=d.score||0; this.locked=!!d.locked; this.lab=d.lab; this.world=+d.world||0; },
  create:function(){
    var self=this, cx=W/2; this.done=false; this.t0=0;
    this.cameras.main.setZoom(RES).centerOn(W/2,H/2);
    var g=this.add.graphics(); g.fillStyle(0x0d0b1e,1); g.fillRect(0,0,W,H);
    ...
    //   capped   — launch cap armed: nothing above it is obtainable by anyone, so no pitch, just
    //              "opening soon" (kept from launch morning).
    //   locked   — the real tier wall (gate on, cap lifted). Copy renders the PUBLISHED terms
    //              (owner, 2026-08-22: $5 of NORMIE → worlds 4-12, $50 → everything) from
    //              nqTerms() — live server config, never a hardcoded number. If terms haven't
    //              arrived (offline boot), it says how access works without naming an amount.
    //   teaser   — mid-run upsell for free players (auto-advances, never soft-locks).
    var _capped=!!nqGateCap(), _cx0=nqGateCap()||nqFreeMax(), _free=nqFreeMax();
    var T=nqTerms(), _deep=this.locked&&this.world>=13;
    function _amt(n){ n=+n||0; return n>=1000?(Math.round(n/100)/10)+'k':String(n); }
    var _a1=T?(T.usdPriced?('$'+T.tier1Usd+' of $NORMIE'):(_amt(T.tier1Normie)+' $NORMIE')):null;
    var _a2=T?(T.usdPriced?('$'+T.tier2Usd):(_amt(T.tier2Normie)+' $NORMIE')):null;
    this.add.text(cx,15, _capped?'🔒 THAT\u2019S THE FRONTIER — FOR NOW':(this.locked?(_deep?'🔒 THE DEEP WORLDS — 13-21':'🔒 HOLDER WORLDS AHEAD'):'⭐ MORE WORLDS AWAIT'),{fontFamily:'"Press Start 2P"',fontSize:'11px',color:'#ffd23f',align:'center',wordWrap:{width:W-20}}).setOrigin(.5);
    var body=_capped
      ? ("You've cleared everything that's open right now — worlds 1-"+_cx0+". The rest of the game unlocks in a few days.")
      : (this.locked
        ? (_deep ? "Worlds 13-21 are the deep end — reserved for the biggest $NORMIE holders."
                 : "Worlds "+(_free+1)+"-12 open to $NORMIE holders. Free players keep worlds 1-"+_free+" — replay them any time.")
        : "You're in the free worlds (1-"+_free+"). Holders go on: worlds "+(_free+1)+"-12, then the deep worlds 13-21.");
    ...
    var mid=_capped
      ? "🏆 Set your score on the board while you wait —\nconnect a wallet and your run plays for prizes."
      : (!T ? "💎 Access is by holding $NORMIE — connect your wallet\nand it's checked live, right here."
        : (_deep ? "🏆 Hold "+_a2+(T.usdPriced?' of $NORMIE':'')+" → everything, worlds 1-21"
                 : "💎 Hold "+_a1+" → worlds "+(_free+1)+"-12\n🏆 Hold "+_a2+" → all 21 worlds"));
    this.add.text(cx,84, mid, {fontFamily:UIFONT,resolution:UIRES,fontSize:'11px',color:'#ffd23f',align:'center',wordWrap:{width:W-52}}).setOrigin(.5,0);
    var _sub=this.add.text(cx,118, _capped?"$NORMIE is the community coin behind Normie Quest — grab some without leaving the game."
                                : "Already holding? Connect your wallet — access is checked live. A signature only, no transaction.", {fontFamily:UIFONT,resolution:UIRES,fontSize:'10px',color:'#9fb0d8',align:'center',wordWrap:{width:W-64}}).setOrigin(.5,0);
    ...
    // The capped body copy is a line longer than the members'-wing copy it replaced, and the old
    // hardcoded button row (152/178/204) sat ON TOP of its second line. Lay the buttons out from
    // the measured bottom of the text instead, so copy length can never overlap them again.
    // Buttons per mode: the tier wall gets an explicit CONNECT (the panel's wallet tab verifies
    // holdings live) — the lounge door only shows to players who already have the wing.
    var _btns=[
      ['🪙  BUY $NORMIE',0xffd23f,'#0a0813',function(){ self.buyNormie(); }],
      (!_capped)?['🔓  I HOLD — CONNECT WALLET',0x9b6bff,'#ffffff',function(){ try{ if(window.__NQ_OPENPREMIUM) window.__NQ_OPENPREMIUM(); }catch(e){} }]
                :(nqIsVipOrAll()?['🏛  PREMIUM LOUNGE',0x9b6bff,'#ffffff',function(){ try{ window.open('/normie-quest-x7/lounge','_blank'); }catch(e){} }]:null),
      [this.locked?(nqHasPicker()?'◀  BACK TO MAP':'◀  BACK TO TITLE'):'▶  KEEP PLAYING',0x2ecc71,'#0a0813',function(){ self.go(); }],
    ].filter(Boolean);
    var _gap=26, _blockH=(_btns.length-1)*_gap+20;
    var _top=Math.ceil(_sub.y+_sub.height)+10, _bot=H-30;   // usable band between the copy and the footer strip
    var _y0=_top+10+Math.max(0,(_bot-_top-_blockH)/2);      // centre the row block in whatever room the copy leaves
    _y0=Math.min(_y0, _bot-_blockH+10);                     // never run into the footer strip
    _btns.forEach(function(b,i){ mkBtn(Math.round(_y0+i*_gap), b[0], b[1], b[2], b[3]); });
    this.add.text(cx,H-10, _capped?(nqHasPicker()?'replay any world you like — your best score is what counts':'your best score is what counts — take another run')
                                  :(this.locked?'you can keep any free world you already unlocked':'not now? just KEEP PLAYING'), {fontFamily:UIFONT,resolution:UIRES,fontSize:'9px',color:'#8891b5',align:'center'}).setOrigin(.5);
  ...
  go:function(){ if(this.done) return; this.done=true; try{ SFX.power(); }catch(e){} /* a sound can never block the next scene — a closed iOS audio context throws on node creation */
    if(this.locked){
      // LevelSelect is the TEST BUILD picker. Until the 2026-08-22 launch gate this branch could
      // only ever fire in the setup lane, so it was safe; now a public player who hits the world
      // cap lands here, and "LEVEL SELECT · TEST BUILD" is not a screen to hand a new player.
      this.scene.start(nqHasPicker()?'LevelSelect':'Title');
      // Back means back — the card's own CONNECT button is the door to the wallet panel now,
      // so nothing auto-opens behind a player who chose to leave.
    }
...
// LAB renderer switch (2026-09-06). Headless Chromium has no GPU, so Phaser.AUTO picks WebGL on
// SwiftShader and the game renders at 0.5 fps — that is the "1-4 fps headless box" behind every
// delayedCall timeout and 90-level run that took hours. The 2D canvas renderer runs the same game
// at a full 60 fps there (measured: 0.5 → 60). A test opts in by setting window.__NQ_RENDER='canvas'
// in an init script; nothing in the shipped game sets it, and the visual gate stays on WebGL because
// its baselines are WebGL pixels.
var NQ_RENDER_TYPE=Phaser.AUTO; try{ if(typeof window!=='undefined' && window.__NQ_RENDER==='canvas') NQ_RENDER_TYPE=Phaser.CANVAS; }catch(e){}
var NQGAME=new Phaser.Game({
  ...
  type:NQ_RENDER_TYPE, parent:'screen', width:W*RES, height:H*RES, pixelArt:true, backgroundColor:'#0a0813', roundPixels:true,
...
// (The private-room door is routed inside Boot's go() — before the tappable Title — so there is no
//  title-screen race here. See "PRIVATE ROOM DOOR" in the Boot scene.)
...
if(typeof document!=='undefined'){ (function(){
  ...
  if(!/[?&#]joystick=0(\b|$)/.test((location.search||'')+(location.hash||''))) return;   // the floating joystick (below) is now the universal touch control; this gutter D-pad runs ONLY as the ?joystick=0 fallback
  var css=document.createElement('style');
...
if(typeof document!=='undefined'){ (function(){
  ...
  if(/[?&#]joystick=0(\b|$)/.test(qs)) return;   // ?joystick=0 = escape hatch: fall back to the old gutter D-pad / tap-zones
  var dbg=/[?&#]joydebug=1(\b|$)/.test(qs);
  var PAD=window.__NQ_PAD||(window.__NQ_PAD={left:false,right:false,jump:false,down:false,throw:false});
  window.__NQ_PAD_ACTIVE=true;
  var css=document.createElement('style');
  css.textContent='#nqjoyB,#nqjoyT{position:fixed;z-index:44;border-radius:50%;pointer-events:none;display:none}'
  ...
  var D=null; if(dbg){ D=document.createElement('div'); D.id='nqjoyD';
    ...
    D.textContent='JOY loaded ✓'; document.body.appendChild(D); }
  ...
  function place(px,py){
    var dx=px-ox, dy=py-oy, d=Math.hypot(dx,dy)||1; if(d>R){ dx=dx*R/d; dy=dy*R/d; }
  ...
  // of truth for "is the pad live", reused by the poll below.
  function gameActive(){ try{ return !!(NQGAME && NQGAME.scene && NQGAME.scene.isActive('Game')); }catch(e){ return false; } }
  // A touch that lands on any real on-screen UI control (the ≡ Levels / Feedback / EXTRAS /
  // Premium buttons, gear, etc.) must reach that control, not spring the joystick — otherwise
  // those overlay buttons are dead during gameplay. The game renders to <canvas>, which is not
  // a control, so play-field touches still drive the stick.
  function isUiTarget(el){ try{ return !!(el && el.closest && el.closest('button,a,input,select,textarea,label,[role="button"],[data-nq-ui]')); }catch(e){ return false; } }
  document.addEventListener('touchstart',function(ev){
    // F9: nqModalUp() covers #nqhow-wrap.on, #nqremap, #nqfb-wrap.on and #nqp-ov.on. Without it a
    // left-half touch ON an open card (the how-to card is z-index 100002 over a running scene, and
    // the wrapper is not a [data-nq-ui] target) sprang the joystick and walked the player behind it.
    if(!gameActive() || nqModalUp()) return;   // let taps fall through to level-select / menu buttons
    ...
    for(var i=0;i<ev.changedTouches.length;i++){ var t=ev.changedTouches[i];
      if(t.target===F || topBand(t.clientY) || isUiTarget(t.target)) continue;   // skip on-screen buttons
      if(t.clientX < window.innerWidth*0.5){
        if(joyId===null){ joyId=t.identifier; ox=t.clientX; oy=t.clientY;
          B.style.left=ox+'px'; B.style.top=oy+'px'; B.style.display='block'; T.style.display='block'; place(ox,oy); acted=true; }
      } else { jumpIds[t.identifier]=1; PAD.jump=true; acted=true; }
    }
    if(acted && ev.cancelable) ev.preventDefault();
  },{passive:false});
  document.addEventListener('touchmove',function(ev){
    if(joyId===null) return;   // nothing grabbed (and never grabbed off the game screen)
    for(var i=0;i<ev.changedTouches.length;i++){ var t=ev.changedTouches[i]; if(t.identifier===joyId){ place(t.clientX,t.clientY); if(ev.cancelable) ev.preventDefault(); } }
  },{passive:false});
  function end(ev){
    for(var i=0;i<ev.changedTouches.length;i++){ var t=ev.changedTouches[i];
      if(t.identifier===joyId) hideJoy();
      if(jumpIds[t.identifier]) delete jumpIds[t.identifier];
    }
    PAD.jump=Object.keys(jumpIds).length>0;
  }
...
if(typeof window!=='undefined' && typeof navigator!=='undefined' && navigator.getGamepads){ (function(){
  ...
  function startRemap(){
    ...
    remapEl.id='nqremap';   // nqModalUp() keys off this — without an id the pad watchers can't see the card
...
if(typeof document!=='undefined'){ (function(){
  ...
  var css=el('style',null,
    ...
    +'overflow-y:auto;-webkit-overflow-scrolling:touch;font-family:"Press Start 2P",monospace}'
    +'#nqhow-wrap.on{display:flex}'
    // FIELD BUG (owner, 2026-08-22): on a phone held SIDEWAYS this card is taller than the
    // viewport (~360-390px) and had neither a height cap nor a scroll — it simply ran off the top
    // and bottom, with LET'S GO unreachable. Reported as "huge, you can't read half of it".
    // dvh, not vh, so a mobile browser's collapsing toolbars are accounted for.
    ...
    // Short viewports: same content, sized to fit without needing the scroll above (that stays as
    // the safety net). Wider max-width because landscape has width to spare.
    +'@media (max-height:520px){'
    +'#nqhow-wrap{align-items:flex-start;padding:8px}'
    +'#nqhow-card{max-width:760px;padding:10px 12px 8px;max-height:calc(100dvh - 16px)}'
    +'#nqhow-card h3{font-size:11px;margin:0 0 6px}'
    +'.nqhow-sec{margin-bottom:6px;break-inside:avoid}'
    +'.nqhow-sec .t{font-size:9px;margin-bottom:2px}'
    +'.nqhow-sec .r{font-size:12px;line-height:1.4}'
    // The CTA must never be the thing you have to scroll to find — stick it to the bottom of the
    // card so it stays put while the instructions scroll behind it. It is already opaque yellow.
    +'#nqhow-go{padding:9px;font-size:10px;margin-top:6px;position:sticky;bottom:0;box-shadow:0 -8px 12px -6px #160f2a}'
    +'}'
    // Short AND wide (a phone held sideways): the sections read as two columns, which is what
    // stops three of the five hiding under a scroll. break-inside above keeps a section whole.
    +'@media (max-height:520px) and (min-width:640px){'
    +'#nqhow-secs{column-count:2;column-gap:18px}'
    +'}'
  ...
  wrap.appendChild(el('div',{id:'nqhow-card'},
    ...
    +'<div class="nqhow-sec"><div class="t">📱 TOUCH</div><div class="r"><span class="k">drag the left side</span> to move (joystick — springs up under your thumb) · <span class="k">right side</span> jump · <span class="k">◎</span> throw · <span class="k">⏸ top-left</span> pause</div></div>'
  ...
  // timer ticking, enemies moving, and (with the joystick fix below) touches on the card driving
  // the player. Mirror what the feedback modal's openM/closeM do: release Phaser's keyboard and
  // pause, resuming only if WE paused (never stomp a manual P/Esc pause).
  var howPaused=false;
  function howKb(on){ try{ if(window.__NQ_KBGAME) window.__NQ_KBGAME(on); }catch(e){} }
...
/* ============================================================================
   ...
   invisible on the real game URL and only existed on the -lab setup lane. It now runs EVERYWHERE.
     • window.__NQ_ACCESS / __NQ_VIP are published on EVERY build since the 2026-08-22 public
       launch (they used to be setup-lane-only). They are the readers nqWorldAllowed /
       nqMegaWhaleUnlocked / nqLevelResumeUnlocked use. What decides whether anything is locked
       is no longer the URL but the SERVER: window.__NQ_GATE = {on,freeMax,cap}, fetched at boot
       from /api/nq/wallet/config. Gate off (the default when the env/override is unset) =>
       nqWorldAllowed short-circuits to "allowed" and these readers are inert. See
       normie-quest/nq-gate.js and docs/NQ_LAUNCH_GATE.md.
     • Copy in THIS block that names a tier, an unlock or a holding is still behind `SETUP`. On
       the public URL the wallet is identity + prize eligibility (a 🏆 on your score, your Lounge
       seat, your rewards) and $NORMIE copy is identity + where-to-buy, per NORMIE_NATION. The
       one public lock card is the Phaser VipPitch scene, which under a launch cap says only
       "unlocks in a few days" — never an amount, because NQ's holder terms are unagreed.
...
(function () {
  ...
  var SETUP = false; try { SETUP = !!window.__NQ_SETUP; } catch (e) {}
  // The server gate (separate script block from the Phaser helpers, so read window directly).
  // Gate ON means tiers are REAL on the public build (owner, 2026-08-22) — tier/access lines
  // below show whenever this is true, not only in the lab.
  function GATE_ON() { try { var g = window.__NQ_GATE; return !!(g && g.on); } catch (e) { return false; } }
  ...
  // TV pairing continuation: truthy when the player asked to enter a TV code (a scanned QR sets
  // it to the CODE string via ?pair=). Connecting a wallet completes it — connectWallet drops
  // straight into the code-entry screen instead of leaving the player to re-find the menu.
  var pendingPhonePair = null;
  try { var _pm = /[?&]pair=([A-Za-z0-9]{4,8})/.exec(location.search); if (_pm) pendingPhonePair = _pm[1].toUpperCase(); } catch (e) {}
  ...
  // The token of the run that just ENDED (game over). CONTINUE trades it at /api/nq/run-continue
  // for a fresh-nonce token carrying the same proven level list, so the banked cumulative score
  // stays inside the budget ceiling instead of instantly flagging suspect (audit #7).
  var endedToken = null;
  ...
  window.NQLB = {
    ...
    abandonRun: function () { run = null; pendingLevels = []; endedToken = null; },
    // Called by the Over scene when the player takes a score-carrying CONTINUE, BEFORE the Game
    // scene restarts. Reserves the run slot synchronously (so enterLevel won't mint a fresh
    // one-level token) and swaps the ended token for a same-level-list reissue. If the swap fails
    // (network blip, token past its 2h TTL) it falls back to a fresh run-start seeded with the
    // resumed level — the pre-fix behavior, whose score the server may mark suspect, but the run
    // still records rather than vanishing.
    continueRun: function () {
      var t = endedToken; endedToken = null;
      if (!t) return;
      run = { token: null, _cpInFlight: false };
      var fallback = function () {
        if (!run || run.token) return;
        var lvl = pendingLevels.length ? pendingLevels[0] : '';
        fetch('/api/nq/run-start?level=' + encodeURIComponent(lvl)).then(function (r) { return r.json(); })
          .then(function (j) { if (j && j.ok && run && !run.token) { run.token = { v: j.v, lv: j.lv, max: j.max, nonce: j.nonce, issuedAt: j.issuedAt, sig: j.sig }; flushPending(); } })
          .catch(function () {});
      };
      fetch('/api/nq/run-continue', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ token: t }) })
        .then(function (r) { return r.json(); })
        .then(function (j) {
          // keep the reissued token WHOLE (it carries the signed lastCpAt) — never rebuild it field-by-field
          if (j && j.ok && j.token && run) { run.token = j.token; flushPending(); }
          else fallback();
        })
        .catch(fallback);
    },
    ...
    enterLevel: function (name) {
      if (!run) {
        endedToken = null;                       // a brand-new run supersedes any continuable old one
        run = { token: null, _cpInFlight: false };
    ...
    submitRun: function (world, score) {
      ...
      endedToken = r.token;                      // kept so a CONTINUE can trade it for a reissue (audit #7)
  ...
  css.textContent =
    ...
    + '#nqp-chars{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-top:10px}'
    + '.nqp-char{display:flex;flex-direction:column;align-items:center;gap:5px;background:#1a1630;border:2px solid #33305a;border-radius:12px;padding:10px 6px;cursor:pointer;color:#cbc6e6;font-family:inherit}'
    + '.nqp-char img{width:62px;height:93px;object-fit:contain;background:#0f0b1e;border-radius:8px}'
    + '.nqp-char.on{border-color:#ffd23f;background:#241b3a}'
    + '.nqp-char-name{font-size:11px;font-weight:700;text-align:center;line-height:1.15}'
    + '.nqp-char-tag{font-size:9px;letter-spacing:.04em;color:#8a84a8}'
    + '.nqp-char.on .nqp-char-tag{color:#ffd23f}'
  ...
  ov.innerHTML =
    ...
    // Tabs are <button>s, not <div>s: on iOS, when the leaderboard name field has focus (soft keyboard
    // up), the first tap on a non-button element is swallowed to dismiss the keyboard and never reaches
    // a div's click handler — so the panel looked stuck on Leaderboard. Buttons are natively tappable
    // and fire click even while the keyboard closes.
    + '<div class="nqp-tabs"><button type="button" class="nqp-tab on" data-tab="lb">🏆 Leaderboard</button>'
    + '<button type="button" class="nqp-tab" data-tab="wal">👛 Wallet</button><button type="button" class="nqp-tab" data-tab="char">🎭 Character</button><button type="button" class="nqp-tab" data-tab="buy">🪙 Buy NORMIE</button>'
    // Hidden until /api/nq/config says the server has armed the shop. An unarmed build must not
    // advertise a shop it cannot open.
    + '<button type="button" class="nqp-tab" data-tab="shop" style="display:none">🛍 Shop</button></div>'
    ...
    + '<div id="nqp-prize" style="display:none"></div>'
    ...
    + '<div class="nqp-pane" data-pane="char"><div class="nqp-sub">Choose who you play as — free &amp; cosmetic, saved on this device. Takes effect on your next run.</div><div id="nqp-chars"></div></div>'
  ...
  function ready() {
    ...
    restoreWallet(function (ok) {
      ...
      if (window._nqLoadPrize) window._nqLoadPrize();   // a restored winner sees the claim banner without reopening
    ...
    // F6: one open path, shared. The end screens point a first-timer with a PARKED (unnamed) run
    // straight at the handle field — before this the only surface that ever revealed a parked run
    // was this panel's Leaderboard tab, which nothing pointed at, so most first scores died with
    // the tab. Never auto-names anyone: the handle is still theirs to type.
    function openBoards() { openOv(); loadBoards(); parkedNote(); if (window._nqLoadPrize) window._nqLoadPrize(); }
    btn.addEventListener('click', function () { openBoards(); });
    try { window.__NQ_OPENBOARD = openBoards; } catch (e) {}
    ...
    // Arrived via the TV's QR (?pair=CODE): open the wallet tab now. Already-proven wallet →
    // straight to code entry, prefilled; otherwise renderWallet shows connect + the pairing hint,
    // and the post-connect continuation above finishes the job.
    if (pendingPhonePair) setTimeout(function () {
      try {
        window.__NQ_OPENPREMIUM();
        var ws0 = window.NQLB.getWallet();
        if (ws0.pubkey && typeof pendingPhonePair === 'string') { var pf0 = pendingPhonePair; pendingPhonePair = null; startPhonePair(ov.querySelector('#nqp-wal'), pf0); }
      } catch (e) {}
    }, 900);
    ...
    function renderChars() {
      var host = ov.querySelector('#nqp-chars'); if (!host) return;
      var C = window.__NQ_CHARS;
      if (!C || !C.list) { host.innerHTML = '<div class="nqp-sub">Characters load with the game — start a run once, then come back.</div>'; return; }
      var cur = C.get();
      host.innerHTML = C.list.map(function (c) {
        var on = c.id === cur;
        return '<button class="nqp-char' + (on ? ' on' : '') + '" data-char="' + c.id + '">'
          + '<img src="' + c.icon + '" alt="" draggable="false"/>'
          + '<span class="nqp-char-name">' + c.name + '</span>'
          + '<span class="nqp-char-tag">' + (on ? '✓ PLAYING' : 'TAP TO PLAY') + '</span></button>';
      }).join('');
      host.querySelectorAll('.nqp-char').forEach(function (b) {
        b.addEventListener('click', function () { try { C.set(b.getAttribute('data-char')); } catch (e) {} renderChars(); });
      });
    }
    ov.querySelectorAll('.nqp-tab').forEach(function (t) {
      t.addEventListener('click', function () {
        try { var a = document.activeElement; if (a && a.blur && a !== t) a.blur(); } catch (e) {}  // drop the iOS keyboard when switching tabs
    ...
    // ---- 🏆 weekly prize claim (winners only) ------------------------------
    // Last completed week's top verified run(s) win a physical prize. The banner shows ONLY to the
    // winner's connected wallet; claiming = enter a shipping address and SIGN a consent message that
    // embeds the address fingerprint (signature only — no transaction, nothing moves). The server
    // stores the address encrypted and the owner ships from a keyed panel. Non-winners never see
    // any of this — the board itself is the public surface.
    var prizeState = null;   // last /claim/status payload for the connected wallet
    function prizeHost() { return ov.querySelector('#nqp-prize'); }
    function loadPrize() {
      var host = prizeHost(); if (!host) return;
      var pk = walletState.pubkey;
      if (!pk) { host.style.display = 'none'; return; }
      fetch('/api/nq/claim/status?pubkey=' + encodeURIComponent(pk)).then(function (r) { return r.json(); })
        .then(function (j) {
          prizeState = j;
          if (!j || !j.ok || !j.enabled || !j.you || !j.you.isWinner) { host.style.display = 'none'; return; }
          host.style.display = '';
          if (j.you.claimed) {
            host.innerHTML = '<div class="nqp-sub" style="border:1px solid #3dff6e55;border-radius:10px;padding:9px 11px;margin:0 0 10px;color:#3dff6e">'
              + '🏆 Prize claim received — rank #' + j.you.rank + ' last week. The prize is on its way.</div>';
          } else if (!j.you.windowOpen) {
            host.style.display = 'none';
          } else {
            host.innerHTML = '<div style="border:1px solid #ffd23f66;border-radius:10px;padding:9px 11px;margin:0 0 10px">'
              + '<div style="color:#ffd23f;font-weight:700">🏆 You topped last week\'s board (rank #' + j.you.rank + ')!</div>'
              + '<div class="nqp-sub" style="margin:4px 0 8px">You won a physical prize. Claim with the wallet that set the score — signature only, nothing moves.</div>'
              + '<button type="button" class="nqp-in" id="nqp-prize-open" style="cursor:pointer;background:#ffd23f;color:#1a1533;font-weight:700;border:0">CLAIM PRIZE</button></div>';
            host.querySelector('#nqp-prize-open').addEventListener('click', showClaimForm);
          }
        }).catch(function () { host.style.display = 'none'; });
    }
    function showClaimForm() {
      var host = prizeHost(); if (!host || !prizeState) return;
      var F = [['name', 'Full name*'], ['line1', 'Street address*'], ['line2', 'Apt / unit (optional)'],
               ['city', 'City*'], ['region', 'State / region'], ['postal', 'Postal code'], ['country', 'Country*']];
      host.innerHTML = '<div style="border:1px solid #ffd23f66;border-radius:10px;padding:9px 11px;margin:0 0 10px">'
        + '<div style="color:#ffd23f;font-weight:700;margin-bottom:6px">🏆 CLAIM — shipping address</div>'
        + F.map(function (f) { return '<input class="nqp-in" id="nqp-adr-' + f[0] + '" placeholder="' + f[1] + '" maxlength="96" style="margin-bottom:6px">'; }).join('')
        + '<div class="nqp-sub" style="margin:2px 0 8px">Your wallet will ask you to sign a message authorizing us to use this address <b>solely to ship this prize</b>. It is stored encrypted and deleted after delivery. Signature only — it cannot move funds.</div>'
        + '<button type="button" class="nqp-in" id="nqp-prize-sign" style="cursor:pointer;background:#ffd23f;color:#1a1533;font-weight:700;border:0">SIGN &amp; CLAIM</button>'
        + '<div class="nqp-sub" id="nqp-prize-msg" style="margin-top:6px"></div></div>';
      var msg = function (t, bad) { var e = host.querySelector('#nqp-prize-msg'); if (e) { e.textContent = t; e.style.color = bad ? '#ff9db8' : ''; } };
      host.querySelector('#nqp-prize-sign').addEventListener('click', function () {
        var addr = {};
        F.forEach(function (f) { var e = host.querySelector('#nqp-adr-' + f[0]); addr[f[0]] = e ? e.value : ''; });
        if (!addr.name || !addr.line1 || !addr.city || !addr.country) { msg('Name, street, city and country are required.', true); return; }
        var pk = walletState.pubkey, week = prizeState.week;
        msg('Preparing claim…');
        fetch('/api/nq/claim/prepare', { method: 'POST', headers: { 'content-type': 'application/json' },
          body: JSON.stringify({ pubkey: pk, week: week, address: addr }) }).then(function (r) { return r.json(); })
          .then(function (prep) {
            if (!prep || !prep.ok) { msg('Could not prepare: ' + ((prep && prep.status) || 'error'), true); return null; }
            msg('Check your wallet — sign the claim message…');
            return getSigner().then(function (p) {
              if (!p || typeof p.signMessage !== 'function') throw new Error('This wallet cannot sign messages here — open the game in your wallet\'s browser.');
              return p.signMessage(new TextEncoder().encode(prep.message), 'utf8');
            }).then(function (signed) {
              var sigBytes = signed && signed.signature ? signed.signature : signed;
              return fetch('/api/nq/claim', { method: 'POST', headers: { 'content-type': 'application/json' },
                body: JSON.stringify({ pubkey: pk, week: week, signature: b58encode(sigBytes) }) }).then(function (r) { return r.json(); });
            }).then(function (sub) {
              if (sub && sub.ok) { msg(''); loadPrize(); }
              else msg('Claim failed: ' + ((sub && sub.status) || 'error'), true);
            });
          }).catch(function (e) { msg((e && e.message) || 'Something went wrong — try again.', true); });
      });
    }
    window._nqLoadPrize = loadPrize;
    ...
    function startTvPair(host) {
      ...
      fetch('/api/nq/pair/new', { method: 'POST' }).then(function (r) { return r.json(); }).then(function (j) {
        ...
        host.innerHTML = '<div class="nqp-sub" style="margin-bottom:10px">📷 <b>Scan with your phone camera</b> — it opens the game with the code filled in. Or type the code by hand: 🎮 → <b>Wallet</b> → <b>I\u2019m the phone</b>.</div>'
          // QR of the game URL WITH the code in it (?pair=CODE) — the code is the PUBLIC half of
          // the pairing (the claim secret never leaves this screen), so showing it as a QR is
          // exactly as safe as showing it as text, and it saves typing a URL on a phone keyboard.
          + '<div style="text-align:center;margin-bottom:10px"><img alt="pairing QR" src="/api/nq/pair/qr?code=' + encodeURIComponent(j.code) + '" '
          + 'style="width:168px;height:168px;border-radius:10px;border:3px solid #ffb43a;background:#fff"></div>'
    ...
    function startPhonePair(host, prefill) {
      stopPairPoll();
      ...
      host.innerHTML = '<div class="nqp-sub" style="margin-bottom:10px">Enter the code shown on your TV to give it access with this wallet '
        ...
        + (prefill ? ('value="' + esc(String(prefill).toUpperCase().replace(/[^A-Z0-9]/g, '').slice(0, 6)) + '" ') : '')
    ...
    function renderWallet() {
      ...
      if (ws.pubkey) {
        ...
        // TIER / ACCESS show whenever gating is LIVE (the server gate is on) or in the lab.
        // Before 2026-08-22 these were setup-only because nothing was gated publicly; now the
        // gate is real and a holder deserves to SEE what their wallet unlocked.
        var TIERED = SETUP || GATE_ON();
        ...
        var intro = '<div class="nqp-sub" style="margin-bottom:10px">'
          + ((SETUP || GATE_ON()) ? 'Prove wallet ownership to unlock worlds by what you HOLD — and your scores play for prizes. '
                   // Gate off: what connecting actually does, and nothing it does not.
        ...
        if (det.length) {
          ...
          host.innerHTML = intro + det.map(function (w, i) {
            var ic = w.img ? '<img src="' + esc(w.img) + '" alt="" style="width:16px;height:16px;vertical-align:-3px;border-radius:4px"> ' : (w.icon + ' ');
          ...
          host.querySelectorAll('button[data-wi]').forEach(function (b) {
            b.addEventListener('click', function () {
              ...
              connectWallet(w.provider, w.name).then(function () {
                renderWallet(); if (window._nqLoadPrize) window._nqLoadPrize();
                // The player came here to pair a TV (tapped "I'm the phone", or scanned the TV's
                // QR) — drop them straight into code entry instead of making them re-find it.
                if (pendingPhonePair) { var pf = typeof pendingPhonePair === 'string' ? pendingPhonePair : null; pendingPhonePair = null; startPhonePair(ov.querySelector('#nqp-wal'), pf); }
              }).catch(function (e) { ov.querySelector('#nqp-walerr').textContent = e.message || String(e); });
            });
          });
        } else {
        ...
        // player still has no way to sign there. BOTH directions are offered, because launch-day
        // field report (owner, 2026-08-22): the phone-side "enter the TV's code" entry only
        // existed AFTER a wallet was connected — the TV pointed people at a button their phone
        // didn't have. Now the phone path exists here too: it asks for the connect first (pairing
        // hands over a PROVEN session; without one there is nothing to hand) and then drops
        // straight into code entry.
        host.insertAdjacentHTML('beforeend',
          '<div style="border-top:1px solid #2a2648;margin-top:14px;padding-top:12px">'
          + '<div class="nqp-sub" style="margin-bottom:8px">📺 <b>Playing on a TV?</b> TVs and consoles can\'t run a wallet — pair with your phone instead, about twenty seconds.</div>'
          + '<button class="nqp-b alt" id="nqp-tvstart">📺 I\u2019m the TV — show a pairing code</button>'
          + '<button class="nqp-b alt" id="nqp-phstart" style="margin-top:6px">📱 I\u2019m the phone — enter the TV\u2019s code</button>'
          + '<div class="nqp-sub" id="nqp-pairhint" style="margin-top:8px"></div></div>');
        host.querySelector('#nqp-tvstart').addEventListener('click', function () { startTvPair(host); });
        host.querySelector('#nqp-phstart').addEventListener('click', function () {
          pendingPhonePair = pendingPhonePair || true;   // keep a scanned code if one is queued
          var h = host.querySelector('#nqp-pairhint');
          if (h) h.innerHTML = '<span style="color:#ffc98a">First connect your wallet with the buttons above — pairing hands the TV your <b>proven</b> session. The code entry opens right after.</span>';
