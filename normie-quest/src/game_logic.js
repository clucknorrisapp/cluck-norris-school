...
var LEVELS=[
  ...
  { name:'5-2', sub:'THE SPAN', time:200, theme:7, bgArt:'w05bridge2', width:8400,
    ...
    enemies:[['fudster',340,230,70],['sniper',620,230,30],['ghost',1000,212,80],['paper',1420,230,70],['sniper',1920,230,30],['bitmaxi',2280,230,70],['flashbot',2720,160,250],['ghost',3220,212,70],['sniper',3480,230,30],['paper',3680,230,70],['bitmaxi',4020,230,70],['flashbot',4420,160,250],['ghost',4840,212,80],['bitmaxi',5760,230,70],['jeet',6160,230,70],['sniper',6590,230,70],['ghost',7020,212,70],['paper',7450,230,70],['sniper',7880,230,70],['sniper',5520,230,40],['ghost',5500,150,70]],
...
var Game=new Phaser.Class({ Extends:Phaser.Scene,
  ...
  update:function(){
    ...
    this.enemies.children.iterate(function(e){
      ...
      if(e.kind==='flashbot'){   // FLASH-LOAN DRONE: hovers, then BLINKS a jump closer to you. Intangible + un-hittable mid-blink; stomp it in the solid window.
        var fty=e.homeY+Math.sin(now/400+e.bob)*8; e.setVelocityX(0); e.setVelocityY((fty-e.y)*5);
        if(!e.phasing && now>e.blinkAt){ e.phasing=true; e.blinkT=now; self.tweens.add({targets:e,alpha:0.12,duration:150}); }
        else if(e.phasing && now>e.blinkT+170){ var bd=(p.x>e.x)?1:-1; e.x=Phaser.Math.Clamp(e.x+bd*150, e.homeX-e.range, e.homeX+e.range); e.setFlipX(bd>0); e.phasing=false; self.tweens.add({targets:e,alpha:1,duration:150}); e.blinkAt=now+1000+Phaser.Math.RND.between(0,700); }
        return;
      }
      if(e.kind==='drillbit'){   // DRILL BIT: winds up (vulnerable, jitter telegraph) then DASH-drills in bursts. drillImmune during the dash = can't be stomped; leap it or stomp on the wind-up.
        ...
        if(e.drillState==='windup'){ e.setVelocityX(0); e.drillImmune=false; e.setRotation(Math.sin(now/40)*0.14);
          if(now>e.drillT+700){ e.drillState='dash'; e.drillT=now; e.dir=(p.x<e.x)?-1:1; e.drillImmune=true; } }
