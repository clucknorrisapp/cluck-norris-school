...
var Game=new Phaser.Class({ Extends:Phaser.Scene,
  ...
  gameOver:function(reason){ if(this.over) return; this.over=true; this.physics.pause();
    ...
    // LEADERBOARD: which level (if any) this run should be credited to. A private room must
    // NEVER touch the board — mirrors Win.create's `!this.private` guard, same room family, same
    // rule; without it a private-room game over reached the public weekly/all-time board. A
    // non-private hidden bonus/speakeasy room reports the FURTHEST NUMBERED campaign world
    // reached (effIdx, already computed above for the checkpoint logic) instead of its own
    // non-numeric name — submitting the room name used to resolve to world 0 and silently drop a
    // legitimate main-run death off every per-world board.
    var boardLevel = null;
    if(!(this.def && this.def.private)){
      boardLevel = (this.def && this.def.hidden) ? ((LEVELS[effIdx] && LEVELS[effIdx].name) || null) : (this.def && this.def.name);
    }
    this.time.delayedCall(600,function(){ this.scene.start('Over',{score:this.score,reason:reason,level:this.def.name,boardLevel:boardLevel,cont:cont,contScore:contScore,contKind:contKind,contName:contName,contTease:contTease}); },[],this); },
...
var Over=new Phaser.Class({ Extends:Phaser.Scene,
  ...
  init:function(d){ this.finalScore=d.score||0; this.reason=d.reason||''; this.level=d.level||''; this.boardLevel=(d.boardLevel!==undefined)?d.boardLevel:d.level; this.cont=d.cont||0; this.contScore=d.contScore||0; this.contKind=d.contKind||''; this.contName=d.contName||''; this.contTease=!!d.contTease; },
  create:function(){
    // SETUP LANE: run ended (game over) — submit the final score to the leaderboard, UNLESS
    // boardLevel is null (a private room — see gameOver()'s boardLevel comment; must never touch
    // the board, same rule Win.create applies via its `!this.private` guard).
    if(window.NQLB && this.boardLevel){ try{ window.NQLB.submitRun(window.NQLB.worldOf(this.boardLevel), this.finalScore); }catch(e){} }
...
(function () {
  ...
  var WALLET_DEFS = [
    // SafePal MUST be listed first (same fix as public/cluck-wallet.js): its in-app browser
    // injects one window.solana that also sets other wallets' compat flags (isPhantom, isGlow) so
    // Phantom-only dapps work — which without this entry ahead of them made one SafePal show up
    // as BOTH "Phantom" and "Glow" below. isSafePal / window.safepal is its true identity and no
    // other wallet sets it. A real Phantom (no isSafePal) skips this and matches phantom below.
    { id: 'safepal',  name: 'SafePal',  icon: '🔐', detect: function () { return (window.safepal && window.safepal.solana) || (window.safepalProvider && window.safepalProvider.solana) || (window.solana && window.solana.isSafePal ? window.solana : null); } },
    ...
    { id: 'coin98',   name: 'Coin98',   icon: '🐋', detect: function () { return (window.coin98 && window.coin98.sol) || (window.solana && window.solana.isCoin98 ? window.solana : null); } },
    ...
    function showClaimForm() {
      ...
      host.querySelector('#nqp-prize-sign').addEventListener('click', function () {
        ...
        fetch('/api/nq/claim/prepare', { method: 'POST', headers: { 'content-type': 'application/json' },
          body: JSON.stringify({ pubkey: pk, week: week, address: addr, token: walletState.token }) }).then(function (r) { return r.json(); })
          .then(function (prep) {
