...
var LEVELS=[
  ...
  { name:'5-2', sub:'THE SPAN', time:200, theme:7, bgArt:'w05bridge2', width:8400,
    ...
    enemies:[['fudster',340,230,70],['sniper',620,230,30],['ghost',1000,212,80],['paper',1420,230,70],['sniper',1920,230,30],['bitmaxi',2280,230,70],['flashbot',2720,200,250],['ghost',3220,212,70],['sniper',3480,230,30],['paper',3680,230,70],['bitmaxi',4020,230,70],['flashbot',4420,200,250],['ghost',4840,212,80],['bitmaxi',5760,230,70],['jeet',6160,230,70],['sniper',6590,230,70],['ghost',7020,212,70],['paper',7450,230,70],['sniper',7880,230,70],['sniper',5520,230,40],['ghost',5500,150,70]],
...
var Game=new Phaser.Class({ Extends:Phaser.Scene,
  ...
  makeEnemy:function(kind,x,y,range){
    ...
    var tex = (kind==='sniper'||kind==='sandwich'||kind==='flashbot'||kind==='drillbit') ? 'bot' : (kind==='gasgoblin' ? 'troll' : kind);   // FLASHBOT / DRILLBIT reuse the bot sprite; their OWN behavior + tint are set below
    var e=this.enemies.create(x,y,tex); e.kind=kind; e.beh=beh;
    e.setScale((kind==='ghost'?26:kind==='bitmaxi'?34:kind==='fudster'?38:kind==='gasgoblin'?28:kind==='laserbot'?32:kind==='mevdrone'?34:kind==='drillbit'?34:30)/e.height);
    ...
    var spd={jeet:50,paper:66,bot:88,ghost:52,bitmaxi:72,sniper:34,fudster:44,gasgoblin:46,sandwich:58,laserbot:34,mevdrone:64,flashbot:40,drillbit:60}[kind]||55;
    ...
    if(kind==='drillbit'){ e.setTint(0xffa838); e.drillState='windup'; e.drillT=this.time.now; e.drillImmune=false; }   // DRILLBIT: winds up then DASH-drills; un-stompable mid-drill, vulnerable on the wind-up
  ...
  update:function(){
    ...
    this.enemies.children.iterate(function(e){
      ...
      if(e.kind==='flashbot'){   // FLASH-LOAN DRONE: HUNTS you — blinks toward you and drops to your level so you can stomp it, then vanishes. Intangible + un-hittable mid-blink.
        var fty=e.homeY+Math.sin(now/400+e.bob)*7; e.setVelocityX(0); e.setVelocityY((fty-e.y)*6);
        if(!e.phasing && now>e.blinkAt){ e.phasing=true; e.blinkT=now; self.tweens.add({targets:e,alpha:0.1,duration:130}); }
        else if(e.phasing && now>e.blinkT+150){
          var bd=(p.x>e.x)?1:-1, step=Math.min(180, Math.abs(p.x-e.x)+40);
          e.x=Phaser.Math.Clamp(e.x+bd*step, e.homeX-e.range, e.homeX+e.range);
          e.homeY=Phaser.Math.Clamp(p.y-26, 150, 214); e.y=e.homeY;   // teleport DOWN to the player's level → engageable, not a far-off floater
          e.setFlipX(bd>0); e.phasing=false; e.setAlpha(1); self.burst(e.x,e.y,0x8fe6ff,9);   // pop of light on arrival so the blink READS
          e.blinkAt=now+750+Phaser.Math.RND.between(0,450);
        }
        return;
      }
      if(e.kind==='drillbit'){   // DRILL BIT: winds up (vulnerable, jitter telegraph) then DASH-drills in bursts. drillImmune during the dash = can't be stomped; leap it or stomp on the wind-up.
        if(e.x<e.homeX-e.range) e.dir=1; if(e.x>e.homeX+e.range) e.dir=-1;
        if(e.body.blocked.left) e.dir=1; if(e.body.blocked.right) e.dir=-1;
        if(e.drillState==='windup'){ e.setVelocityX(0); e.drillImmune=false; e.setRotation(Math.sin(now/32)*0.18); e.setTint((Math.floor(now/70)%2)?0xffffff:0xffa838);   // jitter + flash: clear "about to DASH" tell
          if(now>e.drillT+650){ e.drillState='dash'; e.drillT=now; e.dir=(p.x<e.x)?-1:1; e.drillImmune=true; e.setTint(0xffa838); self.burst(e.x,e.y,0xffb84a,8); } }
        else { var ahead=e.x+e.dir*(e.body.width*0.5+6);
          if(e.body.blocked.left||e.body.blocked.right||(e.body.blocked.down&&self.overPit(ahead))||now>e.drillT+620){ e.drillState='windup'; e.drillT=now; e.drillImmune=false; e.dir=-e.dir; e.setVelocityX(0); e.setRotation(0); }
          else { e.setVelocityX(e.dir*175); e.setRotation(e.rotation+0.5); } }
        e.setFlipX(e.dir>0); return;
      }
      if(e.x<e.homeX-e.range) e.dir=1; if(e.x>e.homeX+e.range) e.dir=-1;
      if(e.body.blocked.left) e.dir=1; if(e.body.blocked.right) e.dir=-1;
