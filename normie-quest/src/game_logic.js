...
var Game=new Phaser.Class({ Extends:Phaser.Scene,
  ...
  makeEnemy:function(kind,x,y,range){
    ...
    e.setScale((kind==='ghost'?26:kind==='shrimp'?22:kind==='bitmaxi'?34:kind==='fudster'?38:kind==='gasgoblin'?28:kind==='laserbot'?32:kind==='mevdrone'?34:30)/e.height);
    ...
    if(kind==='shrimp'){ e.setTint(0xff7a5c); e.hopper=true; e.nextHop=this.time.now+300+Phaser.Math.RND.between(0,500); }   // SHRIMP: small skittish retail seller that HOPS — its OWN behavior, not a jeet reskin
  ...
  update:function(){
    ...
    this.enemies.children.iterate(function(e){
      ...
      // SHRIMP hop: a grounded skittish bounce on a cadence — a moving vertical target, distinct from a plain walker
      if(e.hopper && e.body.blocked.down && now>e.nextHop){ e.setVelocityY(-250); e.nextHop=now+650+Phaser.Math.RND.between(0,450); }
