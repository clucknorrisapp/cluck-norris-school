...
var Game=new Phaser.Class({ Extends:Phaser.Scene,
  ...
  update:function(){
    ...
    // Skins (pr/kd) use a SINGLE run pose instead of cycling nrun1<->nrun2: two detailed
    // illustrations flipping ~10x/sec read as a bounce/blur while moving (owner playtest 2026-08-15).
    // Normie + moon keep the two-frame cycle — their frames are built to tween cleanly.
    var baseKey = this.crouching ? 'nduck'
                : !onGround ? 'njump'
                : (Math.abs(b.velocity.x)>25 ? (this.charPrefix ? 'nrun1' : ((Math.floor(now/(fast?70:100))%2) ? 'nrun2':'nrun1'))
                : 'normie');
    ...
    if(mw){
      ...
      if(this.charPrefix){ p.setRotation(0); p.setScale(bs); }   // skins: steady clean run (no bob) — clarity over bounce (owner 2026-08-15)
      else { var cyc=now/(fast?52:82); p.setRotation(Math.sin(cyc)*0.05);
...
var Win=new Phaser.Class({ Extends:Phaser.Scene,
  ...
  create:function(){
    ...
    if(window.NQLB && !this.private){ try{ window.NQLB.submitRun(worldCount(), this.finalScore); }catch(e){} }
    SFX.win();
    ...
    this.tweens.add({targets:p,alpha:.25,duration:600,yoyo:true,repeat:-1});
    var self=this; this.started=false; var go=function(){ if(self.started) return; self.started=true; self.scene.start('Game',{level:self.replayLevel,score:0,lives:3,lab:(self.private?1:undefined)}); };
    this.time.delayedCall(500,function(){ self.input.once('pointerdown',go); self.input.keyboard.once('keydown',go); });
    padAdvance(self, go);   // controller button also continues
  }
});
...
var LevelClear=new Phaser.Class({ Extends:Phaser.Scene,
  ...
  update:function(){
    ...
    try{
      if(this.runner&&this.runner.scene){ var k=(Math.floor(this.time.now/110)%2)?'nrun2':'nrun1';
...
if(typeof document!=='undefined'){ (function(){
  ...
  document.addEventListener('touchstart',function(ev){
    if(!gameActive()) return;   // let taps fall through to level-select / menu buttons
...
(function () {
  ...
  function ready() {
    ...
    btn.addEventListener('click', function () { openOv(); loadBoards(); parkedNote(); if (window._nqLoadPrize) window._nqLoadPrize(); });
