...
var Boot=new Phaser.Class({ Extends:Phaser.Scene,
  ...
  create:function(){
    ...
    var go=function(){ if(started) return; started=true;
      ...
      try{ var _rm=((location.search||'')+(location.hash||'')).match(/[?&#]room=([A-Za-z0-9_-]{1,24})/);
        if(_rm){ var _rn=_rm[1].toLowerCase();
          for(var _i=0;_i<LEVELS.length;_i++){ if(LEVELS[_i]&&LEVELS[_i].private && String(LEVELS[_i].name||'').toLowerCase()===_rn){ self.scene.start('Game',{level:_i,score:0,lives:3,lab:1}); return; } }
        }
      }catch(e){}
