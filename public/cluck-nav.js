...
(function () {
  ...
  // Shared collision-avoidance for the bottom-pinned floaters (the Listen bar bottom-left, the
  // language toggle bottom-right) now lives in its own file so the bundled store editions — which
  // have no nav script — can ship it too. See public/clkn-dock-float.js.
  if (!document.getElementById("clkn-dock-js")) {
    var df = document.createElement("script");
    df.id = "clkn-dock-js"; df.src = "/clkn-dock-float.js"; df.defer = true;
